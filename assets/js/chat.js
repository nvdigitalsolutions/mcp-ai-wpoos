(function () {
    ...
    /**
     * Check if a result object has the structure of a system logs response.
     * 
     * System logs responses have wp_mcp_ai, wordpress, or plugin_logs objects.
     * 
     * @param {*} result - Tool result to check
     * @return {boolean} True if result appears to be a system logs response
     */
    function isSystemLogsStructure(result) {
        return result && 
               typeof result === 'object' &&
               (result.wp_mcp_ai !== undefined || 
                result.wordpress !== undefined || 
                result.plugin_logs !== undefined);
    }

    /**
     * Extract system logs summary from get_system_logs tool result.
     * 
     * Formats nested log data (wp_mcp_ai, wordpress, plugin_logs) into readable sections.
     * This handles any tool that returns log data in this structure pattern.
     * 
     * @param {Object} result - Tool result object with nested log data
     * @return {string|null} Formatted summary text or null if structure is invalid
     */
    function extractSystemLogsSummary(result) {
        if (!isSystemLogsStructure(result)) {
            return null;
        }
    ...
    function extractGenericToolResponse(result) {
        ...
        } else if (isSystemLogsStructure(result)) {
            // Handle get_system_logs tool result structure with wp_mcp_ai, wordpress, and plugin_logs
            const systemLogsText = extractSystemLogsSummary(result);
            if (systemLogsText) {
                text = systemLogsText;
            }
