		...
		startMonitoring: function (containerId, endpoint, nonce, callback, assistantId, guestToken) {
			...
			if (window.wpMcpAiSSE && window.wpMcpAiSSE.isSupported()) {
				...
				try {
					const sseConnection = window.wpMcpAiSSE.connect(sseUrl, {
						...
						onError: function (error) {
							...
							if (window.console && console.info) {
								console.info('[WP MCP AI] SSE cron status not available, falling back to REST polling');
							}
					...
					if (sseConnection) {
						...
						setTimeout(function () {
							...
							if (self.sseConnections[containerId] && !sseReceived) {
								if (window.console && console.info) {
									console.info('[WP MCP AI] SSE cron status timeout, falling back to REST polling');
								}
								self.stopSSE(containerId);
								self.startFallbackPolling(containerId, endpoint, nonce, callback, assistantId, guestToken);
							}
					...
					// Log fallback at info level - not an error, just informational
					if (window.console && console.info) {
						console.info('[WP MCP AI] SSE cron status connection error, falling back to REST polling');
					}
					// Fall back to REST polling
					this.startFallbackPolling(containerId, endpoint, nonce, callback, assistantId, guestToken);
				}
