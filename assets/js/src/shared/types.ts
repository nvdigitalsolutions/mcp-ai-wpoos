...
// ── WordPress Global Declarations ────────────────────────────────────

/**
 * Augment the global `Window` interface so TypeScript knows about the
 * WordPress globals this plugin sets via `wp_localize_script`.
 *
 * These match the `globals` declared in `.eslintrc.json`.
 */
declare global {
	interface Window {
		wp: Record< string, unknown >;
		wpMcpAiChat: Partial< GlobalChatConfig >;
		wpMcpAiChatInstances: Record< string, ChatInstanceState >;
		wpMcpAiChatStorage: unknown;
		wpMcpAiChatClipboard: unknown;
		wpMcpAiChatMarkdown: unknown;
		wpMcpAiChatDomBatcher: unknown;
		wpMcpAiChatUIUtils: unknown;
		wpMcpAiChatAudio: unknown;
		wpMcpAiUserChats: unknown;
		wpMcpAiAdmin: unknown;
	}
}
