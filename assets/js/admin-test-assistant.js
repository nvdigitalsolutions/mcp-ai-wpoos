...
(function () {
	...
	function init() {
		...
		const modalClose = modal ? modal.querySelector('.wp-mcp-ai-test-modal__close') : null;
		const modalBackdrop = modal ? modal.querySelector('.wp-mcp-ai-test-modal__backdrop') : null;
	...
	function openTestModal(assistantId, assistantTitle) {
		...
		// The chat.js init() function looks for [data-wp-mcp-ai-chat] elements.
		// We need to manually trigger it for dynamically added elements.
	 ...
	 *
	 * This manually triggers chat.js initialization for dynamically created elements.
	 * We create a temporary DOMContentLoaded event on the container to trigger
	 * chat.js's initialization logic.
	 *
	...
	function initializeChatInstance(instanceId) {
		...
		setTimeout(function () {
			...
			// Since chat.js looks for [data-wp-mcp-ai-chat] elements on DOMContentLoaded,
			// we need to manually trigger its initialization logic.
			// The cleanest way is to dispatch a DOMContentLoaded event on the document.
			// However, this may trigger other handlers, so we'll use a more targeted approach.

			// We'll manually run the chat.js init logic by simulating what it does.
			// This requires creating all the necessary event handlers.

			// For simplicity, we'll just trigger a global re-init by dispatching
			// DOMContentLoaded. Chat.js is idempotent and won't re-initialize
			// existing containers.
