...
(function (window) {
	...
	function VADEngine(opts) {
		...
		this._silenceThresholdMs  = o.silenceThresholdMs;
		this._minSpeechDurationMs = o.minSpeechDurationMs;
		this._audioLevelThreshold = o.audioLevelThreshold;
	...
	VADEngine.prototype.init = function (stream) {
		...
		try {
			...
			this._log('VADEngine: initialised', {
				silenceThresholdMs:  this._silenceThresholdMs,
				minSpeechDurationMs: this._minSpeechDurationMs,
				audioLevelThreshold: this._audioLevelThreshold,
			});
			return true;
		} catch (err) {
	...
	VADEngine.prototype._check = function () {
		...
		try {
			...
			var isSpeech = dB > this._audioLevelThreshold;
			...
			if (isSpeech) {
				...
				if (speechDuration >= this._minSpeechDurationMs && silenceDuration >= this._silenceThresholdMs) {
