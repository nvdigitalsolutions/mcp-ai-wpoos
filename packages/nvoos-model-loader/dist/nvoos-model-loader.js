	...
	class ProgressiveModelLoader {
		...
		async checkModelCache(modelId) {
			...
			try {
				...
				console.warn('[nvoos-model-loader] Cache check failed:', error);
				return false;
			}
		}
		...
		updateStage(ui, stageIndex) {
			...
			ui.querySelector('.' + this.classNames.stage).textContent = stage.message;
		...
		updateProgress(ui, progress) {
			const fill = ui.querySelector('.' + this.classNames.progressFill);
			const text = ui.querySelector('.' + this.classNames.progressText);
		...
		updateDetails(ui, details) {
			ui.querySelector('.' + this.classNames.details).textContent = details;
		}
