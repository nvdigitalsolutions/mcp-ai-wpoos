...
**Coding-time skills (`.agents/skills/`) vs runtime skills:** The `.agents/skills/` directory contains **51** coding-time agent skills auto-discovered by the Zed editor (30 design-* skills + 20 wp-* WordPress plugin development patterns + 1 mcp-ai-wpoos-plugin operational guide). These are distinct from the runtime Agent Skills (74 base + 41 Pro bundled skills loaded by NV oOS assistants at runtime). The `.agents/skills/` files are WordPress plugin development patterns that guide coding agents, while the runtime skills (`includes/bundled-skills/`) are SKILL.md files loaded by AI assistants during conversations.
