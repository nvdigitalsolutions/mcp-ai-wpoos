...
| **Bundled with base** | `includes/bundled-skills/` — **74** general-purpose skills (up from 67 in v1.1.54): 29 design-* skills (analytics-reporting through web-research), mcp-ai-wpoos-plugin operational guide, plus existing Anthropic-authored skills (document handling, design, testing) + WordPress-developer skills (security, APIs, plugin patterns) + ui-ux-pro-max design system skill. |
...
**Coding-time skills (`.agents/skills/`) vs runtime skills:** The `.agents/skills/` directory contains **50** coding-time agent skills auto-discovered by the Zed editor (29 design-* skills + 20 wp-* WordPress plugin development patterns + 1 mcp-ai-wpoos-plugin operational guide). These are distinct from the runtime Agent Skills (74 base + 28+ Pro bundled skills loaded by NV oOS assistants at runtime). The `.agents/skills/` files are WordPress plugin development patterns that guide coding agents, while the runtime skills (`includes/bundled-skills/`) are SKILL.md files loaded by AI assistants during conversations.
