...
**New in v1.1.69 (wave 5, #6280–#6308):** no tools added or removed (the Pro coverage manifest only gains the four vision-analysis entries from #6267). Contract changes: `WP_MCP_AI_Enhanced_OpenAI_Client::create_chat_completion()` passes an empty model as the routing base so complexity-based light/advanced selection actually runs (feeding the settings default short-circuited it; routing-disabled calls fall back to the site default) (#6298); `WP_MCP_AI_Token_Budget_Manager::get_model_limit()` retries provider-prefixed slugs without their leading provider segment (`nvidia/nemotron-…`) (#6306); the Pro tool class→file map is cached in `$GLOBALS['wp_mcp_ai_pro_tools_map']` and exposed via `wp_mcp_ai_pro_get_tool_map()` so `WP_MCP_AI_Token_Usage_Service` enumerates gated-off Pro tools without rebuilding the map (#6300); JetEngine availability checks now require the `JET_ENGINE_VERSION` constant (file-scope test stubs leaked process-wide) across Shopify Sync, `get_jetengine_items`, and the JetEngine Pro tool, and the JetEngine Pro tool's `cct_table_exists()` probes with `SELECT 1 … LIMIT 1` instead of `SHOW TABLES` (MySQL 8.0's data-dictionary view misses transactional DDL) (#6300); `WP_MCP_AI_JetEngine_CCT::is_storage_available()` gates conversation-import writer/deleter/tool on the physical CCT table (#6296); the conversation-import tools join the tool presets (#6296); the skill-pack registry requires a pack slug and drops the empty-pack early return (#6302); the OpenAI client caps the resource-aware `max_tokens` default against the model's window/max-output (#6280); `generate_auth0_token` validates the audience by scheme/host syntax instead of `wp_http_validate_url()` (no DNS resolution) (#6301); `WP_MCP_AI_Tool_Image_Base` checks `user_can( $user_id, 'read_post', … )` against the acting user (#6305); `scrape_product` fixes CSS→XPath descendant/predicate ordering and `web_search` preserves `published_date` (Tavily/Brave) (#6306); the MCP controller's legacy HTTP+SSE handshake is filterable via `wp_mcp_ai_legacy_sse_enabled` (#6302).
