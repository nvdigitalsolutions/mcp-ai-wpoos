...
> **New in v1.1.72 (counts changed — +2 Pro):** two new WooCommerce tools — `update_woo_product_price` (regular/sale price, all product types) and `update_woo_product_qty` (stock quantity + management) — join the Pro E-commerce toolkit on a shared `WP_MCP_AI_Woo_Price_Qty_Updater` trait that `bulk_update_products` now also uses (#6388). Pro count → ~1,265, total → ~1,568. No base tool changes; the container's `tool_registry` binding is now transient (live-singleton resolution on every `get()`, #6339) — registry-side, no slug changes.
