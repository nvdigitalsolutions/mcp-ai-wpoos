...
> **New in v1.1.73 (counts unchanged — ~303 base + ~1,265 Pro, ~1,568 total):** both WooCommerce tool changes this window modify existing tools — no slug changes. `bulk_update_products` gains a `scope` argument (`all` default | `product` legacy): under `scope=all`, price/stock fields expand variable parents to their variations and grouped parents to their children via the shared `WP_MCP_AI_Woo_Price_Qty_Updater::resolve_update_targets()`, `WC_Product_Variable::sync()` re-syncs variable parents, and status/featured/category/tag updates always apply to the selected product itself; the response now reports `targets[]` per input ID plus `scope`/`updated_targets` keys (acknowledged shape change; canonical envelope unchanged — #6447). `update_woo_product_qty` gains a `notify` boolean (default `true`): `false` scopes `woocommerce_should_send_low/no_stock_notification` to `__return_false` for the write and removes them in a `finally`; the `woocommerce_low_stock`/`woocommerce_no_stock` actions still fire (#6448). Counts stay ~1,568.
