...
> **v1.1.69 updates (wave 5)**: **exec-disabled shell hardening** — calling a disabled function is a fatal `Error` on PHP 8+ that `@` cannot suppress; all Node.js probes must go through `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` (exec-first, `function_exists`-gated, Process Service fallback) and all OCR CLI calls through `is_cli_tool_available()` / `run_cli_command()`; never call `@exec()`/`shell_exec()` directly (PR #6295). **JetEngine stubs vs real plugin** — availability checks must require `JET_ENGINE_VERSION` (function/class stubs leak process-wide in tests); CCT existence probes use direct `SELECT` because `SHOW TABLES` misses transactional DDL on MySQL 8.0 (PRs #6296/#6300). **Acting-user permission checks** — `WP_MCP_AI_Tool_Image_Base` uses `user_can( $user_id, 'read_post', $attachment_id )` against the executing context user, never the global `current_user_can()` (cron/CLI/token contexts) (PR #6305); `safe_set_current_user()` validates positive ints via `filter_var` (absint flips negatives). **Mesh peer URLs** — validate the raw input with `filter_var( FILTER_VALIDATE_URL )` BEFORE `esc_url_raw()` so scheme-less strings are rejected instead of silently prefixed with `http://` (PR #6285). **Auth0 audience** — validate scheme/host syntax only; `wp_http_validate_url()` DNS-resolves and rejects non-publicly-resolvable identifiers (PR #6301). **Guest memory** — guest captures only land in the shared `guest` bucket behind the opt-in `wp_mcp_ai_memory_auto_capture_guests_allowed` filter (PR #6286).
