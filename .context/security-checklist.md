...
> **v1.1.57 updates**: Plugin updater rework — copy-in-place install with backup/rollback replaces `Plugin_Upgrader` (live directory never renamed; nonce-scoped base-update AJAX actions); Service Status provider detection via `WP_MCP_AI_Credential_Resolver` (settings + WP 7.0 Connectors + env + constants); Hermes WebUI chat async submit/poll.
