...
>
> **New in v1.1.69 (wave 5, #6280/#6302):** the REST validator now sanitizes `display.attachments` entries (URL via `esc_url_raw()`, label/downloadName/meta via `sanitize_text_field`) instead of dropping them; the markup REST controller annotates validation `WP_Error`s with `status 400` so the server surfaces client errors instead of a default 500; the MCP controller's legacy HTTP+SSE handshake reads `WP_MCP_AI_LEGACY_SSE_ENABLED` / the `wp_mcp_ai_legacy_sse_enabled` filter (mixed Accept headers that include application/json never trigger the legacy handshake).
>
