...
>
> **New in v1.1.69 (wave 5):** a fifth PHPUnit repair wave (~18 test PRs + grouped production fixes, #6280–#6308) followed the catch-up merge; the `mcp-ai-wpoos-test-suite` skill's cluster board now lists the 2026-09-04 session clusters (#6294) and a CI-triage artifact landed at `docs/developer/testing-docs/CI-TRIAGE-91771001271.md` (#6296). New production seams the suites depend on: Node.js/OCR CLI probes tolerate `exec`-disabled hosts (exec-first + Process Service fallback, `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` / `is_cli_tool_available()` / `run_cli_command()`) and settings sanitizers clamp with `max( 0, (int) $value )` (absint flips negatives) (#6295); JetEngine detection requires `JET_ENGINE_VERSION` and CCT existence probes use direct `SELECT` (SHOW TABLES misses transactional DDL on MySQL 8.0) (#6296/#6300); `WP_MCP_AI_Enhanced_OpenAI_Client` routes from an empty model base and `get_model_limit()` strips provider prefixes (`nvidia/…`) (#6298/#6306); the REST validator sanitizes `display.attachments` and the OpenAI client caps `max_tokens` against the model window (#6280); markup REST validation errors carry `status 400` and the legacy SSE handshake reads `wp_mcp_ai_legacy_sse_enabled` (#6302); GitHub OAuth redirect termination is filterable (`wp_mcp_ai_github_oauth_redirect_terminate`) and Auth0 audience validation does no DNS resolution (#6301); image permission checks use `user_can()` with the acting user and `safe_set_current_user()` uses `filter_var` (#6305); mesh peer URLs validate pre-sanitization (#6285); memory auto-capture writes guests only behind the opt-in filter (#6286); harness trace durations are millisecond-precise with a 1 ms floor and the logger allowlist adds agentic/token-DB events (#6308); Pro CPT AI integration exposes `reset_for_tests()` and the fine-tune curriculum exporter skips empty input/expected rows (#6287/#6293). Keep those seams when touching the corresponding production paths.
>
