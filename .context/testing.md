...
>
> **New in v1.1.87 (PRs #6776–#6786):** `test-mcp-wordpress-parity-tools.php` (PR #6777; 29 tests) + the L1/L2 contract suites (20 tests, 846 assertions on WP 6.9 + 7.1 — the new `comment_id`/`user_id`/`attachment_id` families). The image-identification cluster (PR #6780; 9 suites — `test-image-dhash.php`, `test-tool-{identify-image,get-image-metadata,find-similar-media,detect-image-content,describe-image-layout,ocr-image-classic,search-similar-images}.php` — 64 tests / 219 assertions incl. `test-vision-tools.php` regression; `test-tool-registry-coverage.php` re-validated at 3,142 assertions with both coverage manifests regenerated). `test-assistant-tool-presets.php` (PR #6785 — the five image tools join the media preset; 9 tests / 2,465 assertions — the root CI failure #6785 fixed). Outbound-booking suites (PR #6786; 8 suites / 41 tests under `addons/pro/tests/tools/outbound-booking/`). `test-restriction-registry.php` +7 (PR #6779 — per-user queue dedupe, distinct-user counts, lift pruning, render-time sweep). Session-budget tests (PR #6776; hook registration, one-shot flagging, once-only injection in the per-call/session-limit + hooks-chat-lifecycle suites). The `mcp-ai-wpoos-test-suite` skill stays at **50** patterns.
>
> **New in v1.1.86 (PRs #6761–#6773):** MCP Server remote-sites suites — `tests/mcp-apps/test-mcp-app-connection-refs.php` + `addons/pro/tests/test-remote-site-manager-mcp-server.php` (PR #6761; 93 tests across portability/registry/manager/admin/chat-exposure/slash-command suites on WP 6.9, 73 core on WP 7.1). `test-get-environment-status-tool.php` (PR #6767; 9/9 on both WP 6.9 + 7.1 — capability gate, warnings, provider-model resolution). `get_system_logs` filters (PR #6768; 8 new in `test-tool-get-system-logs.php` + 4 in `test-get-system-logs-validated-tool.php` + CG AI ecosystem extensions; 28/28 + six Logger suites 66/66 + CG AI standalone 3/3 on both versions). `test-tool-cost-envelope.php` (PR #6771; 6 — null cases, full envelope, estimated flag, omitted provider/model, zero cost). Higgsfield cluster (PR #6772; `test-higgsfield-{client,image-tool,request-tools,video-tool}.php` — registry coverage 56/56 + broader regression 121/121 on WP 6.9 + MySQL; the suite pins the `sanitize_text_field`-not-`sanitize_key` model-ID rule). Action-items/smart-excerpt (PR #6773; `test-pro-result-delivery-chat-format.php` + `test-pro-result-delivery-email-format.php` extended). The `mcp-ai-wpoos-test-suite` skill stays at **50** patterns.
>
> **New in v1.1.85 (PRs #6749–#6759):** the MCP Apps wave adds four suites under `tests/mcp-apps/`: `test-mcp-apps-connection-enhancements.php` (PR #6753, 14 tests — basic-auth encoding/passthrough, session capture + echo, RPC-error decoding, discover→initialize fallback, status persistence + write-skipping, allowlist merge, REST handlers with stored-token resolution; plus `tests/test-security-center.php` field wiring + validation), `test-mcp-app-client-connection-enhancements.php`, `test-mcp-app-registry-connection-enhancements.php`, and `test-rest-mcp-apps-connection-enhancements.php` (PR #6753; see the one-class-per-file lesson below), `test-mcp-app-chat-exposure.php` (PRs #6755/#6758 — the `wp_mcp_ai_chat_effective_tools` seam: exposure merges slugs + registers the bridge, assistant-ID resolution, `handle_tools_list()` inclusion, `rest_pre_dispatch` registration), and `test-mcp-app-client-internal-bridge.php` (PRs #6756/#6757, 4 tests — zero-outbound-HTTP in-process dispatch with auth/session header round-trip, fallback paths, disable filter; plus the `rest_post_dispatch` session-header regression). **One-class-per-file discovery lesson (PR #6754):** PHPUnit 11 discovers only the **first** class in a test file — the #6753 registry/REST suites were split into one class per file and had never executed; always one test class per file. Validation ran on both WP 6.9 and WP 7.1 (60/60 → 70/70 → 85/85 → 77/77 as the wave merged). The `mcp-ai-wpoos-test-suite` skill stays at **50** patterns.
>
