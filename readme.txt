...
Parity suite, image identification, and outbound booking release. Added: a 30-tool mcp-wordpress parity suite ported from the MIT upstream into the base plugin (#6777) — comment CRUD (list/get/create/update/delete_comments), user CRUD (list/create/update/delete_users), content/media/terms (get_post_revisions, get_term, delete_term, get_media, upload_media, update_media, delete_media), site settings + application passwords (get/update_site_settings allowlist-only, list/create/delete_application_passwords with log masking), and a 9-tool SEO toolkit (seo_analyze_content, seo_generate_schema, seo_validate_schema, seo_bulk_update_metadata, seo_site_audit, seo_test_integration, seo_get_live_data, seo_track_serp, seo_keyword_research) with capability checks, multisite guards, the chat-client restriction trait on sensitive write tools, and new comment_id/user_id/attachment_id ID-handoff contracts; a non-LLM image identification ladder (Proposal 043, #6780/#6785) — identify_image orchestrates metadata → perceptual-hash media lookup → classic Cloud Vision detection → deterministic layout description → optional reverse-image web search without ever calling a vision LLM, with five new base tools (identify_image, get_image_metadata, find_similar_media, detect_image_content, describe_image_layout) joining the Media Generation preset and two new Pro tools (ocr_image_classic tesseract-only OCR, search_similar_images Bing Visual Search / SerpApi Google Lens) on a shared WP_MCP_AI_Cloud_Vision_Client; every external rung is key-gated and fails closed (skipped, never an HTTP request without credentials; SSRF-guarded); an outbound appointment booking Pro toolkit (Proposal 044, #6786) gated by enable_outbound_booking_toolkit — ICP list building, hourly sequence engine with send windows/daily caps/consent attestation, email + LinkedIn/Instagram DM channels behind a human approval board, the mcp_ai_oa_angle CPT angle bank with A/B tests, booking links + shortcode + rate-limited honeypotted consent-gated public booking endpoint, and three new Pro tools (outbound_get_pipeline_stats, outbound_import_leads, outbound_manage_angle); and the mcp-wordpress gateway addon (#6778) — an auth-gated Streamable HTTP MCP server for Cloudways Velocity (timing-safe X-MCP-Token auth, 1 MB body cap, deny-wins tool policy, subtree-split deploy to the nvoos-mcp-wordpress mirror). Fixed: the per-session tool-token budget is now visible — blocked-session messages report session state and point at the real reset path, the dormant 75% wp_mcp_ai_session_limit_approaching hook is wired to a one-shot warning on the next chat turn via the new wp_mcp_ai_chat_messages filter, and the Token Manager reset semantics are documented (#6776); the "N users have been restricted" admin notice no longer over-counts (one entry per user, distinct-user counting) and no longer lingers after restrictions are lifted (render-time sweep + lift pruning, Pro command-center banner fixed too, #6779). Changed: update_pro_schedule now edits workflow steps in place and search_upwork_jobs gains exclude_keywords, an all sentinel, quoted-OR skills, tier/budget parsing, and criteria echoing, with a byte-identical CG Pro port (#6784). Docs & skills: the two untracked Elementor bundled skills are committed (75 base bundled skills now genuinely tracked), proposals 043/044 ship, the new .context/image-identification.md lands, and the design-pro-schedule-manager + design-crm coding-time skills document the new surfaces. Sub-projects: content-graph explorer motion/UX polish + remote-source driver fixes on its own track (1.0.8 unchanged). Tool count: ~347 base + ~1,287 Pro (~1,634 total; +35 base +5 Pro; live registry authoritative). Model catalog: v2026.09.22 (unchanged). Stale build ZIPs removed: the 1.1.85 build set (30 files).
...
Tool guidance, Jev decisions, Pro bootstrap hardening, and Upwork-workflow release. Added: every base+Pro tool class now carries model-facing usage guidance — a new usage-guidance interface (when_to_use / when_not_to_use / related_tools / notes) assembles a compact [Usage] suffix on the model-facing payload, legacy-format classes opt in through the legacy tool wrapper, and the Phase 2 sweep completes the full base+pro tree (1,584/1,584 files clean at the enforced severity-5 guidance sniff, so new tool classes without guidance surface on PRs; the post-window close-out adds the last three webchat tools and the CG Pro member mirrors, #6740); an opt-in adaptive tool cap (per-site option plus a per-assistant three-state override) sizes the tools sent per model context window; list_mcp_tools gains tool_slug and include_schemas for lazy schema loading; the TypeSafe Jev "System One" decision provider joins as a first-class decision provider (typed choice/score/noul decisions in 70-500 ms; new base tool typesafe_decide with the canonical envelope and an adversarial-state caveat, OpenRouter decisions bridge, TypeSafe settings subtab, Pro cascade routing via the Jev classifier and an opt-in research source filter, #6728); "The Assistant Builder" meta-assistant seeds on activation and designs/builds/verifies other assistants (default roster now 7, #6727); ID-handoff data contracts now annotate every ID-bearing tool family (post, cron, term, assistant, vector store, batch, Pro schedule, toolkit_cpt, medical record, plan, calendar, WPCode, session, member, webchat room) with produces/consumes handoffs, enforced by permanent manifest-driven honesty and round-trip test suites (#6729-#6739); a Pro bootstrap incomplete-install guard keeps the site running on the base plugin when a partial Pro deploy leaves required files missing (admin notice instead of a site-wide fatal); send_telegram_message auto-chunks messages over 4,096 characters (paragraph, line, then hard splits; parse_mode dropped on hard splits; chunk=false restores legacy single-send). Fixed: the Usage Monitor sub-tab settings save is no longer a silent no-op (the dashboard save handler now applies the monitor bridge filter, #6726); Upwork job search now honors web_search-mode and API credential gates (no more missing-refresh-token failures on scheduled discovery scans), drops category landing pages, extracts budget/job type/recency from search snippets, always runs a second broad pass when the first is thin, and gains sort/location arguments; workflow result deliveries now carry the full result set (50-item cap instead of 5, filterable) with per-item URLs, budget, contract type, and recency, rendered as a single properly numbered list; workflow full deliveries render steps as a compact execution log instead of raw flattened payloads; the literal connection_id "settings" now resolves to the settings-based credential fallback across the Pro Gmail tools, the Drive client, and the base search_gmail tool; load_skill and OKF bundle errors append the available assignments/bundles for self-correction; five webchat tools no longer hit a latent fatal on their unavailable branch (log_activity is now log_event, #6738) and get_webchat_messages SQL prepare warnings are fixed at the root (#6740); the Playground Ollama demo seed sets pretty permalinks so the landing page no longer 404s on fresh installs, and the local npx run is now the documented primary test path; npm advisories closed (adm-zip 0.6.1, js-yaml 4.3.2, colord 2.10.0). Changed: five regulatory tools migrate to the canonical WP_Error envelope; format_code_prettier failure returns migrate to the canonical envelope. Tool count: ~307 base + ~1,279 Pro (~1,586 total; +1 base — typesafe_decide; live registry authoritative). Stale build ZIPs removed: the 1.1.81 set (30 files), then the 1.1.82 set (30 files) after the 1.1.83 wp.org packages rebuilt.
...
Bumped to 1.1.57 across plugin header, WP_MCP_AI_VERSION and WP_MCP_AI_PRO_VERSION constants, readme.txt Stable tag, README.md, CHANGELOG.md, QUICK_REFERENCE.md, and DOCUMENTATION_INDEX.md. Plugin updater rework: new base-only update flow so WordPress.org base installs can update in place without upgrading to the complete build (new "Base Version" panel in Settings → Advanced), copy-in-place installation with backup and rollback replacing Plugin_Upgrader (ZIP folder-name mismatches no longer break updates), and Pro version read from the plugin header instead of the manually maintained constant. Hermes WebUI chat reworked to async submit/poll with approval-gate answers and a still_running status on timeout expiry; answer extraction fixed for live WebUI payloads. Service Status AI-provider detection now resolves credentials through the canonical credential resolver (plugin settings, WP 7.0 Connectors, environment variables, PHP constants). New fleet-operator addon agent-context tree (18 files, 6 operator roles). Post-release fixes: attention routing no longer truncates assistant tool lists (activates only above the 100-tool chat payload cap, returns all tools when scoring is unavailable, and scores against the actual user query); MCP prompts/list and prompts/get scoped to the authenticated assistant; MCP tools/call completes non-background tools synchronously instead of exhausting the async poll budget; queue worker standalone-CLI fatals and empty-queue null envelope fixed; media-worker puppeteer ^25.7.0 and uuid override (GHSA-jmr9-qjv8-65gv, GHSA-w5hq-g745-h8pq) with Node engine floor >=22.12.0.
