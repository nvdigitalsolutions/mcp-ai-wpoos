...
Non-LLM image identification ladder (Proposal 043). Added: a shared WP_MCP_AI_Cloud_Vision_Client service extracted from vision_object_localization (same endpoint, key, and error contract); five new base tools — detect_image_content (classic Cloud Vision labels/text/web-entities/logos/landmarks/faces/safe-search with a normalized envelope), find_similar_media (pure-PHP dHash media-library lookup with post-meta caching and a daily backfill cron), get_image_metadata (alt/title/caption/EXIF/dimensions, no external calls), describe_image_layout (bounding boxes → deterministic text layout description for text-only models), and identify_image (cheap-first orchestration of the ladder with confidence + escalation hints; never calls a vision LLM); two new Pro tools — ocr_image_classic (tesseract-only OCR via the OCR service, worker tesseract.js → system tesseract) and search_similar_images (Bing Visual Search / SerpApi Google Lens reverse-image search with new Vision Analysis settings keys). External rungs are key-gated and skip cleanly without credentials so image bytes never leave the server unconfigured. Tool count: +5 base (~317 base), +2 Pro (~1,284 Pro).
