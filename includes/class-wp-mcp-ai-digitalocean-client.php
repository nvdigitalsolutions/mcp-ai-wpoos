...
if ( ! class_exists( 'WP_MCP_AI_DigitalOcean_Client' ) ) {
			...
			if ( $is_streaming && function_exists( 'curl_init' ) ) {
				...
				if ( null !== $realtime_cb ) {
					if ( class_exists( 'WP_MCP_AI_Logger' ) ) {
						WP_MCP_AI_Logger::log_event(
							'digitalocean_request',
							'Sending real-time streaming request to DigitalOcean via cURL.',
	 ...
	 * each network chunk as it arrives from DigitalOcean.  Every `delta.content`
	...
	protected function do_realtime_curl_stream( $url, array $payload, $model, $timeout, $stream_callback ) {
		...
		if ( $curl_errno ) {
			if ( class_exists( 'WP_MCP_AI_Logger' ) ) {
				WP_MCP_AI_Logger::log_error(
					'DigitalOcean real-time streaming failed.',
		...
		if ( $http_status >= 400 ) {
			if ( class_exists( 'WP_MCP_AI_Logger' ) ) {
				WP_MCP_AI_Logger::log_error(
					'DigitalOcean real-time streaming returned HTTP error.',
					array( 'code' => $http_status )
				);
			}
			return new WP_Error(
				'wp_mcp_ai_api_error',
				__( 'DigitalOcean returned an error during streaming.', 'mcp-ai-wpoos' ),
		...
		if ( class_exists( 'WP_MCP_AI_Logger' ) ) {
			WP_MCP_AI_Logger::log_event( 'digitalocean_realtime_stream', 'Real-time streaming response assembled.', array( 'model' => $model ) );
		}
