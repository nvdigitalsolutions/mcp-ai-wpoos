...
if ( ! class_exists( 'WP_MCP_AI_DigitalOcean_Client' ) ) {
	...
	class WP_MCP_AI_DigitalOcean_Client {
		...
		public function create_chat_completion( array $messages, array $options = array() ) {
			...
			if ( class_exists( 'WP_MCP_AI_Provider_Circuit_Breaker' ) && ! WP_MCP_AI_Provider_Circuit_Breaker::is_allowed( 'digitalocean' ) ) {
				return new WP_Error(
					'provider_circuit_open',
					__( 'DigitalOcean API is temporarily unavailable due to repeated failures. Please try again shortly.', 'mcp-ai-wpoos' ),
					array(
						'status'      => 503,
						'retry_after' => 60,
					)
			...
			if ( class_exists( 'WP_MCP_AI_Provider_Circuit_Breaker' ) ) {
				if ( is_wp_error( $response ) ) {
					WP_MCP_AI_Provider_Circuit_Breaker::record_failure( 'digitalocean' );
				} else {
					$cb_status = wp_remote_retrieve_response_code( $response );
					if ( $cb_status >= 500 ) {
						WP_MCP_AI_Provider_Circuit_Breaker::record_failure( 'digitalocean' );
					} else {
						WP_MCP_AI_Provider_Circuit_Breaker::record_success( 'digitalocean' );
					}
				}
			}
