 ...
 */
class WP_MCP_AI_Mesh_Peer_Tester {
	...
	public static function test_connection( $peer ) {
		...
		$results['details']['reachability'] = array(
			'status'  => 'success',
			'message' => __( 'Site is reachable.', 'mcp-ai-wpoos' ),
		);

		if ( ! is_wp_error( $wellknown ) ) {
			$results['details']['wellknown'] = array(
				'status'  => 'success',
				'message' => __( 'Federation well-known endpoint accessible.', 'mcp-ai-wpoos' ),
			);
		} else {
			$results['details']['wellknown'] = array(
				'status'  => 'warning',
		...
		if ( ! empty( $api_key ) ) {
			if ( ! is_wp_error( $mcp_auth ) ) {
				$results['details']['authentication'] = array(
					'status'  => 'success',
					'message' => __( 'API key authentication successful.', 'mcp-ai-wpoos' ),
				);
			} else {
				$results['details']['authentication'] = array(
					'status'  => 'error',
			...
			$results['details']['authentication'] = array(
				'status'  => 'skipped',
				'message' => __( 'No API key provided.', 'mcp-ai-wpoos' ),
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Site is not reachable: %s', 'mcp-ai-wpoos' ),
		...
		if ( $status_code < 200 || $status_code >= 400 ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Site returned HTTP status %d', 'mcp-ai-wpoos' ),
					$status_code
				)
			);
		}
	...
	protected static function test_wellknown_endpoint( $url ) {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'Well-known endpoint not accessible: %s', 'mcp-ai-wpoos' ),
					$response->get_error_message()
				)
			);
		}
		...
		if ( 200 !== $status_code ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %d: HTTP status code */
					__( 'Well-known endpoint returned status %d', 'mcp-ai-wpoos' ),
					$status_code
				)
			);
		}
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error(
				'wellknown_invalid',
				__( 'Well-known endpoint returned invalid JSON.', 'mcp-ai-wpoos' )
			);
		}
	...
	protected static function test_mcp_authentication( $url, $api_key ) {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'MCP authentication failed: %s', 'mcp-ai-wpoos' ),
					$response->get_error_message()
				)
			);
		}
		...
		if ( 404 === $status_code ) {
			return new WP_Error(
				'mcp_not_found',
				__( 'MCP endpoint not found. The remote site may not have the plugin installed.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function update_peer_test_status( $mesh_peer_id, $test_results ) {
		...
		if ( isset( $test_results['success'] ) && $test_results['success'] ) {
			if ( isset( $test_results['authenticated'] ) && $test_results['authenticated'] ) {
				update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_HEALTH_STATUS, 'healthy' );
			} else {
				update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_HEALTH_STATUS, 'degraded' );
			}
		} else {
			update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_HEALTH_STATUS, 'down' );
		}

		// Store last test timestamp.
		update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_LAST_VERIFIED, current_time( 'mysql', true ) );
