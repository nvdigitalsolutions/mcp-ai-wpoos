<?php
/**
 * Ollama Provider Client Adapter
 *
 * Implements Interface_WP_MCP_AI_Provider_Client by delegating to the
 * concrete WP_MCP_AI_Ollama_Client class.
 *
 * @package WP_MCP_AI
 * @since   1.2.0
...
/**
 * Ollama implementation of the Provider Client interface.
 *
 * @since 1.2.0
 */
class WP_MCP_AI_Ollama_Provider_Client implements Interface_WP_MCP_AI_Provider_Client {
	 ...
	 * Constructor.
	 *
	 * @param WP_MCP_AI_Ollama_Client|null $client Optional concrete client. Defaults to a new instance.
	 */
	public function __construct( $client = null ) {
		$this->client = $client instanceof WP_MCP_AI_Ollama_Client ? $client : new WP_MCP_AI_Ollama_Client();
	}
	 ...
	 *
	 * Ollama streaming is handled via the `stream => true` option within
	 * `create_chat_completion`.  The callback is forwarded via options so that
	 * any stream-aware extension of WP_MCP_AI_Ollama_Client can consume it.
	 *
	 * @param array    $messages  Conversation messages.
	 * @param array    $options   Provider-specific options.
