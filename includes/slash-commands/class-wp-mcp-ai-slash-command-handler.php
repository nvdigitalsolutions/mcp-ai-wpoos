 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Slash_Command_Handler {
		...
		if ( ! isset( $this->commands[ $command ] ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Command "/%s" not found. Use /help to see available commands.', 'mcp-ai-wpoos' ),
		...
		try {
			...
			$error = new WP_Error(
				...
				sprintf(
					...
					__( 'Command execution failed: %s', 'mcp-ai-wpoos' ),
					$e->getMessage()
				)
			);
	...
	private function check_authorization( $command, $config, $context ) {
		...
		if ( ! user_can( $user_id, $config['capability'] ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'You do not have permission to execute /%1$s (requires %2$s capability).', 'mcp-ai-wpoos' ),
					esc_html( $command ),
		...
		if ( ! $authorized ) {
			return new WP_Error(
				...
				__( 'You are not authorized to execute this command.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( $count >= $limit ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Rate limit exceeded. Maximum %d commands per minute allowed.', 'mcp-ai-wpoos' ),
					$limit
				)
			);
		}
