 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Slash_Command_Next_Task {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error(
				...
				__( 'You do not have permission to manage content tasks.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $tasks ) ) {
			return $this->format_response(
				'success',
				__( 'No tasks found! Your site is in great shape.', 'mcp-ai-wpoos' ),
				array( 'tasks' => array() )
			);
		}
		...
		if ( $dry_run ) {
			return $this->format_response(
				'success',
				__( 'Task plan (dry run - no changes made)', 'mcp-ai-wpoos' ),
				array(
					'plan'  => $plan,
					'tasks' => $tasks,
				)
		...
		if ( ! $auto_fix ) {
			return $this->format_response(
				'approval_required',
				__( 'Task plan ready for review. Use --auto flag to execute without approval.', 'mcp-ai-wpoos' ),
				array(
					'plan'  => $plan,
					'tasks' => $tasks,
				)
		...
		return $this->format_response(
			...
			sprintf(
				...
				__( 'Completed %d task(s) successfully.', 'mcp-ai-wpoos' ),
				count( $results )
			),
			array(
				'plan'       => $plan,
		...
		foreach ( $drafts as $draft ) {
			$tasks[] = array(
				...
				'description' => sprintf(
					...
					__( 'Publish draft post: %s', 'mcp-ai-wpoos' ),
		...
		foreach ( $posts as $post ) {
			...
			if ( empty( $meta_desc ) ) {
				$tasks[] = array(
					...
					'description' => sprintf(
						/* translators: %s: post title */
						__( 'Add meta description to: %s', 'mcp-ai-wpoos' ),
	...
	private function find_outdated_content( $options ) {
		...
		foreach ( $posts as $post ) {
			$tasks[] = array(
				...
				'description' => sprintf(
					/* translators: %s: post title */
					__( 'Update outdated content: %s', 'mcp-ai-wpoos' ),
					get_the_title( $post->ID )
				),
				'effort'      => 'high',
	...
	private function estimate_time( $tasks ) {
		...
		return sprintf(
			...
			__( '%1$d hour(s) %2$d minutes', 'mcp-ai-wpoos' ),
			$hours,
			$mins
		);
	}
