 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Slash_Command_Ship {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $user_id, 'publish_posts' ) ) {
			return new WP_Error(
				...
				__( 'You do not have permission to publish posts.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $post_ids ) ) {
			...
			if ( empty( $post_ids ) ) {
				return $this->format_response(
					'info',
					__( 'No posts ready to ship. All clear!', 'mcp-ai-wpoos' ),
					array()
				);
			}
		}
		...
		return $this->format_response(
			...
			sprintf(
				...
				__( 'Processed %d post(s) for shipping.', 'mcp-ai-wpoos' ),
				count( $results )
			),
			array(
				'results' => $results,
	...
	private function run_preflight_checks( $post ) {
		...
		if ( has_post_thumbnail( $post->ID ) ) {
			...
			$checks['issues'][] = __( 'Missing featured image', 'mcp-ai-wpoos' );
		}
		...
		if ( ! empty( $categories ) ) {
			...
			$checks['issues'][] = __( 'No categories assigned', 'mcp-ai-wpoos' );
		}
		...
		if ( ! empty( $post->post_excerpt ) ) {
			...
			$checks['issues'][] = __( 'Missing excerpt', 'mcp-ai-wpoos' );
		}
		...
		if ( $checks['word_count'] < 300 ) {
			$checks['issues'][] = sprintf(
				...
				__( 'Content too short (%d words, minimum 300 recommended)', 'mcp-ai-wpoos' ),
				$checks['word_count']
			);
		}
	...
	private function run_seo_checks( $post ) {
		...
		if ( ! empty( $yoast_title ) || ! empty( $rank_math_title ) ) {
			...
			$checks['issues'][] = __( 'Missing SEO meta title', 'mcp-ai-wpoos' );
		}
		...
		if ( ! empty( $yoast_desc ) || ! empty( $rank_math_desc ) ) {
			...
			$checks['issues'][] = __( 'Missing SEO meta description', 'mcp-ai-wpoos' );
		}
	...
	private function run_quality_checks( $post ) {
		...
		if ( preg_match( '/<h[2-6][^>]*>/i', $content ) ) {
			...
			$checks['issues'][] = __( 'No headings (H2-H6) found in content', 'mcp-ai-wpoos' );
		}
		...
		if ( preg_match( '/<img[^>]+>/i', $content ) ) {
			...
			$checks['issues'][] = __( 'No images in content', 'mcp-ai-wpoos' );
		}
	...
	private function run_image_checks( $post ) {
		...
		if ( $checks['missing_alt_text'] > 0 ) {
			$checks['issues'][] = sprintf(
				...
				__( '%d image(s) missing alt text', 'mcp-ai-wpoos' ),
				$checks['missing_alt_text']
			);
		}
	...
	private function suggest_internal_links( $post ) {
		...
		if ( $suggestions['found_links'] < 2 ) {
			$suggestions['issues'][] = sprintf(
				...
				__( 'Only %d internal link(s) found (recommend at least 2-3)', 'mcp-ai-wpoos' ),
		...
		if ( ! empty( $options['schedule'] ) ) {
			...
			if ( $schedule_date && $schedule_date > time() ) {
				...
				return new WP_Error(
					...
					__( 'Invalid schedule date. Must be in the future.', 'mcp-ai-wpoos' )
				);
			}
		} else {
