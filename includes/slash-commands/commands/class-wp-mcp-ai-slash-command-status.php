...
/**
 * Class WP_MCP_AI_Slash_Command_Status
 *
 * @since 2.1.0
 */
class WP_MCP_AI_Slash_Command_Status {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error( 'insufficient_capability', __( 'You do not have permission to use /status.', 'mcp-ai-wpoos' ) );
		}
	...
	private function get_async_health() {
		if ( ! class_exists( 'WP_MCP_AI_Async_Health_Monitor' ) ) {
			return array(
				'available' => false,
				'status'    => 'unknown',
				'message'   => __( 'Async health monitor not available.', 'mcp-ai-wpoos' ),
			);
		}
	...
	private function get_job_counts( $user_id ) {
		if ( ! class_exists( 'WP_MCP_AI_Cron_Status_Service' ) ) {
			return array(
				'available' => false,
				'message'   => __( 'Cron status service not available.', 'mcp-ai-wpoos' ),
			);
		}
		...
		$out       .= sprintf(
			...
			$tools_ok
				? sprintf(
					...
					__( '%d tools registered', 'mcp-ai-wpoos' ),
					$tool_count
				)
				: __( 'unavailable', 'mcp-ai-wpoos' )
		);
