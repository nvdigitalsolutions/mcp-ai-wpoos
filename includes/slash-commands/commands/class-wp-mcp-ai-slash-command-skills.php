...
/**
 * Class WP_MCP_AI_Slash_Command_Skills
 *
 * @since 2.1.0
 */
class WP_MCP_AI_Slash_Command_Skills {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error( 'insufficient_capability', __( 'You do not have permission to use /skills.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( $install_slug ) {
			if ( ! user_can( $user_id, 'manage_options' ) ) {
				return new WP_Error( 'insufficient_capability', __( 'manage_options capability is required to install skill packs.', 'mcp-ai-wpoos' ) );
			}
	...
	private function install_pack( $slug ) {
		if ( ! class_exists( 'WP_MCP_AI_Skill_Pack_Registry' ) ) {
			return new WP_Error( 'service_unavailable', __( 'Skill Pack Registry is not available.', 'mcp-ai-wpoos' ) );
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Skill pack `%s` installed successfully.', 'mcp-ai-wpoos' ),
				esc_html( $slug )
			),
	...
	private function show_pack( $slug, $as_json ) {
		...
		if ( ! $pack ) {
			...
			if ( $skill_file ) {
				$pack = array(
					'slug'        => $slug,
					'name'        => $slug,
					'description' => __( 'Bundled skill.', 'mcp-ai-wpoos' ),
					'source'      => 'bundled',
		...
		if ( ! $pack ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: skill pack slug */
					__( 'Skill pack "%s" not found.', 'mcp-ai-wpoos' ),
					esc_html( $slug )
				)
			);
		}
	...
	private function render_list( array $packs ) {
		if ( empty( $packs ) ) {
			return __( '_No skill packs found._', 'mcp-ai-wpoos' );
		}
