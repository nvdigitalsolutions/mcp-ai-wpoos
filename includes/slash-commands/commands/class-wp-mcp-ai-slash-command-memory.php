 ...
 */
class WP_MCP_AI_Slash_Command_Memory {
	...
	protected function check_enabled( $context ) {
		...
		if ( ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error(
				...
				__( 'You do not have permission to use memory commands.', 'mcp-ai-wpoos' )
			);
		}
	...
	public function remember( $args, $flags, $context ) {
		...
		if ( '' === $text ) {
			return new WP_Error( 'missing_text', __( 'Usage: /remember <text>', 'mcp-ai-wpoos' ) );
		}

		$content = trim( wp_kses_post( $text ) );
		$title   = mb_substr( wp_strip_all_tags( $content ), 0, 80 );
		if ( '' === $title ) {
			$title = __( 'Untitled memory', 'mcp-ai-wpoos' );
		}
		...
		if ( ! $tool ) {
			return new WP_Error( 'tool_unavailable', __( 'Memory tool is not registered on this site.', 'mcp-ai-wpoos' ) );
		}
		...
		return array(
			'type'    => 'memory.stored',
			'message' => __( '🧠 Memory stored.', 'mcp-ai-wpoos' ),
	...
	public function forget( $args, $flags, $context ) {
		...
		if ( '' === $context_id ) {
			return new WP_Error( 'missing_id', __( 'Usage: /forget <context_id>', 'mcp-ai-wpoos' ) );
		}

		$registry = WP_MCP_AI_Tool_Registry::get_instance();
		$tool     = $registry->get_tool( 'manage_context_lifecycle' );
		if ( ! $tool ) {
			return new WP_Error( 'tool_unavailable', __( 'Memory lifecycle tool is not registered on this site.', 'mcp-ai-wpoos' ) );
		}
		...
		return array(
			'type'    => 'memory.deleted',
			'message' => sprintf( /* translators: %s: context ID */ __( '🧠 Forgot memory %s.', 'mcp-ai-wpoos' ), $context_id ),
			'data'    => is_array( $result ) ? $result : array( 'result' => $result ),
		);
	}
	...
	public function scope( $args, $flags, $context ) {
		...
		if ( empty( $args ) ) {
			return array(
				...
				'message' => __( '🧠 Scope cleared.', 'mcp-ai-wpoos' ),
				'data'    => array(
					'wing' => '',
					'room' => '',
				),
		...
		return array(
			...
			'message' => '' === $room
				? sprintf( /* translators: %s: wing name */ __( '🧠 Active scope: wing "%s".', 'mcp-ai-wpoos' ), $wing )
				: sprintf( /* translators: 1: wing name, 2: room name */ __( '🧠 Active scope: wing "%1$s" / room "%2$s".', 'mcp-ai-wpoos' ), $wing, $room ),
			'data'    => array(
				'wing' => $wing,
				'room' => $room,
			),
