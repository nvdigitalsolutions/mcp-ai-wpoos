...
/**
 * Class WP_MCP_AI_Slash_Command_Preset
 *
 * @since 2.1.0
 */
class WP_MCP_AI_Slash_Command_Preset {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error( 'insufficient_capability', __( 'You do not have permission to use /preset.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( $apply_id ) {
			if ( ! user_can( $user_id, 'manage_options' ) ) {
				return new WP_Error( 'insufficient_capability', __( 'manage_options capability is required to apply a preset.', 'mcp-ai-wpoos' ) );
			}
	...
	private function apply_preset( $preset_id ) {
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Orchestration preset `%s` has been applied.', 'mcp-ai-wpoos' ),
				esc_html( $preset_id )
			),
	...
	private function show_active_preset( $as_json ) {
		...
		if ( ! $active ) {
			return array(
				'success' => true,
				'message' => __( '_No active preset. Using defaults._', 'mcp-ai-wpoos' ),
				'data'    => array(),
			);
		}
	...
	private function show_preset( $preset_id, $as_json ) {
		...
		if ( ! is_array( $presets ) ) {
			return new WP_Error( 'presets_unavailable', __( 'Could not load presets.', 'mcp-ai-wpoos' ) );
		}
	...
	private function list_presets( $as_json ) {
		...
		if ( empty( $presets ) ) {
			return array(
				'success' => true,
				'message' => __( '_No presets found._', 'mcp-ai-wpoos' ),
				'data'    => array(),
			);
		}
