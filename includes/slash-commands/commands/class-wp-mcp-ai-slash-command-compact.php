 ...
 * @since 1.5.0
 */
class WP_MCP_AI_Slash_Command_Compact {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! in_array( $strategy, $valid_strategies, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid strategy. Valid options: %s', 'mcp-ai-wpoos' ),
		...
		if ( $message_count < 3 ) {
			return array(
				'success' => true,
				'message' => __( 'Context is already compact. No action needed.', 'mcp-ai-wpoos' ),
				'data'    => array(
	...
	private function strategy_summarize( $messages, $keep_recent, $assistant_id ) {
		...
		if ( empty( $older_messages ) ) {
			return array(
				'success' => true,
				'message' => __( 'No older messages to summarize.', 'mcp-ai-wpoos' ),
				'data'    => array(
					'messages_before' => $message_count,
					'messages_after'  => $message_count,
					'strategy'        => 'summarize',
		...
		$summary = __( '**[Context Summary]** ', 'mcp-ai-wpoos' );
		...
		if ( ! empty( $topics ) ) {
			...
			if ( ! empty( $unique_topics ) ) {
				$summary .= sprintf(
					...
					__( 'Topics discussed: %s. ', 'mcp-ai-wpoos' ),
		...
		$summary .= sprintf(
			...
			__( '(%d messages compacted)', 'mcp-ai-wpoos' ),
	...
	private function strategy_trim_tools( $messages, $keep_recent, $assistant_id ) {
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Trimmed %1$d tool messages (~%2$s tokens saved). Strategy: trim-tools.', 'mcp-ai-wpoos' ),
				$removed,
				number_format_i18n( $tokens_saved )
			),
			'data'    => array(
	...
	private function strategy_keep_recent( $messages, $keep_recent, $assistant_id ) {
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Kept %1$d recent messages, dropped %2$d (~%3$s tokens saved). Strategy: keep-recent.', 'mcp-ai-wpoos' ),
				count( $compacted ),
				$removed,
				number_format_i18n( $tokens_saved )
			),
	...
	private function strategy_full_compact( $messages, $assistant_id ) {
		...
		$summary = __( '**[Full Session Summary]** ', 'mcp-ai-wpoos' );

		$unique_topics = array_unique( array_filter( $topics ) );
		if ( ! empty( $unique_topics ) ) {
			$summary .= sprintf(
				/* translators: %s: list of topics */
				__( 'Session covered: %s. ', 'mcp-ai-wpoos' ),
		...
		if ( ! empty( $tools_used ) ) {
			...
			$summary .= sprintf(
				...
				__( 'Tools invoked: %s. ', 'mcp-ai-wpoos' ),
		...
		if ( ! empty( $key_facts ) ) {
			$summary .= sprintf(
				...
				__( 'Key outcomes: %s. ', 'mcp-ai-wpoos' ),
		...
		$summary .= sprintf(
			...
			__( 'Original session: %d messages.', 'mcp-ai-wpoos' ),
			$message_count
		);
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Full compact: %1$d → %2$d messages (~%3$s tokens saved). Strategy: full.', 'mcp-ai-wpoos' ),
				$message_count,
				count( $compacted ),
				number_format_i18n( $tokens_saved )
			),
