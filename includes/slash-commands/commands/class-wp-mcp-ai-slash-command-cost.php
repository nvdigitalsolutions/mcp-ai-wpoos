...
/**
 * Class WP_MCP_AI_Slash_Command_Cost
 *
 * @since 2.1.0
 */
class WP_MCP_AI_Slash_Command_Cost {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $current_user_id, 'edit_posts' ) ) {
			return new WP_Error( 'insufficient_capability', __( 'You do not have permission to use /cost.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( $target_uid !== $current_user_id && ! user_can( $current_user_id, 'manage_options' ) ) {
			return new WP_Error( 'insufficient_capability', __( 'manage_options capability is required to view another user\'s costs.', 'mcp-ai-wpoos' ) );
		}
