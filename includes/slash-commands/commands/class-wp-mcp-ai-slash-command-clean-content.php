 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Slash_Command_Clean_Content {
	...
	public function execute( $args, $flags, $context ) {
		...
		if ( ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error(
				...
				__( 'You do not have permission to clean content.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $post_ids ) ) {
			return $this->format_response(
				'success',
				__( 'No posts to check.', 'mcp-ai-wpoos' ),
				array()
			);
		}
		...
		return $this->format_response(
			...
			sprintf(
				...
				__( 'Checked %1$d post(s), found %2$d issue(s).', 'mcp-ai-wpoos' ),
				$results['posts_checked'],
	...
	private function check_post( $post_id, $options ) {
		...
		if ( ! $post ) {
			return array(
				'post_id' => $post_id,
				'status'  => 'error',
				'message' => __( 'Post not found', 'mcp-ai-wpoos' ),
			);
		}
	...
	private function phase1_regex_detection( $post, $options ) {
		...
		if ( preg_match( '/lorem\s+ipsum/i', $content ) ) {
			$issues[] = array(
				...
				'certainty' => 'HIGH',
				'message'   => __( 'Contains "Lorem ipsum" placeholder text', 'mcp-ai-wpoos' ),
				'fixable'   => true,
			);
		...
		if ( preg_match( $marker_pattern, $content ) ) {
			$issues[] = array(
				'type'      => 'draft_markers',
				'certainty' => 'HIGH',
				'message'   => __( 'Contains draft markers like [TODO], [DRAFT]', 'mcp-ai-wpoos' ),
				'fixable'   => true,
			);
		...
		if ( preg_match( '/\[\/?[^\]]*$/m', $content ) ) {
			$issues[] = array(
				...
				'certainty' => 'HIGH',
				'message'   => __( 'Contains broken/unclosed shortcodes', 'mcp-ai-wpoos' ),
				'fixable'   => false,
			);
		}
		...
		if ( preg_match( '/<(?!br|hr|img|input|meta|link)[^>]+><\/[^>]+>/i', $content ) ) {
			$issues[] = array(
				'type'      => 'empty_tags',
				'certainty' => 'HIGH',
				'message'   => __( 'Contains empty HTML tags', 'mcp-ai-wpoos' ),
				'fixable'   => true,
			);
		...
		if ( preg_match( '/  +/', $content ) ) {
			$issues[] = array(
				'type'      => 'extra_spaces',
				'certainty' => 'HIGH',
				'message'   => __( 'Contains multiple consecutive spaces', 'mcp-ai-wpoos' ),
				'fixable'   => true,
			);
		...
		if ( stripos( $content, 'Welcome to WordPress' ) !== false ) {
			$issues[] = array(
				...
				'certainty' => 'HIGH',
				'message'   => __( 'Contains default WordPress content', 'mcp-ai-wpoos' ),
				'fixable'   => false,
			);
		}
	...
	private function phase2_content_analysis( $post ) {
		...
		if ( $word_count < 300 ) {
			$issues[] = array(
				...
				'message'   => sprintf(
					...
					__( 'Content is thin (%d words, minimum 300 recommended)', 'mcp-ai-wpoos' ),
					$word_count
				),
				'fixable'   => false,
			);
		...
		if ( $long_sentences > 3 ) {
			$issues[] = array(
				...
				'message'   => sprintf(
					...
					__( 'Found %d sentences over 30 words (may hurt readability)', 'mcp-ai-wpoos' ),
					$long_sentences
				),
				'fixable'   => false,
			);
		...
		if ( $broken_links > 0 ) {
			$issues[] = array(
				...
				'message'   => sprintf(
					...
					__( 'Found %d broken internal link(s)', 'mcp-ai-wpoos' ),
					$broken_links
				),
				'fixable'   => false,
			);
		...
		if ( empty( $meta_desc ) ) {
			$issues[] = array(
				'type'      => 'missing_meta',
				'certainty' => 'MEDIUM',
				'message'   => __( 'Missing SEO meta description', 'mcp-ai-wpoos' ),
				'fixable'   => false,
			);
		}
		...
		if ( ! empty( $post->post_excerpt ) ) {
			...
			if ( count( $common_words ) / count( $title_words ) > 0.8 ) {
				$issues[] = array(
					...
					'certainty' => 'MEDIUM',
					'message'   => __( 'Excerpt is too similar to title (80%+ word overlap)', 'mcp-ai-wpoos' ),
					'fixable'   => false,
				);
			}
		}
	...
	private function phase3_ai_review( $post, $options ) {
		...
		if ( $informal_count > 2 ) {
			$suggestions[] = array(
				'type'      => 'brand_voice',
				'certainty' => 'LOW',
				'message'   => __( 'Content may be too informal for your brand voice', 'mcp-ai-wpoos' ),
				'fixable'   => false,
			);
		}
		...
		if ( $word_count > 500 && $question_count === 0 ) {
			$suggestions[] = array(
				'type'      => 'engagement',
				'certainty' => 'LOW',
				'message'   => __( 'Long content without questions may reduce engagement', 'mcp-ai-wpoos' ),
				'fixable'   => false,
			);
		}
		...
		if ( $negative_count > 5 ) {
			$suggestions[] = array(
				'type'      => 'tone',
				'certainty' => 'LOW',
				'message'   => __( 'Content tone may be overly negative', 'mcp-ai-wpoos' ),
				'fixable'   => false,
			);
		}
