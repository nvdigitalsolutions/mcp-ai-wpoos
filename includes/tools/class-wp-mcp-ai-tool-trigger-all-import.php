...
class WP_MCP_AI_Tool_Trigger_All_Import implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Trigger WP All Import', 'mcp-ai-wpoos' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'import_id' => array(
					'type'        => 'integer',
					'description' => __( 'The ID of the import template to trigger.', 'mcp-ai-wpoos' ),
					'minimum'     => 1,
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be logged in to trigger imports.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( ! user_can( $user_id, 'manage_options' ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to trigger imports.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( ! class_exists( 'PMXI_Import_Record' ) ) {
			return new WP_Error( 'wp_mcp_ai_class_missing', __( 'PMXI_Import_Record class not found. WP All Import may not be properly installed.', 'mcp-ai-wpoos' ) );
		}

		// Check if import is already processing.
		$processing = get_post_meta( $import_id, 'processing', true );
		if ( $processing ) {
			return new WP_Error( 'wp_mcp_ai_import_processing', __( 'Import is already processing. Please wait for it to complete.', 'mcp-ai-wpoos' ) );
		}
		...
		try {
			...
			if ( ! $import_record->id ) {
				return new WP_Error( 'wp_mcp_ai_import_not_found', __( 'Import record not found.', 'mcp-ai-wpoos' ) );
			}
			...
			if ( $import_key ) {
				...
				if ( is_wp_error( $response ) ) {
					return new WP_Error(
						...
						sprintf(
							...
							__( 'Failed to trigger import: %s', 'mcp-ai-wpoos' ),
				...
				return array(
					'success'     => true,
					'message'     => __( 'Import triggered successfully. Processing in background.', 'mcp-ai-wpoos' ),
					'import_id'   => $import_id,
			...
			return new WP_Error( 'wp_mcp_ai_no_import_key', __( 'Import key not found. Cannot trigger import.', 'mcp-ai-wpoos' ) );
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'Import failed: %s', 'mcp-ai-wpoos' ),
					$e->getMessage()
				)
			);
		}
	...
	public function get_capability_flags() {
		return array(
			...
			'local-only',          // Triggers local site import via home_url(); no external API calls.
