		...
		if ( '' === $api_token ) {
			return new WP_Error(
				...
				array(
					'status'  => 400,
					'actions' => array(
						'configure_cloudflare_api_token' => __( 'Add a Cloudflare API token in the WP MCP AI settings.', 'wp-mcp-ai' ),
					),
				)
			);
		}
		...
		if ( '' === $zone_id ) {
			return new WP_Error(
				...
				array(
					'status'  => 400,
					'actions' => array(
						'configure_cloudflare_zone_id' => __( 'Add the Cloudflare zone ID in the WP MCP AI settings.', 'wp-mcp-ai' ),
					),
				)
			);
		}
