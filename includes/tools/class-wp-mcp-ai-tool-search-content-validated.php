<?php
/**
 * Tool that searches WordPress content with taxonomy and meta filters (Validated version).
 *
 * This is the Symfony Validator version of the search_content tool.
 *
 * @package WP_MCP_AI
 */
...
require_once __DIR__ . '/../validators/arguments/class-search-content-arguments.php';
...
class WP_MCP_AI_Tool_Search_Content_Validated extends WP_MCP_AI_Validated_Tool implements WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	protected function get_validation_class() {
		return \WP_MCP_AI\Tools\Arguments\SearchContentArguments::class;
	}

	/**
	 * Execute the tool with validated arguments.
	 *
	 * @param \WP_MCP_AI\Tools\Arguments\SearchContentArguments $validated_args Validated arguments object.
	...
	/**
	 * Prepare a tax_query argument from validated inputs.
	 *
	 * @param \WP_MCP_AI\Tools\Arguments\SearchContentArguments $validated_args Validated arguments.
	 * @return array
	 */
	protected function prepare_taxonomy_filters( $validated_args ) {
		if ( empty( $validated_args->taxonomy_filters ) || ! is_array( $validated_args->taxonomy_filters ) ) {
			return array();
		}

		$relation = strtoupper( sanitize_text_field( $validated_args->taxonomy_relation ) );

		$tax_query = array();

		foreach ( $validated_args->taxonomy_filters as $filter ) {
			if ( ! is_array( $filter ) ) {
				continue;
			}
	...
	/**
	 * Prepare a meta_query argument from validated inputs.
	 *
	 * @param \WP_MCP_AI\Tools\Arguments\SearchContentArguments $validated_args Validated arguments.
	 * @return array
	 */
	protected function prepare_meta_filters( $validated_args ) {
		if ( empty( $validated_args->meta_filters ) || ! is_array( $validated_args->meta_filters ) ) {
			return array();
		}

		$relation = strtoupper( sanitize_text_field( $validated_args->meta_relation ) );

		$meta_query = array();

		foreach ( $validated_args->meta_filters as $filter ) {
			if ( ! is_array( $filter ) ) {
				continue;
			}
