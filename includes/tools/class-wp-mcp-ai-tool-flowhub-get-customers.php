<?php
/**
 * Tool that retrieves customer data from Flowhub API.
 *
 * @package WP_MCP_AI
 */
...
class WP_MCP_AI_Tool_Flowhub_Get_Customers implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Flowhub Get Customers', 'mcp-ai-wpoos' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id && ! $has_token ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be authenticated to retrieve Flowhub customers.', 'mcp-ai-wpoos' ), array( 'status' => rest_authorization_required_code() ) );
		}
		...
		if ( $user_id ) {
			...
			if ( ! user_can( $user_id, 'manage_woocommerce' ) && ! user_can( $user_id, 'manage_options' ) ) {
				return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to retrieve customer data.', 'mcp-ai-wpoos' ) );
			}
		...
		if ( isset( $arguments['search'] ) ) {
			$options['search'] = sanitize_text_field( $arguments['search'] );
		}
		if ( isset( $arguments['timeout'] ) ) {
			$options['timeout'] = max( 5, min( 60, absint( $arguments['timeout'] ) ) );
		}

		$result = $client->get_customers( $options );
		...
		$summary = __( 'Retrieved customers from Flowhub', 'mcp-ai-wpoos' );
		if ( isset( $result['total'] ) ) {
			$summary = sprintf(
				/* translators: %d: number of customers */
				__( 'Retrieved %d customers from Flowhub', 'mcp-ai-wpoos' ),
				absint( $result['total'] )
			);
		}
		...
		/**
		 * Allow third parties to filter the Flowhub customers result.
		 *
		 * @param array $result    Final response payload.
		 * @param array $arguments Original tool arguments.
		 * @param array $context   Invocation context.
		 */
		$result = apply_filters( 'wp_mcp_ai_flowhub_get_customers_result', $result, $arguments, $context );
