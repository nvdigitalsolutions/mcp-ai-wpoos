<?php
/**
 * Tool that retrieves budget data from Firefly III API.
 *
 * @package WP_MCP_AI
 */
...
/**
 * Provides a tool for retrieving budgets from Firefly III.
 */
class WP_MCP_AI_Tool_Firefly_Get_Budgets implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Firefly III Get Budgets', 'mcp-ai-wpoos' );
	}
	...
	public function get_description() {
		return __( 'Retrieve budget allocations and spending limits from Firefly III personal finance manager. View budget amounts, spending progress, and available funds for the specified time period. Useful for tracking budget adherence, analyzing spending patterns, and financial planning.', 'mcp-ai-wpoos' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'start'         => array(
					'type'        => 'string',
					'description' => __( 'Start date for budget query in YYYY-MM-DD format (e.g., "2024-01-01").', 'mcp-ai-wpoos' ),
				),
				'end'           => array(
					'type'        => 'string',
					'description' => __( 'End date for budget query in YYYY-MM-DD format (e.g., "2024-12-31").', 'mcp-ai-wpoos' ),
				),
				'limit'         => array(
					'type'        => 'integer',
					'description' => __( 'Maximum number of budgets to retrieve (1-100).', 'mcp-ai-wpoos' ),
					'minimum'     => 1,
					'maximum'     => 100,
					'default'     => 20,
				),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id && ! $has_token ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be authenticated to retrieve Firefly III budgets.', 'mcp-ai-wpoos' ), array( 'status' => rest_authorization_required_code() ) );
		}
		...
		$result = $client->get_budgets( $options );
		...
		if ( isset( $result['data'] ) && is_array( $result['data'] ) ) {
			foreach ( $result['data'] as $budget ) {
				if ( ! isset( $budget['attributes'] ) ) {
					continue;
				}

				$attrs = $budget['attributes'];
				$budgets[] = array(
					'id'         => isset( $budget['id'] ) ? $budget['id'] : '',
		...
		$summary = sprintf(
			/* translators: %d: number of budgets */
			__( 'Retrieved %d budget(s) from Firefly III', 'mcp-ai-wpoos' ),
			count( $budgets )
		);
		...
		/**
		 * Allow third parties to filter the Firefly III budgets result.
		 *
		 * @param array $response  Final response payload.
		 * @param array $arguments Original tool arguments.
		 * @param array $context   Invocation context.
		 */
		$response = apply_filters( 'wp_mcp_ai_firefly_get_budgets_result', $response, $arguments, $context );
