...
class WP_MCP_AI_Tool_Search_Drive implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Search Google Drive Files and Folders', 'mcp-ai-wpoos' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( 200 !== $status_code ) {
			...
			if ( 400 === $status_code ) {
				$user_message = __( 'Google Drive query syntax error. The search query may be malformed.', 'mcp-ai-wpoos' );
				if ( $error_message ) {
					/* translators: %s: Google's error message. */
					$user_message = sprintf( __( 'Google Drive query error: %s', 'mcp-ai-wpoos' ), $error_message );
				} else {
					$user_message .= ' ' . __( 'Supported query examples: "report", "name contains \'invoice\'", "mimeType = \'application/pdf\'".', 'mcp-ai-wpoos' );
				}
			} elseif ( 401 === $status_code || 403 === $status_code ) {
				$user_message = __( 'Google Drive authentication error. The access token may have expired or credentials are invalid.', 'mcp-ai-wpoos' );
			} elseif ( 404 === $status_code ) {
				$user_message = __( 'Google Drive resource not found. The folder or file may have been deleted.', 'mcp-ai-wpoos' );
			} else {
				/* translators: %d: HTTP status code. */
				$user_message = sprintf( __( 'Google Drive returned an unexpected HTTP status: %d.', 'mcp-ai-wpoos' ), $status_code );
			}
