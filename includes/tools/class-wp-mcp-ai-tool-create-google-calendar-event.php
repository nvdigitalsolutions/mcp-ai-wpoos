 ...
 */
class WP_MCP_AI_Tool_Create_Google_Calendar_Event implements WP_MCP_AI_Tool_Interface {
    ...
    public function execute( array $arguments = array(), array $context = array() ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error( 'wp_mcp_ai_calendar_request_failed', __( 'Unable to communicate with the Google Calendar API.', 'wp-mcp-ai' ), array( 'status' => 500, 'error' => $response ) );
        }
        ...
        if ( $status < 200 || $status >= 300 ) {
            ...
            return new WP_Error( 'wp_mcp_ai_calendar_error', $message, array( 'status' => $status, 'response' => $decoded ) );
        }
    ...
    protected function exchange_service_account_token( array $credentials, array $arguments, array $context ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error( 'wp_mcp_ai_calendar_token_error', __( 'Unable to obtain a Google access token.', 'wp-mcp-ai' ), array( 'status' => 500, 'error' => $response ) );
        }
        ...
        if ( $status < 200 || $status >= 300 || ! isset( $data['access_token'] ) ) {
            ...
            return new WP_Error( 'wp_mcp_ai_calendar_token_error', $message, array( 'status' => $status, 'response' => $data ) );
        }
    ...
    protected function parse_event_time( $value, $timezone, $label ) {
        ...
        if ( '' === $value ) {
            return new WP_Error( 'wp_mcp_ai_invalid_time', __( 'A valid date or time value must be supplied.', 'wp-mcp-ai' ), array( 'status' => 400, 'field' => $label ) );
        }
        ...
        try {
            ...
            return new WP_Error( 'wp_mcp_ai_invalid_time', __( 'Unable to parse the supplied date/time value.', 'wp-mcp-ai' ), array( 'status' => 400, 'field' => $label ) );
        }
        ...
        if ( $timezone ) {
            try {
                ...
                return new WP_Error( 'wp_mcp_ai_invalid_timezone', __( 'The provided timezone is not valid.', 'wp-mcp-ai' ), array( 'status' => 400, 'field' => $label ) );
            }
        }
