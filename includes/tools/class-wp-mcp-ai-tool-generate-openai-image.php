...
class WP_MCP_AI_Tool_Generate_OpenAI_Image implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Shortcuts_Interface, WP_MCP_AI_Tool_LLM_Sanitizer_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface, WP_MCP_AI_Tool_Model_Requirements_Interface, WP_MCP_AI_Tool_Rules_Interface {
	...
	public function get_name() {
		return __( 'Generate OpenAI Image', 'mcp-ai-wpoos' );
	}
	...
	public function get_description() {
		return __( 'Creates an image with OpenAI and stores it in the Media Library. Supports multi-step orchestration mode with prompt optimization, validation, generation, post-processing, and storage optimization. Set orchestration_mode=true to enable 5-step workflow.', 'mcp-ai-wpoos' );
	}
	...
	protected function execute_legacy( array $arguments, array $context, int $user_id ) {
		...
		if ( empty( $image['image'] ) ) {
			return new WP_Error( 'wp_mcp_ai_image_storage_error', __( 'OpenAI returned an empty image response.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( ! empty( $image['revised_prompt'] ) ) {
			$text_parts[] = sprintf(
				...
				__( 'Revised prompt: %s', 'mcp-ai-wpoos' ),
		...
		$text_parts[] = sprintf(
			...
			__( 'Size: %1$s, Quality: %2$s', 'mcp-ai-wpoos' ),
			$size,
			$quality
		);
	 ...
	 * 2. Parameter Validation (size, quality, model compatibility)
	...
	protected function execute_orchestrated( array $arguments, array $context, int $user_id ) {
		...
		$execution_id = 'image_gen_' . wp_generate_uuid4();
	...
	protected function step_validate_parameters( $arguments ) {
		...
		if ( ! empty( $arguments['size'] ) ) {
			...
			if ( ! in_array( $size, $allowed_sizes, true ) ) {
				$errors[] = sprintf(
					...
					__( 'Invalid size: %s', 'mcp-ai-wpoos' ),
					$size
				);
			}
		}
		...
		if ( ! empty( $arguments['quality'] ) ) {
			...
			$allowed_qualities = self::get_allowed_qualities();
			if ( ! in_array( $quality, $allowed_qualities, true ) ) {
				$errors[] = sprintf(
					/* translators: %s: quality value */
					__( 'Invalid quality: %s', 'mcp-ai-wpoos' ),
					$quality
				);
			}
		}
		...
		if ( ! empty( $arguments['model'] ) ) {
			...
			// Basic check - OpenAI client will do more thorough validation.
	...
	protected function log_orchestration_step( $execution_id, $step, $data ) {
		...
		$steps = get_transient( "wp_mcp_ai_image_exec_{$execution_id}" ) ?: array();
		$steps[] = array(
			'step' => $step,
			'time' => current_time( 'mysql' ),
			'data' => $data,
		);
		set_transient( "wp_mcp_ai_image_exec_{$execution_id}", $steps, HOUR_IN_SECONDS );
	}
	...
	protected function handle_orchestration_error( $step_name, $error, $execution_id ) {
		do_action( 'wp_mcp_ai_image_orchestration_failed', $step_name, $error, $execution_id );

		return new WP_Error(
			'orchestration_failed',
			sprintf(
				/* translators: 1: step name, 2: error message */
				__( 'Image generation orchestration failed at step: %1$s. %2$s', 'mcp-ai-wpoos' ),
				$step_name,
