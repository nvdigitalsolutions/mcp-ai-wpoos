 ...
 */
class WP_MCP_AI_Tool_Run_OpenAI_External_Action implements WP_MCP_AI_Tool_Interface {
    ...
    public function execute( array $arguments = array(), array $context = array() ) {
        ...
        WP_MCP_AI_Logger::log_event( 'openai_external_action_request', 'Sending external action request to OpenAI.', array(
        ...
        if ( $status_code < 200 || $status_code >= 300 ) {
            ...
            WP_MCP_AI_Logger::log_error( 'OpenAI external action returned an error.', array(
                'status'   => $status_code,
                'response' => $decoded,
            ) );

            return new WP_Error( 'wp_mcp_ai_api_error', $message, array( 'status' => $status_code, 'body' => $decoded ) );
        }

        WP_MCP_AI_Logger::log_event( 'openai_external_action_response', 'OpenAI external action completed.', array(
            'response' => $decoded,
        ) );
