...
/**
 * Removes npm packages from a WordPress installation.
 */
class WP_MCP_AI_Tool_NPM_Remove_Package implements WP_MCP_AI_Tool_Interface {
	/**
	 * {@inheritdoc}
	 */
	public function get_slug() {
		return 'npm_remove_package';
	}
	...
	public function get_name() {
		return __( 'NPM Remove Package', 'wp-mcp-ai' );
	}
	...
	public function get_description() {
		return __( 'Removes (uninstalls) one or more npm packages from the WordPress installation and updates package.json accordingly.', 'wp-mcp-ai' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties' => array(
				'packages'    => array(
					'type'        => 'array',
					'description' => __( 'Array of package names to remove.', 'wp-mcp-ai' ),
					'items'       => array(
						'type' => 'string',
					),
					'minItems'    => 1,
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id || ! user_can( $user_id, 'manage_options' ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to remove npm packages.', 'wp-mcp-ai' ) );
		}
		...
		// Build npm uninstall command.
		$uninstall_args = array();
		foreach ( $packages as $package ) {
			$uninstall_args[] = escapeshellarg( $package );
		}

		$command = sprintf(
			'cd %s && %s uninstall %s 2>&1',
			escapeshellarg( $work_dir ),
			escapeshellcmd( $npm_binary ),
			implode( ' ', $uninstall_args )
		);
	...
	protected function is_valid_package_name( $package ) {
		...
		// No version specifications allowed for uninstall command.
