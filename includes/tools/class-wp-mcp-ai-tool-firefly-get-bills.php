<?php
/**
 * Tool that retrieves bill data from Firefly III API.
 *
 * @package WP_MCP_AI
 */
...
/**
 * Provides a tool for retrieving bills from Firefly III.
 */
class WP_MCP_AI_Tool_Firefly_Get_Bills implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Firefly III Get Bills', 'mcp-ai-wpoos' );
	}
	...
	public function get_description() {
		return __( 'Retrieve recurring bills and due dates from Firefly III personal finance manager. View monthly bills, subscriptions, and recurring expenses with payment schedules. Useful for tracking upcoming payments, managing cash flow, and avoiding late fees.', 'mcp-ai-wpoos' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'start'         => array(
					'type'        => 'string',
					'description' => __( 'Start date for bill query in YYYY-MM-DD format (e.g., "2024-01-01").', 'mcp-ai-wpoos' ),
				),
				'end'           => array(
					'type'        => 'string',
					'description' => __( 'End date for bill query in YYYY-MM-DD format (e.g., "2024-12-31").', 'mcp-ai-wpoos' ),
				),
				'limit'         => array(
					'type'        => 'integer',
					'description' => __( 'Maximum number of bills to retrieve (1-100).', 'mcp-ai-wpoos' ),
					'minimum'     => 1,
					'maximum'     => 100,
					'default'     => 20,
				),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id && ! $has_token ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be authenticated to retrieve Firefly III bills.', 'mcp-ai-wpoos' ), array( 'status' => rest_authorization_required_code() ) );
		}
		...
		$result = $client->get_bills( $options );
		...
		if ( isset( $result['data'] ) && is_array( $result['data'] ) ) {
			foreach ( $result['data'] as $bill ) {
				...
				$bills[] = array(
					...
					'amount_min'    => isset( $attrs['amount_min'] ) ? $attrs['amount_min'] : '0',
					'amount_max'    => isset( $attrs['amount_max'] ) ? $attrs['amount_max'] : '0',
					'currency_code' => isset( $attrs['currency_code'] ) ? $attrs['currency_code'] : '',
					'date'          => isset( $attrs['date'] ) ? $attrs['date'] : '',
					'repeat_freq'   => isset( $attrs['repeat_freq'] ) ? $attrs['repeat_freq'] : '',
		...
		$summary = sprintf(
			...
			__( 'Retrieved %d bill(s) from Firefly III', 'mcp-ai-wpoos' ),
			count( $bills )
		);
		...
		/**
		 * Allow third parties to filter the Firefly III bills result.
		 *
		 * @param array $response  Final response payload.
		 * @param array $arguments Original tool arguments.
		 * @param array $context   Invocation context.
		 */
		$response = apply_filters( 'wp_mcp_ai_firefly_get_bills_result', $response, $arguments, $context );
