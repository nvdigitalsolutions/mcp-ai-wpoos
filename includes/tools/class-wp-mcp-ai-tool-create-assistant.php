...
class WP_MCP_AI_Tool_Create_Assistant implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
		...
		return array(
			...
			'properties'           => array(
				...
				'orchestration_mode'    => array(
					'type'        => 'boolean',
					'description' => __( 'Enable multi-step orchestration workflow. When true, executes 6-step process: Research → Validate → Generate → Create → Enhance → Optimize. Default: false (legacy mode).', 'mcp-ai-wpoos' ),
					'default'     => false,
				),
				'auto_research'         => array(
					'type'        => 'boolean',
					'description' => __( 'Automatically research profession and industry information using web_search tool. Only applies when orchestration_mode is true.', 'mcp-ai-wpoos' ),
					'default'     => false,
				),
				'optimize_instructions' => array(
					'type'        => 'boolean',
					'description' => __( 'Use AI to optimize system prompt/instructions for better performance. Only applies when orchestration_mode is true.', 'mcp-ai-wpoos' ),
					'default'     => false,
				),
				'optimize_tools'        => array(
					'type'        => 'boolean',
					'description' => __( 'Intelligently select and optimize tools for the assistant. Only applies when orchestration_mode is true.', 'mcp-ai-wpoos' ),
					'default'     => false,
				),
				'optimize'              => array(
					'type'        => 'boolean',
					'description' => __( 'Optimize assistant after creation (featured image, validation, cache). Only applies when orchestration_mode is true.', 'mcp-ai-wpoos' ),
					'default'     => false,
				),
				'generate_avatar'       => array(
					'type'        => 'boolean',
					'description' => __( 'Auto-generate avatar/featured image for the assistant. Only applies when orchestration_mode is true.', 'mcp-ai-wpoos' ),
					'default'     => false,
				),
				'async'                 => array(
					'type'        => 'boolean',
	...
	/**
	 * Execute orchestrated assistant creation with multi-step workflow.
	 *
	 * @param array $arguments Tool arguments.
	 ...
	 */
	protected function execute_orchestrated( $arguments, $user_id, $context ) {
		$execution_id = wp_generate_uuid4();
		$this->log_orchestration_step( $execution_id, 'started', 'Orchestration workflow initiated' );

		// Step 1: Research (optional).
		if ( $arguments['auto_research'] ?? false ) {
			$research_result = $this->step_research( $arguments, $context, $execution_id );
			if ( is_wp_error( $research_result ) ) {
				$this->log_orchestration_step( $execution_id, 'research_failed', $research_result->get_error_message() );
				// Non-critical: continue with provided data.
			} else {
				$arguments = array_merge( $arguments, $research_result );
				$this->log_orchestration_step( $execution_id, 'research_complete', 'Context enriched with research' );
			}
		}

		// Step 2: Validate.
		$this->log_orchestration_step( $execution_id, 'validating', 'Validating input data' );
		$validation = $this->step_validate( $arguments, $user_id, $execution_id );
		if ( is_wp_error( $validation ) ) {
			$this->log_orchestration_step( $execution_id, 'validation_failed', $validation->get_error_message() );
			return $validation;
		}
		$this->log_orchestration_step( $execution_id, 'validation_complete', 'All validations passed' );

		// Step 3: Generate/Optimize Instructions (optional).
		if ( $arguments['optimize_instructions'] ?? false ) {
			$optimize_result = $this->step_optimize_instructions( $arguments, $context, $execution_id );
			if ( ! is_wp_error( $optimize_result ) ) {
				$arguments['system_prompt'] = $optimize_result;
				$this->log_orchestration_step( $execution_id, 'instructions_optimized', 'System prompt enhanced' );
			}
		}

		// Step 4: Create Assistant.
		$this->log_orchestration_step( $execution_id, 'creating', 'Creating assistant' );
		$result = $this->create_assistant( $arguments, $user_id );

		if ( is_wp_error( $result ) ) {
			$this->log_orchestration_step( $execution_id, 'creation_failed', $result->get_error_message() );
			return $result;
		}

		$assistant_id = $result['assistant_id'];
		$this->log_orchestration_step( $execution_id, 'creation_complete', "Assistant {$assistant_id} created" );

		// Step 5: Enhance (optional).
		if ( $arguments['optimize_tools'] ?? false ) {
			$enhance_result = $this->step_enhance_tools( $assistant_id, $arguments, $context, $execution_id );
			if ( ! is_wp_error( $enhance_result ) ) {
				$result['tools_optimized'] = true;
				$this->log_orchestration_step( $execution_id, 'tools_enhanced', 'Tool selection optimized' );
			}
		}

		// Step 6: Optimize (optional).
		if ( $arguments['optimize'] ?? false ) {
			$optimize_result = $this->step_optimize( $assistant_id, $arguments, $context, $execution_id );
			if ( ! is_wp_error( $optimize_result ) ) {
				$result = array_merge( $result, $optimize_result );
				$this->log_orchestration_step( $execution_id, 'optimization_complete', 'Post-creation optimization complete' );
			}
		}

		$this->log_orchestration_step( $execution_id, 'completed', 'Orchestration workflow completed successfully' );

		// Add orchestration metadata to response.
		$result['orchestration'] = array(
			'enabled'      => true,
			'execution_id' => $execution_id,
			'steps'        => $this->get_orchestration_log( $execution_id ),
		);
	...
	/**
	 * Step 1: Research profession and industry information.
	 *
	 * @param array  $arguments    Tool arguments.
	 * @param array  $context      Execution context.
	 * @param string $execution_id Execution ID for logging.
	 * @return array|WP_Error Research data or error.
	 */
	protected function step_research( $arguments, $context, $execution_id ) {
		$this->log_orchestration_step( $execution_id, 'research_started', 'Researching profession and industry context' );

		$professions    = $arguments['professions'] ?? array();
		$regions        = $arguments['regions'] ?? array();
		$industry_focus = $arguments['industry_focus'] ?? '';

		if ( empty( $professions ) && empty( $industry_focus ) ) {
			return new WP_Error( 'no_research_context', 'No professions or industry focus provided for research' );
		}

		// Build research query.
		$query_parts = array();
		if ( ! empty( $professions ) ) {
			$profession_names = array_map( array( $this, 'get_profession_name' ), array_slice( $professions, 0, 2 ) );
			$query_parts[]    = implode( ' ', $profession_names );
		}
		if ( ! empty( $regions ) ) {
			$region_names  = array_map( array( $this, 'get_region_name' ), array_slice( $regions, 0, 1 ) );
			$query_parts[] = implode( ' ', $region_names );
		}
		if ( ! empty( $industry_focus ) ) {
			$query_parts[] = $industry_focus;
		}
		...
		if ( ! $search_tool ) {
			return new WP_Error( 'search_unavailable', 'Web search tool not available' );
		}
		...
		$research_data = array();
		if ( isset( $search_result['results'] ) && ! empty( $search_result['results'] ) ) {
			$research_data['context_info'] = wp_trim_words( $search_result['results'], 200 );
		}
	...
	/**
	 * Step 2: Validate assistant creation parameters.
	 *
	 * @param array  $arguments    Tool arguments.
	 * @param int    $user_id      User ID.
	 * @param string $execution_id Execution ID for logging.
	 ...
	 */
	protected function step_validate( $arguments, $user_id, $execution_id ) {
		$errors = array();
		...
		// Validate professions count.
		$professions = $arguments['professions'] ?? array();
		if ( ! empty( $professions ) && count( $professions ) > 3 ) {
			$errors[] = 'Maximum 3 professions allowed';
		}

		// Validate regions count.
		$regions = $arguments['regions'] ?? array();
		if ( ! empty( $regions ) && count( $regions ) > 2 ) {
			$errors[] = 'Maximum 2 regions allowed';
		}

		// Validate attachment IDs.
		$attachment_ids = $arguments['attachment_ids'] ?? array();
		if ( ! empty( $attachment_ids ) ) {
			if ( ! is_array( $attachment_ids ) ) {
				$errors[] = 'Attachment IDs must be an array';
			} elseif ( count( $attachment_ids ) > self::MAX_DOCUMENTS ) {
				$errors[] = sprintf( 'Maximum %d documents allowed', self::MAX_DOCUMENTS );
			} else {
				foreach ( $attachment_ids as $att_id ) {
					if ( ! get_post( $att_id ) || get_post_type( $att_id ) !== 'attachment' ) {
						$errors[] = sprintf( 'Invalid attachment ID: %d', $att_id );
						break;
					}
				}
			}
		...
		if ( ! empty( $system_prompt ) && strlen( $system_prompt ) > 32000 ) {
			$errors[] = 'System prompt must be 32,000 characters or less';
		}

		// Validate description length.
		$description = $arguments['description'] ?? '';
		if ( ! empty( $description ) && strlen( $description ) > 5000 ) {
			$errors[] = 'Description must be 5,000 characters or less';
		}
		...
		// Check if at least one of professions, regions, or description is provided.
		if ( empty( $professions ) && empty( $regions ) && empty( $description ) && empty( $system_prompt ) ) {
			$errors[] = 'Must provide at least professions, regions, description, or system_prompt';
		}

		if ( ! empty( $errors ) ) {
			$this->log_orchestration_step( $execution_id, 'validation_errors', implode( '; ', $errors ) );
			return new WP_Error(
				'validation_failed',
	...
	/**
	 * Step 3: Optimize system prompt/instructions using AI.
	 *
	 * @param array  $arguments    Tool arguments.
	 * @param array  $context      Execution context.
	 * @param string $execution_id Execution ID for logging.
	 * @return string|WP_Error Optimized instructions or error.
	 */
	protected function step_optimize_instructions( $arguments, $context, $execution_id ) {
		$this->log_orchestration_step( $execution_id, 'optimizing_instructions', 'Enhancing system prompt with AI' );

		// Get current instructions or generate basic ones.
		$system_prompt = $arguments['system_prompt'] ?? '';
		if ( empty( $system_prompt ) ) {
			// Generate basic instructions first.
			$professions    = $arguments['professions'] ?? array();
			$regions        = $arguments['regions'] ?? array();
			$industry_focus = $arguments['industry_focus'] ?? '';
			$title          = $arguments['title'] ?? '';
			...
			if ( ! empty( $professions ) && ! empty( $regions ) ) {
				$system_prompt = $this->generate_instructions_from_selections( $professions, $regions, $industry_focus, $title );
			} elseif ( ! empty( $description ) ) {
				$system_prompt = $this->generate_instructions_from_description( $description, $title );
			} else {
				$system_prompt = $this->generate_generic_instructions( $title );
			}
		}

		// Use AI to optimize the instructions.
		$ai_client = WP_MCP_AI_Client_Manager::get_instance()->get_client( 'openai' );
		if ( ! $ai_client ) {
			return new WP_Error( 'ai_unavailable', 'AI client not available' );
		}

		$optimization_prompt = "You are an expert at writing AI assistant system prompts. Optimize the following system prompt to be more effective, clear, and comprehensive. Maintain the core intent but improve clarity, structure, and actionability. Keep it under 4000 characters.\n\nOriginal prompt:\n{$system_prompt}\n\nOptimized prompt:";
		...
		try {
			...
			$this->log_orchestration_step( $execution_id, 'optimization_error', $e->getMessage() );
	...
	/**
	 * Step 5: Enhance tool selection for the assistant.
	 *
	...
	protected function step_enhance_tools( $assistant_id, $arguments, $context, $execution_id ) {
		$this->log_orchestration_step( $execution_id, 'enhancing_tools', 'Optimizing tool selection' );

		// Get current tools.
		$current_tools = get_post_meta( $assistant_id, '_wp_mcp_ai_tools', true );
		if ( ! is_array( $current_tools ) ) {
			$current_tools = array();
		}

		// Add commonly useful tools if not already included.
		...
		$professions = $arguments['professions'] ?? array();
		if ( in_array( 'tax_advisor', $professions, true ) || in_array( 'accountant', $professions, true ) ) {
	...
	/**
	 * Step 6: Post-creation optimization.
	 *
	 * @param int    $assistant_id Assistant post ID.
	 * @param array  $arguments    Tool arguments.
	 * @param array  $context      Execution context.
	 ...
	 */
	protected function step_optimize( $assistant_id, $arguments, $context, $execution_id ) {
		$this->log_orchestration_step( $execution_id, 'optimizing', 'Running post-creation optimization' );

		$optimization_results = array();

		// Generate avatar/featured image if requested.
		if ( $arguments['generate_avatar'] ?? false ) {
			$avatar_result = $this->generate_assistant_avatar( $assistant_id, $arguments, $context, $execution_id );
			if ( ! is_wp_error( $avatar_result ) ) {
				$optimization_results['avatar_generated'] = true;
				$optimization_results['featured_image_id'] = $avatar_result;
			}
		}

		// Purge cache for assistant.
		if ( function_exists( 'wp_cache_flush' ) ) {
			wp_cache_delete( $assistant_id, 'posts' );
			wp_cache_delete( $assistant_id, 'post_meta' );
		}
	...
	/**
	 * Generate avatar/featured image for assistant.
	 *
	 * @param int    $assistant_id Assistant post ID.
	 * @param array  $arguments    Tool arguments.
	 * @param array  $context      Execution context.
	 ...
	 */
	protected function generate_assistant_avatar( $assistant_id, $arguments, $context, $execution_id ) {
		$this->log_orchestration_step( $execution_id, 'generating_avatar', 'Creating avatar image' );

		$title       = $arguments['title'] ?? 'AI Assistant';
		$professions = $arguments['professions'] ?? array();

		// Build prompt for avatar generation.
		$prompt = "Professional avatar for {$title}";
		if ( ! empty( $professions ) ) {
			$profession_names = array_map( array( $this, 'get_profession_name' ), array_slice( $professions, 0, 2 ) );
			$prompt          .= ', ' . implode( ' and ', $profession_names );
		}
		$prompt .= '. Professional, friendly, trustworthy appearance. Clean modern style.';

		// Use generate_openai_image tool.
		$image_tool = WP_MCP_AI_Tool_Registry::get_instance()->get_tool( 'generate_openai_image' );
		if ( ! $image_tool ) {
		...
		$attachment_id = $image_result['attachment_id'] ?? 0;
		if ( $attachment_id ) {
			set_post_thumbnail( $assistant_id, $attachment_id );
			return $attachment_id;
		}

		return new WP_Error( 'avatar_generation_failed', 'Failed to generate avatar' );
	}
	 ...
	 */
	protected function log_orchestration_step( $execution_id, $step, $message ) {
		$log_key = 'wp_mcp_ai_orchestration_' . $execution_id;
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			error_log( sprintf( '[WP_MCP_AI Orchestration %s] %s: %s', $execution_id, $step, $message ) );
		}
	}

	/**
	 * Get orchestration log for execution ID.
	 *
	 * @param string $execution_id Execution ID.
	 * @return array Log entries.
	 */
	protected function get_orchestration_log( $execution_id ) {
		$log_key = 'wp_mcp_ai_orchestration_' . $execution_id;
		$log     = get_transient( $log_key );
