<?php
/**
 * Tool that retrieves product data from Flowhub API.
 *
 * @package WP_MCP_AI
 */
...
class WP_MCP_AI_Tool_Flowhub_Get_Products implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Flowhub Get Products', 'mcp-ai-wpoos' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'limit'         => array(
					'type'        => 'integer',
					'description' => __( 'Maximum number of products to retrieve (1-100).', 'mcp-ai-wpoos' ),
					'minimum'     => 1,
					'maximum'     => 100,
					'default'     => 20,
				),
				'offset'        => array(
					'type'        => 'integer',
					'description' => __( 'Number of products to skip for pagination.', 'mcp-ai-wpoos' ),
					'minimum'     => 0,
					'default'     => 0,
				),
				'category'      => array(
					'type'        => 'string',
					'description' => __( 'Filter by product category (e.g., "flower", "concentrate", "edible", "accessories").', 'mcp-ai-wpoos' ),
				),
				'timeout'       => array(
					'type'        => 'integer',
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id && ! $has_token ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be authenticated to retrieve Flowhub products.', 'mcp-ai-wpoos' ), array( 'status' => rest_authorization_required_code() ) );
		}
		...
		if ( isset( $arguments['category'] ) ) {
			$options['category'] = sanitize_text_field( $arguments['category'] );
		}
		if ( isset( $arguments['timeout'] ) ) {
			$options['timeout'] = max( 5, min( 60, absint( $arguments['timeout'] ) ) );
		}

		$result = $client->get_products( $options );
		...
		$summary = __( 'Retrieved products from Flowhub', 'mcp-ai-wpoos' );
		...
		/**
		 * Allow third parties to filter the Flowhub products result.
		 *
		 * @param array $result    Final response payload.
		 * @param array $arguments Original tool arguments.
		 * @param array $context   Invocation context.
		 */
		$result = apply_filters( 'wp_mcp_ai_flowhub_get_products_result', $result, $arguments, $context );
