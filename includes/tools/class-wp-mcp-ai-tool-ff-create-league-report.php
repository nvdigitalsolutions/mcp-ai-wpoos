...
class WP_MCP_AI_Tool_FF_Create_League_Report implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	protected function generate_league_analysis( $standings ) {
		...
		$analysis['highest_scoring'] = sprintf(
			/* translators: 1: Team name, 2: Total points scored. */
			__( '%1$s leads in total points with %2$.1f points scored.', 'mcp-ai-wpoos' ),
		...
		if ( ! empty( $standings[0] ) ) {
			...
			$analysis['leader'] = sprintf(
				/* translators: 1: Team name, 2: Number of wins, 3: Number of losses. */
				__( '%1$s is in first place with a %2$d-%3$d record.', 'mcp-ai-wpoos' ),
				$leader['team_name'],
				$leader['wins'],
				$leader['losses']
			);
		...
		$analysis['league_average'] = sprintf(
			/* translators: %s: Average points per team. */
