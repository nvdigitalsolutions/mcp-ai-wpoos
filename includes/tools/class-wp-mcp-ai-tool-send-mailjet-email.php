 ...
 */
class WP_MCP_AI_Tool_Send_Mailjet_Email implements WP_MCP_AI_Tool_Interface {
		...
		if ( '' === $api_key || '' === $api_secret ) {
			return new WP_Error(
				...
				array(
					'status'  => 400,
					'actions' => array(
						'configure_mailjet_credentials' => __( 'Add a Mailjet API key and secret in the WP MCP AI settings.', 'wp-mcp-ai' ),
					),
				)
			);
		}
		...
		if ( '' === $from_email ) {
			return new WP_Error(
				...
				array(
					'status'  => 400,
					'actions' => array(
						'configure_mailjet_sender' => __( 'Set the Mailjet From Email in the WP MCP AI settings.', 'wp-mcp-ai' ),
					),
				)
			);
		}
