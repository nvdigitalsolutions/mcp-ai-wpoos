...
class WP_MCP_AI_Tool_Search_Gmail implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Search Gmail Messages', 'mcp-ai-wpoos' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'connection_id' => array(
					'type'        => 'string',
					'description' => __( 'Optional Gmail connection ID from Remote Sites. If not provided, uses settings-based credentials.', 'mcp-ai-wpoos' ),
				),
				'query'         => array(
					'type'        => 'string',
					'description' => __( 'Gmail search query string. Supports the same syntax as the Gmail web interface.', 'mcp-ai-wpoos' ),
				),
				'max_results'   => array(
					'type'        => 'integer',
					'description' => __( 'Maximum number of messages to return (1-50).', 'mcp-ai-wpoos' ),
					'minimum'     => 1,
					'maximum'     => 50,
					'default'     => 5,
				),
				'label_ids'     => array(
					'type'        => 'array',
					'description' => __( 'Optional Gmail label IDs to filter the results (for example INBOX or CATEGORY_PROMOTIONS).', 'mcp-ai-wpoos' ),
					'items'       => array(
						'type' => 'string',
					),
				),
				'page_token'    => array(
					'type'        => 'string',
					'description' => __( 'Page token returned by a previous Gmail search response to fetch the next page of results.', 'mcp-ai-wpoos' ),
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		$default_capability  = isset( $settings['search_gmail_capability'] ) ? $settings['search_gmail_capability'] : 'manage_options';
		$required_capability = apply_filters( 'wp_mcp_ai_search_gmail_capability', $default_capability, $context, $arguments, $this );

		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_gmail_forbidden', __( 'You do not have permission to search Gmail.', 'mcp-ai-wpoos' ) );
		}

		if ( is_multisite() && $user_id && ! is_user_member_of_blog( $user_id, get_current_blog_id() ) ) {
			return new WP_Error( 'wp_mcp_ai_gmail_wrong_site', __( 'You do not have access to this site.', 'mcp-ai-wpoos' ) );
		}
			...
			if ( ! empty( $connection['connection_type'] ) && 'gmail' !== $connection['connection_type'] ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Connection "%s" is not a Gmail connection. Please use a Gmail connection type.', 'mcp-ai-wpoos' ),
		...
		if ( '' === $query ) {
			return new WP_Error( 'wp_mcp_ai_gmail_missing_query', __( 'A Gmail search query is required.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'wp_mcp_ai_gmail_http_error', __( 'The Gmail search request failed.', 'mcp-ai-wpoos' ), $response );
		}
		...
		if ( 200 !== $status_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Gmail returned an unexpected HTTP status: %d.', 'mcp-ai-wpoos' ),
					$status_code
				),
				array(
					'status' => $status_code,
		...
		if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $list_payload ) ) {
			...
			return new WP_Error( 'wp_mcp_ai_gmail_invalid_json', __( 'Gmail returned an invalid response.', 'mcp-ai-wpoos' ) );
		}
	...
	protected function request_access_token( $client_id, $client_secret, $refresh_token, $timeout ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'wp_mcp_ai_gmail_token_error', __( 'Failed to refresh the Gmail access token.', 'mcp-ai-wpoos' ), $response );
		}
		...
		if ( 200 !== $status_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %d: HTTP status code. */
					__( 'The Gmail token endpoint returned an unexpected status: %d.', 'mcp-ai-wpoos' ),
					$status_code
				),
				array(
					'status' => $status_code,
		...
		if ( JSON_ERROR_NONE !== json_last_error() || empty( $payload['access_token'] ) ) {
			...
			return new WP_Error( 'wp_mcp_ai_gmail_token_invalid', __( 'Gmail returned an invalid token response.', 'mcp-ai-wpoos' ) );
		}
	...
	protected function fetch_message_details( $user_id, $message_id, $access_token, $timeout ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'wp_mcp_ai_gmail_message_error', __( 'Failed to load Gmail message details.', 'mcp-ai-wpoos' ), $response );
		}
		...
		if ( 200 !== $status_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Gmail returned an unexpected status (%2$d) while loading message %1$s.', 'mcp-ai-wpoos' ),
					$message_id,
					$status_code
				),
				array(
		...
		if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $payload ) ) {
			...
			return new WP_Error( 'wp_mcp_ai_gmail_message_invalid', __( 'Gmail returned an invalid message response.', 'mcp-ai-wpoos' ) );
		}
