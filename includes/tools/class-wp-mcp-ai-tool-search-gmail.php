 ...
 */
class WP_MCP_AI_Tool_Search_Gmail implements WP_MCP_AI_Tool_Interface {
    ...
    public function execute( array $arguments = array(), array $context = array() ) {
        ...
        if ( '' === $client_id || '' === $client_secret || '' === $refresh_token ) {
            return new WP_Error(
                ...
                __( 'Gmail API credentials are not configured. Add the client ID, client secret, and refresh token in the WP MCP AI settings.', 'wp-mcp-ai' )
            );
        }
        ...
        if ( 200 !== $status_code ) {
            ...
            return new WP_Error(
                ...
                array( 'status' => $status_code, 'body' => wp_remote_retrieve_body( $response ) )
            );
        }
    ...
    protected function fetch_message_details( $user_id, $message_id, $access_token, $timeout ) {
        ...
        $detail_url = $this->append_query_string( $detail_url, array(
            'format'           => 'metadata',
        ...
        if ( is_wp_error( $response ) ) {
            WP_MCP_AI_Admin_Settings::log( 'Gmail message detail request failed.', array( 'id' => $message_id, 'error' => $response->get_error_message() ) );
        ...
        $status_code = (int) wp_remote_retrieve_response_code( $response );
        if ( 200 !== $status_code ) {
            WP_MCP_AI_Admin_Settings::log( 'Gmail message detail returned unexpected status.', array( 'id' => $message_id, 'status' => $status_code ) );
        ...
        if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $payload ) ) {
            WP_MCP_AI_Admin_Settings::log( 'Gmail message detail returned invalid JSON.', array( 'id' => $message_id, 'body' => $body ) );
