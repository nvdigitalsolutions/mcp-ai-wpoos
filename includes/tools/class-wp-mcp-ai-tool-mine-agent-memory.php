 ...
 * @since 1.1.0
 */
class WP_MCP_AI_Tool_Mine_Agent_Memory implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface, WP_MCP_AI_Tool_Usage_Guidance_Interface {
	...
	public function get_usage_guidance() {
		return array(
			'when_to_use'     => __( 'Bulk-ingesting posts, URLs, raw text, or chat transcripts into agent memory as verbatim records.', 'mcp-ai-wpoos' ),
			'when_not_to_use' => __( 'Writing a single memory record; use store_agent_context. Reading memory back; use retrieve_agent_memory.', 'mcp-ai-wpoos' ),
			'related_tools'   => array( 'store_agent_context', 'retrieve_agent_memory' ),
			'notes'           => __( 'Runs are capped at 200 records; long content is chunked. Set dry_run=true to preview without writing.', 'mcp-ai-wpoos' ),
		);
	}
