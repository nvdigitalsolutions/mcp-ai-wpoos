 ...
 */
class WP_MCP_AI_Tool_Get_Google_Analytics_Report implements WP_MCP_AI_Tool_Interface {
    ...
    public function execute( array $arguments = array(), array $context = array() ) {
        ...
        if ( is_wp_error( $response ) ) {
            ...
            return new WP_Error( 'wp_mcp_ai_google_analytics_http_error', __( 'The request to the Google Analytics Data API failed.', 'wp-mcp-ai' ), array( 'status' => 500, 'error' => $response ) );
        }
        ...
        if ( $status < 200 || $status >= 300 ) {
            WP_MCP_AI_Admin_Settings::log( 'Google Analytics report returned unexpected status.', array( 'status' => $status, 'body' => $body ) );
            ...
            return new WP_Error( 'wp_mcp_ai_google_analytics_error', $message, array( 'status' => $status, 'response' => $data ) );
        }
    ...
    protected function exchange_service_account_token( array $credentials, $scope, array $arguments, array $context ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error( 'wp_mcp_ai_google_analytics_token_error', __( 'Unable to obtain a Google Analytics access token.', 'wp-mcp-ai' ), array( 'status' => 500, 'error' => $response ) );
        }
        ...
        if ( $status < 200 || $status >= 300 || ! isset( $data['access_token'] ) ) {
            ...
            return new WP_Error( 'wp_mcp_ai_google_analytics_token_error', $message, array( 'status' => $status, 'response' => $data ) );
        }
