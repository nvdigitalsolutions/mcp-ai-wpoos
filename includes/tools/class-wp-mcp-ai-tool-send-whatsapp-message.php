...
class WP_MCP_AI_Tool_Send_WhatsApp_Message implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Send WhatsApp Message', 'wp-mcp-ai' );
	}
	...
	public function get_description() {
		return __( 'Sends a text message to a WhatsApp user via the Meta Cloud API.', 'wp-mcp-ai' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'access_token'    => array(
					'type'        => 'string',
					'description' => __( 'WhatsApp Cloud API access token used for authentication.', 'wp-mcp-ai' ),
				),
				'phone_number_id' => array(
					'type'        => 'string',
					'description' => __( 'Phone number ID assigned to the WhatsApp Business account.', 'wp-mcp-ai' ),
				),
				'to'              => array(
					'type'        => 'string',
					'description' => __( 'E.164 formatted phone number of the message recipient.', 'wp-mcp-ai' ),
				),
				'text'            => array(
					'type'        => 'string',
					'description' => __( 'Body text to send to the recipient.', 'wp-mcp-ai' ),
				),
				'preview_url'     => array(
					'type'        => 'boolean',
					'description' => __( 'Whether WhatsApp should render link previews inside the message.', 'wp-mcp-ai' ),
					'default'     => true,
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to send WhatsApp messages.', 'wp-mcp-ai' ) );
		}
		...
		if ( '' === $access_token ) {
			return new WP_Error( 'wp_mcp_ai_missing_whatsapp_token', __( 'A valid WhatsApp access token is required.', 'wp-mcp-ai' ) );
		}

		$phone_number_id = isset( $arguments['phone_number_id'] ) ? $this->sanitize_phone_number_id( $arguments['phone_number_id'] ) : '';
		if ( '' === $phone_number_id ) {
			return new WP_Error( 'wp_mcp_ai_missing_whatsapp_phone_number_id', __( 'A valid WhatsApp phone number ID must be provided.', 'wp-mcp-ai' ) );
		}

		$recipient = isset( $arguments['to'] ) ? $this->sanitize_phone_number( $arguments['to'] ) : '';
		if ( '' === $recipient ) {
			return new WP_Error( 'wp_mcp_ai_missing_whatsapp_recipient', __( 'A valid WhatsApp recipient phone number must be provided.', 'wp-mcp-ai' ) );
		}

		$message = isset( $arguments['text'] ) ? $this->sanitize_message( $arguments['text'] ) : '';
		if ( '' === $message ) {
			return new WP_Error( 'wp_mcp_ai_missing_whatsapp_message', __( 'Message text must be provided.', 'wp-mcp-ai' ) );
		}
		...
		if ( false === $body ) {
			return new WP_Error( 'wp_mcp_ai_encoding_error', __( 'Failed to encode the WhatsApp request payload.', 'wp-mcp-ai' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error(
				...
				__( 'The WhatsApp API request failed to send.', 'wp-mcp-ai' ),
		...
		if ( 200 !== $code || ! empty( $api_error ) ) {
			$message_text = __( 'The WhatsApp API returned an error.', 'wp-mcp-ai' );
