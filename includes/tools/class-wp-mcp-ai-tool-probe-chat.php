...
class WP_MCP_AI_Tool_Probe_Chat implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Shortcuts_Interface {
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( '' === $message ) {
			$message = __( 'Diagnostics probe issued from the MCP AI troubleshooting tool.', 'wp-mcp-ai' );
		}
		...
		if ( ! $controller instanceof WP_MCP_AI_REST ) {
			return new WP_Error( 'wp_mcp_ai_rest_unavailable', __( 'The MCP AI REST controller is not available for probing.', 'wp-mcp-ai' ) );
		}
	...
	protected function build_warnings( array $assistant_summary ) {
		...
		if ( '' === $provider ) {
			...
			$warnings[] = __( 'OpenAI is selected but the site is missing an API key in the MCP AI settings.', 'wp-mcp-ai' );
