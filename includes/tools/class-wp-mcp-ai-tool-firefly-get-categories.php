<?php
/**
 * Tool that retrieves category data from Firefly III API.
 *
 * @package WP_MCP_AI
 */
...
/**
 * Provides a tool for retrieving categories from Firefly III.
 */
class WP_MCP_AI_Tool_Firefly_Get_Categories implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Firefly III Get Categories', 'mcp-ai-wpoos' );
	}
	...
	public function get_description() {
		return __( 'Retrieve transaction categories from Firefly III personal finance manager. Categories help organize transactions (e.g., Groceries, Utilities, Entertainment, Transportation). Useful for expense tracking, spending analysis, and financial reporting.', 'mcp-ai-wpoos' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'limit'         => array(
					'type'        => 'integer',
					'description' => __( 'Maximum number of categories to retrieve (1-100).', 'mcp-ai-wpoos' ),
					'minimum'     => 1,
					'maximum'     => 100,
					'default'     => 50,
				),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id && ! $has_token ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be authenticated to retrieve Firefly III categories.', 'mcp-ai-wpoos' ), array( 'status' => rest_authorization_required_code() ) );
		}
		...
		$result = $client->get_categories( $options );
		...
		$summary = sprintf(
			...
			__( 'Retrieved %d categor%s from Firefly III', 'mcp-ai-wpoos' ),
			count( $categories ),
			1 === count( $categories ) ? 'y' : 'ies'
		);
		...
		/**
		 * Allow third parties to filter the Firefly III categories result.
		 *
		 * @param array $response  Final response payload.
		 * @param array $arguments Original tool arguments.
		 * @param array $context   Invocation context.
		 */
		$response = apply_filters( 'wp_mcp_ai_firefly_get_categories_result', $response, $arguments, $context );
