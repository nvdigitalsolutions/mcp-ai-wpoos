...
class WP_MCP_AI_REST_Validator {
	...
	/**
	 * Sanitize tool result for sending to chat-client.
	 *
	 * Ensures the result is safe for the chat-client to add to the conversation
	 * array and send back to the API in subsequent requests. Removes large
	 * binary data and extra fields that would violate provider message schemas.
	 *
	 * @param mixed                         $result        The tool result to sanitize.
	 * @param string                        $tool_name     The name of the tool.
	 ...
	 */
	public function sanitize_tool_result_for_chat( $result, $tool_name = '', $tool_instance = null ) {
		$original_result = $result;

		// If tool implements custom chat sanitization interface, use it.
		if ( $tool_instance && $tool_instance instanceof WP_MCP_AI_Tool_Chat_Sanitizer_Interface ) {
			$result = $tool_instance->sanitize_for_chat( $result );
			WP_MCP_AI_Logger::log_event(
				'chat_sanitization_custom',
				sprintf( 'Applied custom chat sanitization for tool: %s', $tool_name ),
				array(
					'tool_name' => $tool_name,
					'has_interface' => 'WP_MCP_AI_Tool_Chat_Sanitizer_Interface',
				)
			);
		} elseif ( $tool_instance && $tool_instance instanceof WP_MCP_AI_Tool_LLM_Sanitizer_Interface ) {
			// Fall back to LLM sanitization if no chat-specific sanitization exists.
			// This ensures tools that strip base64 content for LLM also strip it for chat.
			$result = $tool_instance->sanitize_for_llm( $result );
			WP_MCP_AI_Logger::log_event(
				'chat_sanitization_fallback',
				sprintf( 'Using LLM sanitization fallback for tool: %s', $tool_name ),
				array(
					'tool_name' => $tool_name,
					'has_interface' => 'WP_MCP_AI_Tool_LLM_Sanitizer_Interface',
				)
			);
		}

		// Allow filtering of tool results before sending to chat-client.
		$result = apply_filters( 'wp_mcp_ai_sanitize_tool_result_chat', $result, $tool_name );

		if ( $tool_name ) {
			$result = apply_filters( "wp_mcp_ai_sanitize_tool_result_chat_{$tool_name}", $result );
		}

		// Apply generic chat sanitization to ensure schema compliance.
		if ( is_array( $result ) || is_object( $result ) ) {
			$result = $this->sanitize_complex_data_for_chat( $result );
		}
	...
	/**
	 * Sanitize complex data structures for chat-client consumption.
	 *
	 * Recursively processes arrays and objects to strip large binary data
	 * and ensure compliance with provider message schemas.
	 *
	 * @param mixed $data The data to sanitize.
	 * @return mixed Sanitized data.
	 */
	protected function sanitize_complex_data_for_chat( $data ) {
		if ( is_array( $data ) ) {
			$sanitized      = array();
			$stripped_count = 0;
			foreach ( $data as $key => $value ) {
				// Strip known problematic fields that contain large binary data.
				if ( in_array( $key, array( 'data', 'data_url', 'raw', 'raw_response' ), true ) ) {
					$stripped_count++;
				...
				if ( is_array( $value ) || is_object( $value ) ) {
					$sanitized[ $sanitized_key ] = $this->sanitize_complex_data_for_chat( $value );
				} else {
					// Check if this is a large base64 string or data URL.
					if ( is_string( $value ) && $this->is_large_binary_data( $value ) ) {
						// Replace with a placeholder or skip.
						$stripped_count++;
						continue;
					}
					$sanitized[ $sanitized_key ] = $this->sanitize_scalar_for_chat( $value );
				}
			}

			// Log if we stripped any large data fields.
			if ( $stripped_count > 0 ) {
				WP_MCP_AI_Logger::log_event(
					'chat_sanitization_stripped_fields',
					sprintf( 'Stripped %d large binary data field(s) for chat-client compliance', $stripped_count ),
					array(
						'stripped_count' => $stripped_count,
						'context'        => 'chat_client_sanitization',
					)
				);
			}
		...
		if ( is_object( $data ) ) {
			return $this->sanitize_complex_data_for_chat( (array) $data );
		}

		return $this->sanitize_scalar_for_chat( $data );
	}

	/**
	 * Check if a string contains large binary data that should be stripped.
	 *
	 * @param string $value The value to check.
	 * @return bool True if the value contains large binary data.
	 */
	protected function is_large_binary_data( $value ) {
		// Check for data URLs (e.g., data:image/png;base64,...)
		if ( strpos( $value, 'data:' ) === 0 && strpos( $value, 'base64' ) !== false ) {
			return true;
		}

		// Check for large base64-encoded strings (>1KB indicates likely binary data).
		if ( strlen( $value ) > 1024 && $this->looks_like_base64( $value ) ) {
			return true;
		}
	 ...
	 *
	 * @param string $value The value to check.
	 * @return bool True if the value looks like base64.
	 */
	protected function looks_like_base64( $value ) {
		// Base64 strings only contain alphanumeric chars, +, /, and = for padding.
		// If more than 90% of a long string matches this pattern, it's likely base64.
		if ( strlen( $value ) < 100 ) {
			return false; // Too short to be meaningful binary data.
		}

		$base64_chars = preg_match_all( '/[A-Za-z0-9+\/=]/', $value );
		$ratio        = $base64_chars / strlen( $value );

		return $ratio > 0.9;
	}

	/**
	 * Sanitize scalar values for chat-client consumption.
	 *
	 * @param mixed $value The value to sanitize.
	 * @return mixed Sanitized value.
	 */
	protected function sanitize_scalar_for_chat( $value ) {
		if ( is_string( $value ) ) {
			return wp_check_invalid_utf8( $value, true );
		}
