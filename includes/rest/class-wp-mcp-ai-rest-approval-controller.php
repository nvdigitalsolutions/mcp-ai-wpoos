...
class WP_MCP_AI_REST_Approval_Controller extends WP_REST_Controller {
	 ...
	 *
	 * @param WP_REST_Request $request Parameter description.
	...
	public function get_item_permissions_check( $request ) {
		...
		$queue  = WP_MCP_AI_Approval_Queue::get_instance();
		$record = $queue->get( (int) $request['id'] );
		if ( $record && (int) $record['requester_id'] === get_current_user_id() ) { // phpcs:ignore WordPress.PHP.YodaConditions.NotYoda -- Intentional readability order.
			return true;
		}
