<?php
/**
 * Paper Index — Inverted index for flat-file record collections.
 *
 * Maintains a lightweight inverted index that maps tags, statuses, types,
 * author IDs, and date-buckets to record IDs within a collection.
 * Stored as JSON at `_indexes/{collection}.idx.json`.
 *
 * @package WP_MCP_AI
 * @since   1.3.0
...
/**
 * Class WP_MCP_AI_Paper_Index
 *
 * Each collection gets one index file. The index is lazily built on first
 * query and auto-updated on write/delete operations. File-level locking
 * (flock) prevents corruption during concurrent writes.
 */
class WP_MCP_AI_Paper_Index {
	...
	public function __construct( $collection, $collection_dir, $indexes_dir ) {
		...
		$this->index_path     = trailingslashit( $indexes_dir ) . $this->collection . '.idx.json';
	 ...
	 *
	 * @param WP_MCP_AI_Paper_Driver_Interface $driver  The format driver for this collection.
	 * @param string                           $extension File extension to scan (e.g. '.json').
	 * @return bool True on success.
	 */
	public function rebuild( WP_MCP_AI_Paper_Driver_Interface $driver, $extension ) {
		$this->data  = $this->empty_structure();
		$this->dirty = true;
		...
		$pattern = trailingslashit( $this->collection_dir ) . '*' . $extension;
		$files   = glob( $pattern );
	 ...
	 */
	public function ensure_exists( WP_MCP_AI_Paper_Driver_Interface $driver, $extension ) {
		if ( file_exists( $this->index_path ) ) {
			return true;
		}
