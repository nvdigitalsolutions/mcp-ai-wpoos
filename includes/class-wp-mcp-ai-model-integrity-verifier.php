 ...
 *
 * Verifies model hashes, checks for known-vulnerable model versions,
 * and validates provider endpoint integrity before establishing connections.
 *
 * @package WP_MCP_AI
 * @since   1.1.51
 ...
 * @since 1.1.51
 */
class WP_MCP_AI_Model_Integrity_Verifier {

	/**
	 * Option key for known model hashes.
	 *
	 * @var string
	 */
	...
	/**
	 * Option key for blocked model versions.
	 *
	 * @var string
	 */
	...
	/**
	 * Option key for integrity check results.
	 *
	 * @var string
	 */
		...
		if ( isset( $blocked[ $key ] ) && ! empty( $blocked[ $key ] ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Model %1$s from %2$s is blocked: %3$s', 'mcp-ai-wpoos' ),
					$model,
					$provider,
					$reason
				)
		...
		if ( empty( $parsed['scheme'] ) || 'https' !== $parsed['scheme'] ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Self-hosted provider %s must use HTTPS for non-localhost connections to protect API keys in transit.', 'mcp-ai-wpoos' ),
					$provider
				)
			);
		}
	 ...
	 *
	 * Queries a local allowlist of known-vulnerable model versions.
	 * For production deployments, integrate with a threat intelligence feed.
	 *
	 * @param string $model    Model identifier.
	 * @param string $provider Provider identifier.
	 * @return true|WP_Error
		...
		if ( isset( $known_vulnerabilities[ $key ] ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Model %1$s from %2$s has a known vulnerability: %3$s', 'mcp-ai-wpoos' ),
					$model,
					$provider,
