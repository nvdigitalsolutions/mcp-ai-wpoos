	...
	public function parse_file( $file_path ) {
		if ( ! file_exists( $file_path ) || ! is_readable( $file_path ) ) {
			return new WP_Error(
				...
				__( 'The skill file could not be found or is not readable.', 'mcp-ai-wpoos' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( false === $content ) {
			return new WP_Error(
				...
				__( 'Failed to read the skill file.', 'mcp-ai-wpoos' ),
				array( 'status' => 500 )
			);
		}
	...
	public function parse( $content ) {
		if ( ! is_string( $content ) || '' === trim( $content ) ) {
			return new WP_Error(
				...
				__( 'Skill content is empty.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
	...
	private function extract_frontmatter( $content ) {
		...
		if ( ! preg_match( '/\A---\s*\n(.*?)\n---\s*\n?/s', $content, $matches ) ) {
			return new WP_Error(
				...
				__( 'No valid YAML frontmatter found. SKILL.md must start with --- delimited YAML.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
	...
	private function validate_metadata( $metadata ) {
		...
		if ( empty( $metadata['name'] ) ) {
			return new WP_Error(
				...
				__( 'Skill name is required in YAML frontmatter.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( strlen( $name ) > self::MAX_NAME_LENGTH ) {
			return new WP_Error(
				...
				sprintf( __( 'Skill name must not exceed %d characters.', 'mcp-ai-wpoos' ), self::MAX_NAME_LENGTH ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! preg_match( '/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/', $name ) || false !== strpos( $name, '--' ) ) {
			return new WP_Error(
				...
				__( 'Skill name must contain only lowercase letters, digits, and hyphens. It cannot start/end with a hyphen or contain consecutive hyphens.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( empty( $metadata['description'] ) ) {
			return new WP_Error(
				...
				__( 'Skill description is required in YAML frontmatter.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( strlen( $metadata['description'] ) > self::MAX_DESCRIPTION_LENGTH ) {
			return new WP_Error(
				...
				/* translators: %d: maximum allowed length */
				sprintf( __( 'Skill description must not exceed %d characters.', 'mcp-ai-wpoos' ), self::MAX_DESCRIPTION_LENGTH ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! empty( $metadata['compatibility'] ) && strlen( $metadata['compatibility'] ) > self::MAX_COMPATIBILITY_LENGTH ) {
			return new WP_Error(
				...
				/* translators: %d: maximum allowed length */
				sprintf( __( 'Skill compatibility field must not exceed %d characters.', 'mcp-ai-wpoos' ), self::MAX_COMPATIBILITY_LENGTH ),
				array( 'status' => 400 )
			);
		}
