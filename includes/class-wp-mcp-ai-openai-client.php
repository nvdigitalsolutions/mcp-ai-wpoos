...
if ( ! class_exists( 'WP_MCP_AI_OpenAI_Client' ) ) {
	...
	class WP_MCP_AI_OpenAI_Client {
		...
		public function create_chat_completion( array $messages, array $options = array() ) {
			...
			// Enable SSE streaming when requested (real-time curl streaming in REST handler).
		...
		$request_headers = $this->build_request_headers( $api_key );
			...
			if ( $is_streaming && function_exists( 'curl_init' ) ) {
				...
				if ( null !== $realtime_cb ) {
					WP_MCP_AI_Logger::log_event(
						'openai_request',
						'Sending real-time streaming request to OpenAI via cURL.',
						array(
							'model'    => $model,
							'realtime' => true,
						)
	 ...
	 * each network chunk as it arrives from OpenAI.  Every `delta.content`
	...
	protected function do_realtime_curl_stream( $url, array $payload, $model, $timeout, $stream_callback ) {
		...
		if ( $http_status >= 400 ) {
			...
			return new WP_Error(
				'wp_mcp_ai_api_error',
				__( 'OpenAI returned an error during streaming.', 'mcp-ai-wpoos' ),
