...
| `WP_MCP_AI_Agent_Harness_Evolver` | `class-wp-mcp-ai-agent-harness-evolver.php` | Continual Harness (Karten et al., 2026) — reads audit trail trajectories, detects failure signatures, applies CRUD to prompt/roles/skills/memory mid-session. Includes `WP_MCP_AI_Agent_Harness_Evolver_Role_Adapter` for evolved role registration. Since 1.9.0: `analyze_failures()`, component-scoped `evolve()`, dry-run mode, per-assistant budget guard. |
...
| `WP_MCP_AI_Evolved_Prompt_Resolver` (static) | `class-wp-mcp-ai-evolved-prompt-resolver.php` | Opt-in consumption of `_wp_mcp_ai_evolved_system_prompt` via `wp_mcp_ai_resolved_system_prompt` (priority 15); no-op unless `wp_mcp_ai_harness_use_evolved_prompt` returns true. Since 1.9.0 also consults `WP_MCP_AI_Artifact_Shadow` (Phase F) — shadow-served variants never write the deployed meta |
...
- **Harness Evolver events:** `wp_mcp_ai_harness_evolution_enabled`, `wp_mcp_ai_harness_evolution_frequency`, `wp_mcp_ai_harness_evolution_warmup`, `wp_mcp_ai_harness_evolution_max_window` (filters — configure evolution behaviour), `wp_mcp_ai_harness_evolution_completed`, `wp_mcp_ai_harness_evolution_failed` (actions — fire after evolution passes), `wp_mcp_ai_agent_roles` (filter at priority 20 — registers evolved roles). Since 1.9.0: `wp_mcp_ai_harness_evolution_budget_usd` (filter — hourly budget per assistant, default $5.00), `wp_mcp_ai_harness_use_evolved_prompt` (filter — opt-in evolved-prompt consumption, default false). Since 1.9.0 the same switches can be managed from Settings → Orchestration Layer: `WP_MCP_AI_Evolution_Settings_Bridge` (priority 5) feeds saved values into these filters; unsaved settings leave the code defaults untouched.
