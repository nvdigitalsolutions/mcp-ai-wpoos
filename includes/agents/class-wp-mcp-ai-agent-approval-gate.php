 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Agent_Approval_Gate {
	...
	public static function request_approval( $session_id, $action_type, $action_data, $risk_level ) {
		...
		if ( ! in_array( $action_type, self::VALID_ACTION_TYPES, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid action type: %s.', 'mcp-ai-wpoos' ),
		...
		if ( ! in_array( $risk_level, self::VALID_RISK_LEVELS, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid risk level: %s.', 'mcp-ai-wpoos' ),
					esc_html( $risk_level )
				)
			);
		}
		...
		if ( 'critical' === $risk_level ) {
			...
			if ( true !== $override ) {
				return new WP_Error(
					'approval_denied',
					__( 'Critical-risk actions require explicit administrator approval.', 'mcp-ai-wpoos' ),
					array(
	...
	public static function resolve_approval( $approval_id, $approved, $reason ) {
		...
		if ( empty( $approval_id ) ) {
			return new WP_Error(
				'missing_approval_id',
				__( 'Approval ID is required.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! is_array( $approval ) ) {
			return new WP_Error(
				'approval_not_found',
				__( 'Approval record not found or expired.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( isset( $approval['status'] ) && 'pending' !== $approval['status'] ) {
			return new WP_Error(
				'already_resolved',
				__( 'This approval has already been resolved.', 'mcp-ai-wpoos' )
			);
		}
	...
	private static function create_pending_approval( $session_id, $action_type, $action_data, $risk_level ) {
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'Action requires %1$s-risk approval. Approval ID: %2$s.', 'mcp-ai-wpoos' ),
				esc_html( $risk_level ),
