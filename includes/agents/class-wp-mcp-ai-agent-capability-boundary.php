 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Agent_Capability_Boundary {
 ...
 * @since 1.2.0
 */
// phpcs:ignore Generic.Files.OneObjectStructurePerFile.MultipleFound -- Related helper class.
/**
\t * Class WP_MCP_AI_Agent_Capability_Boundary_Hooks
\t *
\t * Registers capability boundary hooks for agent tool execution.
\t */
class WP_MCP_AI_Agent_Capability_Boundary_Hooks {
 // phpcs:ignore Generic.Files.OneObjectStructurePerFile.MultipleFound -- Related helper class.
	...
	public static function on_before_tool_execution( $tool_slug, $arguments, $context ) {
		...
		// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_die() with WP_Error handles escaping internally via its default error handler.
			...
			wp_die(
				new WP_Error(
					...
					sprintf(
						...
						__( 'Tool "%s" is not allowed in this agent session.', 'mcp-ai-wpoos' ),
						esc_html( $tool_slug )
					),
					array( 'status' => 403 )
				),
		...
		if ( ! $boundary->check_rate_limit( $tool_slug ) ) {
			wp_die(
				new WP_Error(
					...
					sprintf(
						/* translators: %s: tool slug */
						__( 'Tool "%s" has exceeded its rate limit for this session.', 'mcp-ai-wpoos' ),
						esc_html( $tool_slug )
					),
					array( 'status' => 429 )
				),
		...
		if ( $boundary->is_budget_exhausted() ) {
			wp_die(
				new WP_Error(
					...
					__( 'Agent session budget exhausted — maximum iterations reached.', 'mcp-ai-wpoos' ),
					array( 'status' => 429 )
				),
				429
			);
		...
		$approval = WP_MCP_AI_Agent_Approval_Gate::request_approval(
			$session_id,
			'tool_execution',
			array(
				'tool_slug'    => $tool_slug,
	 ...
	 * @since 1.2.0
	 *
	 * @return WP_MCP_AI_Agent_Capability_Boundary|null
	 */
	private static function get_current_boundary() {
		...
		return new WP_MCP_AI_Agent_Capability_Boundary( $session_id, $tool_slugs );
	}
