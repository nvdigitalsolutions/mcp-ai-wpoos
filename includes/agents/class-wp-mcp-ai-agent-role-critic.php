 ...
 * @since 1.1.0
 */
class WP_MCP_AI_Agent_Role_Critic extends WP_MCP_AI_Agent_Role_Base {
	...
	public function __construct() {
		...
		$this->role_name        = __( 'Critic', 'mcp-ai-wpoos' );
		$this->role_description = __( 'Validates results from other agents, checks quality, and provides feedback for improvement.', 'mcp-ai-wpoos' );
	...
	public function execute_role_task( $task, $context ) {
		...
		if ( empty( $task['result_to_validate'] ) ) {
			return new WP_Error(
				...
				__( 'No result provided for validation.', 'mcp-ai-wpoos' )
			);
		}
	...
	protected function check_completeness( $result, $criteria ) {
		...
		if ( empty( $result ) ) {
			$score    = 0.0;
			$issues[] = __( 'Result is empty', 'mcp-ai-wpoos' );
		} elseif ( is_array( $result ) && count( $result ) === 0 ) {
			$score    = 0.0;
			$issues[] = __( 'Result array is empty', 'mcp-ai-wpoos' );
		}
			...
			foreach ( $criteria['required_fields'] as $field ) {
				if ( ! isset( $result[ $field ] ) ) {
					$issues[] = sprintf(
						...
						__( 'Missing required field: %s', 'mcp-ai-wpoos' ),
						$field
					);
					$score -= 0.1;
				}
	...
	protected function check_quality( $result, $criteria ) {
		...
		if ( is_string( $result ) && strlen( $result ) < 10 ) {
			$score   -= 0.2;
			$issues[] = __( 'Result is too short to be meaningful', 'mcp-ai-wpoos' );
		}
	...
	protected function generate_feedback( $validation, $criteria ) {
		...
		if ( ! empty( $feedback['issues'] ) ) {
			$feedback['recommendations'][] = __( 'Address all identified issues before proceeding', 'mcp-ai-wpoos' );
			$feedback['summary']           = sprintf(
				/* translators: %d: number of issues */
				_n( '%d issue found that needs attention.', '%d issues found that need attention.', count( $feedback['issues'] ), 'mcp-ai-wpoos' ),
				count( $feedback['issues'] )
			);
		} else {
			$feedback['summary'] = __( 'Result meets all validation criteria.', 'mcp-ai-wpoos' );
		}
