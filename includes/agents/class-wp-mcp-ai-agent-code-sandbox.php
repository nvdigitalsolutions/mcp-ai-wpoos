 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Agent_Code_Sandbox {
	...
	public function execute( $code, $language, $timeout_seconds = null, $working_dir = '' ) {
		...
		if ( ! in_array( $language, $allowed_languages, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Unsupported language "%1$s". Supported: %2$s.', 'mcp-ai-wpoos' ),
					esc_html( $language ),
		...
		if ( '' === $code ) {
			return new WP_Error(
				'empty_code',
				__( 'No code provided for execution.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( false === $written ) {
			...
			return new WP_Error(
				'write_failed',
				__( 'Failed to write code to temporary file.', 'mcp-ai-wpoos' )
			);
		}
			...
			default:
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Unknown language: %s.', 'mcp-ai-wpoos' ),
						esc_html( $language )
					)
				);
		}
	...
	private static function run_process( $command, $working_dir, $timeout ) {
		...
		if ( ! is_resource( $process ) ) {
			return new WP_Error(
				'process_failed',
				__( 'Failed to start execution process.', 'mcp-ai-wpoos' )
			);
		}
		...
		while ( true ) {
			...
			if ( ( time() - $start_time ) >= $timeout ) {
				...
				$killed    = true;
				$truncated = true;
				$stderr   .= "\n[" . esc_html__( 'Process terminated: maximum execution time exceeded.', 'mcp-ai-wpoos' ) . ']';
				break;
			}
	...
	private static function create_temp_directory() {
		...
		if ( ! wp_mkdir_p( $temp_dir ) ) {
			return new WP_Error(
				'temp_dir_failed',
				__( 'Failed to create temporary sandbox directory.', 'mcp-ai-wpoos' )
			);
		}
