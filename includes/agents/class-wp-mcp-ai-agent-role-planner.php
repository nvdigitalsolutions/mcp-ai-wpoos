 ...
 * @since 1.1.0
 */
class WP_MCP_AI_Agent_Role_Planner extends WP_MCP_AI_Agent_Role_Base {
	...
	public function __construct() {
		...
		$this->role_name        = __( 'Planner', 'mcp-ai-wpoos' );
		$this->role_description = __( 'Analyzes complex tasks and breaks them down into coordinated subtasks for other agents to execute.', 'mcp-ai-wpoos' );
	...
	protected function decompose_task( $task, $complexity ) {
		...
		switch ( $complexity ) {
			case 'very_complex':
				$subtasks[] = $this->create_subtask( 'research', __( 'Gather and analyze required information', 'mcp-ai-wpoos' ), 1 );
				$subtasks[] = $this->create_subtask( 'process', __( 'Process and organize findings', 'mcp-ai-wpoos' ), 2 );
				$subtasks[] = $this->create_subtask( 'synthesis', __( 'Synthesize comprehensive results', 'mcp-ai-wpoos' ), 3 );
				$subtasks[] = $this->create_subtask( 'validation', __( 'Validate and refine output', 'mcp-ai-wpoos' ), 4 );
				break;

			case 'complex':
				$subtasks[] = $this->create_subtask( 'analysis', __( 'Analyze requirements', 'mcp-ai-wpoos' ), 1 );
				$subtasks[] = $this->create_subtask( 'execution', __( 'Execute main task', 'mcp-ai-wpoos' ), 2 );
				$subtasks[] = $this->create_subtask( 'review', __( 'Review and finalize', 'mcp-ai-wpoos' ), 3 );
				break;

			case 'moderate':
				$subtasks[] = $this->create_subtask( 'prepare', __( 'Prepare and gather context', 'mcp-ai-wpoos' ), 1 );
				$subtasks[] = $this->create_subtask( 'execute', __( 'Execute task', 'mcp-ai-wpoos' ), 2 );
				break;
