 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Agent_Harness_Bootstrap {
	...
	public static function save_state( $assistant_id, $session_id ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				...
				__( 'A valid assistant ID is required to save harness state.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( empty( $session_id ) ) {
			return new WP_Error(
				...
				__( 'A valid session ID is required to save harness state.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $saved ) {
			return new WP_Error(
				...
				__( 'Failed to save harness bootstrap bundle.', 'mcp-ai-wpoos' ),
				array( 'status' => 500 )
			);
		}
	...
	public static function load_state( $assistant_id, $bundle_id ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				'wp_mcp_ai_bootstrap_invalid_assistant',
				__( 'A valid assistant ID is required to load harness state.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $bundle_assistant !== $assistant_id ) {
			return new WP_Error(
				...
				__( 'This bootstrap bundle does not belong to the specified assistant.', 'mcp-ai-wpoos' ),
				array( 'status' => 403 )
			);
		}
	...
	protected static function get_bundle( $bundle_id ) {
		if ( empty( $bundle_id ) ) {
			return new WP_Error(
				...
				__( 'Bundle ID is required.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( null === $bundle || ! is_array( $bundle ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Bootstrap bundle "%s" not found.', 'mcp-ai-wpoos' ),
					esc_html( $bundle_id )
				),
				array( 'status' => 404 )
			);
