 ...
 * @since 1.2.0
 */
class WP_MCP_AI_Agent_Audit_Trail {
	...
	public static function start_session( $session_id, $context ) {
		...
		if ( empty( $session_id ) ) {
			return new WP_Error(
				...
				__( 'Session ID is required and must not be empty.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $context['assistant_id'] ) ) {
			return new WP_Error(
				...
				__( 'Context must include assistant_id.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function log_decision( $trail_id, $step, $data ) {
		...
		if ( empty( $trail_id ) || empty( $step ) ) {
			return new WP_Error(
				...
				__( 'Trail ID and step are required.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function log_tool_call( $trail_id, $tool_slug, $arguments, $result, $duration_ms ) {
		...
		if ( empty( $trail_id ) || empty( $tool_slug ) ) {
			return new WP_Error(
				...
				__( 'Trail ID and tool slug are required.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function log_output( $trail_id, $output ) {
		...
		if ( empty( $trail_id ) ) {
			return new WP_Error(
				...
				__( 'Trail ID is required.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function end_session( $trail_id, $status, $metadata = array() ) {
		...
		if ( empty( $trail_id ) || empty( $status ) ) {
			return new WP_Error(
				...
				__( 'Trail ID and status are required.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! in_array( $status, self::VALID_TRAIL_STATUSES, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid trail status. Valid statuses: %s', 'mcp-ai-wpoos' ),
		...
		if ( self::is_trail_closed( $trail_id ) ) {
			return new WP_Error(
				...
				__( 'This audit trail is already closed and cannot accept new events.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function get_trails_by_session( $session_id ) {
		...
		if ( empty( $session_id ) ) {
			return new WP_Error(
				'wp_mcp_ai_audit_invalid_session',
				__( 'Session ID is required.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function register_cpt() {
		...
		$labels = array(
			'name'          => __( 'Audit Events', 'mcp-ai-wpoos' ),
			'singular_name' => __( 'Audit Event', 'mcp-ai-wpoos' ),
		);
		...
		if ( is_wp_error( $post_id ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Failed to store audit event: %s', 'mcp-ai-wpoos' ),
	...
	private static function store_event_options( $event ) {
		...
		if ( ! $updated ) {
			return new WP_Error(
				...
				__( 'Failed to store audit event in options.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $events ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Audit trail not found: %s', 'mcp-ai-wpoos' ),
					$trail_id
				)
			);
		}
	...
	private static function get_trail_meta( $trail_id ) {
		...
		if ( empty( $events ) ) {
			return new WP_Error(
				...
				__( 'Audit trail contains no events.', 'mcp-ai-wpoos' )
			);
		}
