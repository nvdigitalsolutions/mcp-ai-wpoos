...
/**
 * Renders the `wp-mcp-ai/tools-grid` block on the server.
 *
 ...
 */
function wp_mcp_ai_render_tools_grid_block( $attributes, $content = '', $block = null ) {
	...
	// Get tool groups.
	$groups    = wp_mcp_ai_get_tool_groups_for_blocks();
	...
	if ( null !== $block && function_exists( 'get_block_wrapper_attributes' ) ) {
		...
		$wrapper_attributes = 'class="wp-block-wp-mcp-ai-tools-grid" data-block-id="' . esc_attr( $unique_id ) . '"';
	}
...
/**
 * Get tool groups data for blocks.
 *
 * @return array
 */
function wp_mcp_ai_get_tool_groups_for_blocks() {
	$groups = array();
