<?php
/**
 * Site Blueprint Compiler — loads, validates, and compiles reusable subgraph
 * blueprints into executable pipeline graphs.
 *
 * This is the site-builder analogue of ComfyUI's subgraph / template system:
 * a pre-wired collection of nodes and edges with exposed input/output ports.
 * When instantiated with concrete values, the compiler substitutes {placeholders}
 * and resolves internal node IDs to produce a flat pipeline graph that the
 * WP_MCP_AI_Site_Pipeline_Executor can execute.
 *
 * Blueprints are stored as JSON files in registered directories (default:
 * config/site-blueprints/). Third-party plugins can register additional
 * directories via the 'wp_mcp_ai_site_blueprint_directories' filter.
 *
 ...
 *     },
 *     "outputs": {
 *       "html": { "node": "hero_flex", "port": "html" }
 *     },
 *     "internalGraph": {
 *       "nodes": { ... },
 *       "edges": [ ... ],
 *       "outputNode": "hero_flex"
 *     }
 *   }
 *
 * @package    WP_MCP_AI
 ...
 */
class WP_MCP_AI_Site_Blueprint_Compiler {

	/**
	 * Default blueprint directory within the plugin.
	 *
	 * @since 1.2.0
	 * @var string
	 */
	const DEFAULT_DIR = WP_MCP_AI_PATH . 'config/site-blueprints/';
	 ...
	 * @return array|WP_Error Pipeline graph ready for WP_MCP_AI_Site_Pipeline_Executor,
		...
		if ( empty( $internal['nodes'] ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Blueprint "%s" contains no internal nodes.', 'mcp-ai-wpoos' ),
					esc_html( $slug )
				)
			);
		}
