<?php
/**
 * Site Pipeline Executor — walks a node-graph DAG, resolves inputs,
 * executes nodes in topological order, and caches intermediate results.
 *
 * This is the site-builder analogue of ComfyUI's execution.py.
 * Given a pipeline graph (nodes + edges), the executor:
 *   1. Topologically sorts the nodes based on edge dependencies.
 *   2. For each node, resolves inputs from upstream nodes' outputs
 *      (connected via edges) and static node config.
 *   3. Computes an input hash; if unchanged from a previous run, the
 *      cached output is reused (ComfyUI's "only re-execute changed parts").
 *   4. Executes the node, caches the result, and feeds it downstream.
 *
 * Caching is backed by WordPress transients (configurable TTL).
 * The cache can be cleared per-pipeline via clear_cache( $pipeline_id ).
 *
 * @package    WP_MCP_AI
 ...
 */
class WP_MCP_AI_Site_Pipeline_Executor {
	 ...
	 *
	 * @since 1.2.0
	 * @var WP_MCP_AI_Site_Node_Registry|null
	 */
	protected $registry;
	 ...
	 * @since 1.2.0
	 *
	 * @return WP_MCP_AI_Site_Node_Registry
	 */
	protected function get_registry(): WP_MCP_AI_Site_Node_Registry {
	...
	public function execute( array $pipeline_graph, string $pipeline_id = '' ) {
		...
		if ( empty( $nodes ) ) {
			return new WP_Error(
				...
				__( 'The pipeline graph contains no nodes.', 'mcp-ai-wpoos' )
			);
		}
			...
			if ( ! isset( $node_config['slug'] ) ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Node "%s" is missing a slug.', 'mcp-ai-wpoos' ),
						esc_html( $node_id )
					)
				);
			}
	...
	protected function topological_sort( array $nodes, array $edges ) {
		...
		if ( count( $sorted ) !== count( $node_ids ) ) {
			return new WP_Error(
				...
				__( 'The pipeline graph contains a cycle. DAGs must be acyclic.', 'mcp-ai-wpoos' )
			);
		}
				...
				if ( ! isset( $outputs[ $src_node ] ) ) {
					return new WP_Error(
						...
						sprintf(
							...
							__( 'Cannot resolve input for node "%1$s": upstream node "%2$s" has not been executed.', 'mcp-ai-wpoos' ),
							esc_html( $node_id ),
							esc_html( $src_node )
						)
					);
