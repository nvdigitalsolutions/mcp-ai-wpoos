 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Learning_Log {
	...
	public static function record( array $entry ) {
		...
		if ( '' === $artifact_type ) {
			return new WP_Error(
				...
				__( 'Learning-log entries require an artifact type.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
	...
	private static function related_hashes( $hash, $strategy ) {
		if ( self::STRATEGY_SIBLINGS === $strategy ) {
			...
			$entry  = WP_MCP_AI_Artifact_Population::get_entry( $hash );
		...
		if ( class_exists( 'WP_MCP_AI_Artifact_Population' ) ) {
			...
			for ( $depth = 0; $depth < 5; $depth++ ) {
				$entry = WP_MCP_AI_Artifact_Population::get_entry( $current );
