 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Approval_Queue {
	...
	public static function enqueue( $assistant_id, $action, $artifact_type, $payload, array $options = array() ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				...
				__( 'A valid assistant ID is required to queue an approval.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! in_array( $action, self::ALLOWED_ACTIONS, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Queue action "%s" is not supported.', 'mcp-ai-wpoos' ),
					$action
				)
			);
		}
		...
		if ( '' === $artifact_type ) {
			return new WP_Error(
				...
				__( 'A queue item requires an artifact type.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( $pending_here >= self::MAX_PENDING_PER_ASSISTANT ) {
			return new WP_Error(
				...
				__( 'This assistant already has the maximum number of pending approvals.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function approve( $item_id, $approver_id = 0, $note = '' ) {
		...
		if ( null === $item ) {
			return new WP_Error(
				...
				__( 'The approval item does not exist.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( 'pending' !== $item['status'] ) {
			return new WP_Error(
				...
				__( 'This approval item has already been decided.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! current_user_can( 'edit_post', (int) $item['assistant_id'] ) ) {
			return new WP_Error(
				...
				__( 'You are not allowed to approve artifacts for this assistant.', 'mcp-ai-wpoos' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( 'promote' === $item['action'] ) {
			if ( ! class_exists( 'WP_MCP_AI_Artifact_Deploy' ) ) {
				return new WP_Error(
					...
					__( 'The artifact deploy subsystem is not available.', 'mcp-ai-wpoos' )
				);
			}
			...
			$result = WP_MCP_AI_Artifact_Deploy::rollback( (int) $item['assistant_id'], $item['artifact_type'] );
		}
	...
	public static function reject( $item_id, $approver_id = 0, $note = '' ) {
		...
		if ( ! current_user_can( 'edit_post', (int) $item['assistant_id'] ) ) {
			return new WP_Error(
				'wp_mcp_ai_artifact_queue_forbidden',
				__( 'You are not allowed to reject artifacts for this assistant.', 'mcp-ai-wpoos' ),
				array( 'status' => 403 )
			);
		}
