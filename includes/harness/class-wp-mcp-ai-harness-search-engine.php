...
class WP_MCP_AI_Harness_Search_Engine {
	...
	public static function start_search( $assistant_id, array $seed_profiles, array $search_set_suites, array $opts = array() ) {
		...
		if ( empty( $seed_profiles ) ) {
			return new WP_Error(
				...
				__( 'At least one seed profile is required.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $search_set_suites ) ) {
			return new WP_Error(
				...
				__( 'At least one eval suite slug is required for the search set.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( false !== $existing ) {
			return new WP_Error(
				...
				sprintf( __( 'A search run is already active for assistant %d. Wait for it to complete or cancel it.', 'mcp-ai-wpoos' ), $assistant_id )
			);
		}
		...
		if ( empty( $seeds ) ) {
			...
			return new WP_Error(
				...
				__( 'No valid seed profiles after sanitization.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function step_search( $assistant_id ) {
		...
		if ( ! isset( self::$active_searches[ $assistant_id ] ) ) {
			...
			if ( false === $lock_time || empty( $population ) ) {
				return new WP_Error(
					...
					__( 'No active search run found for this assistant.', 'mcp-ai-wpoos' )
				);
			}
		...
		$result = array(
			...
			'message'   => sprintf(
				...
				__( 'Evaluated %1$d candidates. Iteration %2$d/%3$d.', 'mcp-ai-wpoos' ),
				$evaluated_total,
				$search['current_iter'],
				$search['iterations']
			),
	...
	public static function validate_candidate( array $profile ) {
		if ( empty( $profile ) ) {
			return new WP_Error(
				...
				__( 'Profile must not be empty.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $clean['enabled'] ) ) {
			return new WP_Error(
				...
				__( 'Profile must have enabled=true to participate in search.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function diff_profiles( $assistant_id, $hash_a, $hash_b ) {
		...
		if ( ! isset( $population[ $hash_a ] ) ) {
			return new WP_Error( 'wp_mcp_ai_search_hash_not_found', __( 'Profile A not found.', 'mcp-ai-wpoos' ) );
		}
		if ( ! isset( $population[ $hash_b ] ) ) {
			return new WP_Error( 'wp_mcp_ai_search_hash_not_found', __( 'Profile B not found.', 'mcp-ai-wpoos' ) );
		}
	...
	private static function evaluate_candidate( $assistant_id, array $profile, array $suite_slugs ) {
		if ( ! class_exists( 'WP_MCP_AI_Eval_Suite_Registry' ) ) {
			return new WP_Error(
				...
				__( 'Eval suite registry is not available.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $all_results ) ) {
			return new WP_Error(
				...
				__( 'No eval suites could be evaluated.', 'mcp-ai-wpoos' )
			);
		}
	 ...
	 */
	private static function build_profile_generator( $assistant_id, array $profile, WP_MCP_AI_Eval_Suite $suite ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- $suite provides context for the generator; required by the eval runner contract.
		...
		return static function ( WP_MCP_AI_Eval_Case $case, array $suite_context ) use ( $_profile, $_assistant_id ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase,Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Universal.NamingConventions.NoReservedKeywordParameterNames.caseFound -- $case required by WP_MCP_AI_Eval_Case type; $suite_context required by eval runner contract.
			...
			if ( ! empty( $_profile['cues'] ) && class_exists( 'WP_MCP_AI_Prompt_Cue_Library' ) ) {
	...
	private static function best_of_n_proposer( array $population, $k ) {
		...
		for ( $i = 0; $i < $attempts && $candidate_count < $k; ++$i ) {
			...
			$clean = WP_MCP_AI_Harness_Profile::sanitize( $mutated );
			$hash  = self::profile_hash( $clean );
	...
	private static function apply_mutation( array $profile, $mutation ) {
		switch ( $mutation ) {
			...
			case 'swap_cue':
				if ( ! class_exists( 'WP_MCP_AI_Prompt_Cue_Library' ) ) {
					break;
				}
				$library  = WP_MCP_AI_Prompt_Cue_Library::get_instance();
