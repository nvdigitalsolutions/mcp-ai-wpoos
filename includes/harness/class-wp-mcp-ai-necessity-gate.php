 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Necessity_Gate {
			...
			default:
				...
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Action blocked by necessity gate: %1$s — %2$s', 'mcp-ai-wpoos' ),
						$tool_slug,
						$verdict['reason']
					)
				);
	...
	private static function require_approval( $tool_slug, $arguments, $verdict ) {
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: tool name, 2: reason */
				__( 'Human approval required: %1$s — %2$s. Please approve or deny this action.', 'mcp-ai-wpoos' ),
				$tool_slug,
				$verdict['reason']
			)
		);
	...
	private static function log_decision( $tool_slug, $action, $reason ) {
		...
		WP_MCP_AI_Logger::log_event(
			...
			sprintf(
				...
				__( 'Necessity gate: %1$s — %2$s', 'mcp-ai-wpoos' ),
				$action,
				$tool_slug
			),
			array(
