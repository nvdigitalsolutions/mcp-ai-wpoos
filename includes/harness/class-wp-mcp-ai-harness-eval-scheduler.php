...
class WP_MCP_AI_Harness_Eval_Scheduler {
	...
	public static function run_suite_for_assistant( $assistant_id, $suite_slug ) {
		...
		if ( $assistant_id <= 0 || '' === $suite_slug ) {
			return new WP_Error( 'wp_mcp_ai_harness_eval_invalid_args', __( 'Invalid assistant or suite.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( ! $suite instanceof WP_MCP_AI_Eval_Suite ) {
			return new WP_Error(
				...
				sprintf( __( 'Eval suite "%s" is not registered.', 'mcp-ai-wpoos' ), $suite_slug )
			);
		}
		...
		if ( ! is_callable( $generator ) ) {
			return new WP_Error(
				...
				__( 'No harness eval generator is wired up. Hook `wp_mcp_ai_harness_eval_generator` to enable scheduled runs.', 'mcp-ai-wpoos' )
			);
		}
