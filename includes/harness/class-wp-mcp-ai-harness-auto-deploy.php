...
class WP_MCP_AI_Harness_Auto_Deploy {
	...
	public static function evaluate( $assistant_id, array $candidate_profile, array $held_in_scores, array $held_out_scores, array $baseline_scores ) {
		...
		if ( ! $held_in_improved ) {
			$reason         = __( 'No improvement on held-in (search set) data.', 'mcp-ai-wpoos' );
			$recommendation = __( 'Reject: candidate does not improve over baseline.', 'mcp-ai-wpoos' );
		} elseif ( $held_in_delta < self::MIN_HELD_IN_IMPROVEMENT ) {
			$reason = sprintf(
				/* translators: 1: actual improvement, 2: required minimum */
				__( 'Held-in improvement of %1$.1f%% is below the %2$.1f%% minimum threshold.', 'mcp-ai-wpoos' ),
				$held_in_delta * 100,
				self::MIN_HELD_IN_IMPROVEMENT * 100
			);
			$recommendation = __( 'Reject: improvement is below minimum threshold.', 'mcp-ai-wpoos' );
		} elseif ( $held_out_regressed ) {
			$reason = sprintf(
				/* translators: 1: actual regression, 2: maximum allowed */
				__( 'Held-out regression of %1$.1f%% exceeds the %2$.1f%% maximum.', 'mcp-ai-wpoos' ),
				abs( $held_out_delta ) * 100,
				self::MAX_HELD_OUT_REGRESSION * 100
			);
			$recommendation = __( 'Reject: regression on held-out data.', 'mcp-ai-wpoos' );
		} else {
			$approved       = true;
			$reason         = __( 'All safety gates passed.', 'mcp-ai-wpoos' );
			$recommendation = __( 'Approve: candidate is safe to deploy.', 'mcp-ai-wpoos' );
		}
	...
	public static function apply_with_rollback( $assistant_id, array $new_profile, array $eval_results = array() ) {
		...
		$current = WP_MCP_AI_Harness_Profile::get( $assistant_id );
		update_post_meta( $assistant_id, self::META_PREVIOUS_PROFILE, wp_json_encode( $current ) );

		// Apply new profile.
		$saved = WP_MCP_AI_Harness_Profile::save( $assistant_id, $new_profile );
	...
	public static function rollback( $assistant_id ) {
		...
		$saved = WP_MCP_AI_Harness_Profile::save( $assistant_id, $previous );
