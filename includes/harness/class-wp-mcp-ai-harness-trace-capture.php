 ...
 *   - reasoning_trace.json — from `WP_MCP_AI_Reasoning_Trace`
 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Harness_Trace_Capture {
	...
	public static function on_before_chat_request( $assistant_id, $messages, $options, $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- $messages, $options, $request required by hook.
		...
		$run_id = WP_MCP_AI_Harness_Trace_Store::start_run(
			$assistant_id,
			array(
				'model'    => $model,
		...
		if ( class_exists( 'WP_MCP_AI_Harness_Profile' ) ) {
			...
			WP_MCP_AI_Harness_Trace_Store::write_artifact( $run_id, 'profile.json', $profile );
		}
	}
	...
	public static function on_after_chat_response( $assistant_id, $response, $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- $request required by hook.
		...
		if ( '' !== $response_text ) {
			WP_MCP_AI_Harness_Trace_Store::write_text( $run_id, 'model_response.txt', $response_text );
		}

		// Write self-refine data if any was captured.
		if ( ! empty( self::$refine_iterations ) ) {
			WP_MCP_AI_Harness_Trace_Store::write_artifact(
				$run_id,
				'self_refine.json',
				array(
	...
	public static function on_after_tool_execution( $tool_slug, $arguments, $context, $result, $descriptor = null ) {
		...
		WP_MCP_AI_Harness_Trace_Store::append_jsonl( self::$active_run_id, 'tool_calls.jsonl', $record );
	}
	 ...
	 *
	 * Called externally from `WP_MCP_AI_Self_Refine_Loop` or its callers.
	 *
	 * @since 1.9.0
	 *
	 ...
	 * @since 1.9.0
	 *
	 * @param array $trace Reasoning trace array (from WP_MCP_AI_Reasoning_Trace).
	 * @return void
	 */
