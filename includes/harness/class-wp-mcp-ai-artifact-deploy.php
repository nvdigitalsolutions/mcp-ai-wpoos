 ...
 *
 * Safety guarantees (mirroring WP_MCP_AI_Harness_Auto_Deploy):
 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Deploy {
	...
	public static function promote( $assistant_id, $artifact_type, $candidate, array $options = array() ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				...
				__( 'A valid assistant ID is required to deploy artifacts.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! in_array( $artifact_type, self::DEPLOYABLE_TYPES, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Artifact type "%s" cannot be deployed.', 'mcp-ai-wpoos' ),
					$artifact_type
				)
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $assistant_id ) ) {
			return new WP_Error(
				...
				__( 'You are not allowed to deploy artifacts for this assistant.', 'mcp-ai-wpoos' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( is_wp_error( $applied ) || true !== $applied ) {
			...
			return is_wp_error( $applied )
				...
				: new WP_Error(
					...
					__( 'The candidate could not be applied.', 'mcp-ai-wpoos' )
				);
		}
	...
	public static function rollback( $assistant_id, $artifact_type ) {
		...
		if ( $assistant_id <= 0 || ! in_array( $artifact_type, self::DEPLOYABLE_TYPES, true ) ) {
			return new WP_Error(
				...
				__( 'Invalid assistant or artifact type for rollback.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $assistant_id ) ) {
			return new WP_Error(
				'wp_mcp_ai_artifact_deploy_forbidden',
				__( 'You are not allowed to roll back artifacts for this assistant.', 'mcp-ai-wpoos' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( empty( $raw ) ) {
			return new WP_Error(
				...
				__( 'There is no rollback target for this artifact type.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! is_array( $previous ) || ! array_key_exists( 'payload', $previous ) ) {
			return new WP_Error(
				...
				__( 'The rollback target is unreadable.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( is_wp_error( $applied ) || true !== $applied ) {
			return is_wp_error( $applied )
				...
				: new WP_Error(
					'wp_mcp_ai_artifact_deploy_apply_failed',
					__( 'The rollback payload could not be applied.', 'mcp-ai-wpoos' )
				);
		}
	...
	public static function get_deployed( $assistant_id, $artifact_type ) {
		...
		if ( 'prompt' === $artifact_type ) {
			$meta_key = class_exists( 'WP_MCP_AI_Agent_Harness_Evolver' ) && defined( 'WP_MCP_AI_Agent_Harness_Evolver::EVOLVED_PROMPT_META_KEY' )
	...
	public static function detect_drift( $assistant_id, $artifact_type, array $thresholds = array() ) {
		...
		if ( ! class_exists( 'WP_MCP_AI_Eval_Regression_Detector' ) || ! class_exists( 'WP_MCP_AI_Eval_Run_Store' ) ) {
			return array(
				'actionable' => false,
				'detector'   => array(),
	...
	private static function validate_candidate( $artifact_type, $candidate ) {
		if ( 'prompt' === $artifact_type ) {
			if ( ! is_string( $candidate ) || '' === trim( $candidate ) ) {
				return new WP_Error(
					...
					__( 'A deployed prompt must be a non-empty string.', 'mcp-ai-wpoos' )
				);
			}
			...
			if ( strlen( $candidate ) > $max_chars ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'The prompt exceeds the %d character deployment limit.', 'mcp-ai-wpoos' ),
						$max_chars
					)
				);
			}
		...
		if ( ! is_array( $candidate ) || empty( $candidate['name'] ) || empty( $candidate['instructions'] ) ) {
			return new WP_Error(
				...
				__( 'A deployed skill must include a name and instructions.', 'mcp-ai-wpoos' )
			);
		}
	...
	private static function resolve_holdout( $assistant_id, $artifact_type, array $options ) {
		...
		if ( ! $require_holdout ) {
			return array(
				'source' => 'skipped',
				'reason' => __( 'Holdout requirement disabled by site policy.', 'mcp-ai-wpoos' ),
			);
		}
		...
		if ( empty( $verification ) ) {
			...
			if (
				...
				return new WP_Error(
					...
					__( 'Promotion requires holdout evidence: pass a verification payload or generators with a suite.', 'mcp-ai-wpoos' )
				);
			}
			...
			if ( ! class_exists( 'WP_MCP_AI_Artifact_Verification_Gate' ) ) {
				return new WP_Error(
					...
					__( 'The artifact verification gate is not available.', 'mcp-ai-wpoos' )
				);
			}

			$verification = WP_MCP_AI_Artifact_Verification_Gate::evaluate(
			...
			if ( ! is_array( $verification ) ) {
				return new WP_Error(
					...
					__( 'The holdout evaluation could not be completed.', 'mcp-ai-wpoos' )
				);
			}
		}
		...
		if ( ! $accepted || $regressed > 0 || $pass_rate < self::MIN_HELD_OUT_PASS_RATE ) {
			return new WP_Error(
				...
				__( 'The candidate failed the holdout gate: regressions detected or pass rate below the minimum.', 'mcp-ai-wpoos' )
			);
		}
	...
	private static function apply( $assistant_id, $artifact_type, $payload ) {
		if ( 'prompt' === $artifact_type ) {
			...
			if ( ! is_string( $payload ) ) {
				return new WP_Error(
					'wp_mcp_ai_artifact_deploy_invalid_prompt',
					__( 'A deployed prompt must be a string.', 'mcp-ai-wpoos' )
				);
			}
		...
		if ( ! is_array( $payload ) || empty( $payload['name'] ) ) {
			return new WP_Error(
				'wp_mcp_ai_artifact_deploy_invalid_skill',
				__( 'A deployed skill must include a name.', 'mcp-ai-wpoos' )
			);
		}
		...
		return update_option( self::SKILLS_OPTION_KEY, $skills, false ) ? true : new WP_Error(
			'wp_mcp_ai_artifact_deploy_apply_failed',
			__( 'The skill could not be saved.', 'mcp-ai-wpoos' )
		);
	}
	 ...
	 * evolver's canonical archive shape (see WP_MCP_AI_Artifact_Lineage::hash_for).
	 *
	 * @since 1.9.0
	 *
	 * @param string $artifact_type Artifact type.
