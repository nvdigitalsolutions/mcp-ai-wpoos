 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Admission_Gate {
	...
	public static function evaluate( $artifact_type, $candidate_artifact, $incumbent_artifact, $verification = null, $assistant_id = 0 ) {
		...
		if ( $any_failed ) {
			...
			if ( 'admit' === $on_no_evidence ) {
				...
				$decision  = self::DECISION_REJECT;
				$reasons[] = __( 'No marginal-gain evidence is available and the no-evidence policy fails closed.', 'mcp-ai-wpoos' );
			} else {
	...
	private static function critic_structural( $artifact_type, $candidate ) {
		if ( '' === $artifact_type ) {
			return array(
				'passed' => false,
				'reason' => __( 'Artifact type is required.', 'mcp-ai-wpoos' ),
			);
		}
		...
		if ( 'prompt' === $artifact_type ) {
			...
			if ( '' === trim( $text ) ) {
				return array(
					'passed' => false,
					'reason' => __( 'The prompt artifact is empty.', 'mcp-ai-wpoos' ),
				);
			}
			...
			if ( strlen( $text ) > max( 100, $max_chars ) ) {
				return array(
					'passed' => false,
					'reason' => __( 'The prompt artifact exceeds the maximum length.', 'mcp-ai-wpoos' ),
				);
			}
		...
		if ( 'skill' === $artifact_type ) {
			...
			if ( '' === trim( $name ) || '' === trim( $instructions ) ) {
				return array(
					'passed' => false,
					'reason' => __( 'Skill artifacts require a name and instructions.', 'mcp-ai-wpoos' ),
				);
			}
		...
		if ( empty( $candidate ) ) {
			return array(
				'passed' => false,
				'reason' => __( 'The artifact payload is empty.', 'mcp-ai-wpoos' ),
			);
		}
	...
	private static function critic_harmlessness( $candidate ) {
		...
		if ( '' === trim( $text ) ) {
			return array(
				'passed' => false,
				'reason' => __( 'The artifact has no content to screen.', 'mcp-ai-wpoos' ),
			);
		}

		// Deterministic secret/PII scan.
		if ( class_exists( 'WP_MCP_AI_Pii_Filter' ) && WP_MCP_AI_Pii_Filter::contains_secret( $text ) ) {
			return array(
				'passed' => false,
				'reason' => __( 'The artifact contains PII or secrets.', 'mcp-ai-wpoos' ),
			);
		}

		// Jailbreak / injection / diversion detection.
		if ( class_exists( 'WP_MCP_AI_Guardrails' ) && method_exists( 'WP_MCP_AI_Guardrails', 'analyze_message' ) ) {
			$analysis = WP_MCP_AI_Guardrails::analyze_message( $text, 'high' );
			if ( isset( $analysis['result'] ) && 'safe' !== $analysis['result'] ) {
				return array(
					'passed' => false,
					'reason' => sprintf(
						/* translators: 1: guardrail family, 2: severity */
						__( 'The artifact was flagged by the guardrails (%1$s, %2$s).', 'mcp-ai-wpoos' ),
	...
	private static function critic_marginal_gain( $verification ) {
		if ( ! is_array( $verification ) || ! isset( $verification['improved_cases'] ) || ! isset( $verification['regressed_cases'] ) ) {
			return array(
				'passed'   => false,
				'reason'   => __( 'No marginal-gain evidence was provided.', 'mcp-ai-wpoos' ),
				'evidence' => false,
			);
		}
		...
		if ( self::MODE_NET_GAIN === $mode ) {
			...
			$reason = $passed
				? __( 'The candidate produced a net gain over the incumbent.', 'mcp-ai-wpoos' )
				: __( 'The candidate produced no net gain over the incumbent.', 'mcp-ai-wpoos' );
		} else {
			$passed = $improved > 0 && 0 === $regressed;
			$reason = $passed
				? __( 'The candidate improved on failure cases with zero regressions.', 'mcp-ai-wpoos' )
				: __( 'The candidate must improve on at least one failure case with zero regressions.', 'mcp-ai-wpoos' );
		}
