	...
	public static function start_run( $assistant_id, array $meta = array() ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				...
				__( 'A valid assistant ID is required to start a trace run.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! self::ensure_directory( $dir ) ) {
			return new WP_Error(
				...
				__( 'Could not create trace run directory.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! $written ) {
			return new WP_Error(
				...
				__( 'Could not write run metadata.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function get_run_manifest( $run_id, $assistant_id = 0 ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				...
				__( 'Could not determine assistant ID for run.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! is_dir( $dir ) ) {
			return new WP_Error(
				...
				__( 'Run directory not found.', 'mcp-ai-wpoos' )
			);
		}
