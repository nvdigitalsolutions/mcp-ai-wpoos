 ...
 *
 * @see WP_MCP_AI_Artifact_Verification_Gate Consumer of these cases.
 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Failure_Replay {
	...
	public static function collect_failures( $assistant_id, $options = array() ) {
		...
		if ( $assistant_id <= 0 ) {
			return new WP_Error(
				...
				__( 'A valid assistant ID is required for failure replay.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! class_exists( 'WP_MCP_AI_Harness_Trace_Store' ) ) {
			return new WP_Error(
				...
				__( 'The harness trace store is not available.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( empty( $runs ) ) {
			return new WP_Error(
				...
				__( 'No harness trace runs found for this assistant.', 'mcp-ai-wpoos' )
			);
		}
		...
		foreach ( $runs as $run ) {
			...
			$records = WP_MCP_AI_Harness_Trace_Store::read_artifact( $run['run_id'], 'tool_calls.jsonl', $assistant_id );
			if ( ! is_array( $records ) ) {
				continue;
			}
		...
		if ( empty( $failures ) ) {
			return new WP_Error(
				'wp_mcp_ai_failure_replay_no_cases',
				__( 'No failed tool calls found in the recent harness traces.', 'mcp-ai-wpoos' )
			);
		}
	...
	public static function build_cases( $assistant_id, $options = array() ) {
		...
		$verifier_slug = isset( $options['verifier_slug'] ) ? sanitize_key( (string) $options['verifier_slug'] ) : WP_MCP_AI_Artifact_Replay_Verifier::SLUG;
		if ( '' === $verifier_slug ) {
			$verifier_slug = WP_MCP_AI_Artifact_Replay_Verifier::SLUG;
		}
		...
		foreach ( $failures as $failure ) {
			...
			$cases[] = array(
				...
				'label'         => sprintf(
					...
					__( 'Replay: %1$s failure (run %2$s)', 'mcp-ai-wpoos' ),
					$failure['tool_slug'],
					$failure['run_id']
				),
				'input'         => array(
	...
	public static function build_suite( $assistant_id, $options = array() ) {
		if ( ! class_exists( 'WP_MCP_AI_Eval_Suite' ) ) {
			return new WP_Error(
				...
				__( 'The eval suite subsystem is not available.', 'mcp-ai-wpoos' )
			);
		}
			...
			$suite = new WP_MCP_AI_Eval_Suite(
				array(
					...
					'label'             => sprintf(
						...
						__( 'Failure replay for assistant %d', 'mcp-ai-wpoos' ),
						(int) $assistant_id
					),
					'description'       => __( 'Auto-generated replay cases from failed tool calls in harness traces.', 'mcp-ai-wpoos' ),
					'cases'             => $cases,
					'artifact_type'     => $artifact_type,
	...
	private static function build_replay_prompt( $failure ) {
		return sprintf(
			...
			__( 'A previous attempt to use the tool "%1$s" failed with error: %2$s. The call arguments were: %3$s. Produce a corrected response or plan that avoids this failure.', 'mcp-ai-wpoos' ),
			$failure['tool_slug'],
			'' !== $failure['error'] ? $failure['error'] : __( '(unknown error)', 'mcp-ai-wpoos' ),
