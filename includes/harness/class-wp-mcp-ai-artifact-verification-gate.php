 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Verification_Gate {
	 ...
	 */
	public static function evaluate( $incumbent_generator, $candidate_generator, WP_MCP_AI_Eval_Suite $suite, array $options = array() ) {
		if ( ! is_callable( $incumbent_generator ) || ! is_callable( $candidate_generator ) ) {
			return new WP_Error(
				'wp_mcp_ai_verification_generator_not_callable',
				__( 'Both generators must be callable.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}

		if ( ! class_exists( 'WP_MCP_AI_Eval_Runner' ) ) {
			return new WP_Error(
				'wp_mcp_ai_verification_no_runner',
				__( 'The eval runner subsystem is not available.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( 0 === $suite->count_cases() ) {
			return array(
				'decision' => 'skip',
				'mode'     => $mode,
				'reason'   => __( 'The verification suite contains no cases.', 'mcp-ai-wpoos' ),
			);
		}
		...
		if ( self::MODE_NO_REGRESSION === $mode ) {
			...
			$reason   = $accepted
				? __( 'Candidate caused no regressions and held the pass rate within tolerance.', 'mcp-ai-wpoos' )
				: __( 'Candidate regressed on cases the incumbent passed, or dropped the pass rate beyond tolerance.', 'mcp-ai-wpoos' );
		} else {
			$accepted = $improved > 0;
			$reason   = $accepted
				? sprintf(
					/* translators: %d: number of improved cases */
					_n( 'Candidate improved on %d failure case.', 'Candidate improved on %d failure cases.', $improved, 'mcp-ai-wpoos' ),
					$improved
				)
				: __( 'Candidate did not improve on any failure case.', 'mcp-ai-wpoos' );
		}
