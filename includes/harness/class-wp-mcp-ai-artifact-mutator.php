 ...
 * @since 1.9.0
 */
class WP_MCP_AI_Artifact_Mutator {
	...
	public static function failure_driven( $llm_callable, array $context ) {
		if ( empty( $context['parent'] ) || ! is_array( $context['parent'] ) ) {
			return new WP_Error(
				...
				__( 'A parent artifact is required for mutation.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
	...
	public static function crossover( $llm_callable, array $context ) {
		...
		if ( count( $parents ) < 2 ) {
			return new WP_Error(
				...
				__( 'Crossover requires at least two parent artifacts.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		$parts[] = '';
		$parts[] = __( 'Combine the best ideas from the parent artifacts above into a single improved artifact. Return ONLY a JSON object with keys "prompt" (the combined prompt text) and "change_summary" (one sentence describing the combination).', 'mcp-ai-wpoos' );

		$messages = array(
			array(
				'role'    => 'system',
				'content' => __( 'You are an artifact crossover mutator for an AI agent evolution loop. You recombine independently discovered improvements from multiple parent artifacts into one coherent artifact. Your output must be valid JSON only — no commentary, no markdown code fences.', 'mcp-ai-wpoos' ),
			),
			array(
				'role'    => 'user',
				'content' => implode( "\n", $parts ),
	 ...
	 * @since 1.9.0
	 *
	 * @param mixed $parent    Parent artifact payload.
	 ...
	 */
	public static function diff_artifacts( $parent, $child, $max_lines = 60 ) {
		$max_lines = max( 10, (int) $max_lines );

		$old = self::artifact_to_text( $parent );
		...
		if ( count( $old_lines ) > self::MAX_DIFF_INPUT_LINES || count( $new_lines ) > self::MAX_DIFF_INPUT_LINES ) {
			return sprintf(
				...
				__( 'Large change: %1$d → %2$d lines.', 'mcp-ai-wpoos' ),
				count( $old_lines ),
				count( $new_lines )
			);
		}
			...
			$diff = array_merge(
				...
				array(
					sprintf(
						...
						__( '… %d more changed lines …', 'mcp-ai-wpoos' ),
	...
	private static function build_failure_messages( $context, $include_learning = false ) {
		...
		$parts[] = __( 'Current parent artifact:', 'mcp-ai-wpoos' );
		$parts[] = '---';
		$parts[] = self::artifact_to_text( isset( $parent['artifact'] ) ? $parent['artifact'] : array() );
		$parts[] = '---';
		$parts[] = '';
		...
		if ( empty( $failures ) ) {
			$parts[] = __( 'No specific failure cases were supplied; propose a general quality improvement.', 'mcp-ai-wpoos' );
		} else {
			$parts[] = __( 'Failure cases this artifact produced (fix these):', 'mcp-ai-wpoos' );
		...
		if ( $include_learning ) {
			...
			if ( empty( $log ) ) {
				$parts[] = __( 'No learning-log entries were available for this lineage.', 'mcp-ai-wpoos' );
			} else {
				$parts[] = __( 'Learning log (past mutations in this lineage and their score impact — build on improvements, do NOT repeat regressions):', 'mcp-ai-wpoos' );
				foreach ( $log as $entry ) {
					...
					$parts[] = sprintf(
						...
						__( 'Score delta %1$s — %2$s', 'mcp-ai-wpoos' ),
						sprintf( '%+.3f', $delta ),
		...
		$parts[] = __( 'Propose an improved artifact that addresses the failures above. Return ONLY a JSON object with keys "prompt" (the improved prompt text) and "change_summary" (one sentence describing what changed and why).', 'mcp-ai-wpoos' );

		return array(
			array(
				'role'    => 'system',
				'content' => __( 'You are an artifact mutator for an AI agent evolution loop. You improve artifacts through targeted, minimal edits driven by concrete failure evidence. Your output must be valid JSON only — no commentary, no markdown code fences.', 'mcp-ai-wpoos' ),
			),
			array(
				'role'    => 'user',
				'content' => implode( "\n", $parts ),
	...
	private static function call_and_parse( $llm_callable, $messages, $kind, $context ) {
		if ( ! is_callable( $llm_callable ) ) {
			return new WP_Error(
				...
				__( 'The mutator LLM callable is not callable.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( '' === trim( $prompt ) ) {
			return new WP_Error(
				...
				__( 'The mutator returned an empty prompt.', 'mcp-ai-wpoos' )
			);
		}
	...
	private static function parse_json( $content ) {
		...
		if ( '' === $content ) {
			return new WP_Error(
				...
				__( 'The mutator returned empty content.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( JSON_ERROR_NONE !== json_last_error() ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Failed to parse the mutator response: %s', 'mcp-ai-wpoos' ),
					json_last_error_msg()
				)
			);
		}
		...
		if ( ! is_array( $parsed ) ) {
			return new WP_Error(
				'wp_mcp_ai_mutator_invalid_response',
				__( 'The mutator response is not a JSON object.', 'mcp-ai-wpoos' )
			);
		}
	...
	private static function failure_to_text( $failure ) {
		...
		return sprintf(
			...
			__( 'Tool "%1$s" failed: %2$s', 'mcp-ai-wpoos' ),
			'' !== $tool ? $tool : __( '(unknown)', 'mcp-ai-wpoos' ),
			'' !== $error ? $error : __( '(unknown error)', 'mcp-ai-wpoos' )
		);
	}
	...
	private static function log_entry_to_text( $entry ) {
		...
		return '' !== $summary ? $summary : __( '(no description)', 'mcp-ai-wpoos' );
	}
