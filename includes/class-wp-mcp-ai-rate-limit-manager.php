...
class WP_MCP_AI_Rate_Limit_Manager {
	...
	/**
	 * Transient prefix for storing idempotency keys.
	 */
	const IDEMPOTENCY_PREFIX = 'wp_mcp_ai_idempotent_';

	/**
	 * Default idempotency key TTL in seconds (1 hour).
	 */
	const DEFAULT_IDEMPOTENCY_TTL = 3600;
	 ...
	 *
	 * Creates a unique key based on the operation parameters that can be used
	 * to prevent duplicate execution of the same operation during retries.
	 *
	 * @since 1.0.0
	 *
	 * @param string $operation Operation identifier (e.g., 'tool_execute', 'chat_completion').
	 * @param array  $params    Operation parameters that define uniqueness.
	 * @return string Idempotency key.
	 */
	public static function generate_idempotency_key( $operation, array $params = array() ) {
		$normalized_params = self::normalize_params_for_idempotency( $params );
		$key_data          = array(
			'operation' => sanitize_key( $operation ),
			'params'    => $normalized_params,
		);

		return 'idem_' . md5( wp_json_encode( $key_data ) );
	}

	/**
	 * Check if an operation with the given idempotency key has been executed.
	 *
	 * @since 1.0.0
	 *
	 * @param string $idempotency_key Idempotency key to check.
	 * @return mixed|null Cached result if operation was already executed, null otherwise.
	 */
	public static function get_idempotent_result( $idempotency_key ) {
		$transient_key = self::IDEMPOTENCY_PREFIX . md5( $idempotency_key );
		$cached_result = get_transient( $transient_key );

		if ( false !== $cached_result ) {
			WP_MCP_AI_Logger::log_event(
				'idempotent_cache_hit',
				'Idempotent operation result returned from cache.',
				array( 'idempotency_key' => substr( $idempotency_key, 0, 20 ) . '...' )
			);
	 ...
	 * @param mixed  $result          Operation result to cache.
	 * @param int    $ttl             Time to live in seconds. Default 1 hour.
	 * @return bool True on success, false on failure.
	 */
	public static function store_idempotent_result( $idempotency_key, $result, $ttl = null ) {
		if ( null === $ttl ) {
			/**
			 * Filter the idempotency key TTL.
			 *
			 * @since 1.0.0
			 *
			 * @param int $ttl Time to live in seconds. Default 3600 (1 hour).
			 */
			$ttl = apply_filters( 'wp_mcp_ai_idempotency_ttl', self::DEFAULT_IDEMPOTENCY_TTL );
		}

		$transient_key = self::IDEMPOTENCY_PREFIX . md5( $idempotency_key );

		return set_transient( $transient_key, $result, absint( $ttl ) );
	}

	/**
	 * Execute an idempotent operation with retry logic.
	 *
	 * Combines idempotency checking with exponential backoff retries.
	 * If the operation was already executed (based on idempotency key),
	 ...
	 * @param callable $callable       Function to execute.
	 * @param array    $args           Arguments to pass to the callable.
	 * @param string   $idempotency_key Idempotency key for this operation.
	 * @param array    $retry_options  Optional retry configuration.
	 *
	 * @return mixed|WP_Error Result from callable or WP_Error on failure.
	 */
	public static function execute_idempotent_with_retry( $callable, array $args = array(), $idempotency_key = '', array $retry_options = array() ) {
		if ( ! is_callable( $callable ) ) {
			return new WP_Error(
				'wp_mcp_ai_invalid_callable',
				__( 'The provided callable is not valid.', 'wp-mcp-ai' )
		...
		// Check for cached result if idempotency key is provided.
		if ( ! empty( $idempotency_key ) ) {
			$cached_result = self::get_idempotent_result( $idempotency_key );
		...
		$result = self::execute_with_retry( $callable, $args, $retry_options );

		// Cache successful results.
		if ( ! empty( $idempotency_key ) && ! is_wp_error( $result ) ) {
			self::store_idempotent_result( $idempotency_key, $result );
		}
	...
	/**
	 * Normalize parameters for idempotency key generation.
	 *
	 * Removes non-deterministic parameters that shouldn't affect idempotency.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	protected static function normalize_params_for_idempotency( array $params ) {
		// Remove non-deterministic parameters.
		$exclude_keys = array( 'timestamp', 'nonce', 'request_id', 'session_id' );
		...
		foreach ( $params as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				$normalized[ $key ] = self::normalize_params_for_idempotency( $value );
			} else {
