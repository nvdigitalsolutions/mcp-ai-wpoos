 ...
 * prefixed with the current tenant scope.  This keeps tenant-specific
 * settings isolated without requiring every caller to manually build
 * composite keys.
 *
 * Key format: wp_mcp_ai_{tenant_type}_{tenant_id}_{original_key}
 *
 * A static "global" variant is also provided for tenant-type-level settings
 * that span all tenant IDs (e.g. default preferences per type).
 *
 * @package WP_MCP_AI
 * @since   3.1.0
 ...
 * Tenant-scoped options helper.
 */
class WP_MCP_AI_Tenant_Options {
	 ...
	 *
	 * @return WP_MCP_AI_Tenant_Options|null Null if tenant context not resolved.
	 */
