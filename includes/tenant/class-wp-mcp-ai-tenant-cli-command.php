<?php
/**
 * Tenant Management WP-CLI Commands
 *
 * Provides `wp mcp tenant` commands for managing tenants, user assignments,
 * and data migration from the command line.
 *
 * @package WP_MCP_AI
 * @since   3.1.0
 ...
 */
class WP_MCP_AI_Tenant_CLI_Command {
	...
	public function list( $args, $assoc_args ) {
		...
		if ( empty( $rows ) ) {
			WP_CLI::success( __( 'No tenants found.', 'mcp-ai-wpoos' ) );
			return;
		}
	...
	public function create( $args, $assoc_args ) {
		...
		if ( $existing ) {
			WP_CLI::error(
				sprintf(
					...
					__( 'A tenant of type "%1$s" with name "%2$s" already exists (ID: %3$d).', 'mcp-ai-wpoos' ),
					$type,
					$name,
					$existing
				)
		...
		if ( false === $result ) {
			WP_CLI::error( __( 'Failed to create tenant.', 'mcp-ai-wpoos' ) );
		}
		...
		WP_CLI::success(
			sprintf(
				...
				__( 'Created tenant #%1$d (%2$s: %3$s).', 'mcp-ai-wpoos' ),
				$tenant_id,
				$type,
				$name
			)
		...
		if ( ! $user ) {
			WP_CLI::error(
				sprintf(
					...
					__( 'User "%s" not found.', 'mcp-ai-wpoos' ),
					$user_id_raw
				)
			);
		}

		$result = WP_MCP_AI_Tenant_Database::assign_user( $user->ID, $tenant_type, $tenant_id, $is_primary );

		if ( false === $result ) {
			WP_CLI::error( __( 'Failed to assign user to tenant.', 'mcp-ai-wpoos' ) );
		}

		WP_CLI::success(
			sprintf(
				/* translators: 1: user login, 2: tenant type, 3: tenant ID */
				__( 'Assigned user "%1$s" to tenant %2$s:%3$d.', 'mcp-ai-wpoos' ),
				$user->user_login,
				$tenant_type,
				$tenant_id
			)
	...
	public function migrate( $args, $assoc_args ) {
		...
		if ( 0 === strpos( $target, 'cpt:' ) ) {
			...
			if ( $dry_run ) {
				WP_CLI::log(
					sprintf(
						...
						__( 'Dry run: Would add tenant meta (%2$s:%3$d) to %1$d posts of type "%4$s".', 'mcp-ai-wpoos' ),
						$total,
						$tenant_type,
						$tenant_id,
						$post_type
			...
			$migrated = WP_MCP_AI_Tenant_Migration::migrate_cpt_posts( $post_type, $tenant_type, $tenant_id );

			WP_CLI::success(
				sprintf(
					/* translators: 1: count, 2: tenant type, 3: tenant ID, 4: post type */
					__( 'Migrated %1$d posts of type "%4$s" to tenant %2$s:%3$d.', 'mcp-ai-wpoos' ),
					$migrated,
					$tenant_type,
					$tenant_id,
					$post_type
		...
		if ( ! $exists ) {
			WP_CLI::error(
				sprintf(
					...
					__( 'Table "%s" does not exist.', 'mcp-ai-wpoos' ),
					$table
				)
			);
		}
		...
		if ( ! WP_MCP_AI_Tenant_Migration::has_tenant_columns( $table ) ) {
			if ( $dry_run ) {
				WP_CLI::log(
					sprintf(
						/* translators: %s: table name */
						__( 'Dry run: Would add tenant columns to table "%s".', 'mcp-ai-wpoos' ),
						$table
					)
				);
			} else {
				...
				WP_CLI::log(
					sprintf(
						/* translators: %s: table name */
						__( 'Added tenant columns to table "%s".', 'mcp-ai-wpoos' ),
						$table
					)
				);
			}
		...
		if ( $dry_run ) {
			WP_CLI::log(
				sprintf(
					...
					__( 'Dry run: Would backfill table "%1$s" with tenant %2$s:%3$d.', 'mcp-ai-wpoos' ),
					$table,
					$tenant_type,
					$tenant_id
				)
			...
			$updated = WP_MCP_AI_Tenant_Migration::backfill_table( $table, $tenant_type, $tenant_id );

			WP_CLI::success(
				sprintf(
					/* translators: 1: count, 2: tenant type, 3: tenant ID, 4: table name */
					__( 'Backfilled %1$d rows in table "%4$s" with tenant %2$s:%3$d.', 'mcp-ai-wpoos' ),
					$updated,
					$tenant_type,
					$tenant_id,
					$table
	...
	public function status( $args, $assoc_args ) {
		$global_enabled = WP_MCP_AI_Tenant_Feature_Flags::is_enabled() ? 'yes' : 'no';
		$toolkits       = WP_MCP_AI_Tenant_Feature_Flags::get_enabled_toolkits();

		WP_CLI::log( sprintf( 'Global tenant isolation: %s', $global_enabled ) );

		if ( empty( $toolkits ) ) {
			WP_CLI::log( __( 'No toolkits have tenant isolation enabled.', 'mcp-ai-wpoos' ) );
		} else {
			WP_CLI::log( __( 'Toolkits with tenant isolation:', 'mcp-ai-wpoos' ) );
	...
	public function toggle( $args, $assoc_args ) {
		...
		if ( 'on' === $state ) {
			...
			WP_CLI::success( __( 'Tenant isolation enabled globally.', 'mcp-ai-wpoos' ) );
		} elseif ( 'off' === $state ) {
			WP_MCP_AI_Tenant_Feature_Flags::disable();
			WP_CLI::success( __( 'Tenant isolation disabled globally.', 'mcp-ai-wpoos' ) );
		} else {
			WP_CLI::error( __( 'State must be "on" or "off".', 'mcp-ai-wpoos' ) );
		}
	}
}

if ( class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'mcp tenant', 'WP_MCP_AI_Tenant_CLI_Command' );
}
