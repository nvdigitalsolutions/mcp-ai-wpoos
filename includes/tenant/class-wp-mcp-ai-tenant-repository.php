<?php
/**
 * Tenant Repository — Abstract Base Class
 *
 * Every data-access class that deals with tenant-scoped data MUST extend
 * this class.  It provides:
 *
 *   - set_tenant_context()    — bind the repository to a specific tenant
 *   - tenant_where()          — returns a prepared SQL WHERE clause
 *   - require_tenant()        — throws if no tenant context is active
 *   - get_tenant_type() / get_tenant_id() — accessors
 *
 * The class defaults to a "bypass" mode (tenant_id = 0) so that existing
 * code continues to work until the feature flag is enabled per-toolkit.
 *
 * @package WP_MCP_AI
 * @since   3.1.0
 ...
 */
abstract class WP_MCP_AI_Tenant_Repository {
	...
	protected function require_tenant(): void {
		if ( $this->strict && ( empty( $this->tenant_type ) || $this->tenant_id <= 0 ) ) {
			throw new \RuntimeException(
				esc_html__( 'Tenant context not set. Call set_tenant_context() before executing queries.', 'mcp-ai-wpoos' )
			);
		}
	}
