 ...
 */
class WP_MCP_AI_ACP_Session_Manager {
	...
	public function load_session( $session_id ) {
		...
		if ( $session['user_id'] !== get_current_user_id() && ! current_user_can( 'manage_options' ) ) { // phpcs:ignore WordPress.PHP.YodaConditions.NotYoda -- Intentional readability order.
