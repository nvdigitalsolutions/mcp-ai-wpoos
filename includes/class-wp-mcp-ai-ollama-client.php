...
class WP_MCP_AI_Ollama_Client {
	...
	public function test_connection() {
		...
		return array( 'success' => true, 'message' => __( 'Connected to Ollama.', 'wp-mcp-ai' ) );
	}
	...
	public function create_chat_completion( array $messages, array $options = array() ) {
		...
		$response = wp_remote_post( $url, array( 'headers' => array( 'Content-Type' => 'application/json' ), 'body' => wp_json_encode( $payload ), 'timeout' => $this->resolve_timeout( $options ) ) );
	...
	protected function build_payload( array $messages, array $options, $model ) {
		...
		foreach ( $messages as $message ) {
			...
			$ollama_messages[] = array( 'role' => $role, 'content' => $content );
		}

		$payload = array( 'model' => $model, 'messages' => $ollama_messages, 'stream' => false );
	...
	protected function normalize_response( array $response, $model ) {
		...
		if ( '' !== $content ) {
			$message['content'] = array( array( 'type' => 'text', 'text' => $content ) );
		}

		$normalized = array(
			'choices'  => array( array( 'index' => 0, 'message' => $message, 'finish_reason' => isset( $response['done'] ) && $response['done'] ? 'stop' : 'length' ) ),
			'provider' => 'ollama',
			'model'    => $model,
		);
