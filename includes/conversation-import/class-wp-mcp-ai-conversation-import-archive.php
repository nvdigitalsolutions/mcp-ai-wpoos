<?php
/**
 * Safe archive handling for conversation imports.
 *
 * Extracts uploaded ZIP archives (ChatGPT exports ship as ZIP) with
 * zip-slip protection and discovers JSON payload files in extracted
 * trees (Google Takeout ships nested folders).
 *
 * @package WP_MCP_AI
 * @since   1.1.60
 ...
 */
class WP_MCP_AI_Conversation_Import_Archive {
	...
	public function prepare( $source_path ) {
		...
		return new WP_Error(
			...
			__( 'Unsupported file type. Provide a .zip, .json, or .jsonl export file.', 'mcp-ai-wpoos' )
		);
	}
	...
	protected function validate_source( $source_path ) {
		...
		if ( '' === $source_path ) {
			return new WP_Error(
				...
				__( 'No import source path was provided.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! file_exists( $source_path ) ) {
			return new WP_Error(
				...
				__( 'The import source file does not exist.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( ! is_readable( $source_path ) ) {
			return new WP_Error(
				...
				__( 'The import source file is not readable.', 'mcp-ai-wpoos' )
			);
		}
	...
	protected function extract_zip( $zip_path ) {
		if ( ! class_exists( 'ZipArchive' ) ) {
			return new WP_Error(
				...
				__( 'The PHP ZipArchive extension is required to import ZIP exports.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( true !== $open_result ) {
			return new WP_Error(
				...
				sprintf( __( 'Could not open the ZIP archive (code %d).', 'mcp-ai-wpoos' ), $open_result )
			);
		}
		...
		if ( empty( $files ) ) {
			...
			return new WP_Error(
				...
				__( 'The ZIP archive contains no conversation JSON files.', 'mcp-ai-wpoos' )
			);
		}
	...
	protected function create_temp_dir() {
		...
		if ( ! wp_mkdir_p( $candidate ) ) {
			return new WP_Error(
				...
				__( 'Could not create a temporary directory for the import.', 'mcp-ai-wpoos' )
			);
		}
