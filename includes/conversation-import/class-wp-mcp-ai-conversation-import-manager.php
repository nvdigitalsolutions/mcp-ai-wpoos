<?php
/**
 * Conversation import orchestration.
 *
 * Coordinates archive preparation, format detection, adapter extraction,
 * deduplication, CCT writes, batching, checkpoints, and reporting for
 ...
 */
class WP_MCP_AI_Conversation_Import_Manager {
	 ...
	 *
	 * @var WP_MCP_AI_Conversation_Import_CCT_Writer|null
	 */
	protected $writer;
	 ...
	 * Constructor.
	 *
	 * @param WP_MCP_AI_Conversation_Import_CCT_Writer|null $writer Optional writer override (tests).
	 */
	public function __construct( $writer = null ) {
		if ( $writer instanceof WP_MCP_AI_Conversation_Import_CCT_Writer ) {
			$this->writer = $writer;
		}
	}
	 ...
	 *
	 * @return WP_MCP_AI_Conversation_Import_CCT_Writer
	 */
	protected function get_writer() {
		if ( null === $this->writer ) {
			$this->writer = new WP_MCP_AI_Conversation_Import_CCT_Writer();
		}
	...
	public function run( array $args = array() ) {
		...
		if ( '' !== $resume_token && empty( $resume_state ) ) {
			return new WP_Error(
				...
				__( 'The resume token does not match an existing import checkpoint.', 'mcp-ai-wpoos' )
			);
		}
		...
		$media     = $sideload_media ? new WP_MCP_AI_Conversation_Import_Media() : null;
		$processed = 0;
		...
		foreach ( $files as $file_index => $file_path ) {
			...
			if ( isset( $args['format'] ) && '' !== sanitize_key( (string) $args['format'] ) ) {
				...
				if ( $requested !== $platform ) {
					$file_report['error'] = sprintf(
						...
						__( 'Skipped: requested format "%1$s" but file detected as "%2$s".', 'mcp-ai-wpoos' ),
						$requested,
						$platform
					);
					$report['files'][] = $file_report;
			...
			foreach ( $extraction as $conversation ) {
				if ( ! $conversation instanceof WP_MCP_AI_Conversation_Import_Conversation ) {
					continue;
				}
				...
				if ( null !== $media && 'chatgpt' === $conversation->get_platform() ) {
					...
					if ( $resolved instanceof WP_MCP_AI_Conversation_Import_Conversation ) {
	 ...
	 *
	 * @param WP_MCP_AI_Conversation_Import_CCT_Writer $writer      CCT writer.
	...
	public function get_status( $token ) {
		...
		if ( empty( $state ) ) {
			return array(
				'token'   => $token,
				'status'  => 'not_found',
				'message' => __( 'No checkpoint found for this token. Completed runs are not retained; use the run report instead.', 'mcp-ai-wpoos' ),
			);
		}
	 ...
	 *
	 * @param WP_MCP_AI_Conversation_Import_Format_Detector $detector  Detector instance.
	...
	protected function resolve_source_path( array $args ) {
		if ( empty( $args['source'] ) ) {
			return new WP_Error(
				...
				__( 'Provide an import source: a file path or a media library attachment ID.', 'mcp-ai-wpoos' )
			);
		}
