 ...
 *
 * Parses OpenAI's native `conversations.json` (Settings → Data Controls →
 * Export data). The export is a flat JSON array of conversation objects whose
 * `mapping` field is a tree of message nodes. Branches occur when a user
 * edited or regenerated a response; per project decision, the active branch is
 * always resolved by walking `current_node` backwards to the root.
 *
 * @link    https://help.openai.com/en/articles/7260999-how-do-i-export-my-chatgpt-history-and-data
 * @credit  ChatGPT export schema documentation by xuy (docs-for-agents), MIT.
 * @package WP_MCP_AI
 * @since   1.1.60
 ...
 */
class WP_MCP_AI_Conversation_Import_Adapter_Chatgpt implements WP_MCP_AI_Conversation_Import_Adapter_Interface {
	...
	public function extract( $decoded, array $options = array() ) {
		if ( ! $this->supports_decoded( $decoded ) ) {
			return new WP_Error(
				...
				__( 'The file does not look like a ChatGPT conversations.json export.', 'mcp-ai-wpoos' )
			);
		}
	 ...
	 * @return WP_MCP_AI_Conversation_Import_Conversation|null Null when no visible
	 *                                                        messages remain.
	 */
			...
			if ( is_array( $part ) ) {
				if ( isset( $part['content_type'] ) && 'image_asset_pointer' === $part['content_type'] ) {
					...
					if ( '' !== $pointer ) {
						...
						$parts[] = sprintf( __( '[Image: %s]', 'mcp-ai-wpoos' ), $pointer );
					}
					continue;
				}
