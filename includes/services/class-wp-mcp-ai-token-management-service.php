 ...
 *
 * Handles token usage tracking, statistics, and limit management.
 * Extracted from WP_MCP_AI_Section_Token_Manager as part of service layer refactoring.
 *
 * @package WP_MCP_AI
 * @since 1.0.0
 */
...
class WP_MCP_AI_Token_Management_Service {
	 ...
	 * @return array Usage statistics with totals.
	 */
	...
	/**
	 * Get all users with token usage data.
	 *
	 * @return array Array of user IDs.
	 */
	public function get_users_with_usage() {
		global $wpdb;
	 ...
	 */
	public function calculate_usage_totals( $usage ) {
		$totals = array(
			'requests'          => 0,
			'prompt_tokens'     => 0,
			'completion_tokens' => 0,
	...
	/**
	 * Get all available tools for token management.
	 *
	 ...
	 */
	public function get_tool_statistics( $tool_slug ) {
		return WP_MCP_AI_Tool_Token_Limits::get_tool_statistics( $tool_slug );
	}
	 ...
	 *
	 * @param string $tool_slug Tool slug.
	 * @return int Tool token limit.
	 */
	public function get_tool_limit( $tool_slug ) {
		return WP_MCP_AI_Tool_Token_Limits::get_tool_limit( $tool_slug );
	}
	 ...
	 */
	public function update_tool_limit( $tool_slug, $limit ) {
		return WP_MCP_AI_Tool_Token_Limits::set_tool_limit( $tool_slug, absint( $limit ) );
	}

	/**
	 * Get site-wide token usage statistics.
	 *
	...
	/**
	 * Reset token usage for a specific user.
	 *
	 * @param int $user_id User ID.
	 * @return bool Success status.
	 */
	public function reset_user_usage( $user_id ) {
		return delete_user_meta( $user_id, WP_MCP_AI_Usage_Tracker::USER_META_KEY );
	}
	 ...
	 */
	public function reset_all_usage() {
		$user_ids = $this->get_users_with_usage();
		$count    = 0;

		foreach ( $user_ids as $user_id ) {
			if ( $this->reset_user_usage( $user_id ) ) {
				++$count;
			}
		}
	 ...
	 * @return array Formatted usage details.
	 */
	public function get_user_usage_details( $user_id, $usage = null ) {
		if ( null === $usage ) {
			$usage = WP_MCP_AI_Usage_Tracker::get_usage_for_user( $user_id );
		}
