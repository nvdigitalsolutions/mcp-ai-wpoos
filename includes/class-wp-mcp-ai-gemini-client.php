...
/**
 * Prepare tool definitions in Gemini format.
 *
 * @param array $tools Array of tool definitions.
 * @return array Gemini-specific tool definitions.
 */
protected function prepare_tool_definitions( array $tools ) {
// Convert to OpenAI-style format first.
$openai_tools = array();
foreach ( $tools as $tool ) {
if ( ! isset( $tool['name'] ) ) {
continue;
...
// Use translate_tools to convert to Gemini format.
return $this->translate_tools( $openai_tools );
}
...
/**
 * Build tool response message for Gemini.
 *
 * Gemini uses functionResponse format, but we'll use the standard
 * OpenAI-style tool response which will be converted in build_payload.
 *
...
if ( isset( $tool_definition['functionDeclarations'] ) && is_array( $tool_definition['functionDeclarations'] ) ) {
// Return the first declaration name.
if ( isset( $tool_definition['functionDeclarations'][0]['name'] ) ) {
return $tool_definition['functionDeclarations'][0]['name'];
}
}
...
// Gemini format has functionDeclarations array.
if ( isset( $tool_definition['functionDeclarations'] ) && is_array( $tool_definition['functionDeclarations'] ) ) {
// Return the first declaration parameters.
if ( isset( $tool_definition['functionDeclarations'][0]['parameters'] ) ) {
return $tool_definition['functionDeclarations'][0]['parameters'];
}
}
// Fallback to OpenAI-style format.
