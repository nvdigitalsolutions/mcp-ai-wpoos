...
if ( ! class_exists( 'WP_MCP_AI_Gemini_Client' ) ) {
	...
	class WP_MCP_AI_Gemini_Client {
		...
		/** @var string Managed Agents / Antigravity Interactions API endpoint. @since 2.x */
		const API_INTERACTIONS_ENDPOINT  = 'https://generativelanguage.googleapis.com/v1beta/interactions';
		/** @var string Managed Agents creation endpoint (custom agents). @since 2.x */
		const API_MANAGED_AGENTS_ENDPOINT = 'https://generativelanguage.googleapis.com/v1beta/agents';
		/** @var string Environment file download endpoint template. @since 2.x */
		const API_ENV_DOWNLOAD_ENDPOINT  = 'https://generativelanguage.googleapis.com/v1beta/files/environment-%s:download';
		/** @var string Required Api-Revision header for the Interactions API. @since 2.x */
		const INTERACTIONS_API_REVISION  = '2026-05-20';
		/** @var string Current Antigravity managed agent ID. @since 2.x */
		...
		protected function decode_inline_image_data( $data ) {
			$data    = (string) $data;
			$decoded = base64_decode( $data, true ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Decoding binary image data from API response.
			...
			if ( '' !== $normalised ) {
				$decoded = base64_decode( $normalised, true ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Decoding binary image data from API response.
			...
			$decoded = base64_decode( $data ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Decoding binary image data from API response.
		...
		public function generate_speech( $text, array $options = array() ) {
			...
			$audio_data = base64_decode( $decoded['audioContent'] ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Decoding binary image data from API response.
