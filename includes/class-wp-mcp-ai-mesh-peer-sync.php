...
class WP_MCP_AI_Mesh_Peer_Sync {
	...
	public function __construct() {
		...
		add_action( 'update_option_' . WP_MCP_AI_Admin_Settings::OPTION_NAME, array( $this, 'sync_mesh_peers_on_option_update' ), 10, 3 );
	}
	...
	private function create_mesh_peer_post( $name, $url, $mesh_peer_id ) {
		$post_data = array(
			...
			'post_content' => sprintf(
				...
				__( 'Mesh peer connection to %s', 'mcp-ai-wpoos' ),
				$url
			),
		);
		...
		update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_SITE_URL, $url );
		update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_SITE_NAME, $name );

		// Set default health status as unknown until tested.
		update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_HEALTH_STATUS, 'unknown' );
		...
		update_post_meta( $post_id, WP_MCP_AI_AI_Peer_CPT::META_CAPABILITIES, wp_json_encode( $default_capabilities ) );
