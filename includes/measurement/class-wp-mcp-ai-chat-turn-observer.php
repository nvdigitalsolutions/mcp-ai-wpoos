 ...
 * Chat-turn observer.
 */
class WP_MCP_AI_Chat_Turn_Observer {
	 ...
	 * Singleton instance.
	 *
	 * @var WP_MCP_AI_Chat_Turn_Observer|null
	 */
	 ...
	 *
	 * @return WP_MCP_AI_Chat_Turn_Observer
	 */
	...
	public function on_after( $assistant_id = null, $response = null, $request = null ) {
		...
		$collector->record( WP_MCP_AI_Chat_Turn_Metrics::CHAT_TURN_COUNT, 1, $ctx );
		if ( 'error' === $outcome ) {
			$collector->record( WP_MCP_AI_Chat_Turn_Metrics::CHAT_TURN_ERROR_COUNT, 1, $ctx );
		}
		...
		if ( is_array( $response ) && isset( $response['usage'] ) && is_array( $response['usage'] ) ) {
			...
			if ( $prompt > 0 ) {
				$collector->record( WP_MCP_AI_Chat_Turn_Metrics::TOKEN_USAGE_PROMPT, $prompt, $ctx );
			}
			if ( $completion > 0 ) {
				$collector->record( WP_MCP_AI_Chat_Turn_Metrics::TOKEN_USAGE_COMPLETION, $completion, $ctx );
			}
		}
	}
