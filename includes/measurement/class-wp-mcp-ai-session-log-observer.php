 ...
 * @since 1.3.0
 */
class WP_MCP_AI_Session_Log_Observer {
	...
	private function project_turn_ended( array $data, float $time ) {
		...
		$collector->record( WP_MCP_AI_Chat_Turn_Metrics::CHAT_TURN_COUNT, 1, $context );

		if ( 'error' === $outcome ) {
			$collector->record( WP_MCP_AI_Chat_Turn_Metrics::CHAT_TURN_ERROR_COUNT, 1, $context );
		}
		...
		if ( null !== $started_at ) {
			...
			$collector->record(
				WP_MCP_AI_Chat_Turn_Metrics::CHAT_TURN_DURATION_MS,
		...
		if ( isset( $data['iterations'] ) && is_numeric( $data['iterations'] ) && (int) $data['iterations'] > 0 ) {
			$collector->record( WP_MCP_AI_Chat_Turn_Metrics::CHAT_AGENTIC_ITERATIONS, (int) $data['iterations'], $context );
		}
	}
	...
	/**
	 * The metric collector, when available.
	 *
