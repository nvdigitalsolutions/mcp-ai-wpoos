 ...
 */
class WP_MCP_AI_Reward_Function_Registry {

	/**
	 * Singleton instance.
	 *
	 * @var WP_MCP_AI_Reward_Function_Registry|null
	 */
	 ...
	 * Get the singleton.
	 *
	 * @return WP_MCP_AI_Reward_Function_Registry
	 */
	...
	public function register( array $definition ) {
		...
		if ( '' === $slug ) {
			return new WP_Error( 'wp_mcp_ai_reward_invalid_slug', __( 'Reward function slug is required.', 'mcp-ai-wpoos' ) );
		}
		if ( isset( $this->functions[ $slug ] ) ) {
			return new WP_Error(
				'wp_mcp_ai_reward_already_registered',
				/* translators: %s: reward function slug. */
				sprintf( __( 'Reward function "%s" is already registered.', 'mcp-ai-wpoos' ), $slug )
			);
		}
		if ( empty( $definition['callback'] ) || ! is_callable( $definition['callback'] ) ) {
			return new WP_Error( 'wp_mcp_ai_reward_invalid_callback', __( 'Reward function callback must be callable.', 'mcp-ai-wpoos' ) );
		}
		if ( empty( $definition['label'] ) ) {
			return new WP_Error( 'wp_mcp_ai_reward_missing_label', __( 'Reward function label is required.', 'mcp-ai-wpoos' ) );
		}
		if ( ! isset( $definition['output_min'] ) || ! isset( $definition['output_max'] ) ) {
			return new WP_Error( 'wp_mcp_ai_reward_missing_range', __( 'Reward function output range is required.', 'mcp-ai-wpoos' ) );
		}
		$output_min = (float) $definition['output_min'];
		$output_max = (float) $definition['output_max'];
		if ( $output_max <= $output_min ) {
			return new WP_Error( 'wp_mcp_ai_reward_invalid_range', __( 'Reward function output_max must be greater than output_min.', 'mcp-ai-wpoos' ) );
		}
		if ( empty( $definition['anti_gaming'] ) || ! is_string( $definition['anti_gaming'] ) ) {
			return new WP_Error(
				'wp_mcp_ai_reward_missing_anti_gaming',
				__( 'Reward function must declare an anti-gaming safeguard. This is a Goodhart guard and cannot be bypassed.', 'mcp-ai-wpoos' )
			);
		}
	...
	public function evaluate( $slug, array $inputs, array $context = array() ) {
		...
		if ( null === $def ) {
			return new WP_Error(
				...
				/* translators: %s: reward function slug. */
				sprintf( __( 'Reward function "%s" is not registered.', 'mcp-ai-wpoos' ), $slug )
			);
		}
		...
		foreach ( $def['inputs'] as $required ) {
			if ( ! array_key_exists( $required, $inputs ) ) {
				return new WP_Error(
					...
					sprintf( __( 'Missing required input "%1$s" for reward function "%2$s".', 'mcp-ai-wpoos' ), $required, $slug )
				);
			}
		}
		...
		if ( ! is_numeric( $value ) ) {
			return new WP_Error(
				...
				/* translators: %s: reward function slug. */
				sprintf( __( 'Reward function "%s" returned a non-numeric value.', 'mcp-ai-wpoos' ), $slug )
			);
		}
		$value = (float) $value;
