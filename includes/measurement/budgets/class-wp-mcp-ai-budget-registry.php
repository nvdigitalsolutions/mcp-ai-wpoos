 ...
 * Budget Registry
 *
 * Tracks consumption against {@see WP_MCP_AI_Budget_Envelope}s and fires
 ...
 */
class WP_MCP_AI_Budget_Registry {
	 ...
	 * Singleton instance.
	 *
	 * @var WP_MCP_AI_Budget_Registry|null
	 */
	 ...
	 *
	 * @var array<string,WP_MCP_AI_Budget_Envelope>
	 */
	 ...
	 * Get singleton.
	 *
	 * @return WP_MCP_AI_Budget_Registry
	 */
	...
	public function boot() {
		 ...
		 * @since 1.3.0
		 *
		 * @param WP_MCP_AI_Budget_Registry $registry Registry instance.
		 */
	 ...
	 *
	 * @param WP_MCP_AI_Budget_Envelope|array $envelope Envelope or definition.
	 * @return WP_MCP_AI_Budget_Envelope|WP_Error
	 */
	public function register( $envelope ) {
		if ( is_array( $envelope ) ) {
			try {
				$envelope = new WP_MCP_AI_Budget_Envelope( $envelope );
		...
		if ( ! $envelope instanceof WP_MCP_AI_Budget_Envelope ) {
			return new WP_Error(
				...
				__( 'Budget must be an array definition or a WP_MCP_AI_Budget_Envelope instance.', 'mcp-ai-wpoos' )
			);
		}
	 ...
	 *
	 * @param string $slug Slug.
	 * @return WP_MCP_AI_Budget_Envelope|null
	 */
	 ...
	 * All envelopes.
	 *
	 * @return array<string,WP_MCP_AI_Budget_Envelope>
	 */
	public function all() {
	...
	public function get_consumption( $slug ) {
		...
		if ( WP_MCP_AI_Budget_Envelope::SCOPE_PERSISTENT === $envelope->get_scope() ) {
	 ...
	 *
	 * @param WP_MCP_AI_Budget_Envelope $envelope Envelope.
	 * @param float                     $value    Value to add.
	 * @return float New accumulator value.
	 */
	private function accumulate( WP_MCP_AI_Budget_Envelope $envelope, $value ) {
	 ...
	 * @return void
	 */
	private function maybe_fire_signals( WP_MCP_AI_Budget_Envelope $envelope, $consumed ) {
		$slug  = $envelope->get_slug();
		$limit = $envelope->get_limit();
	 ...
	 */
	private function state_for( WP_MCP_AI_Budget_Envelope $envelope, $consumed ) {
	 ...
	 * @return void
	 */
	private function maybe_roll_window( WP_MCP_AI_Budget_Envelope $envelope ) {
