 ...
 * Metric event store.
 */
class WP_MCP_AI_Metric_Event_Store {
	 ...
	 * Singleton instance.
	 *
	 * @var WP_MCP_AI_Metric_Event_Store|null
	 */
	 ...
	 *
	 * @return WP_MCP_AI_Metric_Event_Store
	 */
	...
	private function event_to_row( array $event ) {
		...
		if ( WP_MCP_AI_Measurement_Registry::PRIVACY_RESTRICTED === $privacy ) {
			// Defensive barrier: restricted events must never land in the table.
			return null;
		}

		$allowed_privacies = array(
			WP_MCP_AI_Measurement_Registry::PRIVACY_PUBLIC,
			WP_MCP_AI_Measurement_Registry::PRIVACY_INTERNAL,
			WP_MCP_AI_Measurement_Registry::PRIVACY_SENSITIVE,
		);
		if ( ! in_array( $privacy, $allowed_privacies, true ) ) {
			...
			$privacy = WP_MCP_AI_Measurement_Registry::PRIVACY_INTERNAL;
		}
