 ...
 *
 * Enables NV oOS assistants to discover, authenticate with,
...
class WP_MCP_AI_A2A_Client {
	...
	public static function discover_agent( $agent_url ) {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Failed to discover A2A agent: %s', 'mcp-ai-wpoos' ),
		...
		if ( 200 !== $status_code ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'A2A agent discovery returned status %1$d from %2$s', 'mcp-ai-wpoos' ),
					$status_code,
					$card_url
				)
			);
		...
		if ( ! is_array( $card ) || empty( $card['name'] ) ) {
			return new WP_Error(
				'a2a_invalid_card',
				__( 'Invalid Agent Card format received.', 'mcp-ai-wpoos' )
			);
		}
	...
	protected static function send_jsonrpc_request( $endpoint, $rpc_request, $options = array() ) {
		...
		$headers = array(
			...
			'Accept'       => 'application/json',
			'A2A-Version'  => WP_MCP_AI_A2A_Agent_Card::PROTOCOL_VERSION,
		);
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error(
				...
				__( 'Invalid JSON response from remote A2A agent.', 'mcp-ai-wpoos' )
			);
		}
		...
		if ( isset( $data['error'] ) ) {
			return new WP_Error(
				'a2a_remote_error',
				isset( $data['error']['message'] ) ? $data['error']['message'] : __( 'Unknown remote error.', 'mcp-ai-wpoos' ),
				array(
