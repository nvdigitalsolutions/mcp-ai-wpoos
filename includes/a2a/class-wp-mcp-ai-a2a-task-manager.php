 ...
 */
class WP_MCP_AI_A2A_Task_Manager {
	...
	public static function transition_state( $task_id, $new_state, $message = null ) {
		...
		if ( in_array( $current_state, self::TERMINAL_STATES, true ) ) {
			return new WP_Error(
				...
				__( 'Cannot transition from a terminal state.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! in_array( $new_state, $allowed, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid state transition from %1$s to %2$s.', 'mcp-ai-wpoos' ),
					$current_state,
					$new_state
				),
				array( 'status' => 400 )
	...
	public static function cancel_task( $task_id ) {
		...
		if ( in_array( $task['status']['state'], self::TERMINAL_STATES, true ) ) {
			return new WP_Error(
				...
				__( 'Task is in a terminal state and cannot be canceled.', 'mcp-ai-wpoos' ),
				array( 'status' => 400 )
			);
		}
