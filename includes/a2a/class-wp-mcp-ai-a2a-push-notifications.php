 ...
 */
class WP_MCP_AI_A2A_Push_Notifications {
	...
	public static function get_config( $task_id, $config_id ) {
		...
		if ( ! isset( $all_configs[ $task_id ][ $config_id ] ) ) {
			return new WP_Error(
				'a2a_task_not_found',
				__( 'Push notification configuration not found.', 'mcp-ai-wpoos' ),
				array( 'status' => 404 )
			);
		}
	...
	public static function fire_notifications( $task_id ) {
		...
		if ( WP_MCP_AI_A2A_Task_Manager::is_terminal_state( $task['status']['state'] ) ) {
	 ...
	 * @return array Sanitized authentication info.
	 */
