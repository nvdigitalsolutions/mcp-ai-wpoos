...
class WP_MCP_AI_Elementor_Chat_Bubble_Widget extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		if ( $is_editor ) {
			$this->render_bubble_html( $classes, $data_attrs, $css_vars, $panel_title, $tooltip, $settings, $shortcode_html, true );
			return;
		}
		 ...
		 */
		ob_start();
		$this->render_bubble_html( $classes, $data_attrs, $css_vars, $panel_title, $tooltip, $settings, $shortcode_html, false );
	 ...
	 * @param bool   $use_kses       Whether to wrap shortcode output in kses.
	 */
	protected function render_bubble_html( $classes, $data_attrs, $css_vars, $panel_title, $tooltip, $settings, $shortcode_html, $use_kses ) {
		...
		if ( $use_kses ) {
			// Editor context: use kses with data-* support for safe inline rendering.
			...
			// Footer context: shortcode output is generated by our own
			// render_shortcode() where all values are escaped with esc_attr(),
			// esc_html(), etc.  Using kses_chat_output() to satisfy WPCS while
