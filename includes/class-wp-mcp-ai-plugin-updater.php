...
class WP_MCP_AI_Plugin_Updater {
	...
	/**
	 * The asset filename pattern to download from the release.
	 */
	const ASSET_PATTERN = 'nvdigital-open-operator-system-oos-complete';
	...
	public static function check_for_update() {
		...
		// Find the complete package asset URL.
		$download_url = '';
		if ( ! empty( $data['assets'] ) ) {
			foreach ( $data['assets'] as $asset ) {
				if ( false !== strpos( $asset['name'], self::ASSET_PATTERN ) && false !== strpos( $asset['name'], '.zip' ) ) {
		...
		// Also check the ZIP ball as fallback.
		...
		$result = array(
			...
			'update_available' => version_compare( $latest_version, $installed, '>' ),
