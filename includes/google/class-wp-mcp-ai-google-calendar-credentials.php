<?php
/**
 * Google Calendar credential resolution.
 *
 * Resolves Google Calendar credentials from, in priority order:
 *
 * 1. A Pro Remote Sites connection identified by `connection_id`.
 * 2. Base-plugin settings (`wp_mcp_ai_settings`), the single-connection surface.
 * 3. The legacy `wp_mcp_ai_google_calendar_*` filters, so any site already
 *    injecting a service account or a pre-minted access token keeps working.
 *
 * Centralising this prevents the copy-paste divergence visible between the Gmail
 * and Drive tools, where the same resolution block is duplicated inline in each
 * `execute()` with slightly different error codes.
 *
 * @package WP_MCP_AI
...
if ( ! class_exists( 'WP_MCP_AI_Google_Calendar_Credentials' ) ) {
	 ...
	 */
	class WP_MCP_AI_Google_Calendar_Credentials {
		...
		protected static function resolve_from_connection( $connection_id ) {
			if ( ! class_exists( 'WP_MCP_AI_Pro_Remote_Site_Manager' ) ) {
				if ( ! defined( 'WP_MCP_AI_PRO_PATH' ) ) {
					return new WP_Error(
						...
						__( 'Remote Site connections require the NV oOS Pro addon. Configure Google Calendar in Tools → Connections instead.', 'mcp-ai-wpoos' ),
						array( 'status' => 400 )
					);
				}
			...
			if ( empty( $connection ) ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Google Calendar connection "%s" was not found. Check your Remote Sites configuration.', 'mcp-ai-wpoos' ),
						$connection_id
					),
					array( 'status' => 404 )
				);
			...
			if ( '' !== $type && self::CONNECTION_TYPE !== $type ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Connection "%s" is not a Google Calendar connection. Select a Google Calendar connection type.', 'mcp-ai-wpoos' ),
						$type
					),
					array( 'status' => 400 )
				);
			...
			if ( isset( $connection['enabled'] ) && ! $connection['enabled'] ) {
				return new WP_Error(
					...
					__( 'This Google Calendar connection is disabled.', 'mcp-ai-wpoos' ),
					array( 'status' => 400 )
				);
			}
			...
			if ( '' === $client_id || '' === $client_secret || '' === $refresh_token ) {
				return new WP_Error(
					...
					__( 'This Google Calendar connection is not fully authorised. Open the connection and complete the OAuth flow.', 'mcp-ai-wpoos' ),
					array( 'status' => 400 )
				);
			}
			...
			return array(
				...
				'scope_profile'   => WP_MCP_AI_Google_Calendar_Scopes::normalise_profile(
		...
		protected static function resolve_from_settings() {
			...
			if ( '' === $client_id || '' === $client_secret || '' === $refresh_token ) {
				return new WP_Error(
					...
					__( 'Google Calendar is not connected. Open Tools → Connections → Google Calendar, save your OAuth client ID and secret, then click Connect. Pro sites can also use a Google Calendar connection under Remote Sites.', 'mcp-ai-wpoos' ),
					array( 'status' => 400 )
				);
			}
		...
		protected static function resolve_from_filters( array $context = array(), array $arguments = array() ) {
			...
			if ( '' === $token && empty( $service_account ) ) {
				return new WP_Error(
					'wp_mcp_ai_calendar_missing_credentials',
					__( 'Google Calendar credentials were not provided.', 'mcp-ai-wpoos' ),
					array( 'status' => 400 )
				);
			}
			...
			return array(
				...
				'granted_scopes'  => '',
				'scope_profile'   => WP_MCP_AI_Google_Calendar_Scopes::DEFAULT_PROFILE,
				'timezone'        => self::default_timezone(),
		 ...
		 * @return WP_MCP_AI_Google_Calendar_Client|WP_Error
		 */
		public static function make_client( array $credentials, array $options = array() ) {
			...
			if ( '' !== $access_token ) {
				...
				if ( '' === $client_id || '' === $client_secret || '' === $refresh_token ) {
					return new WP_Error(
						'wp_mcp_ai_calendar_missing_credentials',
						__( 'Google Calendar credentials are incomplete.', 'mcp-ai-wpoos' ),
						array( 'status' => 400 )
					);
				}
			...
			return new WP_MCP_AI_Google_Calendar_Client( $provider, $options );
		}
		...
		protected static function mint_service_account_token( array $service_account, array $credentials ) {
			...
			if ( '' === $client_email || '' === $private_key ) {
				return new WP_Error(
					...
					__( 'Incomplete Google service account credentials.', 'mcp-ai-wpoos' ),
					array( 'status' => 500 )
				);
			}

			if ( '' === $token_uri ) {
				$token_uri = WP_MCP_AI_Google_OAuth_Service::TOKEN_ENDPOINT;
			}
			...
			if ( '' === $scopes ) {
				...
				$scopes  = WP_MCP_AI_Google_Calendar_Scopes::get_profile_scope_string( $profile );
			}
			...
			$response = wp_remote_post(
				$token_uri,
				array(
					'timeout' => WP_MCP_AI_Google_OAuth_Service::DEFAULT_TIMEOUT,
					'headers' => array(
			...
			if ( is_wp_error( $response ) ) {
				return new WP_Error(
					...
					__( 'Unable to obtain a Google access token for the service account.', 'mcp-ai-wpoos' ),
					array( 'status' => 500 )
				);
			}
			...
			if ( $status < 200 || $status >= 300 || empty( $data['access_token'] ) ) {
				$message = __( 'Google rejected the service account token request.', 'mcp-ai-wpoos' );
			...
			$ttl          = $expires_in - WP_MCP_AI_Google_OAuth_Service::TOKEN_CACHE_MARGIN;
		...
		protected static function build_jwt_assertion( array $claims, $private_key ) {
			if ( ! function_exists( 'openssl_sign' ) ) {
				return new WP_Error(
					...
					__( 'The OpenSSL extension is required to sign Google service account assertions.', 'mcp-ai-wpoos' ),
					array( 'status' => 500 )
				);
			}
			...
			if ( ! openssl_sign( implode( '.', $segments ), $signature, $private_key, 'sha256' ) ) {
				return new WP_Error(
					...
					__( 'Unable to sign the Google service account assertion.', 'mcp-ai-wpoos' ),
					array( 'status' => 500 )
				);
			}
		...
		public static function require_scope( array $credentials, $required ) {
			...
			if ( WP_MCP_AI_Google_Calendar_Scopes::has_scope( $granted, $required ) ) {
				return true;
			}

			return WP_MCP_AI_Google_Calendar_Scopes::missing_scope_error( $required );
		}
