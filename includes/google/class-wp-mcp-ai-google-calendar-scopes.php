<?php
/**
 * Google Calendar OAuth scope registry.
 *
 * Single source of truth for Google Calendar API v3 OAuth scopes across both
 * connection surfaces (base Tools → Connections and Pro Remote Sites). Having
 * one registry prevents the scope drift that already exists between the base
 * and Pro Google Drive flows, which request different scope sets for the same
 * product.
 *
 * Scope tiers follow Google's OAuth verification model:
 *
 * - Non-sensitive scopes require no app review.
 * - Sensitive scopes require Google review plus a demo video (3-5 business days).
 * - Restricted scopes additionally require an annual CASA security assessment.
 *
 * No Google Calendar scope is restricted, so a Calendar-only integration never
...
if ( ! class_exists( 'WP_MCP_AI_Google_Calendar_Scopes' ) ) {
	 ...
	 */
	class WP_MCP_AI_Google_Calendar_Scopes {
		...
		public static function get_profiles() {
			$profiles = array(
				self::PROFILE_MINIMAL  => array(
					'label'                 => __( 'Minimal — plugin-owned calendar only (no Google review)', 'mcp-ai-wpoos' ),
					'description'           => __( 'NV oOS creates and manages its own secondary calendar. It cannot read or write your existing calendars. This profile uses only non-sensitive scopes, so your Google Cloud project does not need OAuth app verification.', 'mcp-ai-wpoos' ),
					'scopes'                => array(
				...
				self::PROFILE_FULL     => array(
					'label'                 => __( 'Full — complete calendar management (Google review required)', 'mcp-ai-wpoos' ),
					'description'           => __( 'Full access to all calendars including creation, deletion, sharing permissions, and settings. Uses sensitive scopes, so a published Google Cloud project must pass OAuth app verification.', 'mcp-ai-wpoos' ),
					'scopes'                => array(
						self::SCOPE_CALENDAR,
		...
		public static function missing_scope_error( $required ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'This Google Calendar connection was not granted the "%s" permission. Reconnect the account and approve all requested Calendar permissions.', 'mcp-ai-wpoos' ),
					$required
				),
				array(
					'status'         => 403,
