<?php
/**
 * Google Calendar push notification receiver and channel manager.
 *
 * Google Calendar push notifications carry **no request body** — they only signal
 * that *something* changed. The receiver therefore validates the request, returns
 * `200` immediately, and defers the actual read to a scheduled incremental sync.
 * Doing API work inside the webhook request would be both slow and unreliable.
 *
 * Google also states plainly that notifications are not 100% reliable and that a
 * small percentage are dropped under normal conditions. Push is consequently a
 * *trigger*, never a transport: `WP_MCP_AI_Google_Calendar_Sync` always runs a
 * jittered safety-net poll alongside it.
 *
 * Operational constraints this class encodes:
 *
 * - The receiving URL must be HTTPS with a valid CA-signed certificate. Self-signed,
 *   untrusted, revoked, and hostname-mismatched certificates are rejected outright,
 *   so local and staging sites can never receive push. `is_push_eligible()` detects
 *   this up front rather than letting `watch` fail opaquely.
 * - Google's `APIs-Google` agent respects `robots.txt`, so a disallowed `/wp-json/`
 *   silently kills delivery.
 * - Channels expire after at most 7 days and there is no auto-renewal. Renewal must
 *   use a *new* channel ID, which produces an overlap window where two channels are
 *   live for the same resource — hence the message-number high-water mark.
 * - Google may deliver the `sync` handshake *before* the `watch` HTTP response
 *   returns, so the channel row is written before `watch` is called.
 *
 * @package WP_MCP_AI
...
if ( ! class_exists( 'WP_MCP_AI_Google_Calendar_Push' ) ) {
	 ...
	 */
	class WP_MCP_AI_Google_Calendar_Push {
		...
		public function verify_notification( $request ) {
			...
			if ( '' === $channel_id || '' === $token ) {
				return new WP_Error(
					...
					__( 'Missing Google notification headers.', 'mcp-ai-wpoos' ),
					array( 'status' => 401 )
				);
			}
			...
			if ( empty( $channel ) || empty( $channel['token'] ) ) {
				...
				return new WP_Error(
					...
					__( 'Unrecognised Google notification channel.', 'mcp-ai-wpoos' ),
					array( 'status' => 403 )
				);
			}
			...
			if ( ! hash_equals( (string) $channel['token'], $token ) ) {
				return new WP_Error(
					...
					__( 'Google notification token mismatch.', 'mcp-ai-wpoos' ),
					array( 'status' => 403 )
				);
			}
		...
		public function handle_notification( $request ) {
			...
			if ( ! empty( $channel ) && 'exists' === $resource_state ) {
				...
				if ( ! wp_next_scheduled( WP_MCP_AI_Google_Calendar_Sync::SYNC_HOOK, array( $connection_id, $calendar_id ) ) ) {
		...
		public static function is_push_eligible() {
			...
			if ( 'https' !== wp_parse_url( $url, PHP_URL_SCHEME ) ) {
				return new WP_Error(
					...
					__( 'Google push notifications require the site to be served over HTTPS with a valid certificate. Incremental polling will be used instead.', 'mcp-ai-wpoos' )
				);
			}
			...
			if ( ! $host || 'localhost' === $host || filter_var( $host, FILTER_VALIDATE_IP ) ) {
				return new WP_Error(
					...
					__( 'Google push notifications require a publicly resolvable domain name. Incremental polling will be used instead.', 'mcp-ai-wpoos' )
				);
			}
			...
			if ( $resolved && $resolved !== $host && ! filter_var( $resolved, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE ) ) {
				return new WP_Error(
					...
					__( 'This site resolves to a private network address, which Google cannot reach. Incremental polling will be used instead.', 'mcp-ai-wpoos' )
				);
			}
		...
		public static function watch( $connection_id = '', $calendar_id = '' ) {
			...
			WP_MCP_AI_Google_Calendar_Sync::save_state(
		...
		public static function stop( $channel_id ) {
			...
			if ( empty( $channel ) ) {
				return new WP_Error(
					...
					__( 'That Google notification channel is not registered.', 'mcp-ai-wpoos' )
				);
			}

			$credentials = WP_MCP_AI_Google_Calendar_Credentials::resolve(
