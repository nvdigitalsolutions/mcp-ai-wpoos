 ...
 *
 * A thin, quota-aware wrapper around the Calendar v3 REST surface. The client
 * deliberately owns three concerns that are easy to get wrong and expensive to
 * get wrong twice:
 *
 * 1. **Retry policy.** Google returns `rateLimitExceeded` as either `403` or
 *    `429` and documents them as functionally identical. Both, plus `5xx`, are
 *    retried with exponential backoff and per-attempt jitter. Client errors are
 *    never retried.
 * 2. **HTTP 410 discrimination.** `410` carries three distinct meanings on this
 *    API. `fullSyncRequired` and `updatedMinTooLongAgo` require dropping local
 *    state and resyncing; `deleted` on a DELETE is a *success*. Branching on the
 *    status code alone is a bug.
 * 3. **Sync-mode parameter safety.** Eight `events.list` parameters are legal on
 *    an initial full sync but return `400` when combined with `syncToken`.
 *    `build_sync_params()` enforces the split so callers cannot leak `timeMin`
 *    into an incremental request — the single most common Calendar sync defect.
 *
 * @package WP_MCP_AI
...
if ( ! class_exists( 'WP_MCP_AI_Google_Calendar_Client' ) ) {
	 ...
	 * Google Calendar API v3 client.
	 */
	class WP_MCP_AI_Google_Calendar_Client {
		...
		public function request( $method, $path, array $params = array(), $body = null ) {
			...
			return $last_error instanceof WP_Error
				...
				: new WP_Error(
					...
					__( 'The Google Calendar API remained unavailable after several retries.', 'mcp-ai-wpoos' ),
					array( 'status' => 503 )
				);
		}
		...
		protected function interpret_response( $response ) {
			if ( is_wp_error( $response ) ) {
				...
				return array(
					...
					'error'       => new WP_Error(
						...
						__( 'Unable to reach the Google Calendar API.', 'mcp-ai-wpoos' ),
						array(
							'status' => 503,
			...
			if ( 410 === $status ) {
				if ( 'deleted' === $reason ) {
					return array(
						...
						'error'       => new WP_Error(
							...
							__( 'The Google Calendar resource has already been deleted.', 'mcp-ai-wpoos' ),
							array(
								'status' => 410,
								'reason' => $reason,
							)
				...
				return array(
					...
					'error'       => new WP_Error(
						...
						__( 'The Google Calendar sync token is no longer valid. A full resynchronisation is required.', 'mcp-ai-wpoos' ),
						array(
							'status' => 410,
							'reason' => $reason,
						)
			...
			if ( $this->is_retryable( $status, $reason ) ) {
				return array(
					...
					'error'       => new WP_Error(
						...
						'' !== $message ? $message : __( 'The Google Calendar API is rate limiting this request.', 'mcp-ai-wpoos' ),
						array(
							'status' => $status,
							'reason' => $reason,
						)
			...
			return array(
				...
				'error'       => new WP_Error(
					...
					'' !== $message ? $message : __( 'The Google Calendar API rejected the request.', 'mcp-ai-wpoos' ),
					array(
						'status'   => $status,
						'reason'   => $reason,
						'response' => $data,
		...
		protected function resolve_token() {
			...
			if ( '' === $token ) {
				return new WP_Error(
					...
					__( 'No Google access token is available for this Calendar request.', 'mcp-ai-wpoos' ),
					array( 'status' => 401 )
				);
			}
