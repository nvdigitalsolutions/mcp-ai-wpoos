	...
	class WP_MCP_AI_Anthropic_Client {
		...
		public function get_api_key() {
			...
			if ( empty( $key ) && class_exists( 'WP_MCP_AI_Credential_Resolver' ) ) {
				$key = WP_MCP_AI_Credential_Resolver::get_api_key( 'anthropic' ) ?? '';
			}
		...
		public function create_chat_completion( array $messages, array $options = array() ) {
			...
			if ( class_exists( 'WP_MCP_AI_Provider_Circuit_Breaker' ) && ! WP_MCP_AI_Provider_Circuit_Breaker::is_allowed( 'anthropic' ) ) {
				return new WP_Error(
					'provider_circuit_open',
					__( 'Anthropic API is temporarily unavailable due to repeated failures. Please try again shortly.', 'mcp-ai-wpoos' ),
					array(
						'status'      => 503,
						'retry_after' => 60,
					)
			...
			if ( class_exists( 'WP_MCP_AI_Provider_Circuit_Breaker' ) ) {
				if ( is_wp_error( $response ) ) {
					WP_MCP_AI_Provider_Circuit_Breaker::record_failure( 'anthropic' );
				} else {
					$cb_status = wp_remote_retrieve_response_code( $response );
					if ( $cb_status >= 500 ) {
						WP_MCP_AI_Provider_Circuit_Breaker::record_failure( 'anthropic' );
					} else {
						WP_MCP_AI_Provider_Circuit_Breaker::record_success( 'anthropic' );
					}
				}
			}
