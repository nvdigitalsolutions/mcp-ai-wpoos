<?php
/**
 * Value object describing a pending markup elicitation request.
 *
 * @package WP_MCP_AI
 * @since   1.3.0
...
/**
 * Class WP_MCP_AI_Markup_Request
 *
 * Immutable description of a markup elicitation: what asset to mark up,
 * which interaction mode the canvas should use, what extra fields the
 * tool needs alongside the markup, and the scoping/expiry metadata the
 * store uses for replay protection.
 *
 * @since 1.3.0
 */
class WP_MCP_AI_Markup_Request {
	...
	/**
	 * Target descriptor: either ['attachment_id' => int] or ['url' => string]
	 * or ['data_uri' => string] or ['text' => string] (for plain-text mode).
	 *
	 * @var array
	 */
	private $target;
	...
	/**
	 * Optional JSON Schema fragment describing extra fields the tool wants
	 * collected alongside the markup (for example a follow-up prompt).
	 *
	 * @var array
	 */
	private $schema;
	...
	/**
	 * Human-readable instructions shown above the canvas. Kept short.
	 *
	 * @var string
	 */
	...
	/**
	 * Creating user ID (0 for guest). Used for capability checks.
	 *
	 * @var int
	 */
	private $user_id;
	...
	public function __construct( array $args ) {
		...
		$this->expires_at     = isset( $args['expires_at'] ) ? (int) $args['expires_at'] : ( time() + $ttl );
	}
	 ...
	 * @return WP_MCP_AI_Markup_Request|WP_Error
	 */
