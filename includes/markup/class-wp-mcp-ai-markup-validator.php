 ...
 *
 * Performs:
 *  - Schema-level checks of the W3C Web Annotation document.
 *  - Capability checks (read-asset capability for the asset and write
 *    capability for any new mask attachments).
 *  - DoS guards (max shapes, max SVG size, max mask dimensions).
 *  - Attachment-id resolution against the request's declared target.
 *
 * @package WP_MCP_AI
 * @since   1.3.0
...
/**
 * Class WP_MCP_AI_Markup_Validator
 *
 * @since 1.3.0
 */
class WP_MCP_AI_Markup_Validator {
	 ...
	 * @param array                    $annotation Submitted annotation.
	 * @return array|WP_Error Cleaned annotation array, or error.
	 */
	public function validate( WP_MCP_AI_Markup_Request $request, $annotation ) {
		if ( ! is_array( $annotation ) ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_payload', __( 'Markup payload must be an object.', 'mcp-ai-wpoos' ) );
		}

		$encoded = wp_json_encode( $annotation );
		if ( false === $encoded ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_payload', __( 'Markup payload is not JSON-serialisable.', 'mcp-ai-wpoos' ) );
		}
		if ( strlen( $encoded ) > self::MAX_ANNOTATION_BYTES ) {
			return new WP_Error( 'wp_mcp_ai_markup_too_large', __( 'Markup payload exceeds the maximum allowed size.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( 'Annotation' !== $type && ! ( is_array( $type ) && in_array( 'Annotation', $type, true ) ) ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_type', __( 'Annotation type must be "Annotation".', 'mcp-ai-wpoos' ) );
		}

		// Body items describe the user's marks. Normalize to an array of items.
		$body = isset( $annotation['body'] ) ? $annotation['body'] : array();
		if ( ! is_array( $body ) ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_body', __( 'Annotation body must be an array.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( count( $body ) > self::MAX_SHAPES ) {
			return new WP_Error(
				...
				sprintf( __( 'Markup contains too many shapes (max %d).', 'mcp-ai-wpoos' ), self::MAX_SHAPES )
			);
		}
		...
		if ( ! is_array( $target ) ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_target', __( 'Annotation target must be an object or array.', 'mcp-ai-wpoos' ) );
		}
		...
		$cleaned = array(
			...
			'motivation' => isset( $annotation['motivation'] ) && is_string( $annotation['motivation'] )
				...
				: WP_MCP_AI_Markup_Elicitation::motivation_for_mode( $request->get_mode() ),
			'body'       => $cleaned_body,
	 ...
	 */
	private function validate_body_item( WP_MCP_AI_Markup_Request $request, $item ) {
		if ( ! is_array( $item ) ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_body_item', __( 'Each annotation body item must be an object.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( '' !== $type && ! in_array( $type, $allowed, true ) ) {
			return new WP_Error(
				...
				sprintf( __( 'Annotation body type "%s" is not supported.', 'mcp-ai-wpoos' ), $type )
			);
		}
		...
		if ( isset( $item['shape'] ) && is_array( $item['shape'] ) ) {
			...
			if ( '' !== $shape_kind && ! in_array( $shape_kind, $allowed_shapes, true ) ) {
				return new WP_Error(
					...
					sprintf( __( 'Shape "%s" is not supported.', 'mcp-ai-wpoos' ), $shape_kind )
				);
			}
			$points = isset( $item['shape']['points'] ) && is_array( $item['shape']['points'] ) ? $item['shape']['points'] : array();
			if ( count( $points ) > 2048 ) {
				return new WP_Error( 'wp_mcp_ai_markup_too_many_points', __( 'Shape has too many vertices.', 'mcp-ai-wpoos' ) );
			}
			$clean_points = array();
	 ...
	 * @return array|WP_Error
	 */
	private function validate_target( WP_MCP_AI_Markup_Request $request, array $target ) {
		...
		if ( empty( $cleaned ) ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_target', __( 'Annotation target is empty.', 'mcp-ai-wpoos' ) );
		}
		return $cleaned;
	}
	 ...
	 * @return array|WP_Error
	 */
	private function validate_single_target( WP_MCP_AI_Markup_Request $request, array $target ) {
		$cleaned = array();
		...
		if ( isset( $target['selector'] ) ) {
			...
			if ( ! is_array( $selector ) ) {
				return new WP_Error( 'wp_mcp_ai_markup_invalid_selector', __( 'Selector must be an object or array.', 'mcp-ai-wpoos' ) );
			}
		...
		switch ( $type ) {
			case 'SvgSelector':
				...
				if ( strlen( $value ) > self::MAX_SVG_BYTES ) {
					return new WP_Error( 'wp_mcp_ai_markup_svg_too_large', __( 'SVG selector exceeds maximum size.', 'mcp-ai-wpoos' ) );
				}
			...
			default:
				return new WP_Error(
					...
					sprintf( __( 'Selector type "%s" is not supported.', 'mcp-ai-wpoos' ), $type )
				);
		}
	}
	 ...
	 */
	private function check_target_capability( WP_MCP_AI_Markup_Request $request ) {
		...
		if ( $attachment_id <= 0 ) {
			return new WP_Error( 'wp_mcp_ai_markup_invalid_attachment', __( 'Invalid target attachment.', 'mcp-ai-wpoos' ) );
		}
		$attachment = get_post( $attachment_id );
		if ( ! $attachment || 'attachment' !== $attachment->post_type ) {
			return new WP_Error( 'wp_mcp_ai_markup_attachment_not_found', __( 'Target attachment not found.', 'mcp-ai-wpoos' ) );
		}
		...
		if ( $user_id > 0 ) {
			if ( ! current_user_can( 'read_post', $attachment_id ) ) {
				return new WP_Error(
					...
					__( 'You are not allowed to mark up this asset.', 'mcp-ai-wpoos' ),
					array( 'status' => 403 )
				);
			}
			return true;
		...
		if ( $request->get_user_id() <= 0 && $request->get_assistant_id() > 0 ) {
			if ( (int) $attachment->post_author > 0 ) {
				...
				return new WP_Error(
					'wp_mcp_ai_markup_forbidden',
					__( 'Guests are not allowed to mark up this asset.', 'mcp-ai-wpoos' ),
					array( 'status' => 403 )
				);
			}
		}
