 ...
 *
 * Subscribes to the four markup lifecycle actions and aggregates outcome,
 * per-tool, and per-mode counters into a single bounded WordPress option
 * (`wp_mcp_ai_markup_telemetry`).
 *
 * The recorder is intentionally lightweight: every callback runs in O(1)
 * with a single `update_option()` call. It does not persist anything
 * about individual users or payload contents — only aggregate counts +
 * the timestamp of the most recent event per outcome.
 *
 * Each event also produces a corresponding `WP_MCP_AI_Logger::log_event()`
 * entry under the `markup_*` event-type family so the existing recent
 * activity feed picks it up when logging is enabled.
 *
 * @package WP_MCP_AI
 * @since   1.3.0
...
/**
 * Class WP_MCP_AI_Markup_Telemetry
 *
 * @since 1.3.0
 */
class WP_MCP_AI_Markup_Telemetry {
	 ...
	 *
	 * @param mixed $request Markup request (expected WP_MCP_AI_Markup_Request).
	...
	protected function record( $outcome, $tool_slug, $mode ) {
		...
		$summary['last_seen'][ $outcome ] = time();
