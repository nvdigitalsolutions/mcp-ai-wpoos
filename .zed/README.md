...
- **`nv-oos-local`** — point Zed at the `mcp-ai/v1` REST endpoint of a local NV oOS site (e.g. `docker compose up -d` per [`MAINTAINER_MAP.md`](../MAINTAINER_MAP.md)). Gives the agent access to all ~830 in-product NV oOS tools while developing.
