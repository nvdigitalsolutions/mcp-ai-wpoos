...
| **P2b** | Close the payload-filter leak where ~290 registered slugs (per the Capability-Fence Audit) declared neither `get_required_capability()` nor a `'required_capability'` key in `get_definition()` and therefore fell through `build_tools_payload()`'s `method_exists()` check with no fence. | M | v1.2.1 | ✅ Landed (May 2026) — new authoritative slug → capability map at [`includes/class-wp-mcp-ai-tool-capability-map.php`](../../includes/class-wp-mcp-ai-tool-capability-map.php) with `WP_MCP_AI_Tool_Capability_Map::resolve()` honouring per-class method → map → safe default (`edit_posts`); [`includes/class-wp-mcp-ai-rest.php::build_tools_payload()`](../../includes/class-wp-mcp-ai-rest.php) wired to the resolver with a guest-bypass that preserves existing guest assistants; coverage tests at [`tests/test-tool-required-capability-coverage.php`](../../tests/test-tool-required-capability-coverage.php) assert `*_validated` wrappers mirror their un-validated peers and sensitive destructive slugs are locked to `manage_options` / `install_plugins` / `install_themes` / `delete_posts`. Audit doc at [`docs/proposals/audits/P2b-required-capability-assignment-2026-05.md`](audits/P2b-required-capability-assignment-2026-05.md). Filters `wp_mcp_ai_tool_capability_map` and `wp_mcp_ai_tool_required_capability` provide deployment-level overrides. Per-class `get_required_capability()` methods for the ~290 mapped slugs land incrementally in follow-up PRs (plan's PR-A through PR-F) — security gap is already closed by the map. |
