...
The cap default stays at 100 (filter `wp_mcp_ai_max_chat_tools`, clamped 1–128, plus the 48,000-token budget filter `wp_mcp_ai_max_chat_tool_tokens`). A new opt-in adaptive cap (option `wp_mcp_ai_adaptive_tool_cap`, default off) lowers the effective cap per model context window via `WP_MCP_AI_Tool_Payload_Advisor` — 40 tools for ≤128K contexts, 64 for ≤256K, 100 above — and never raises it.
