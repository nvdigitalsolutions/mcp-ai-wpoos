...
NV oOS implements these security enhancements while maintaining backward compatibility with existing clients.
...
* **Simple JWT Login tokens** (optional) reuse the JWTs minted by the Simple JWT Login plugin. NV oOS validates them via Simple JWT Login’s services, falls back to manual decoding when necessary, and imports assistant or scope claims so REST requests inherit the correct context.【F:includes/class-wp-mcp-ai-simple-jwt-login-integration.php†L47-L214】【F:includes/integrations/class-wp-mcp-ai-integration-simple-jwt.php†L240-L378】
...
3. **Configure WordPress.** In **Settings → NV oOS**, enable **Auth0 GitHub bridge** and paste the Management API Client ID/Secret alongside your Auth0 domain. These values are stored with the rest of the plugin settings and sanitised on save.【F:includes/admin/class-wp-mcp-ai-admin-settings.php†L59-L119】【F:includes/admin/class-wp-mcp-ai-admin-settings.php†L1184-L1241】【F:includes/admin/class-wp-mcp-ai-admin-settings.php†L1658-L1698】
...
1. With the Auth0 1-click setup wizard (Settings → NV oOS → Auth0 Setup) to auto-configure your Auth0 integration
...
1. `WP_MCP_AI_REST::permissions_check()` extracts the bearer token, normalises the Auth0 domain configured in **Settings → NV oOS**, and downloads the tenant's JWKS (cached for one hour).
