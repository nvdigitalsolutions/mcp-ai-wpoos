# API & Technical Reference

Complete technical reference documentation for NV oOS APIs, tools, models, and specifications.

## 📂 Reference Sections

### [API Documentation](api/)
REST API, MCP endpoint, authentication:
- [REST API Reference](api/rest-api.md) ⭐ **ESSENTIAL**
- [MCP Endpoint](api/mcp-endpoint.md) - MCP 2024-11-05 specification
- [Authentication](api/authentication.md) - Auth methods and security
- [MCP Server Authentication](api/mcp-server-authentication.md)
- [MCP & SSE](api/MCP-AND-SSE.md) - Server-Sent Events
- [Gemini API](api/gemini/) - Gemini-specific API docs

### [Tools Reference](tools/)
Complete tool documentation:
- [Tool Reference](tools/tool-reference.md) ⭐ **ESSENTIAL** - All 133 tools
- [Tool Grouping](tools/tool-grouping.md) - Organization by dependency
- [Capability Flags](tools/capability-flags-usage.md) - Tool capabilities
- [Tool Multipliers](tools/TOOL-MULTIPLIERS.md) - Token usage multipliers
- [Tool Token Limits](tools/TOOL-TOKEN-LIMITS.md) - Per-tool limits

### [Models Reference](models/)
AI model specifications:
- [New AI Models 2024](models/NEW-AI-MODELS-2024.md) - Latest models
- [Model Rate Limits](models/MODEL-RATE-LIMITS-CCT.md) - Rate limiting
- [Model Config Comparison](models/model-config-visual-comparison.md)
- [Core vs Pro Features](models/FEATURE-MATRIX-CORE-PRO.md)

### [Technical Documentation](technical/)
Technical specifications:
- [Token Context Window](technical/TOKEN-CONTEXT-WINDOW-EXPLAINED.md)
- [Token Counting](technical/token-counting.md) - Accurate token estimation
- [Generic Tool Response](technical/generic-tool-response.md)
- [Base vs Full Comparison](technical/base-vs-full-comparison.md)
- [PHP Version Compatibility](technical/php-version-compatibility.md)

## 🔗 Related Documentation
- [Getting Started](../getting-started/)
- [Architecture](../architecture/)
- [Developer Guides](../guides/developer/)
