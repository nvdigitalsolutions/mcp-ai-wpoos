...
- **Web Search** (`web_search`) performs lightweight web searches via the configured provider — DuckDuckGo Instant Answer (default, no key required), Brave Search, or Tavily (AI-first, recommended for agents). Accepts optional `country` (ISO 3166-1 alpha-2), `language` (ISO 639-1), and `freshness` (`pd`/`pw`/`pm`/`py`) parameters for geo-targeting and recency filtering. Brave results are enriched with `extra_snippets`; Tavily returns page-excerpt content optimised for LLM consumption. All results are deduplicated by URL, snippet-grounded for the LLM via `sanitize_for_llm()`, and subject to per-user rate limiting and result caching.【F:includes/tools/class-wp-mcp-ai-tool-web-search.php†L1-L320】
