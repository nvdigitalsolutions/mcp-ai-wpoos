...
**Tool Count:** ~347 base tools + ~1,287 Pro tools = ~1,634 total (live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative; +35 base +5 Pro in v1.1.87 — the 30-tool mcp-wordpress parity suite `list_comments` … `seo_keyword_research` from PR #6777 plus the five-tool image identification ladder `identify_image`/`get_image_metadata`/`find_similar_media`/`detect_image_content`/`describe_image_layout` from PR #6780, and the Pro `ocr_image_classic`/`search_similar_images` (#6780) + `outbound_get_pipeline_stats`/`outbound_import_leads`/`outbound_manage_angle` (#6786); +4 base in v1.1.86 — the Higgsfield quartet `generate_higgsfield_video`, `generate_higgsfield_image`, `check_higgsfield_request`, `cancel_higgsfield_request` from PR #6772; unchanged in v1.1.85 — the MCP App bridge slugs `mcp_app_<label>_<tool>` are registered dynamically at chat time via the `wp_mcp_ai_chat_effective_tools` seam and are not counted in the static totals; +2 base in v1.1.84 — `typesafe_decide` from PR #6728, the TypeSafe Jev decision tool, and `typesafe_guardrail`, the TypeSafe Jev safety-screening tool from PR #6747; +3 Pro in v1.1.84 — `typesafe_rerank`, `typesafe_eval`, `typesafe_skill_select`, the TypeSafe Jev decision toolset from PR #6747; unchanged in v1.1.83 — no tool registrations in-window, the Tool Description Engineering sweep adds model-facing usage guidance to every tool class without adding slugs; unchanged in v1.1.82 — no tool registrations in-window; +13 Pro in v1.1.81 — five CRM tools `bulk_move_deal_stages`, `create_tracked_link`, `record_crm_reply`, `get_crm_handover`, `get_pipeline_digest` from PR #6636 and eight financial tools `market_screener`, `macro_data_fetcher`, `economic_calendar_fetcher`, `earnings_calendar_fetcher`, `options_chain_fetcher`, `crypto_market_data`, `portfolio_transaction_log`, `price_alerts` from PR #6639)
**Last Updated:** September 27, 2026 (v1.1.87 — the mcp-wordpress parity suite (+30 base), the image identification ladder (+5 base +2 Pro), and the outbound booking tools (+3 Pro) move the count above; note: none of the 40 new window slugs landed in `docs/tool-status.txt` and only the image ladder's `identify_image` entry is documented below — tracked as OI-8; the v1.1.86 Higgsfield entries remain OI-7)
...
- **Vocabulary rule (D1):** a contract name is the *exact* envelope / parameter key that flows between tools — `job_id`, `plan_id`, `session_id`, `room_id`, `post_id`, `term_id`, `assistant_id`, `vector_store_id`, `batch_id` (base) and `schedule_id`, `item_id`, `record_id`, `member_id`, `event_id`, `snippet_id` (Pro). A tool `produces X` iff its success envelope carries key `X`; a tool `consumes X` iff its parameters schema has property `X`.
...
- **TypeSafe Decide (Jev)** (`typesafe_decide`) asks the TypeSafe Jev decision model to answer typed questions (choice, score, or yes/no) about supplied text or structured state. Jev returns constrained, probabilistic decisions with confidence — it never generates prose, so it is suited to routing, triage, classification, and relevance judgments. State containing user-controlled content is treated as untrusted; consequential actions must stay gated in code. Requires `manage_options` and a TypeSafe key (or an OpenRouter key with `transport: openrouter`). Input-only billing ($0.042/M input tokens, output free).【F:includes/tools/class-wp-mcp-ai-tool-typesafe-decide.php†L17-L330】
- **TypeSafe Guardrail (Jev)** (`typesafe_guardrail`) screens content for safety hazards with Jev noul questions and thresholds the probabilities into advisory verdicts (pass / review / block) per category. The verdicts are advisory only — enforcement stays in the caller's code gates. Requires `manage_options` and a TypeSafe key (or an OpenRouter key with `transport: openrouter`).【F:includes/tools/class-wp-mcp-ai-tool-typesafe-guardrail.php†L17-L330】
...
### TypeSafe Jev decision tools (Pro)

- **TypeSafe Rerank** (`typesafe_rerank`) reranks a candidate set (search results, vector-store hits, media) by relevance to a query: each candidate is one score question batched into a single Jev call, then reordered most-relevant-first with a keep-minimum floor. Capability: `manage_options`.【F:addons/pro/includes/tools/jev/class-wp-mcp-ai-pro-tool-typesafe-rerank.php†L17-L330】
- **TypeSafe Eval** (`typesafe_eval`) calibration harness: runs inline labeled examples through Jev and reports overall and per-confidence-bucket accuracy plus per-example grading (report-only, nothing stored or trained). Capability: `manage_options`.【F:addons/pro/includes/tools/jev/class-wp-mcp-ai-pro-tool-typesafe-eval.php†L17-L330】
- **TypeSafe Skill Select** (`typesafe_skill_select`) ranks the bundled skill catalog against a task in one choice question, then re-checks the top candidates with noul appropriateness questions (TypeSafe's two-stage skill-suggestion pattern) for the caller to load via `load_skill`. Capability: `manage_options`.【F:addons/pro/includes/tools/jev/class-wp-mcp-ai-pro-tool-typesafe-skill-select.php†L17-L330】
