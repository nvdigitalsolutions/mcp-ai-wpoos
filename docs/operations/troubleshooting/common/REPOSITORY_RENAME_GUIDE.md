...
This document outlines the repository rename from `nvdigitalsolutions/wp-mcp-ai` to `nvdigitalsolutions/mcp-ai-wpoos` to better align the repository name with the actual plugin name "Open Operator System (NV oOS)".

## Rationale

**Why rename from `wp-mcp-ai` to `mcp-ai-wpoos`?**

1. **Better alignment with plugin branding**: The plugin is branded as "NV oOS" (Open Operator System)
