...
| v1.1.28 | June 8, 2026 | CRM Phase C (IMAP/Twilio/WhatsApp/Gmail inbound), Customer CPT + 360 dashboard, Support Ticket module (10 AI tools + SLA), TF-IDF + BM25, QKV Attention Routing, Funiq Bridge addon, NVOOS Graphify ecosystem. |
| v1.1.29–v1.1.42 | Jun–Jul 2026 | Multiple releases: Pro Toolkit Optimizations, DietPi Toolkit, LibreChat, Layer I Guardrails, Context Window Management, WP 7.0 Connectors, Media Command Center, Content Format Templates, 33 MCP servers, OAuth 2.0 MCP auth, OKF v0.1 integration, security compliance fixes (11 HIGH/P0), Meta-Harness auto-optimization, Agent delegation rework, Page Agent addon, Pro SPA v2, security infrastructure (7 classes), nvoos/core framework-agnostic engine, Status Page & Incident Communication, 21 Agent Skills + 6 BMAD Agents, Algorave addon. |
| v1.1.43 | August 1, 2026 | MCP 2026-07-28 stateless core upgrade, Security v1.1.43 hardening (SSRF/CSRF/SQL/XSS), OKF v0.2 trust-signal support, ICP System (Pro CRM Phase G), Pro Module Registry PSR-4, Hexagonal architecture purity, 7 playbook/profession sync fixes, Phase 3 operational security hardening, WPCS 3.4.1 (CVE-2026-45293). |
| v1.1.46 | August 6, 2026 | Comprehensive Backup & Restore (11 modular export providers), GitHub-based Plugin Updater (772 lines) for full-build auto-updates, Abilities API selective adoption (includes/abilities/ framework with 5 classes + 5 test files) for AI agent discovery, Status Page fixes (REST fatal error, JS, i18n), Knowledge base auto-build CI, PHPCS cleanup (parse errors, text domain, WPCS formatting across 100+ files). |
| v1.1.45 | August 5, 2026 | Self-hosted OCR (Unlimited-OCR + DeepSeek-OCR, 17 files, +4,087 lines): unified vLLM client, 2 Pro tools, structured extraction service, Embedded backend + dashboard, admin settings UI. Embedded addon v0.2.0 (voice, OpenMed, MCP abilities). AI transparency & SGI compliance. Comic Reader v0.2.0. Graphify standalone plugins v1.0.1. Build/release automation. |
| v1.1.44 | August 4, 2026 | CCT stability (mutex lock, FlowHub guard, base-plugin fatal fix, Veo async context), API key resolution fixes, Proposal 016 architecture hardening (all waves), Proposal 017 polling/queue/load-balancing hardening, Deferred security items #5755, Docs: FOR_REVIEWERS update, 16 broken links fixed, Graphify ecosystem audit. |
