# Getting Started with NV oOS

Quick start guides and installation documentation for Open Operator System (NV oOS).

## 📂 Contents

### Use Cases & Quickstart Guides
- **[Use Cases & Quickstart Guides](USE_CASES_AND_QUICKSTARTS.md) ⭐ NEW** - Comprehensive guide covering:
  - Content Creation & Management
  - E-Commerce Automation
  - Media Generation & Processing
  - Business Operations
  - Research & Data Analysis
  - Developer & Technical Integration
  - Education & Knowledge Management
- [5-Minute Quick Start](QUICK_START_5_MINUTES.md) - From zero to first chat in 5 minutes

### Installation & Setup
- [Plugin Setup Checklist](installation-setup/mcp-ai-plugin-setup-checklist.md) - Complete setup guide
- [Deployment Troubleshooting](installation-setup/deployment-troubleshooting.md) - Common deployment issues
- [Cloudflare Tunnel Setup](installation-setup/cloudflare-tunnel-setup.md) - Secure remote access
- [Multisite Support](installation-setup/multisite-support.md) - WordPress multisite configuration

### Quick Starts
- [MCP Testing Quick Start](quick-starts/QUICK-START-MCP-TESTING.md) - Test MCP endpoint quickly
- [Remote Client Quickstart](quick-starts/remote-client-quickstart.md) - Connect external clients
- [Chat History Persistence](quick-starts/chat-history-persistence-quickstart.md) - Save chat sessions
- [LM Studio Setup](quick-starts/lm-studio-setup.md) - Local AI model setup

### First Steps
- [Assistant Tool Shortcuts](first-steps/assistant-tool-shortcuts.md) - Pre-configured task shortcuts
- [Console Testing](first-steps/console-testing.md) - Browser console testing
- [Test Assistant Visual Reference](first-steps/test-assistant-visual-reference.md) - Visual testing guide

## 🔗 Related Documentation
- [Main Documentation Index](../DOCUMENTATION_INDEX.md)
- [User Guides](../guides/user/)
- [Developer Guides](../guides/developer/)
- [Troubleshooting](../troubleshooting/)
