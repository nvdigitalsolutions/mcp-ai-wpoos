...
> **SEPTEMBER 27, 2026 UPDATE (v1.1.87):** PARITY SUITE, IMAGE IDENTIFICATION & OUTBOUND BOOKING. **MCP-WORDPRESS PARITY SUITE (PR #6777)** — 30 NEW NATIVE BASE TOOLS PORTED FROM THE MIT UPSTREAM: COMMENT CRUD (`list_comments`–`delete_comment`), USER CRUD (`list_users`–`delete_user`), CONTENT/MEDIA/TERMS (`get_post_revisions`, `get_term`, `delete_term`, `get_media`, `upload_media`, `update_media`, `delete_media`), SITE SETTINGS + APPLICATION PASSWORDS (`get/update_site_settings` ALLOWLIST-ONLY, `list/create/delete_application_passwords` LOG-MASKED), AND A 9-TOOL SEO TOOLKIT — WITH CAPABILITY CHECKS, MULTISITE GUARDS, AND NEW `comment_id`/`user_id`/`attachment_id` ID-HANDOFF CONTRACTS. **NON-LLM IMAGE IDENTIFICATION LADDER (PROPOSAL 043, PRS #6780/#6785)** — `identify_image` ORCHESTRATES METADATA → PERCEPTUAL-HASH MEDIA LOOKUP → CLASSIC CLOUD VISION DETECTION → LAYOUT DESCRIPTION → OPTIONAL REVERSE-IMAGE SEARCH WITHOUT A VISION LLM; 5 BASE TOOLS JOIN THE MEDIA GENERATION PRESET + 2 PRO TOOLS (`ocr_image_classic`, `search_similar_images`); EVERY EXTERNAL RUNG IS KEY-GATED AND FAILS CLOSED. **OUTBOUND APPOINTMENT BOOKING TOOLKIT (PROPOSAL 044, PR #6786)** — GATED PRO TOOLKIT (`enable_outbound_booking_toolkit`) WITH ICP LISTS, HOURLY SEQUENCE ENGINE, DM APPROVAL BOARDS, ANGLE-BANK A/B TESTS, BOOKING LINKS + SHORTCODE + RATE-LIMITED HONEYPOTTED PUBLIC BOOKING ENDPOINT, AND 3 NEW PRO TOOLS. **MCP-WORDPRESS GATEWAY ADDON (PR #6778)** — AUTH-GATED STREAMABLE HTTP MCP SERVER FOR CLOUDWAYS VELOCITY (`addons/mcp-wordpress-gateway/`, SUBTREE-SPLIT DEPLOY TO THE `nvoos-mcp-wordpress` MIRROR). **SESSION-BUDGET WARNINGS (PR #6776)** — BLOCKED-SESSION MESSAGES POINT AT THE REAL RESET PATH; THE DORMANT 75% HOOK INJECTS A ONE-SHOT WARNING VIA THE NEW `wp_mcp_ai_chat_messages` FILTER; THE TWO UNTRACKED ELEMENTOR BUNDLED SKILLS ARE COMMITTED. **RESTRICTION NOTICE FIXED (PR #6779)** — DISTINCT-USER COUNTING + RENDER-TIME SWEEP + LIFT PRUNING. **UPWORK REFINEMENTS (PR #6784)** — IN-PLACE `workflow_steps` EDITING + `exclude_keywords`/TIER/BUDGET PARSING, BYTE-IDENTICAL CG PRO PORT. TOOL COUNT: ~347 BASE + ~1,287 PRO (~1,634 TOTAL; +35 BASE +5 PRO). ADDONS: 27 → 28. MODEL CATALOG: v2026.09.22 (UNCHANGED). STALE BUILD ZIPS REMOVED: THE 1.1.85 SET (30 FILES). UPDATED DOCS: THIS PLAN, README/CHANGELOG/readme.txt/QUICK_REFERENCE + `docs/developer/mcp-wordpress-tool-parity.md` + `docs/features/performance/*` + `docs/operations/deployment/mcp-wordpress-velocity-setup.md` + `docs/reference/tools/tool-reference.md` + PROPOSALS 043/044 (IN-WINDOW) + `docs/features/agent-skills.md` + `docs/project/ADDON_INVENTORY.md`.

> **SEPTEMBER 25, 2026 UPDATE (v1.1.86):** MCP SERVER CONNECTIONS, HIGGSFIELD MEDIA PROVIDER, LOG FILTERS & DELIVERY TEMPLATES. **MCP APPS AS A REMOTE SITES CONNECTION TYPE (PR #6761, PROPOSAL 041)** — NEW `mcp_server` CONNECTION TYPE WITH AES-256-CBC-ENCRYPTED CENTRAL CREDENTIALS, A REAL JSON-RPC HANDSHAKE TEST CONNECTION, TOOL DISCOVERY WITH PERSISTED SNAPSHOTS, RESTRICTED-HOST ENFORCEMENT, AND AUTH MAPPING ONTO MCP (ELEMENTOR APPLICATION PASSWORDS → `basic`); PER-ASSISTANT `connection_ref` REFERENCE MODE RESOLVES CREDENTIALS DECRYPT-ON-USE AT CHAT TIME (NEVER WRITTEN BACK TO POST META) WITH AUTO-DISABLE ON MISSING IMPORT REFS; ASSISTANT EXPORT/IMPORT REDACTS MCP APP `token`/`oauth_data` BY DEFAULT (OPT-OUT FILTERS). **FOUR NEW HIGGSFIELD BASE TOOLS (PR #6772, PROPOSAL 042)** — `generate_higgsfield_video` (FIVE VERIFIED MODELS BEHIND ONE `model` PARAM), `generate_higgsfield_image` (SOUL V2/SOUL CINEMA), `check_higgsfield_request`, `cancel_higgsfield_request` ON THE SHARED `WP_MCP_AI_Higgsfield_Client` (TWO-PART `KEY ID:SECRET` AUTH, SUBMIT/STATUS/CANCEL LIFECYCLE, BACKOFF+JITTER POLLING, IMMEDIATE DOWNLOAD); ALSO FIXES THE TYPESAFE TOOL SCHEMAS + PRO COVERAGE MANIFEST. **`get_system_logs` FILTERS (PR #6768)** — OPTIONAL AND-COMBINED `since`/`levels`/`search` FILTERS OVER THE STRUCTURED BUFFERS + FILE LOGS WITH A FILTERS SUMMARY (RESOLVED UTC CUTOFF) AND PER-FILE `filtered_out` COUNTS; `parse_since()` IS THE SHARED CANONICAL PARSER; CG AI PORTS BASE-IDENTICAL. **ACTION-ITEMS TEMPLATE + SMART EXCERPTS (PR #6773)** — SCHEDULED DIGESTS SEND ONLY THE ACTIONABLE SECTION (`action_items`, EMAIL + CHAT) AND PREFER THE RESPONSE'S OWN SUMMARY/TL;DR/ACTION SECTIONS OVER BLIND 80-WORD TRIMS. **TOOL COSTS IN FINAL LABELS (PR #6771)** — TOP-LEVEL `toolResult.cost` ENVELOPE ON THE AGENTIC LOOP + SSE STREAMING WITH TOOL-BUBBLE BADGES. **ENVIRONMENT STATUS FIXED (PR #6767)** — ALWAYS-ON "NO ASSISTANTS PUBLISHED" WARNING + DEAD DEFAULT-ASSISTANT BRANCH REPAIRED; NEW `plugin.default_provider_model` RESOLVES THE EFFECTIVE PER-PROVIDER MODEL. TOOL COUNT: ~312 BASE + ~1,282 PRO (~1,594 TOTAL; +4 BASE — THE HIGGSFIELD QUARTET). MODEL CATALOG: v2026.09.22 (UNCHANGED). STALE BUILD ZIPS REMOVED: THE 1.1.84 WP.ORG PACKAGE SET (6 FILES). UPDATED DOCS: THIS PLAN, README/CHANGELOG/readme.txt/QUICK_REFERENCE + `docs/assistant-import-export.md` + `docs/features/remote-sites.md` + `docs/tool-status.txt` (IN-WINDOW, #6761/#6772) + `docs/project/proposals/041-*.md` + `042-higgsfield-video-provider-integration.md` (IN-WINDOW).

> **SEPTEMBER 22, 2026 UPDATE (v1.1.84):** TYPESAFE JEV ENHANCEMENT WAVE. **API FIDELITY (PHASE 0, PR #6747)** — NOUL CRITERIA + STRUCTURED ENTRYTYPE FIELDS (RECURSIVE TWO-GATE SANITISATION WALK); BOUNDED 429/5XX RETRIES HONOURING `retry-after` ON THE NATIVE CLIENT + OPENROUTER DECISIONS BRIDGE (4XX/TRANSPORT NEVER RETRIED; THE BRIDGE DEFAULTS TO `typesafe/jev-1.13`); AN OPT-IN ADVISORY DECISION CACHE (`enable_typesafe_cache`, ZEROED USAGE ON HITS, ENDPOINT/BASE/MODEL/PAYLOAD KEYING); A `typesafe_endpoint` OVERRIDE; THE `jev-preview` ALIAS; `min_confidence`/`weights`/ADVISORY TOKEN WARNINGS + USAGE ALIASES ON `typesafe_decide`. **NEW BASE TOOL `typesafe_guardrail` (PHASE 1, PR #6747)** — ONE NOUL QUESTION PER HAZARD CATEGORY → ADVISORY PASS/REVIEW/BLOCK VERDICTS; NEW BUNDLED SKILL `mcp-ai-wpoos-jev-decisions` (BUNDLED SKILLS 74 → 75). **PRO JEV INTEGRATIONS (PHASE 2, PR #6747)** — OPT-IN FAIL-OPEN GUEST-CHAT GUARDRAIL (`enable_jev_guest_guardrail` ON THE LAYER I PRE-CHAT FILTER), ADVISORY CITATION CHECKING ON `generate_research_report`/`research_eca`, AND THREE NEW `manage_options`-GATED TOOLS `typesafe_rerank`/`typesafe_eval`/`typesafe_skill_select`. **CAPABILITY FIX (PR #6745)** — `typesafe_decide`'S DECLARED CAPABILITY MATCHES ITS ENFORCED ADMIN GATE (METADATA-ONLY, CI-PINNED). **PROPOSAL + PLAN 040** (PR #6746) — EXTRACTION TOOLS, THE CG PORT CLUSTER, AND NV CLOUD PASSTHROUGH REMAIN DEFERRED. TOOL COUNT: ~308 BASE + ~1,282 PRO (~1,590 TOTAL; +1 BASE +3 PRO). MODEL CATALOG: v2026.09.22 (+`jev-preview`). STALE BUILD ZIPS: NONE REMOVED THIS PASS (THE 1.1.83 SET IS RETAINED AS THE CURRENT RELEASE ARTIFACTS). UPDATED DOCS: THIS PLAN, README/CHANGELOG/readme.txt/QUICK_REFERENCE + `docs/features/ai-providers/typesafe.md` + `docs/reference/tools/tool-reference.md` (IN-WINDOW, #6745/#6747) + `docs/project/proposals/040-typesafe-jev-enhancements*.md` (IN-WINDOW, #6746).

> **SEPTEMBER 21, 2026 UPDATE (v1.1.83):** TOOL GUIDANCE, JEV DECISIONS, PRO BOOTSTRAP HARDENING & UPWORK-WORKFLOW RELEASE. **TOOL DESCRIPTION ENGINEERING** (PRs #6686, #6695, #6687–#6723) — EVERY BASE+PRO TOOL CLASS (~1,487 TOOLS ACROSS 1,584 FILES) NOW CARRIES MODEL-FACING USAGE GUIDANCE: A NEW `WP_MCP_AI_Tool_Usage_Guidance_Interface` (WHEN_TO_USE / WHEN_NOT_TO_USE / RELATED_TOOLS / NOTES) ASSEMBLES A COMPACT `[Usage: …]` SUFFIX ON THE MODEL-FACING PAYLOAD, LEGACY-FORMAT CLASSES OPT IN THROUGH `WP_MCP_AI_Legacy_Tool_Wrapper`, AND THE `WPMCPAI.Tools.ToolDescriptionGuidance` SNIFF IS ENFORCED AT SEVERITY 5 (FULL-TREE GATE 1,584/1,584 FILES, 0 WARNINGS — EVERY NEW TOOL CLASS WITHOUT GUIDANCE SURFACES ON PRs). **TYPESAFE JEV DECISION PROVIDER** (PR #6728) — A FIRST-CLASS *DECISION* PROVIDER DELIBERATELY SEPARATE FROM CHAT: TYPED CHOICE/SCORE/NOUL DECISIONS (70–500 MS) VIA A NEW DECISION-CLIENT CONTRACT + OPENROUTER DECISIONS BRIDGE + THE NEW BASE TOOL `typesafe_decide` (CANONICAL ENVELOPE, ADVERSARIAL-STATE CAVEAT); PRO CASCADE ROUTING (`jev_routing`) + OPT-IN RESEARCH SOURCE FILTER. **"THE ASSISTANT BUILDER" META-ASSISTANT** (PR #6727) — SEEDED ON ACTIVATION; DESIGNS/BUILDS/VERIFIES OTHER ASSISTANTS (ROSTER 6 → 7). **ID-HANDOFF DATA CONTRACTS** (PRs #6729–#6739) — EVERY ID-BEARING TOOL FAMILY (POST, CRON, TERM, ASSISTANT, VECTOR STORE, BATCH, PRO SCHEDULE, TOOLKIT_CPT, MEDICAL RECORD, PLAN, CALENDAR, WPCODE, SESSION, MEMBER, WEBCHAT ROOM) DECLARES PRODUCES/CONSUMES HANDOFFS WITH PERMANENT MANIFEST-DRIVEN HONESTY + ROUND-TRIP SUITES. **OPT-IN ADAPTIVE TOOL CAP** (`wp_mcp_ai_adaptive_tool_cap` + PER-ASSISTANT THREE-STATE OVERRIDE) + **LAZY SCHEMA LOADING** (`tool_slug`/`include_schemas` ON `list_mcp_tools`, PR #6686). **PRO BOOTSTRAP INCOMPLETE-INSTALL GUARD** (PR #6677) — PARTIAL PRO DEPLOYS DEGRADE TO AN ADMIN NOTICE INSTEAD OF A SITE-WIDE FATAL. **TELEGRAM AUTO-CHUNKING** (PR #6677) — `send_telegram_message` SPLITS >4,096-CHAR MESSAGES AT PARAGRAPH → LINE → HARD BOUNDARIES. **UPWORK PIPELINE** (PRs #6678–#6680, #6682, #6684) — WEB_SEARCH-MODE + API CREDENTIAL GATING, CATEGORY-PAGE DROPPING + SERP BUDGET/TYPE/RECENCY EXTRACTION, ALWAYS-ON BROAD SECOND PASS, `sort`/`location` ARGS, AND WORKFLOW DELIVERIES SHIP THE **FULL 50-ITEM RESULT SET** WITH PER-ITEM URLS RENDERED AS ONE PROPERLY NUMBERED LIST; STEPS RENDER AS A COMPACT EXECUTION LOG (PR #6679). **GMAIL `connection_id: "settings"` + SKILL/OKF SELF-CORRECTION** (PR #6677). **USAGE MONITOR SAVES AGAIN** (PR #6726) — THE DASHBOARD SAVE HANDLER NOW APPLIES THE MONITOR BRIDGE FILTER. **WEBCHAT FIXES** (PRs #6738/#6740) — `log_activity()` LATENT FATAL → `log_event`; SQL PREPARE WARNINGS FIXED AT THE ROOT. **PLAYGROUND OLLAMA DEMO** (PR #6683) — PERMALINK SEED FIX ENDS THE FRESH-INSTALL LANDING 404; LOCAL `npx` IS THE PRIMARY TEST PATH. **NPM ADVISORIES CLOSED** (PR #6681 — ADM-ZIP 0.6.1, JS-YAML 4.3.2, COLORD 2.10.0). **CANONICAL ENVELOPE MIGRATIONS** — FIVE REGULATORY TOOLS (PR #6689) + `format_code_prettier` (PR #6737). TOOL COUNT: ~307 BASE + ~1,279 PRO (~1,586 TOTAL; +1 BASE — `typesafe_decide`, PR #6728). STALE 1.1.81 BUILD ZIPS REMOVED (30 FILES), THEN THE 1.1.82 SET (30 FILES) AFTER THE 1.1.83 WP.ORG PACKAGES REBUILT. UPDATED DOCS: THIS PLAN, README/CHANGELOG/readme.txt/QUICK_REFERENCE + `docs/features/tool-description-guidelines.md` (IN-WINDOW, #6686) + `docs/project/proposals/P3-data-contract-rollout-plan-2026-09.md` (IN-WINDOW, #6739).
