...
| **Onboarding Wizard** (built-in) | ⭐ **NEW:** 4-step Getting Started wizard at `/wp-admin/admin.php?page=wp-mcp-ai-getting-started`. 9 use-case presets seed fully-configured assistants with tools, system prompts, and temperatures — including the featured **Knowledge Graph Companion**, which is auto-selected when the NV oOS Content Graph (or the bundled Graphify addon) is detected. WCAG 2.1 accessible. See [README.md](../README.md#-installation). | Everyone |
