...
> **SEPTEMBER 24, 2026 UPDATE (v1.1.85):** MCP APPS CONNECTION & EXPOSURE RELEASE. **METABOX DIAGNOSTICS (PR #6753)** — PER-ROW TEST CONNECTION / DISCOVER TOOLS BUTTONS WITH INLINE RESULTS (NEGOTIATED PROTOCOL, HANDSHAKE TYPE, SERVER INFO, SESSION STATE, LATENCY, LIVE TOOL COUNT, VERBATIM ERRORS), A PERSISTED PER-APP STATUS BADGE + TOOL-COUNT CHIP, AND TEST ALL; BASIC AUTH END-TO-END (RAW `user:pass` AUTO-ENCODED OR PRE-ENCODED BASE64) WITH TOKEN MASKING (STORED CREDENTIALS NEVER ECHOED; EMPTY = KEEP); `Mcp-Session-Id` CAPTURE + ECHO, LEGACY-HANDSHAKE FALLBACK, JSON-RPC ERROR DECODING ON HTTP 4XX, MCP SERVERS JSON IMPORT, LOOPBACK DETECTION WITH A PHP-FPM DEADLOCK WARNING; **SECURITY CENTER → MCP APP ALLOWED HOSTS** SETTING (CONSTANT HARD OVERRIDE → FILTER + SAVED SETTING MERGED). **NEGOTIATED PROTOCOL HEADER (PR #6754)** — THE CLIENT ADVERTISES THE NEGOTIATED `protocolVersion` POST-INITIALIZE AND SUPPRESSES THE `_meta` ENVELOPE IN LEGACY SESSIONS; PHPUNIT-11 ONE-CLASS-PER-FILE DISCOVERY FIXED. **BRIDGE TOOLS REACH THE CHAT PAYLOAD (PR #6755)** — NEW `wp_mcp_ai_chat_effective_tools` FILTER SEAM IN `build_tools_payload()` (AFTER ATTENTION FILTERING, BEFORE CAPABILITY CHECKS) + PRO `wp_mcp_ai_mcp_apps_expose_tools()`. **IN-PROCESS SAME-SITE BRIDGE (PR #6756)** — SAME-ORIGIN MCP ENDPOINTS DISPATCH VIA `rest_do_request()` (NO OUTBOUND SOCKET/TLS/EXTRA PHP-FPM WORKER) WITH OUTBOUND-HTTP SAFETY RAILS, AND `rest_post_dispatch` PARITY (PR #6757) KEEPS RESPONSE-SIDE SESSION HEADERS ALIVE. **EVERY TOOL SURFACE CARRIES THE SEAM (PR #6758)** — `handle_tools_list()`, `handle_tool_request()`, `execute_tool_call_internal()`, AND THE `list_mcp_tools` CATALOGUE, PLUS `rest_pre_dispatch` REGISTRATION, A 60 S NEGATIVE DISCOVERY CACHE, AND SAVE-TIME CACHE INVALIDATION. **DOCS HUB 0.5.1 (PRS #6749/#6750/#6759)** — LOCAL-FIRST UPLOADS SOURCE, OPT-IN REMOTE IMPORT, SYMLINK-ESCAPE HARDENING, SLUG-NAMED UPLOADS FOLDER WITH A ONE-TIME MIGRATION (0 BLOCKING PLUGIN CHECK ERRORS). **README ANCHORS + CONSOLIDATION (PRS #6751/#6752)** — VS16-FALLBACK TOC ANCHORS FIXED, VALIDATOR REPAIRED TO GITHUB'S REAL ANCHOR RULES + CI ENFORCEMENT, AND ONE 12-RELEASE RELEASE HISTORY SECTION + COMPLETE PREVIOUS RELEASES TABLE (README 4,423 → 3,301 LINES). TOOL COUNT: ~308 BASE + ~1,282 PRO (~1,590 TOTAL; UNCHANGED — BRIDGE SLUGS ARE DYNAMIC CHAT-TIME REGISTRATIONS). MODEL CATALOG: v2026.09.22 (UNCHANGED). STALE BUILD ZIPS REMOVED: THE 1.1.83 SET (30 FILES) + SUPERSEDED DOCS-HUB 0.4.7/0.5.0 ZIPS. UPDATED DOCS: THIS PLAN, README/CHANGELOG/readme.txt/QUICK_REFERENCE + `addons/docs-hub/` DOCS (IN-WINDOW, #6749/#6750/#6759).
