...
- **v1.1.85** (September 24): MCP Apps connection & exposure release. **Metabox diagnostics (#6753)** — per-row Test Connection / Discover Tools buttons with inline results (negotiated protocol, handshake type, server info, session state, latency, live tool count, verbatim errors), a persisted per-app status badge + tool-count chip, and Test All; **basic auth end-to-end** (raw `user:pass` auto-encoded or pre-encoded base64) with token masking (stored credentials never echoed; empty = keep); `Mcp-Session-Id` capture + echo, legacy-handshake fallback, JSON-RPC error decoding on HTTP 4xx, mcpServers JSON import, loopback detection with a PHP-FPM deadlock warning; **Security Center → MCP App Allowed Hosts** setting (constant hard override → filter + saved setting merged). **Negotiated protocol header (#6754)** — the client advertises the negotiated `protocolVersion` post-initialize and suppresses the `_meta` envelope in legacy sessions; PHPUnit-11 one-class-per-file discovery fixed. **Bridge tools reach the chat payload (#6755)** — new `wp_mcp_ai_chat_effective_tools` seam in `build_tools_payload()` (after attention filtering, before capability checks) + Pro `wp_mcp_ai_mcp_apps_expose_tools()`. **In-process same-site bridge (#6756)** — same-origin MCP endpoints dispatch via `rest_do_request()` (no outbound socket/TLS/extra worker) with outbound-HTTP safety rails, and `rest_post_dispatch` parity (#6757) keeps response-side session headers alive. **Every tool surface carries the seam (#6758)** — `handle_tools_list()`, `handle_tool_request()`, `execute_tool_call_internal()`, and the `list_mcp_tools` catalogue, plus `rest_pre_dispatch` registration, a 60 s negative discovery cache, and save-time cache invalidation. **Docs Hub 0.5.1 (#6749/#6750/#6759)** — local-first uploads source, opt-in remote import, symlink-escape hardening, slug-named uploads folder with a one-time migration (0 blocking Plugin Check errors). **README anchors + consolidation (#6751/#6752)** — VS16-fallback TOC anchors fixed, validator repaired to GitHub's real rules + CI enforcement, and one 12-release Release History section + complete Previous Releases table. Tool count: ~308 base + ~1,282 Pro (~1,590 total; unchanged — bridge slugs are dynamic chat-time registrations). Model catalog: v2026.09.22 (unchanged). Stale build ZIPs removed: the 1.1.83 set (30 files) + superseded docs-hub 0.4.7/0.5.0 ZIPs. See `docs/project/plans/v1.1.85-docs-catch-up.md`.
...
- [Tool Reference](reference/tools/tool-reference.md) - All ~1,590 tools detailed (~308 base + ~1,282 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
