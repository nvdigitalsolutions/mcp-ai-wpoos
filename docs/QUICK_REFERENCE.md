...
- **v1.1.87** (September 27): Parity suite, image identification & outbound booking. **30 new base tools from the mcp-wordpress parity suite (#6777)** — comment/user CRUD, content/media/terms, site settings + application passwords (log-masked), and a 9-tool SEO toolkit on the gap matrix in `docs/developer/mcp-wordpress-tool-parity.md`. **Non-LLM image identification ladder (Proposal 043, #6780/#6785)** — `identify_image` orchestrates metadata → dHash media lookup → Cloud Vision detection → layout description → optional reverse-image search without a vision LLM; 5 base tools join the Media Generation preset + 2 Pro tools (`ocr_image_classic`, `search_similar_images`); every external rung is key-gated and fails closed. **Outbound appointment booking toolkit (Proposal 044, #6786)** — gated Pro toolkit with ICP lists, hourly sequence engine (send windows/daily caps/consent attestation), DM approval boards, angle-bank A/B tests, booking links + shortcode + rate-limited honeypotted public booking endpoint, and 3 new Pro tools. **mcp-wordpress gateway addon (#6778)** — auth-gated Streamable HTTP MCP server for Cloudways Velocity. **Session-budget warnings (#6776)** — blocked-session messages point at the real reset path, the 75% hook injects a one-shot warning via the new `wp_mcp_ai_chat_messages` filter; the two untracked Elementor bundled skills are committed. **Restriction notice fixed (#6779)** — distinct-user counting + render-time sweep/lift pruning. **Upwork refinements (#6784)** — `update_pro_schedule` edits workflow steps in place; `search_upwork_jobs` gains `exclude_keywords`/tier/budget parsing; CG Pro port byte-identical. Tool count: ~347 base + ~1,287 Pro (~1,634 total; +35 base +5 Pro). Addons: 27 → 28. Model catalog: v2026.09.22 (unchanged). Stale 1.1.85 build ZIPs removed (30 files). See `docs/project/plans/v1.1.87-docs-catch-up.md`.
