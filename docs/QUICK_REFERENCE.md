...
- **v1.1.57** (August 15): Plugin updater rework — base-only update flow for wp.org installs (new "Base Version" panel in Settings → Advanced; `ASSET_BASE` package matching), copy-in-place install with backup/rollback replacing `Plugin_Upgrader` (ZIP folder-name mismatches no longer break updates), Pro version read from plugin header (`get_pro_installed_version()`, constant fallback). Hermes WebUI chat reworked to async submit/poll (`/api/chat/start` + `/api/chat/stream/status`; `still_running` on timeout, `HERMES_APPROVAL_MODE` gate answers; live payload answer-extraction fix). Service Status AI-provider detection now uses `WP_MCP_AI_Credential_Resolver` (settings + WP 7.0 connectors + env + constants) with early-bootstrap fallback; new test suite (135 lines). New `addons/fleet-operator/.context/` agent-context tree (18 files, 6 operator roles) registered in AGENTS.md. Fix docs: `docs/history/2026/fixes/base-update-path-fix.md`, `docs/history/2026/fixes/service-status-provider-detection-fix.md`.
