...
- **v1.1.77** (September 11): Model-refresh, base+pro & checkout-launch release. **DeepSeek V4.1 Flash refresh** — new `deepseek-flash` (vision, $0.15/$0.60 off-peak), V4 Flash/Vision Exp retired, V4 Pro deprecated (sunset 2026-09-14, fallback `deepseek-flash`), migration map rewrites six legacy ids; cost calculator gains **peak/off-peak pricing** (`PEAK_WINDOWS`, `is_peak_time()`, `calculate_cost_at()`; legacy `calculate_cost()` time-independent, #6555). **Knowledge Graph Companion preset** — featured, auto-selected when a content graph is detected, 20 base tools + 14 `graphify_*` when enabled (#6570). **Base+pro gating fixed** — Pro toolkits now load in base+pro installs (all 21 init gates + ~550 tool gates); new 15-test `tests/basepro/` matrix + CI job (#6561). **Pro WP-CLI load-order guard** — no more `WP_MCP_AI_PATH` fatal when Pro activates first (#6585). **Memory CCT phantom slug fixed** — Graphify + retention read canonical `ai_agent_memories` (sweeps, caps, pruning, health stats work, #6591). **Checkout launch complete** — Checkout API **0.1.0 → 0.1.1** (boolean serialization fix, 424/502 contract, connection test, `GET /health`) and Content Graph **1.0.6 → 1.0.7** (CCT sources + status notes, trust modal, Stripe element + release-tag fixes, #6568–#6594). **Security** — sharp/nodemailer/joi/postcss-selector-parser bumps (#6592). **Wave G** — law-firm, cre-debt, quiz, eca, chat-channels complete; places starts (#6551–#6584). Tool count unchanged: ~303 base + ~1,265 Pro (~1,568 total). Stale 1.1.75 build ZIPs removed (30 files). See `docs/project/plans/v1.1.77-docs-catch-up.md`.
