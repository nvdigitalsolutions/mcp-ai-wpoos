...
- **A tool was reported as "executed" but nothing happened** — `POST /tools/execute/{slug}` fails in two different ways and both are now caught.

  1. *Composio rejected the call* — `HTTP 200` with `{"successful": false, "error": "..."}` (fixed in 1.4.1). The old code returned that body as a success, so a revoked-token failure reached the assistant as "Composio tool X executed."
  2. *Composio delivered the call and the provider refused it* — `HTTP 200` with `{"successful": true, "data": {"message": "HTTP 401: Request had invalid authentication credentials."}}` (fixed in 1.4.2). Composio is telling the truth here: it *did* reach Google. The failure is the provider's, and it arrives nested inside `data`, which earlier builds neither read nor checked.

  Both become a `WP_Error` — `wp_mcp_ai_composio_account_auth_required` for auth-class failures (a plain-language message naming the app, the provider's reason and a `reconnect_url`) or `wp_mcp_ai_composio_tool_failed` otherwise — carrying `log_id`, `upstream_error` and, for case 2, `provider_status`. Auth failures are written to the health ledger, so the dead account is skipped on the next call. Nested messages under `data.error`, `data.message` and `data.response_data.error` are read, so the error you see is the provider's own wording rather than a generic "unsuccessful" placeholder.

  Only those error-carrying fields are scanned, and a proxied status must lead the string — fetching an email thread titled "HTTP 401 errors on staging" is still a success, not a failure.
- **`wp_mcp_ai_composio_unknown_account`** — the `connected_account_id` you passed does not exist on this connection: it was deleted, or it belongs to a different Composio project. The usual cause is reusing a `ca_...` remembered from earlier in the conversation after the account was pruned or reconnected. The error names the accounts that *do* exist (`available_accounts`); either use one of those, or omit `connected_account_id` and let auto-resolution pick the healthiest. Earlier builds sent the dead ID upstream and surfaced whatever opaque error Composio returned.
