...
- **After optimization:** 2.7MB (1,236 files)
- **Reduction:** 37% smaller, 714 fewer files
...
- Vendor CI/dev config files - .gitignore, .gitattributes, phpstan.neon, psalm.xml, .php-cs-fixer.dist.php, phpspec.yml, phpunit.xml.dist, .travis.yml, .circleci, Makefile, .pullapprove.yml
- Admin test pages - class-wp-mcp-ai-admin-test-*.php (test assistants, teams, professions)
- Admin test assets - admin-test-*.css (test page styles)
- Remote tester - class-wp-mcp-ai-remote-tester.php (development tool)
- Performance test widgets - class-wp-mcp-ai-elementor-test-*.php, class-wp-mcp-ai-elementor-performance-test-*.php
...
**Why this matters:** Smaller uploads with fewer files are faster, more reliable, and prevent 504 Gateway Timeout errors when uploading to wordpress.org. Removing CI/development tool config files and test pages reduces processing time during WordPress.org validation.
...
- [Architecture Quick Reference](../../architecture/core/ARCHITECTURE_QUICK_REFERENCE.md)
