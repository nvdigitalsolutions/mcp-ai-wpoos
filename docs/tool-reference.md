...
- **Run Crawl4AI Job** (`run_crawl4ai_job`) executes Crawl4AI collections locally or via remote endpoints (v0.7.7+ compatible), collating Markdown, HTML, and error metadata so assistants can ingest large batches of URLs in a single invocation. Supports browser pool configuration (permanent, hot, cold) for optimal performance. Remote submissions return an immediate task token while the background crawler schedules WP-Cron polling so results are cached and surfaced to the assistant once Crawl4AI finishes processing. Monitor job status via Settings → Crawl4AI Manager.【F:includes/tools/class-wp-mcp-ai-tool-run-crawl4ai-job.php†L17-L1856】【F:includes/crawler/class-wp-mcp-ai-crawler.php†L1-L214】【F:includes/admin/class-wp-mcp-ai-admin-crawl4ai-manager.php†L1-L785】
