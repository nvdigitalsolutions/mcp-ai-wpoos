...
| D-UI-4 | Assistant builder/test/add pages | 🟡 | Pages + CPT complete (31 new ecosystem tests): `src/Admin/AssistantPostType.php` (CPT args/meta keys/sanitizers/REST schema byte-identical, standalone-only), `src/Admin/AssistantPages.php` hub, `src/Admin/AssistantPages/BuildAssistantPage.php` (4 tabs, profession/region lists, provider list, stats, `nvoos_cg_ai_create_assistant` flow), `AddAssistantPage.php` (template grid + modal + `nvoos_cg_ai_create_from_professional` with byte-identical meta keys), `TestPageBase.php` + `TestAssistantPage.php` (`nvoos-cg-test-assistant` table + `test_assistant` query-param chat modal embedding `[nvoos_content_graph_chat]`), admin JS/CSS assets; deviations: Tools Grid/KB block components + base prompt construction deferred to their owning waves (profession runtime), background create answers `async_unavailable` until E2, test modal uses server-rendered widget selection; **remaining: assistant-selector/builder blocks** (port with the block-set wave — tools grid + knowledge base components) |
