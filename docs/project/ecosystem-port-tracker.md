...
| D-UI-4 | Assistant builder/test/add pages | 🟡 | D-UI-4a done (26 new ecosystem tests): `src/Admin/AssistantPostType.php` (CPT args/meta keys/sanitizers/REST schema byte-identical, standalone-only), `src/Admin/AssistantPages.php` hub (menus + AJAX actions), `src/Admin/AssistantPages/BuildAssistantPage.php` (4 tabs, profession/region lists, provider list, stats, `nvoos_cg_ai_create_assistant` flow), `src/Admin/AssistantPages/AddAssistantPage.php` (template grid + modal + `nvoos_cg_ai_create_from_professional` with byte-identical meta keys), admin JS/CSS assets; deviations: Tools Grid/KB block components, base prompt construction, and background create deferred to their owning waves (E2/profession runtime); **Test Assistant page = next D-UI-4 slice** (needs the admin chat-tester asset surface) |
