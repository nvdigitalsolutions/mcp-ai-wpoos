...
| D1e | Message attachments + response attachments (attachment pipeline) | ✅ | `src/Chat/{MessageAttachments,ResponseAttachments}.php` (byte-identical meta key `_wp_mcp_ai_openai_file`, filters, error codes, cache group) + `FileServiceBridge`/`FileClientBridge` collaborators; remote file APIs delegate to base in monolith, degrade to the `local-` reference path in standalone (D2-gated gap, tracked); cleanup/response hooks registered standalone-only via `Plugin.php`; 28 characterization tests |
| D1 | Chat runtime core (clients, router, selector, caches, summarizer, RAG bridge, threads, transcripts, SSE, attachments, quick actions) | 🟡 | D1a–D1e done; transcript recorder/retention + chatkit next; remote file-API uploads gated on D2 |
