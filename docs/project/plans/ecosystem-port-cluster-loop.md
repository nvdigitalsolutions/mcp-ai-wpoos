...
| 7 | E-UI-2 | Managers (tool/token/cron/DAG/DLQ/approvals) | L | In progress — sub-cluster 1 (approvals UI) landed (`src/Admin/Managers/ApprovalsManager.php`); 13 characterization tests green both matrices, full suite 1006/0 both matrices. Next: token manager, then cron manager, DAG builder, DLQ manager, media-library columns, asset inventory (one PR each). |
