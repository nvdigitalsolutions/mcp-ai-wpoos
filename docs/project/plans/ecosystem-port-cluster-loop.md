...
| 7 | E-UI-2 | Managers (tool/token/cron/DAG/DLQ/approvals) | L | In progress — sub-clusters 1 (approvals UI) + 2 (token manager) + 3 (cron manager) + 4 (DAG builder) + 5 (DLQ manager) landed (`src/Admin/Managers/ApprovalsManager.php`, `TokenManager.php`, `CronManagerPage.php`, `DagBuilder.php`, `DlqManager.php`); 13 + 16 + 23 + 14 + 27 characterization tests green both matrices, full suite 1086/0 both matrices. Next: media-library columns, then asset inventory (one PR each). |
