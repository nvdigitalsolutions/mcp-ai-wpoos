...
|---|---|---|---|
| **0 — Execution spine** | CG-AI | `McpController::mcp_tools_call()` execution chain (items 2–8 above); `RegistryBridge` helper for slug→tool resolution; fix the stale "(D-Tools)" docblock reference | `tools/call` on the 13 AI tools + graph tools returns byte-parity envelopes; 503 gone |
| **1 — Register the pre-ported inventory** | CG-AI | Registration manifest mapping the 148 core-parity slugs (+ optionally the 43 core utilities) to `lib/core` FQCNs, instantiated with `CoreBridge`'s `ErrorFactory` and registered via `$bridge->tools->register()` + `notifyRegistered()`; feature-flag gating mirrors the base's `is_available()` contract | Standalone `tools/list` slug set == monolith base-registry dump ∩ (base ∩ core); every registered tool passes a smoke execute() |
