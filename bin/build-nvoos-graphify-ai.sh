#!/bin/bash
#
# Build NV oOS Graphify AI Addon Plugin ZIP
#
# Produces a distribution ZIP of the nvoos-graphify-ai plugin
# from plugins/nvoos-graphify-ai/. This is a companion addon that
# adds AI chat, providers, AI tools, embeddings, and agent memory
# to the standalone nvoos-graphify plugin. Includes vendor/autoload.php
...
#
# Usage:
#   ./bin/build-nvoos-graphify-ai.sh
#
...
PLUGIN_DIR="plugins/nvoos-graphify-ai"
PLUGIN_FILE="${PLUGIN_DIR}/nvoos-graphify-ai.php"
...
echo "=========================================="
echo "Building NV oOS Graphify AI Addon Plugin"
...
OUTPUT_DIR="build"
TMP_DIR="build/.tmp-graphify-ai"
ARTIFACT="nvoos-graphify-ai-v${VERSION}"
ZIP_FILE="${OUTPUT_DIR}/${ARTIFACT}.zip"

mkdir -p "$OUTPUT_DIR"
rm -rf "$TMP_DIR"
mkdir -p "${TMP_DIR}/${ARTIFACT}/nvoos-graphify-ai"

# ---------------------------------------------------------------------------
# Remove any previously built ZIPs for this slug
# ---------------------------------------------------------------------------
rm -f "${OUTPUT_DIR}"/nvoos-graphify-ai-v*.zip
rm -f "${OUTPUT_DIR}"/nvoos-graphify-ai-v*.sha256

# ---------------------------------------------------------------------------
# Assemble plugin directory for ZIP
# ---------------------------------------------------------------------------
echo "📁 Assembling plugin directory..."
rsync -a "${PLUGIN_DIR}/" "${TMP_DIR}/${ARTIFACT}/nvoos-graphify-ai/" \
	--exclude '.git/' \
	--exclude '.gitignore' \
	--exclude '.github/' \
	--exclude '.distignore' \
...
find "${TMP_DIR}/${ARTIFACT}/nvoos-graphify-ai" -type f | sort
echo ""

# ---------------------------------------------------------------------------
# Verify vendor/autoload.php is present
# ---------------------------------------------------------------------------
if [ ! -f "${TMP_DIR}/${ARTIFACT}/nvoos-graphify-ai/vendor/autoload.php" ]; then
...
(
	cd "${TMP_DIR}/${ARTIFACT}"
	zip -r -q "${ROOT_DIR}/${ZIP_FILE}" nvoos-graphify-ai/
)
...
echo "🎉 NV oOS Graphify AI addon plugin ZIP built successfully!"
echo "   ${ZIP_FILE}"
