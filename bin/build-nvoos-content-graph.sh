#!/bin/bash
#
# Build NV oOS Content Graph Standalone Plugin ZIP
#
# Produces a distribution ZIP of the standalone nvoos-content-graph plugin
# from plugins/nvoos-content-graph/. Includes vendor/autoload.php for PSR-4
...
PLUGIN_DIR="plugins/nvoos-content-graph"
PLUGIN_FILE="${PLUGIN_DIR}/nvoos-content-graph.php"
...
echo "=========================================="
echo "Building NV oOS Content Graph Standalone Plugin"
...
OUTPUT_DIR="build"
TMP_DIR="build/.tmp-content-graph"
ARTIFACT="nvoos-content-graph-v${VERSION}"
ZIP_FILE="${OUTPUT_DIR}/${ARTIFACT}.zip"

mkdir -p "$OUTPUT_DIR"
rm -rf "$TMP_DIR"
mkdir -p "${TMP_DIR}/${ARTIFACT}/nvoos-content-graph"

# ---------------------------------------------------------------------------
# Remove any previously built ZIPs for this slug
# ---------------------------------------------------------------------------
rm -f "${OUTPUT_DIR}"/nvoos-content-graph-v*.zip
rm -f "${OUTPUT_DIR}"/nvoos-content-graph-v*.sha256

# ---------------------------------------------------------------------------
# Assemble plugin directory for ZIP
# ---------------------------------------------------------------------------
echo "📁 Assembling plugin directory..."
rsync -a "${PLUGIN_DIR}/" "${TMP_DIR}/${ARTIFACT}/nvoos-content-graph/" \
	--exclude '.git/' \
	--exclude '.gitignore' \
	--exclude '.github/' \
	--exclude '.distignore' \
...
find "${TMP_DIR}/${ARTIFACT}/nvoos-content-graph" -type f | sort
echo ""

# ---------------------------------------------------------------------------
# Verify vendor/autoload.php is present
# ---------------------------------------------------------------------------
if [ ! -f "${TMP_DIR}/${ARTIFACT}/nvoos-content-graph/vendor/autoload.php" ]; then
...
(
	cd "${TMP_DIR}/${ARTIFACT}"
	zip -r -q "${ROOT_DIR}/${ZIP_FILE}" nvoos-content-graph/
)
...
echo "🎉 NV oOS Content Graph standalone plugin ZIP built successfully!"
echo "   ${ZIP_FILE}"
