...
if [ -f "addons/pro/wp-mcp-ai-pro.php" ]; then
    echo "   ✓ Pro addon file exists at: addons/pro/wp-mcp-ai-pro.php"
else
    echo "   ✗ Pro addon file NOT found"
    exit 1
fi

# Verify loading order in wp-mcp-ai.php
echo ""
echo "2. Verifying file loading order in wp-mcp-ai.php..."

TOOL_REGISTRY_LINE=$(grep -n "require_once.*class-tool-registry.php" wp-mcp-ai.php | cut -d: -f1)
PRO_ADDON_LINE=$(grep -n "require_once \$pro_addon_file" wp-mcp-ai.php | head -1 | cut -d: -f1)
TOOLS_INIT_LINE=$(grep -n "require_once.*tools-init.php" wp-mcp-ai.php | head -1 | cut -d: -f1)
...
if grep -q "did_action.*plugins_loaded" addons/pro/wp-mcp-ai-pro.php; then
...
if grep -q "wp_mcp_ai_pro_tool_group_map" addons/pro/wp-mcp-ai-pro.php; then
    echo "   ✓ Pro addon registers tool group map filter"
    
    # Show which groups Pro tools are in
    echo "   Pro tool groups:"
    grep -A20 "pro_tools = array" addons/pro/wp-mcp-ai-pro.php | grep "=>" | head -10 | sed 's/^/      /'
else
...
if grep -A5 "function wp_mcp_ai_bootstrap" wp-mcp-ai.php | grep -q "Pro addon is now loaded earlier"; then
...
echo "  1. wp-mcp-ai.php loads Tool Registry class"
echo "  2. wp-mcp-ai.php loads Pro addon (if exists)"
echo "  3. Pro addon calls wp_mcp_ai_pro_init() immediately"
echo "  4. Pro addon registers hook for 'wp_mcp_ai_register_tools'"
echo "  5. wp-mcp-ai.php loads tools-init.php"
