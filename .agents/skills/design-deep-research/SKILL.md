...
| `semantic_content_search` | Search site content via vector embeddings (any embedding provider) | Finds related content already on your WordPress site |
...
| `list_terms` | List categories/tags/custom taxonomy terms | Discover the site's taxonomy structure before mapping new posts |
| `list_taxonomies` | List registered taxonomies and their properties | Find which taxonomies exist on the site |
...
**Provider flexibility:** embeddings are generated by whichever embedding
backend is configured (OpenAI, Gemini, Ollama, or DigitalOcean) —
**independent of the assistant's chat provider** (e.g. a DeepSeek-backed
assistant still works when only a Gemini key is configured). The `embedding_provider`
setting pins a specific backend when multiple are configured.

**Graceful degradation:** when no embedding provider is configured, the tool
falls back to keyword search instead of failing. A degraded response carries
`fallback_mode: "keyword"`, `semantic_search_enabled: false`, and a `warning`
explaining how to restore semantic search.
...
threshold      — Minimum similarity score 0–1 (default: 0.7, semantic results only)
...
- `query_embedding_provider` / `query_embedding_model` — which backend generated the query vector.
- `skipped_dimension_mismatch` — stored vectors skipped because a different embedding model produced them (a non-zero count explains sparse results).
- `fallback_mode: "keyword"` — semantic search was unavailable; results are keyword matches with `similarity_score: null`.
...
1. SITE CHECK (optional)
   ...
   (falls back to keyword search when embeddings aren't configured)

2. TAXONOMY DISCOVERY (when creating content)
   list_taxonomies → which taxonomies exist?
   list_terms { taxonomy: "category" } → existing category structure
...
5. ACT ON IT
   ...
   → Map the post against list_terms output via create_term/update_term
