 ...
 *
 * Usage:
 *   1. Replace {{ASSISTANT_TITLE}}, the {{PROMPT}} string content, and any
 ...
 *
 * @package NV_oOS_Pro_Toolkit_Skills
 */
...
// The prompt is a single-quoted string — no variable interpolation, safe to
// paste any text (literal newlines are fine). Avoid angle-bracket
// placeholders; sanitization strips them.
$prompt = '{{PASTE SYSTEM PROMPT HERE}}';
...
if ( ! empty( $existing ) ) {
	$assistant_id = (int) $existing[0]->ID;
	wp_update_post(
		array(
			'ID'          => $assistant_id,
			'post_status' => 'publish',
	...
	echo esc_html( "Updated existing assistant ID: {$assistant_id}\n" );
} else {
	...
	if ( is_wp_error( $assistant_id ) ) {
		die( esc_html( 'Failed to create assistant: ' . $assistant_id->get_error_message() ) . "\n" );
	}
	echo esc_html( "Created assistant ID: {$assistant_id}\n" );
}
...
update_post_meta( $assistant_id, '_wp_mcp_ai_provider', '{{PROVIDER_SLUG}}' );
update_post_meta( $assistant_id, '_wp_mcp_ai_model', '{{MODEL_ID}}' );
update_post_meta( $assistant_id, '_wp_mcp_ai_system_prompt', $prompt );
update_post_meta( $assistant_id, '_wp_mcp_ai_tools', $tools );
update_post_meta( $assistant_id, '_wp_mcp_ai_classification', 'internal' );
update_post_meta( $assistant_id, 'mcp_ai_required_capability', 'manage_options' );

echo esc_html( 'Meta updated. Tools assigned: ' . count( $tools ) ) . "\n";
echo esc_html( "Next step: wsl docker compose exec -T WordPress wp --allow-root mcp-ai credential issue {$assistant_id} --user=1 --porcelain" ) . "\n";
