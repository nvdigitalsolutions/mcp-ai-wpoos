...
class WP_MCP_AI_Tool_Slug_Integrity_Tests extends WP_UnitTestCase {
	...
	public function test_all_tools_have_correct_slugs() {
		...
		foreach ( $tools as $tool ) {
			...
			// Legacy tools register through WP_MCP_AI_Legacy_Tool_Wrapper;
			// derive the expected slug from the *declaring* class.
	...
	/**
	 * Test that the send_group_email tool resolves via the registry.
	 */
	public function test_send_group_email_tool_exists() {
		...
		// The registry transparently auto-upgrades to the validated variant
		// when it is registered (documented in get_tool()).
		if ( $registry->is_tool_registered( 'send_group_email_validated' ) ) {
			$this->assertInstanceOf( 'WP_MCP_AI_Tool_Send_Group_Email_Validated', $tool );
			$this->assertEquals( 'send_group_email_validated', $tool->get_slug() );
		} else {
	...
	/**
	 * Test that the get_open_meteo_forecast tool resolves via the registry.
	 */
	...
	/**
	 * Test that retrieving tools by slug returns the expected classes.
	 */
	public function test_tool_retrieval_by_slug_is_correct() {
		...
		foreach ( $test_cases as $slug => $expected_class ) {
			...
			if ( $tool ) {
				// get_tool() auto-upgrades to the *_validated variant when it is
				// registered — the validated class follows the
				// "<BaseClass>_Validated" naming convention.
				$expected = $registry->is_tool_registered( $slug . '_validated' )
					? $expected_class . '_Validated'
					: $expected_class;

				$this->assertEquals(
					$expected,
					get_class( $tool ),
					"Tool slug '$slug' should return class '$expected'"
				);
			}
		}
	}
	...
	/**
	 * Test that no two registered tools share a slug.
	 */
