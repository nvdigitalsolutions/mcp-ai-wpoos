...
class Test_Chat_Channels extends WP_UnitTestCase {
	...
	/**
	 * Resolve_contact_from_cpt populates connection_id from CPT meta.
	 */
	public function test_resolve_contact_from_cpt_returns_connection_id() {
		...
		$post_id = wp_insert_post(
			array(
				'post_type'   => WP_MCP_AI_Channel_Contacts_CPT::POST_TYPE,
				'post_title'  => 'Bot Contact',
				'post_status' => 'publish',
			)
		);
		update_post_meta( $post_id, '_channel', 'telegram' );
		update_post_meta( $post_id, '_channel_contact_id', '999' );
		update_post_meta( $post_id, '_connection_id', 'bot_abc' );
		...
		$this->assertSame( '999', $channel_contact_id );
		$this->assertSame( 'bot_abc', $connection_id );

		wp_delete_post( $post_id, true );
	}

	/**
	 * Resolve_contact_from_cpt defaults connection_id to empty when meta is absent.
	 */
	public function test_resolve_contact_from_cpt_defaults_connection_id_to_empty() {
		$this->load_pro_class( 'WP_MCP_AI_Chat_Channels_REST_Controller', 'includes/rest/class-wp-mcp-ai-chat-channels-rest-controller.php' );
		$this->load_pro_class( 'WP_MCP_AI_Channel_Contacts_CPT', 'includes/class-wp-mcp-ai-channel-contacts-cpt.php' );
		...
		update_post_meta( $post_id, '_channel', 'whatsapp' );
		update_post_meta( $post_id, '_channel_contact_id', '+15551234567' );
		...
		$this->assertSame( 'whatsapp', $channel );
		$this->assertSame( '+15551234567', $channel_contact_id );
		$this->assertSame( '', $connection_id, 'connection_id must default to empty when meta is absent' );

		wp_delete_post( $post_id, true );
	}

	/**
	 * Resolve_contact_from_cpt still works when called without the connection_id param (backward compat).
	 */
	public function test_resolve_contact_from_cpt_backward_compat_no_connection_id_param() {
		$this->load_pro_class( 'WP_MCP_AI_Chat_Channels_REST_Controller', 'includes/rest/class-wp-mcp-ai-chat-channels-rest-controller.php' );
		$this->load_pro_class( 'WP_MCP_AI_Channel_Contacts_CPT', 'includes/class-wp-mcp-ai-channel-contacts-cpt.php' );
		...
		update_post_meta( $post_id, '_channel', 'slack' );
		update_post_meta( $post_id, '_channel_contact_id', 'U12345' );
		...
		$channel            = '';
		$channel_contact_id = '';
		// Deliberately not passing connection_id — should work via default param.
		$method->invokeArgs( $controller, array( $post_id, &$channel, &$channel_contact_id ) );

		$this->assertSame( 'slack', $channel );
		$this->assertSame( 'U12345', $channel_contact_id );
