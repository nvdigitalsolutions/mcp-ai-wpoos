...
class WP_MCP_AI_Model_Rate_Limits_CCT_Test extends WP_UnitTestCase {
	...
	/**
	 * Test that LM Studio models are included in defaults.
	 */
	public function test_default_models_include_lm_studio() {
		$reflection = new ReflectionClass( 'WP_MCP_AI_Model_Rate_Limits_CCT' );
		$method     = $reflection->getMethod( 'get_default_model_data' );
		$method->setAccessible( true );
		...
		$lm_studio_models = array_filter(
			$default_models,
			function ( $model ) {
				return 'lm_studio' === $model['provider'];
			}
		);

		$this->assertNotEmpty( $lm_studio_models, 'Should have LM Studio models in defaults' );

		// Check for the 3 specific models added.
		$model_names = array_column( $lm_studio_models, 'model_name' );
		$this->assertContains( 'ibm/granite-4-h-tiny', $model_names, 'IBM Granite 4 H Tiny should be in defaults' );
		$this->assertContains( 'qwen/qwen3-coder-30b', $model_names, 'Qwen3 Coder 30B should be in defaults' );
		$this->assertContains( 'openai/gpt-oss-20b', $model_names, 'OpenAI GPT-OSS 20B should be in defaults' );
	}

	/**
	 * Test that new LM Studio models have correct function calling support.
	 */
	public function test_lm_studio_models_support_function_calling() {
		$reflection = new ReflectionClass( 'WP_MCP_AI_Model_Rate_Limits_CCT' );
		$method     = $reflection->getMethod( 'get_default_model_data' );
		$method->setAccessible( true );
		...
		// Test IBM Granite 4 H Tiny.
		$granite = array_values(
			array_filter(
				$default_models,
				function ( $model ) {
					return 'ibm/granite-4-h-tiny' === $model['model_name'];
				}
			)
		);

		$this->assertNotEmpty( $granite, 'Should find IBM Granite in defaults' );
		$granite_model = reset( $granite );
		$this->assertSame( 'lm_studio', $granite_model['provider'], 'Provider should be lm_studio' );
		$this->assertTrue( $granite_model['supports_function_calling'], 'Granite should support function calling' );
		$this->assertTrue( $granite_model['supports_streaming'], 'Granite should support streaming' );
		$this->assertFalse( $granite_model['supports_vision'], 'Granite should not support vision' );
		$this->assertSame( 128000, $granite_model['context_window'], 'Granite should have 128K context window' );
		$this->assertSame( 0, $granite_model['tpm_limit'], 'Local models should have 0 TPM limit' );
		$this->assertSame( 0, $granite_model['rpm_limit'], 'Local models should have 0 RPM limit' );
		...
		$qwen = array_values(
			array_filter(
				$default_models,
				function ( $model ) {
					return 'qwen/qwen3-coder-30b' === $model['model_name'];
				}
			)
		);

		$this->assertNotEmpty( $qwen, 'Should find Qwen3 Coder in defaults' );
		$qwen_model = reset( $qwen );
		$this->assertSame( 'lm_studio', $qwen_model['provider'], 'Provider should be lm_studio' );
		$this->assertTrue( $qwen_model['supports_function_calling'], 'Qwen3 should support function calling' );
		$this->assertTrue( $qwen_model['supports_streaming'], 'Qwen3 should support streaming' );
		$this->assertFalse( $qwen_model['supports_vision'], 'Qwen3 should not support vision' );
		$this->assertSame( 262144, $qwen_model['context_window'], 'Qwen3 should have 256K context window' );

		// Test OpenAI GPT-OSS 20B.
		$gpt_oss = array_values(
			array_filter(
				$default_models,
				function ( $model ) {
					return 'openai/gpt-oss-20b' === $model['model_name'];
				}
			)
		);

		$this->assertNotEmpty( $gpt_oss, 'Should find GPT-OSS 20B in defaults' );
		$gpt_oss_model = reset( $gpt_oss );
		$this->assertSame( 'lm_studio', $gpt_oss_model['provider'], 'Provider should be lm_studio' );
		$this->assertTrue( $gpt_oss_model['supports_function_calling'], 'GPT-OSS should support function calling' );
		$this->assertTrue( $gpt_oss_model['supports_streaming'], 'GPT-OSS should support streaming' );
		$this->assertFalse( $gpt_oss_model['supports_vision'], 'GPT-OSS should not support vision' );
		$this->assertSame( 131072, $gpt_oss_model['context_window'], 'GPT-OSS should have 131K context window' );
	}
}
