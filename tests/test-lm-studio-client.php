...
class WP_MCP_AI_LM_Studio_Client_Tests extends WP_UnitTestCase {
	...
	/**
	 * Test that normalise_tools_for_payload method exists and is callable.
	 */
	public function test_normalise_tools_for_payload_method_exists() {
		$reflection = new ReflectionClass( $this->client );
		$this->assertTrue(
			$reflection->hasMethod( 'normalise_tools_for_payload' ),
			'LM Studio client should have normalise_tools_for_payload method for OpenAI-compatible tool calling'
		);
	}

	/**
	 * Test that tools are included in chat completion payload when provided.
	 */
	public function test_chat_completion_includes_tools_in_payload() {
		...
		$this->assertArrayHasKey( 'tools', $captured_payload, 'Payload should include tools parameter' );
	...
	/**
	 * Test normalise_tools_for_payload extracts tool names correctly.
	 */
	public function test_normalise_tools_for_payload_extracts_names() {
		$reflection = new ReflectionClass( $this->client );
	...
	/**
	 * Test normalise_tools_for_payload handles empty tools array.
	 */
	public function test_normalise_tools_for_payload_handles_empty_array() {
		$reflection = new ReflectionClass( $this->client );
		$method     = $reflection->getMethod( 'normalise_tools_for_payload' );
		$method->setAccessible( true );
	...
	/**
	 * Test normalise_tools_for_payload skips tools without names.
	 */
	public function test_normalise_tools_for_payload_skips_tools_without_names() {
		$reflection = new ReflectionClass( $this->client );
		$method     = $reflection->getMethod( 'normalise_tools_for_payload' );
		$method->setAccessible( true );
		...
		$this->assertIsArray( $result );
		$this->assertCount( 1, $result, 'Should skip tool without name' );
		$this->assertEquals( 'valid_tool', $result[0]['name'] );
	}

	/**
	 * Test normalise_tools_for_payload uses slug as fallback name.
	 */
	public function test_normalise_tools_for_payload_uses_slug_fallback() {
		$reflection = new ReflectionClass( $this->client );
		$method     = $reflection->getMethod( 'normalise_tools_for_payload' );
		$method->setAccessible( true );
		...
		$this->assertIsArray( $result );
		$this->assertCount( 1, $result );
		$this->assertEquals( 'tool_slug', $result[0]['name'] );
	}

	/**
	 * Test that chat completions with tools work end-to-end.
	 */
	public function test_chat_completion_with_tools_end_to_end() {
		update_option(
			WP_MCP_AI_Admin_Settings::OPTION_NAME,
			array(
				'lm_studio_endpoint_url' => 'http://localhost:1234',
