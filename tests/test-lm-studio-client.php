...
class WP_MCP_AI_LM_Studio_Client_Tests extends WP_UnitTestCase {
	...
	/**
	 * Test that tools are properly formatted in the request payload.
	 */
	public function test_chat_completion_includes_tools_in_payload() {
		update_option(
			WP_MCP_AI_Admin_Settings::OPTION_NAME,
			array(
				'lm_studio_endpoint_url' => 'http://localhost:1234',
				'lm_studio_model'        => 'qwen/qwen3-coder-30b:2',
			)
		);
		...
		$this->assertCount( 1, $payload['tools'], 'Should have one tool' );
		$this->assertEquals( 'function', $payload['tools'][0]['type'], 'Tool type should be function' );
		$this->assertEquals( 'search_products', $payload['tools'][0]['function']['name'], 'Tool name should be search_products' );
	 ...
	 */
	public function test_chat_completion_preserves_tool_calls_in_response() {
		update_option(
			WP_MCP_AI_Admin_Settings::OPTION_NAME,
			array(
				'lm_studio_endpoint_url' => 'http://localhost:1234',
		...
		$result = $this->client->create_chat_completion( $messages, array( 'tools' => array() ) );

		$this->assertIsArray( $result, 'Result should be an array' );
		$this->assertArrayHasKey( 'choices', $result );
		$this->assertArrayHasKey( 'message', $result['choices'][0] );
		$this->assertArrayHasKey( 'tool_calls', $result['choices'][0]['message'], 'Response should preserve tool_calls' );
		$this->assertIsArray( $result['choices'][0]['message']['tool_calls'] );
		$this->assertCount( 1, $result['choices'][0]['message']['tool_calls'] );
		$this->assertEquals( 'search_products', $result['choices'][0]['message']['tool_calls'][0]['function']['name'] );
		$this->assertEquals( 'tool_calls', $result['choices'][0]['finish_reason'], 'Finish reason should be tool_calls' );

		remove_all_filters( 'pre_http_request' );
	}

	/**
	 * Test that content normalization is skipped when tool_calls are present.
	 */
	public function test_content_not_normalized_when_tool_calls_present() {
		update_option(
			WP_MCP_AI_Admin_Settings::OPTION_NAME,
			array(
				'lm_studio_endpoint_url' => 'http://localhost:1234',
		...
		// When tool_calls are present, content should remain as returned (null or string).
		// It should NOT be normalized to array format.
		$this->assertArrayHasKey( 'tool_calls', $result['choices'][0]['message'] );
		$content = $result['choices'][0]['message']['content'];
		// Content should be null or not normalized to array when tool_calls present.
		$this->assertTrue(
			is_null( $content ) || is_string( $content ),
			'Content should remain null or string when tool_calls are present, not be normalized to array'
		);
	...
	/**
	 * Test normalise_tools_for_payload method with various tool formats.
	 */
	public function test_normalise_tools_for_payload() {
		...
		// Test with proper OpenAI format.
		$tools = array(
			array(
				'type'     => 'function',
				'function' => array(
		...
		$result = $method->invoke( $this->client, $tools );
		$this->assertIsArray( $result );
		$this->assertCount( 1, $result );
		$this->assertEquals( 'get_weather', $result[0]['name'] );

		// Test with slug format (internal format).
		$tools = array(
			array(
				'slug'        => 'search_posts',
		...
		$this->assertIsArray( $result );
		$this->assertCount( 1, $result );
		$this->assertEquals( 'search_posts', $result[0]['name'] );
		...
		// Test with invalid tools (should be filtered out).
		$tools = array(
			array( 'invalid' => 'tool' ), // No name.
			array(
				'type'     => 'function',
				'function' => array(
					'name' => 'valid_tool',
		...
		$result = $method->invoke( $this->client, $tools );
		$this->assertIsArray( $result );
		$this->assertCount( 1, $result, 'Invalid tools should be filtered out' );
		$this->assertEquals( 'valid_tool', $result[0]['name'] );
	}
}
