    ...
    public function test_execute_uses_access_token_filter() {
        ...
        $result = $tool->execute(
            array(
                ...
                'calendar_id' => 'primary',
                'attendees'   => array( 'one@example.com', array( 'email' => 'two@example.com', 'name' => 'Two' ) ),
            ),
            array( 'user_id' => $user_id )
        );
    ...
    public function test_execute_exchanges_service_account_credentials() {
        ...
        $http_stub = function( $preempt, $args, $url ) use ( &$requests ) {
            ...
            if ( false !== strpos( $url, 'oauth2.googleapis.com/token' ) ) {
                ...
                return array(
                    'body'     => wp_json_encode( array( 'access_token' => 'service-token', 'expires_in' => 3600 ) ),
                    'response' => array( 'code' => 200 ),
                    'headers'  => array(),
                );
            }
            ...
            return array(
                'body'     => wp_json_encode( array( 'id' => 'evt_service', 'status' => 'confirmed' ) ),
                'response' => array( 'code' => 200 ),
                'headers'  => array(),
            );
        };
