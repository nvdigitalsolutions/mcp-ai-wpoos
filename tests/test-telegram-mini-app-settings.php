...
class Test_Telegram_Mini_App_Settings extends WP_UnitTestCase {
	...
	// =========================================================================
	// Slash command hyphen → underscore normalisation tests
	// =========================================================================

	/**
	 * Test that get_registered_slash_commands replaces hyphens with underscores.
	 */
	public function test_get_registered_slash_commands_normalises_hyphens() {
		if ( ! $this->webhook_controller ) {
			$this->markTestSkipped( 'Webhook controller not available.' );
			return;
		}
		...
		if ( ! class_exists( 'WP_MCP_AI_Slash_Command_Handler' ) ) {
			$this->markTestSkipped( 'WP_MCP_AI_Slash_Command_Handler not available.' );
			return;
		}

		// Set up a mock slash command handler with a hyphenated command.
		global $wp_mcp_ai_slash_command_handler;
		$original_handler             = $wp_mcp_ai_slash_command_handler;
		...
		$method = new ReflectionMethod( $this->webhook_controller, 'get_registered_slash_commands' );
		$method->setAccessible( true );

		$commands = $method->invoke( $this->webhook_controller );

		// Restore original handler.
		$wp_mcp_ai_slash_command_handler = $original_handler;

		$this->assertArrayHasKey( 'content_translate', $commands, 'Hyphens should be replaced with underscores in command names.' );
		$this->assertArrayNotHasKey( 'content-translate', $commands, 'Original hyphenated name should not appear.' );
		$this->assertEquals( 'Translate content', $commands['content_translate'], 'Command description should be preserved during normalisation.' );
	}

	/**
	 * Test that get_default_commands normalises hyphenated slash command names.
	 */
	public function test_get_default_commands_normalises_hyphens() {
		if ( ! $this->webhook_controller ) {
			$this->markTestSkipped( 'Webhook controller not available.' );
			return;
		}
		...
		$this->assertContains( 'content_translate', $command_names, 'Hyphenated command should appear with underscores.' );
		$this->assertNotContains( 'content-translate', $command_names, 'Hyphenated command should not appear as-is.' );
	...
	/**
	 * Test that handle_bot_command dispatches correctly when Telegram sends
	 * an underscore command that maps to a hyphenated internal command.
	 */
	public function test_handle_bot_command_resolves_underscore_to_hyphen() {
		if ( ! $this->webhook_controller ) {
			$this->markTestSkipped( 'Webhook controller not available.' );
			return;
		}
		...
		$wp_mcp_ai_slash_command_handler->register(
			...
			array(
				'description' => 'Translate content',
				'handler'     => static function() use ( &$executed ) {
					$executed = true;
					return 'Translated!';
				},
			)
		...
		$method = new ReflectionMethod( $this->webhook_controller, 'handle_bot_command' );
		$method->setAccessible( true );

		// Simulate Telegram sending /content_translate (underscored form).
		$parsed  = array(
			'command' => 'content_translate',
			'args'    => '',
		);
		$message = array(
			'chat'     => array( 'id' => 12345, 'type' => 'private' ),
			'from'     => array( 'id' => 99999 ),
			'message_id' => 1,
		);

		// We only need to verify the command lookup succeeds; suppress the actual
		// send_command_reply HTTP call by catching the WP_Error or mock.
		try {
			$method->invoke( $this->webhook_controller, $parsed, $message, '12345', '99999', 'private' );
		} catch ( \Exception $e ) {
			// Ignore any exceptions from Telegram API calls in the test env.
		}

		$wp_mcp_ai_slash_command_handler = $original_handler;

		$this->assertTrue( $executed, 'Handler for content-translate should have been called when /content_translate was received.' );
	}
}
