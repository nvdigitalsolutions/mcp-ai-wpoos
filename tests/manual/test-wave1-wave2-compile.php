...
$ok = 0; $fail = 0;
function check(string $class): void { global $ok, $fail;
    if (class_exists($class) || interface_exists($class)) { $ok++; } else { $fail++; printf("  FAIL %s\n", $class); }
}
function verify(string $cls, string $iface): void { global $ok, $fail;
    $obj = new $cls();
...
$contracts = ['SemanticCompressorInterface','DataBudgetTrackerInterface','ErlangCInterface',
    'ErrorTrackingServiceInterface','CostTrackingServiceInterface','ModelCatalogInterface',
    'TokenBudgetServiceInterface','FileValidationServiceInterface','FileUploadServiceInterface',
    'FileOrchestrationInterface','RateLimiterInterface','EmbeddingServiceInterface',
    'MemoryStoreInterface','AssistantServiceInterface','ProfessionRepositoryInterface',
    'CronStatusInterface','PerformanceMonitorInterface','ContextCompressionInterface',
    'TokenUsageInterface','TimeoutDetectionInterface','ToolExecutionInterface',
    'AgentOrchestrationInterface','ChatContinuationInterface','OrchestrationControlInterface',
    'OcrServiceInterface','LanguageDetectionInterface','EmailServiceInterface',
    'ValidationServiceInterface','CodeFormattingInterface','MusicGenerationInterface',
    'FinancialDataInterface','VisionInferenceInterface',
];
foreach ($contracts as $i) { check($C . $i); }
...
echo "\n=== Adapters (29) ===\n";
foreach (['SemanticCompressor','DataBudgetTracker','ErrorTrackingService','CostTrackingService',
    'ModelCatalog','TokenBudgetService','FileValidationService','FileUploadService',
    'FileOrchestration','RateLimiter','EmbeddingService','MemoryStore',
    'AssistantService','ProfessionRepository','CronStatus','PerformanceMonitor',
    'ContextCompression','TokenUsage','TimeoutDetection','ToolExecution',
    'AgentOrchestration','ChatContinuation','OrchestrationControl',
    'OcrService','LanguageDetection','EmailService','ValidationService',
    'CodeFormatting','MusicGeneration','FinancialData','VisionInference',
] as $a) { check($W . $a); }
