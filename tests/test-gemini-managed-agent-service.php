...
class Test_Gemini_Managed_Agent_Service extends WP_UnitTestCase {
	...
	// -------------------------------------------------------------------------
	// send_interaction — input validation
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// Environment normalisation (via Reflection)
	// -------------------------------------------------------------------------
	...
	public function test_normalise_environment_passthrough() {
		...
		$config = array( 'type' => 'remote', 'sources' => array() );
	...
	public function test_build_multimodal_input_mixed_text_and_image() {
		...
		$result = $reflection->invoke(
			$this->service,
			array(
				array( 'type' => 'text', 'text' => 'Analyze this chart:' ),
				array(
					'type'      => 'image',
					'data'      => 'base64png',
					'mime_type' => 'image/png',
	...
	public function test_normalise_interaction_result_with_steps() {
		...
		$result = $reflection->invoke(
			...
			array(
				'id'    => 'int_def456',
				'steps' => array(
					array( 'tool_calls' => array( array( 'name' => 'code_execution', 'args' => array( 'language' => 'python' ) ) ) ),
					array( 'tool_calls' => array( array( 'name' => 'google_search', 'args' => array( 'query' => 'test' ) ) ) ),
				),
			)
		);
	...
	public function test_aggregate_stream_events_with_text_deltas() {
		...
		$events = array(
			array( 'id' => 'int_1', 'delta' => array( 'text' => 'Hello ' ) ),
	...
	public function test_aggregate_stream_events_with_steps() {
		...
		$events = array(
			array(
				'id'   => 'int_2',
				'step' => array( 'type' => 'reasoning', 'text' => 'Planning...' ),
			),
			array(
				'step' => array( 'type' => 'tool_call', 'tool' => 'google_search' ),
			),
		);
	...
	public function test_get_environment_for_interaction_returns_id() {
		...
		set_transient(
			...
			array( 'environment_id' => $env_id, 'created_at' => time() ),
			3600
		);
	...
	// -------------------------------------------------------------------------
	// create_managed_agent — input validation
	// -------------------------------------------------------------------------
	...
	public function test_build_agent_environment_skips_malformed_sources() {
		...
		$result = $reflection->invoke(
			...
			array(
				'sources' => array(
					array( 'no_type' => true ), // Missing type/target.
					array(
						'type'    => 'inline',
						'target'  => '.agents/AGENTS.md',
	...
	// -------------------------------------------------------------------------
	// continue_interaction input validation
	// -------------------------------------------------------------------------
	...
	public function test_continue_interaction_preserves_ids() {
		...
		// by checking that the availability gate is passed and the next error
		// is the expected network error (not an argument validation error).
		$result = $this->service->continue_interaction( $interaction_id, 'Follow-up task.' );

		// Should fail at the API request stage (no real endpoint), not at validation.
		$this->assertWPError( $result );

		// The error should NOT be a missing input or missing key error —
		// those would indicate the method didn't pass args correctly.
		$code = $result->get_error_code();
		$this->assertNotEquals( 'wp_mcp_ai_missing_input', $code, 'Should not fail on input validation' );
		$this->assertNotEquals( 'wp_mcp_ai_missing_interaction_id', $code, 'Should not fail on interaction ID validation' );
