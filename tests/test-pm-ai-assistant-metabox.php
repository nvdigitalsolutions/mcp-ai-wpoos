...
class WP_MCP_AI_PM_AI_Assistant_Metabox_Test extends WP_UnitTestCase {
	...
	/**
	 * Test that AJAX handler returns chat configuration for valid requests.
	 */
	public function test_ajax_handler_returns_chat_config() {
		// Skip if Pro addon is not available.
		if ( ! defined( 'WP_MCP_AI_PRO_PATH' ) ) {
			$this->markTestSkipped( 'Pro addon not available' );
		}
		...
		// Create assistant and task.
		$assistant_id = $this->factory->post->create(
			array(
				'post_type'   => 'mcp_ai_assistant',
				'post_title'  => 'Ajax Assistant',
				'post_status' => 'publish',
			)
		);
		...
		$task_id = $this->factory->post->create(
			array(
				'post_type'    => 'mcp_ai_task',
				'post_title'   => 'Ajax Task',
				'post_content' => 'Task content',
				'post_status'  => 'publish',
			)
		);

		// Set current user with required capabilities.
		...
		$_POST['context_message'] = 'Context data';
