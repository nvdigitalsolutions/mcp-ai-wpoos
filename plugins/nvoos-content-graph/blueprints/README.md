...
| `demo.json` | **Generated.** Standalone blueprint for shareable links — installs the plugin from wordpress.org, seeds Asteria, builds the graph, lands on the Graph Explorer. |
...
opening `<?php` tag) and writes both blueprint files. The generated JSON is
committed — the generator is optional tooling for content edits.
