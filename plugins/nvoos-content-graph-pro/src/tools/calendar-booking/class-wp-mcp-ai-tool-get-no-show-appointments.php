 ...
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/calendar-booking/class-wp-mcp-ai-tool-get-no-show-appointments.php` for the standalone
...
class WP_MCP_AI_Tool_Get_No_Show_Appointments implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get No-Show Appointments', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves a list of appointments marked as no-show, with optional date range and service filters. Useful for identifying missed appointments and planning reschedule outreach.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties' => array(
				'date_from'    => array(
					'type'        => 'string',
					'description' => __( 'Filter appointments on or after this date (YYYY-MM-DD).', 'nvoos-content-graph-pro' ),
					'pattern'     => '^\d{4}-\d{2}-\d{2}$',
				),
				'date_to'      => array(
					'type'        => 'string',
					'description' => __( 'Filter appointments on or before this date (YYYY-MM-DD).', 'nvoos-content-graph-pro' ),
					'pattern'     => '^\d{4}-\d{2}-\d{2}$',
				),
				'service_type' => array(
					'type'        => 'string',
	...
	public static function get_unavailable_reason() {
		return __( 'The Get No-Show Appointments tool requires the Calendar Booking toolkit to be enabled in plugin settings.', 'nvoos-content-graph-pro' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $current_user_id || ! user_can( $current_user_id, 'read' ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to list appointments.', 'nvoos-content-graph-pro' ) );
		}
		...
		return array(
			...
			'message'      => sprintf(
				...
				__( 'Found %d no-show appointments.', 'nvoos-content-graph-pro' ),
				$query->found_posts
			),
