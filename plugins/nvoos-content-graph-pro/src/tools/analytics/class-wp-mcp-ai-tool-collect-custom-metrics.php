...
class WP_MCP_AI_Tool_Collect_Custom_Metrics implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public static function get_unavailable_reason() {
		...
		return __( 'Custom metrics collection tool is not available.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_name() {
		return __( 'Collect Custom Metrics', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Track custom business metrics and KPIs. Record events with values, metadata, and timestamps for analysis and reporting.', 'nvoos-content-graph-pro' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( empty( $metric_name ) ) {
			return new WP_Error(
				'missing_metric_name',
				__( 'Metric name is required.', 'nvoos-content-graph-pro' )
			);
		}
		...
		if ( false === $result ) {
			return new WP_Error(
				'insert_failed',
				__( 'Failed to record custom metric.', 'nvoos-content-graph-pro' ),
		...
		return array(
			...
			'message'      => sprintf(
				...
				__( 'Recorded metric "%1$s" with value %2$s.', 'nvoos-content-graph-pro' ),
				$metric_name,
				$metric_value
			),
		);
