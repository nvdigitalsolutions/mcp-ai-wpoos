<?php
/**
 * wellness/medical-records/class-wp-mcp-ai-tool-delete-medical-record.php (ecosystem port — Wave F4, healthcare wellness CRUD batch 2).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/healthcare/wellness/medical-records/class-wp-mcp-ai-tool-delete-medical-record.php` for the
...
class WP_MCP_AI_Tool_Delete_Medical_Record implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Delete Medical Record', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Permanently deletes a medical record from the system.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'record_id' => array(
					'type'        => 'integer',
					'description' => __( 'Record ID to delete (required)', 'nvoos-content-graph-pro' ),
					'minimum'     => 1,
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $current_user_id || ! user_can( $current_user_id, 'delete_posts' ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to delete medical records.', 'nvoos-content-graph-pro' ) );
		}
		...
		if ( ! $result ) {
			return new WP_Error( 'wp_mcp_ai_delete_failed', __( 'Failed to delete medical record.', 'nvoos-content-graph-pro' ) );
		}

		return array(
			'success' => true,
			'message' => __( 'Medical record deleted successfully.', 'nvoos-content-graph-pro' ),
		);
	}
}
