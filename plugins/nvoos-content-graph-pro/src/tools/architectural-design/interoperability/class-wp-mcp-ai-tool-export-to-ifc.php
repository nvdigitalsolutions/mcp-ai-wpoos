<?php
/**
 * Tool_Export_To_Ifc (ecosystem port - Wave F2, architectural-design tool batch).
 *
...
class WP_MCP_AI_Tool_Export_To_Ifc implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Export to IFC', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Generate an IFC 4.3 STEP-format text body (HEADER + DATA) from a normalised floor plan. Output is a valid STEP file body — geometry is minimal but the entity graph (project → site → building → storeys → spaces / walls / openings) is structurally complete.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'floor_plan'   => array(
					'type'        => 'object',
					'description' => __( 'Normalised floor-plan structure (use import_dwg_floor_plan or import_ifc_model first if needed).', 'nvoos-content-graph-pro' ),
				),
				'author'       => array(
					'type'        => 'string',
					'description' => __( 'Author name for the IFC header.', 'nvoos-content-graph-pro' ),
				),
				'organization' => array(
					'type'        => 'string',
					'description' => __( 'Organization name for the IFC header.', 'nvoos-content-graph-pro' ),
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id || ! user_can( $user_id, 'edit_posts' ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to export to IFC.', 'nvoos-content-graph-pro' ) );
		}
		...
		return array(
			...
			'note'          => __( 'Geometry is minimal. For coordinated geometry, run the result through ifcopenshell or a BIM authoring tool.', 'nvoos-content-graph-pro' ),
		);
	}
}
