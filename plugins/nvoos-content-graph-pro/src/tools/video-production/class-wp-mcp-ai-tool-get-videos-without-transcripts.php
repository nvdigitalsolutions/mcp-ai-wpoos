 ...
 *
 * Tool for retrieving videos lacking transcripts or captions.
 *
...
/**
 * Retrieves videos that lack transcripts or captions, optionally
 * filtered by platform and language.
 *
 * @since 2.8.0
 */
class WP_MCP_AI_Tool_Get_Videos_Without_Transcripts implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() {
		return 'get_videos_without_transcripts';
	}
	...
	public function get_name() {
		return __( 'Get Videos Without Transcripts', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves videos that lack transcripts or captions.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties' => array(
				...
				'language' => array(
					'type'        => 'string',
					'description' => __( 'Language code for transcripts (e.g. "en").', 'nvoos-content-graph-pro' ),
					'default'     => 'en',
				),
				'limit'    => array(
					'type'        => 'integer',
	...
	public static function get_unavailable_reason() {
		...
		return __( 'Get Videos Without Transcripts tool is not available.', 'nvoos-content-graph-pro' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $current_user_id || ! user_can( $current_user_id, 'read' ) ) {
			return new WP_Error(
				'wp_mcp_ai_forbidden',
				__( 'You do not have permission to view video transcripts.', 'nvoos-content-graph-pro' )
			);
		}
		...
		// Build query args for video attachments without transcripts.
		$query_args = array(
			'post_type'      => 'attachment',
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				$videos[] = array(
					...
					'transcript_lang' => get_post_meta( $attachment_id, '_mcp_video_transcript_lang', true ),
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Found %d videos without transcripts.', 'nvoos-content-graph-pro' ),
				count( $videos )
			),
			'total'   => count( $videos ),
			'videos'  => $videos,
