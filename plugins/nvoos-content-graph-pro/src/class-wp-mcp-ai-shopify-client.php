		...
		private function storefront_catalog_rpc( $method, array $params ) {
			...
			if ( is_wp_error( $raw_response ) ) {
				return new WP_Error(
					...
					/* translators: %s: error message */
					sprintf( __( 'Shopify Storefront Catalog request failed: %s', 'nvoos-content-graph-pro' ), $raw_response->get_error_message() )
				);
			}
			...
			if ( $response_code < 200 || $response_code >= 300 ) {
				...
				return new WP_Error(
					...
					/* translators: 1: HTTP status code, 2: optional response detail */
					sprintf( __( 'Shopify Storefront Catalog MCP endpoint returned HTTP %1$d.%2$s', 'nvoos-content-graph-pro' ), $response_code, $detail )
				);
			}
			...
			if ( json_last_error() !== JSON_ERROR_NONE ) {
				return new WP_Error(
					...
					__( 'Shopify Storefront Catalog MCP endpoint returned an invalid JSON response.', 'nvoos-content-graph-pro' )
				);
			}
			...
			if ( isset( $decoded['error'] ) && is_array( $decoded['error'] ) ) {
				...
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Shopify Storefront Catalog MCP error %1$s: %2$s', 'nvoos-content-graph-pro' ),
						$rpc_code,
						$rpc_message
					)
				);
