# [nvoos_graphify_chat] Shortcode — Design Plan
...
| Concern | Pro SPA-v2 | `[nvoos_graphify_chat]` |
|---|---|---|
...
```
POST /wp-json/nvoos-graphify/v1/ai/chat
```
...
```json
{
  ...
  "data": { "content": "..." },
  "tool_results": [
    { "name": "nvoos_graphify_query_graph", "result": { ... }, "cost": 0.002 }
  ],
  "iterations": 2,
...
type:tool_start
data:{"id":"call_1","name":"nvoos_graphify_query_graph"}
...
```php
namespace NvoosGraphifyAi\Frontend;

class ChatShortcode {
    public function register(): void {
        add_shortcode( 'nvoos_graphify_chat', array( $this, 'render' ) );
    }
    ...
    public function render( array $atts ): string {
        ...
        $containerId = 'nvoos-graphify-chat-' . wp_unique_id();

        $this->enqueueAssets();

        wp_add_inline_script(
            'nvoos-graphify-ai-chat-frontend',
            'window.NvoosGraphifyChat_' . $containerId . ' = ' . wp_json_encode( array(
                'container'    => $containerId,
                'restUrl'      => rest_url( 'nvoos-graphify/v1' ),
                'nonce'        => wp_create_nonce( 'wp_rest' ),
                'provider'     => $atts['provider'],
                'showCost'     => (bool) $atts['show_cost'],
                'placeholder'  => $atts['placeholder'] ?: __( 'Ask about your knowledge graph…', 'nvoos-graphify-ai' ),
                'isAdmin'      => false,
                'i18n'         => array(
                    'send'         => __( 'Send', 'nvoos-graphify-ai' ),
                    'thinking'     => __( 'Thinking…', 'nvoos-graphify-ai' ),
                    'error'        => __( 'Something went wrong.', 'nvoos-graphify-ai' ),
                    'toolsUsed'    => __( 'Tools used', 'nvoos-graphify-ai' ),
                    'cost'         => __( 'Cost', 'nvoos-graphify-ai' ),
                    'clear'        => __( 'Clear', 'nvoos-graphify-ai' ),
                    'graphQuery'   => __( 'Queried your knowledge graph', 'nvoos-graphify-ai' ),
                    'noProvider'   => __( 'No AI provider configured. Set an API key in NV Graphify → AI Providers.', 'nvoos-graphify-ai' ),
                ),
        ...
        return sprintf(
            '<div id="%s" class="nvoos-graphify-chat-widget" style="height:%s"></div>',
    ...
    private function enqueueAssets(): void {
        wp_enqueue_style(
            'nvoos-graphify-ai-chat-frontend',
            NVOOS_GRAPHIFY_AI_URL . 'assets/css/graphify-ai-chat.css',
            array(),
        ...
        wp_enqueue_script(
            'nvoos-graphify-ai-chat-frontend',
            NVOOS_GRAPHIFY_AI_URL . 'assets/js/graphify-ai-chat-frontend.js',
            array(),
            NVOOS_GRAPHIFY_AI_VERSION,
            true
        );
...
if ( class_exists( 'NvoosGraphifyAi\Frontend\ChatShortcode' ) ) {
    ( new \NvoosGraphifyAi\Frontend\ChatShortcode() )->register();
}
```

---

### JavaScript — `assets/js/graphify-ai-chat-frontend.js`

**~250 lines, vanilla JS, no dependencies.**

```
graphify-ai-chat-frontend.js
...
function handleFrame(frame) {
    switch (frame.type) {
        ...
        case 'tool_start':  // "Queried graph: nvoos_graphify_query_graph"
...
│    Tool: nvoos_graphify_query_graph      │
...
| File | Action | Lines |
|---|---|---|
| `plugins/nvoos-graphify-ai/src/Frontend/ChatShortcode.php` | **Create** | ~100 |
| `plugins/nvoos-graphify-ai/assets/js/graphify-ai-chat-frontend.js` | **Create** | ~250 |
| `plugins/nvoos-graphify-ai/assets/css/graphify-ai-chat.css` | **Update** (add frontend styles) | ~50 added |
| `plugins/nvoos-graphify-ai/src/Plugin.php` | **Edit** — wire `ChatShortcode` in `registerFrontend()` | ~5 |
...
```
src/Frontend/ChatShortcode.php        ← [nvoos_graphify_chat] shortcode
src/Admin/Sections/ChatInterface.php  ← Admin "Chat Tester" tab
    ↓ both call
assets/js/graphify-ai-chat.js         ← Single JS file (already exists)
    ↓ handles
SSE parsing + DOM rendering + sessionStorage
```

The existing `graphify-ai-chat.js` already handles SSE — it just needs:
1. A frontend entry point (check `window.NvoosGraphifyAiChat.isAdmin`)
...
|---|---|
| POST `/nvoos-graphify/v1/ai/chat` | ✅ Same endpoint, same request shape |
...
- `graphify-ai-chat.js` detects `isAdmin` vs `isFrontend` from config
- Adds sessionStorage persistence
- Adds tool card rendering

### Step 2: Create `ChatShortcode.php`
- Registers `[nvoos_graphify_chat]`
