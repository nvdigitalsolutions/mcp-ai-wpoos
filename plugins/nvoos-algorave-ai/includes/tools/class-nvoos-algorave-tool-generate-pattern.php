...
class NV_oOS_Algorave_Tool_Generate_Pattern implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Generate Algorave Pattern', 'nvoos-algorave-ai' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'description'  => array(
					'type'        => 'string',
					'description' => __( 'Natural language description of the desired pattern (e.g. "a techno beat at 130bpm with a rolling bassline", "ambient pads in D minor", "drum and bass rhythm").', 'nvoos-algorave-ai' ),
					'minLength'   => 1,
					'maxLength'   => 2000,
				),
				'engine'       => array(
					'type'        => 'string',
					'description' => __( 'Target synthesis engine. "strudel" for TidalCycles mini-notation (default), "tonejs" for Tone.js JavaScript code.', 'nvoos-algorave-ai' ),
					'enum'        => array( 'strudel', 'tonejs' ),
					'default'     => 'strudel',
				),
				'bpm'          => array(
					'type'        => 'integer',
					'description' => __( 'Beats per minute (20–300). Defaults to 120.', 'nvoos-algorave-ai' ),
					'minimum'     => 20,
					'maximum'     => 300,
					'default'     => 120,
				),
				'scale'        => array(
					'type'        => 'string',
					'description' => __( 'Musical scale/key (e.g. "C minor", "A major", "F# dorian"). Defaults to "C minor".', 'nvoos-algorave-ai' ),
					'maxLength'   => 50,
				),
				'genre'        => array(
					'type'        => 'string',
					'description' => __( 'Musical genre for style guidance (e.g. "techno", "ambient", "drum and bass", "house", "experimental").', 'nvoos-algorave-ai' ),
					'maxLength'   => 100,
				),
				'save_pattern' => array(
					'type'        => 'boolean',
					'description' => __( 'Whether to save the generated pattern to the pattern library.', 'nvoos-algorave-ai' ),
					'default'     => false,
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( empty( $description ) ) {
			return array(
				'success' => false,
				'error'   => __( 'A description of the desired pattern is required.', 'nvoos-algorave-ai' ),
			);
		}
		...
		$result = array(
			...
			'message'      => sprintf(
				...
				__( 'Generated a %1$s pattern at %2$d BPM in %3$s. Copy the code into the live coder to play it, or ask me to modify it.', 'nvoos-algorave-ai' ),
				$engine,
				$bpm,
				$scale
			),
			'instructions' => __( 'To play this pattern, paste the code into the Algorave Live Coder or use the [algorave_live_coder] shortcode. You can also ask me to modify the tempo, add effects, or change the scale.', 'nvoos-algorave-ai' ),
		);
		...
		if ( ! empty( $arguments['save_pattern'] ) ) {
			...
			if ( ! is_wp_error( $post_id ) ) {
				...
				$result['message']   .= ' ' . __( 'Pattern saved to the library.', 'nvoos-algorave-ai' );
			}
		}
