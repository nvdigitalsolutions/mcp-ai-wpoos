...
class Sparql implements RemoteSource {
	...
	public function getConfigSchema(): array {
		return array(
			'endpoint'  => array(
				...
				'description' => __( 'SPARQL 1.1 endpoint that accepts JSON results (e.g. https://query.wikidata.org/sparql).', 'nvoos-graphify' ),
				'required'    => true,
			),
			'query'     => array(
				'type'        => 'textarea',
				'label'       => __( 'SPARQL Query', 'nvoos-graphify' ),
				'description' => __( 'SPARQL SELECT query that returns ?id ?label ?type ?url and optionally ?source ?target ?relation for edges.', 'nvoos-graphify' ),
				'required'    => true,
			),
			'node_type' => array(
				'type'        => 'text',
				'label'       => __( 'Default Node Type', 'nvoos-graphify' ),
				'description' => __( 'Fallback node type when the result has no ?type binding.', 'nvoos-graphify' ),
				'default'     => 'entity',
			),
			'max_items' => array(
				'type'        => 'number',
				'label'       => __( 'Max Items', 'nvoos-graphify' ),
				'description' => __( 'Max results to ingest per sync.', 'nvoos-graphify' ),
				'default'     => 500,
			),
		);
	}
	...
	public function testConnection(): array {
		...
		if ( empty( $endpoint ) ) {
			return array(
				'success' => false,
				'message' => __( 'No endpoint configured.', 'nvoos-graphify' ),
			);
		}
		...
		if ( empty( $query ) ) {
			return array(
				'success' => false,
				'message' => __( 'No SPARQL query configured.', 'nvoos-graphify' ),
			);
		}
