...
/**
 * Database access object for the NV oOS Graphify plugin.
 *
 * Manages five custom tables:
 *   - nvoos_graphify_nodes            — graph entities (posts, terms, users, media, topics)
 *   - nvoos_graphify_edges            — relationships between nodes
 *   - nvoos_graphify_meta             — key/value addon metadata
 *   - nvoos_graphify_remote_sources   — external data source configurations
 *   - nvoos_graphify_embeddings       — vector embeddings for RAG
 *
...
class Db
{
        ...
        if ( $type ) {
            $results = $wpdb->get_results(
                $wpdb->prepare(
                    ...
                    $like, sanitize_text_field( $type ), $limit
                )
            );
    ...
    public static function upsertEdge( array $edge ) {
        ...
        if ( $existing ) {
            $wpdb->update(
                $table,
                array( 'confidence' => max( (float) $existing->confidence, $confidence ), 'provenance' => $provenance, 'properties' => $properties ),
        ...
        $result = $wpdb->insert(
            $table,
            array( 'source_node_id' => $source, 'target_node_id' => $target, 'relation' => $relation, 'confidence' => $confidence, 'provenance' => $provenance, 'properties' => $properties ),
            array( '%s', '%s', '%s', '%f', '%s', '%s' )
        );
        return $result ? $wpdb->insert_id : false;
    }
        ...
        if ( $relation ) {
            $results = $wpdb->get_results(
                $wpdb->prepare(
                    ...
                    $nodeId, $nodeId, sanitize_text_field( $relation ), $limit
                )
            );
            return is_array( $results ) ? $results : array();
        }
    ...
    public static function setMeta( string $key, $value ): void {
        ...
        if ( $exists ) {
            ...
            $wpdb->insert( $table, array( 'meta_key' => sanitize_text_field( $key ), 'meta_value' => $serialized ), array( '%s', '%s' ) );
        }
    }
    ...
    public static function saveRemoteSource( array $data ) {
        ...
        if ( $exists ) {
            $wpdb->update( $table, array( 'driver' => $driver, 'label' => $label, 'enabled' => $enabled, 'config_json' => $configJson ), array( 'slug' => $slug ), array( '%s', '%s', '%d', '%s' ), array( '%s' ) );
        } else {
            $wpdb->insert( $table, array( 'slug' => $slug, 'driver' => $driver, 'label' => $label, 'enabled' => $enabled, 'config_json' => $configJson ), array( '%s', '%s', '%s', '%d', '%s' ) );
        }
        return true;
    }
    ...
    public static function upsertEmbedding( array $data ) {
        ...
        if ( $existingId ) {
            $wpdb->update( $table, array( 'dim' => $dim, 'vector' => $vector ), array( 'id' => absint( $existingId ) ), array( '%d', '%s' ), array( '%d' ) );
            return absint( $existingId );
        }
        $result = $wpdb->insert( $table, array( 'node_id' => $nodeId, 'model' => $model, 'dim' => $dim, 'vector' => $vector ), array( '%s', '%s', '%d', '%s' ) );
        return $result ? $wpdb->insert_id : false;
    }
