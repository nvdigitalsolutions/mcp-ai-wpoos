 ...
 *
 * @since 1.0.0
 * @package NvoosGraphifyAiPlatform\Admin
 */
...
use NvoosGraphifyAiPlatform\Schema\Defaults;
...
final class PlatformDashboard {
	 ...
	 *
	 * Called from {@see \NvoosGraphifyAiPlatform\Plugin::registerAdmin()}.
	 *
	 * @since 1.0.0
	 *
	 * @return void
	...
	public function registerMenu(): void {
		...
		add_menu_page(
			__( 'NV Platform Settings', 'nvoos-graphify-ai-platform' ),
			__( 'NV Platform', 'nvoos-graphify-ai-platform' ),
			'manage_options',
			self::PAGE_SLUG,
			array( $this, 'renderDashboard' ),
			'dashicons-layout',
			null // Let WordPress auto-position (below NV Graphify at 85).
		);
		...
		add_submenu_page(
			self::PAGE_SLUG,
			__( 'Platform Settings', 'nvoos-graphify-ai-platform' ),
			__( 'Platform Settings', 'nvoos-graphify-ai-platform' ),
			'manage_options',
			self::PAGE_SLUG,
			array( $this, 'renderDashboard' )
	...
	public function handleSaveSettings(): void {
		...
		if ( ! isset( $_POST['_wpnonce'] ) ||
			...
			wp_die( esc_html__( 'Security check failed. Please try again.', 'nvoos-graphify-ai-platform' ) );
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to manage platform settings.', 'nvoos-graphify-ai-platform' ) );
		}
	...
	public function renderDashboard(): void {
		...
		<div class="wrap ai-platform-dashboard">
			<h1><?php esc_html_e( 'NV Platform', 'nvoos-graphify-ai-platform' ); ?></h1>

			<?php if ( $updated ) : ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'Settings saved.', 'nvoos-graphify-ai-platform' ); ?></p>
				</div>
			<?php endif; ?>
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					if ( empty( $sections ) ) :
						...
						<div class="ai-platform-empty-tab" style="background:#fff;border:1px solid #c3c4c7;padding:2rem;text-align:center;">
							<h2><?php esc_html_e( 'No content yet', 'nvoos-graphify-ai-platform' ); ?></h2>
							<p><?php esc_html_e( 'This tab has no registered sections. Content will appear as subsystems are activated.', 'nvoos-graphify-ai-platform' ); ?></p>
						</div>
						<?php
					else :
						...
						<p class="submit">
							<?php submit_button( __( 'Save Settings', 'nvoos-graphify-ai-platform' ), 'primary', 'submit', false ); ?>
						</p>
					<?php endif; ?>
				</form>
			</div>
