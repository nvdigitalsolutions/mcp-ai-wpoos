 ...
 *
 * Displays system status cards: Graphify version, AI addon status,
...
final class OverviewSection extends PlatformSection {
	...
	public function get_title(): string {
		return __( 'Platform Overview', 'nvoos-graphify-ai-platform' );
	}
	...
	public function render_wrapper( string $page_slug = '' ): void {
		...
		?>
		<h2><?php esc_html_e( 'Platform Overview', 'nvoos-graphify-ai-platform' ); ?></h2>
		...
		<div class="ai-platform-stats-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:1rem;margin-bottom:1.5rem;">
			...
			$this->renderStatCard(
				$project_count,
				__( 'Projects', 'nvoos-graphify-ai-platform' ),
				admin_url( 'edit.php?post_type=ai_platform_project' )
			);
			$this->renderStatCard(
				$resource_count,
				__( 'Resources', 'nvoos-graphify-ai-platform' ),
				admin_url( 'edit.php?post_type=ai_platform_resource' )
			);
			$this->renderStatCard(
				$template_count,
				__( 'Templates', 'nvoos-graphify-ai-platform' ),
				admin_url( 'edit.php?post_type=ai_platform_template' )
			);
			?>
		</div>

		<h3><?php esc_html_e( 'Quick Links', 'nvoos-graphify-ai-platform' ); ?></h3>
		<div style="display:flex;flex-wrap:wrap;gap:0.75rem;">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . PlatformDashboard::PAGE_SLUG . '&tab=agents' ) ); ?>" class="button">
				<?php esc_html_e( 'Manage Agents', 'nvoos-graphify-ai-platform' ); ?>
			</a>
			<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=' . Agents::POST_TYPE ) ); ?>" class="button">
				<?php esc_html_e( 'New Agent', 'nvoos-graphify-ai-platform' ); ?>
			</a>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=nvoos-graphify' ) ); ?>" class="button">
				<?php esc_html_e( 'Knowledge Graph', 'nvoos-graphify-ai-platform' ); ?>
			</a>
			<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=ai_platform_project' ) ); ?>" class="button">
				<?php esc_html_e( 'New Project', 'nvoos-graphify-ai-platform' ); ?>
			</a>
		</div>
		<?php
	}
	...
	private function renderStatCard( int $count, string $label, string $url ): void {
		...
		<div class="ai-platform-stat-card" style="background:#fff;border:1px solid #c3c4c7;padding:1rem;text-align:center;">
			...
			<a href="<?php echo esc_url( $url ); ?>" class="button button-small"><?php esc_html_e( 'View all', 'nvoos-graphify-ai-platform' ); ?></a>
		</div>
		<?php
	}
