...
final class CreateAgentButton {
	...
	public function register(): void {
		...
		add_action( 'wp_ajax_nvoos_graphify_platform_create_agent_modal', array( $this, 'handleAjaxCreate' ) );
		add_action( 'wp_ajax_nvoos_graphify_platform_build_agent_conversation', array( $this, 'handleAjaxBuildFromConversation' ) );
		add_action( 'wp_ajax_nvoos_graphify_platform_upload_agent_attachment', array( $this, 'handleAjaxUploadAttachment' ) );
	}
	...
	public function enqueueScripts( string $hook ): void {
		...
		wp_enqueue_style(
			'nvoos-graphify-ai-platform-agents-create-button',
			NVOOS_GRAPHIFY_AI_PLATFORM_URL . 'assets/css/agents-create-button.css',
			array(),
		...
		wp_enqueue_script(
			'nvoos-graphify-ai-platform-agents-create-button',
			NVOOS_GRAPHIFY_AI_PLATFORM_URL . 'assets/js/agents-create-button.js',
			array( 'jquery' ),
			NVOOS_GRAPHIFY_AI_PLATFORM_VERSION,
			true
		);
		...
		wp_localize_script(
			'nvoos-graphify-ai-platform-agents-create-button',
			'nvoosGraphifyPlatformCreateAgentButton',
			array(
				'buildUrl'   => admin_url( 'edit.php?post_type=' . Agents::POST_TYPE . '&page=wp-mcp-ai-build-assistant' ),
				'buttonText' => __( 'Build AI Agent', 'nvoos-graphify-ai-platform' ),
			)
		);
	}
	...
	public function handleAjaxCreate(): void {
		...
		if ( empty( $title ) || empty( $professions ) || empty( $regions ) ) {
			wp_send_json_error( array( 'message' => __( 'Title, professions, and regions are required.', 'nvoos-graphify-ai-platform' ) ) );
		}

		// Use the create_assistant tool from the base plugin's tool registry.
		if ( ! class_exists( 'WP_MCP_AI_Tool_Registry' ) ) {
			wp_send_json_error( array( 'message' => __( 'Tool registry not available.', 'nvoos-graphify-ai-platform' ) ) );
		}
		...
		if ( ! $tool ) {
			wp_send_json_error( array( 'message' => __( 'Create agent tool not available.', 'nvoos-graphify-ai-platform' ) ) );
		}
	...
	public function handleAjaxBuildFromConversation(): void {
		...
		if ( empty( $conversation_json ) ) {
			wp_send_json_error( array( 'message' => __( 'No conversation data provided.', 'nvoos-graphify-ai-platform' ) ) );
		}

		$conversation = json_decode( $conversation_json, true );

		if ( ! is_array( $conversation ) || empty( $conversation['messages'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid conversation data.', 'nvoos-graphify-ai-platform' ) ) );
		}

		$assistant_config = self::extractConfigFromConversation( $conversation['messages'] );

		if ( empty( $assistant_config['title'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Could not determine a title for the agent. Please describe what you want the agent to be called.', 'nvoos-graphify-ai-platform' ) ) );
		}
	...
	public function handleAjaxUploadAttachment(): void {
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions to upload files.', 'nvoos-graphify-ai-platform' ) ) );
		}

		if ( empty( $_FILES['file'] ) ) {
			wp_send_json_error( array( 'message' => __( 'No file provided.', 'nvoos-graphify-ai-platform' ) ) );
		}
		...
		if ( ! in_array( $file_ext, $allowed_types, true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid file type. Allowed types: txt, md, pdf, doc, docx.', 'nvoos-graphify-ai-platform' ) ) );
		}
