...
final class AddAgentPage {
	...
	public function register(): void {
		...
		add_action( 'wp_ajax_nvoos_graphify_platform_create_agent', array( $this, 'handleAjaxCreate' ) );
	}
	...
	public function registerPage(): void {
		$this->page_hook = add_submenu_page(
			...
			__( 'Add Agent', 'nvoos-graphify-ai-platform' ),
			__( 'Add Agent', 'nvoos-graphify-ai-platform' ),
			'edit_posts',
			'nvoos-graphify-ai-platform-add-agent',
	...
	public function enqueueScripts( string $hook ): void {
		...
		wp_enqueue_style(
			'nvoos-graphify-ai-platform-agents-add',
			NVOOS_GRAPHIFY_AI_PLATFORM_URL . 'assets/css/agents-add.css',
			array(),
		...
		wp_enqueue_script(
			'nvoos-graphify-ai-platform-agents-add',
			NVOOS_GRAPHIFY_AI_PLATFORM_URL . 'assets/js/agents-add.js',
			array( 'jquery' ),
			NVOOS_GRAPHIFY_AI_PLATFORM_VERSION,
			true
		);
		...
		wp_localize_script(
			'nvoos-graphify-ai-platform-agents-add',
			'nvoosGraphifyPlatformAgentsAdd',
			array(
	...
	public function renderPage(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Add New Agent', 'nvoos-graphify-ai-platform' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Select a professional template to create a new AI agent. Each professional has pre-configured instructions, tools, and knowledge base.', 'nvoos-graphify-ai-platform' ); ?>
			</p>
				...
				<div class="notice notice-warning">
					<p>
						...
						printf(
							...
							esc_html__( 'No professional templates found. Please %s first to create agents from templates.', 'nvoos-graphify-ai-platform' ),
							'<a href="' . esc_url( admin_url( 'post-new.php?post_type=mcp_ai_profession' ) ) . '">' . esc_html__( 'create a professional', 'nvoos-graphify-ai-platform' ) . '</a>'
						);
						?>
					</p>
					<p>
						...
						printf(
							...
							esc_html__( 'Alternatively, you can %s to create a custom agent without using a template.', 'nvoos-graphify-ai-platform' ),
							'<a href="' . esc_url( admin_url( 'post-new.php?post_type=' . Agents::POST_TYPE ) ) . '">' . esc_html__( 'add a new agent directly', 'nvoos-graphify-ai-platform' ) . '</a>'
						);
						?>
					</p>
				</div>
						...
						<div class="wp-mcp-ai-professional-card" data-profession-id="<?php echo esc_attr( (string) $profession->ID ); ?>">
									...
									<?php if ( $tools_count > 0 ) : ?>
										<span class="meta-item">
											...
											printf(
												...
												esc_html( _n( '%d tool', '%d tools', $tools_count, 'nvoos-graphify-ai-platform' ) ),
												absint( $tools_count )
											);
											?>
										</span>
									...
									<?php if ( $expertise_count > 0 ) : ?>
										<span class="meta-item">
											...
											printf(
												...
												esc_html( _n( '%d expertise', '%d expertise areas', $expertise_count, 'nvoos-graphify-ai-platform' ) ),
							...
							<div class="professional-actions">
								...
								<a href="<?php echo esc_url( get_edit_post_link( $profession->ID ) ); ?>" class="button button-secondary">
									<?php esc_html_e( 'View Template', 'nvoos-graphify-ai-platform' ); ?>
								</a>
							</div>
						</div>
					<?php endforeach; ?>
		...
		<div id="wp-mcp-ai-create-modal" class="wp-mcp-ai-modal" style="display:none;">
			...
			<div class="wp-mcp-ai-modal-content">
				<div class="wp-mcp-ai-modal-header">
					<h2><?php esc_html_e( 'Create Agent from Template', 'nvoos-graphify-ai-platform' ); ?></h2>
					...
					<form id="wp-mcp-ai-create-form">
						...
						<p>
							<label for="assistant-title">
								<strong><?php esc_html_e( 'Agent Title', 'nvoos-graphify-ai-platform' ); ?> <span class="required">*</span></strong>
							</label>
							<input type="text" id="assistant-title" name="title" class="regular-text widefat" required placeholder="<?php esc_attr_e( 'e.g., "Jamaica Tax Agent"', 'nvoos-graphify-ai-platform' ); ?>">
							<span class="description"><?php esc_html_e( 'Give your agent a descriptive name', 'nvoos-graphify-ai-platform' ); ?></span>
						</p>

						<p>
							<label for="assistant-provider">
								<strong><?php esc_html_e( 'AI Provider', 'nvoos-graphify-ai-platform' ); ?></strong>
							</label>
							<select id="assistant-provider" name="provider" class="regular-text widefat">
								<option value=""><?php esc_html_e( '-- Use Template Default --', 'nvoos-graphify-ai-platform' ); ?></option>
								<?php
							...
							<span class="description"><?php esc_html_e( 'Override the template default if needed', 'nvoos-graphify-ai-platform' ); ?></span>
						</p>
					</form>
				</div>
				<div class="wp-mcp-ai-modal-footer">
					<button type="button" class="button button-secondary wp-mcp-ai-modal-close">
						<?php esc_html_e( 'Cancel', 'nvoos-graphify-ai-platform' ); ?>
					</button>
	...
	public function handleAjaxCreate(): void {
		check_ajax_referer( 'nvoos_graphify_platform_create_agent', 'nonce' );

		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'nvoos-graphify-ai-platform' ) ) );
		}
		...
		if ( ! $profession_id || 'mcp_ai_profession' !== get_post_type( $profession_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid professional template.', 'nvoos-graphify-ai-platform' ) ) );
		}

		if ( empty( $title ) ) {
			wp_send_json_error( array( 'message' => __( 'Title is required.', 'nvoos-graphify-ai-platform' ) ) );
		}
		...
		if ( ! empty( $knowledge_base ) ) {
			$system_prompt .= "\n\n" . __( 'Knowledge Base:', 'nvoos-graphify-ai-platform' ) . "\n" . $knowledge_base;
		}
		...
		wp_send_json_success(
			array(
				...
				'message'  => __( 'Agent created successfully!', 'nvoos-graphify-ai-platform' ),
			)
		);
	}
}
