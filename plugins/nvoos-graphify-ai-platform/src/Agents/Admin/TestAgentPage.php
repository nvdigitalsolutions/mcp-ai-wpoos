...
final class TestAgentPage {
	...
	public function registerPage(): void {
		$this->page_hook = add_submenu_page(
			...
			__( 'Test Agent', 'nvoos-graphify-ai-platform' ),
			__( 'Test Agent', 'nvoos-graphify-ai-platform' ),
			'manage_options',
			'nvoos-graphify-ai-platform-test-agent',
	...
	public function enqueueAssets( string $hook ): void {
		...
		wp_enqueue_style(
			'nvoos-graphify-ai-platform-agents-test',
			NVOOS_GRAPHIFY_AI_PLATFORM_URL . 'assets/css/agents-test.css',
		...
		wp_enqueue_script(
			'nvoos-graphify-ai-platform-agents-test',
			NVOOS_GRAPHIFY_AI_PLATFORM_URL . 'assets/js/agents-test.js',
			array( 'wp-mcp-ai-chat' ),
			NVOOS_GRAPHIFY_AI_PLATFORM_VERSION,
			true
		);
	...
	private function getChatStrings(): array {
		return array(
			'placeholder'               => __( 'Ask something…', 'nvoos-graphify-ai-platform' ),
			'send'                      => __( 'Send', 'nvoos-graphify-ai-platform' ),
			'sending'                   => __( 'Sending message…', 'nvoos-graphify-ai-platform' ),
			'waiting'                   => __( 'Waiting for the agent…', 'nvoos-graphify-ai-platform' ),
			...
			'uploadError'               => __( 'Upload failed.', 'nvoos-graphify-ai-platform' ),
			'uploadInProgress'          => __( 'Upload in progress.', 'nvoos-graphify-ai-platform' ),
			'unsupportedFileType'       => __( 'Unsupported file type.', 'nvoos-graphify-ai-platform' ),
			'newConversation'           => __( 'New conversation', 'nvoos-graphify-ai-platform' ),
			'loadConversation'          => __( 'Load conversation', 'nvoos-graphify-ai-platform' ),
			'historyLoading'            => __( 'Loading…', 'nvoos-graphify-ai-platform' ),
			'historyEmpty'              => __( 'No previous conversations.', 'nvoos-graphify-ai-platform' ),
			'historyError'              => __( 'Unable to load history.', 'nvoos-graphify-ai-platform' ),
			'deleteConversation'        => __( 'Delete conversation', 'nvoos-graphify-ai-platform' ),
			'confirmDeleteConversation' => __( 'Delete this conversation?', 'nvoos-graphify-ai-platform' ),
			'roleLabels'                => array(
				'assistant' => __( 'Agent', 'nvoos-graphify-ai-platform' ),
	...
	public function renderPage(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'nvoos-graphify-ai-platform' ) );
		}
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Test AI Agents', 'nvoos-graphify-ai-platform' ); ?></h1>
			<p><?php esc_html_e( 'Test your AI agents directly from the admin dashboard. Click "Test" next to any agent to open a chat interface and validate its behavior.', 'nvoos-graphify-ai-platform' ); ?></p>
				...
				<div class="notice notice-warning">
					<p>
						...
						printf(
							...
							esc_html__( 'No agents found. %s to get started.', 'nvoos-graphify-ai-platform' ),
							'<a href="' . esc_url( admin_url( 'post-new.php?post_type=' . Agents::POST_TYPE ) ) . '">' . esc_html__( 'Create your first agent', 'nvoos-graphify-ai-platform' ) . '</a>'
						);
						?>
					</p>
				</div>
			<?php else : ?>
				<table class="wp-list-table widefat fixed striped">
					<thead>
						<tr>
							<th scope="col"><?php esc_html_e( 'Agent Name', 'nvoos-graphify-ai-platform' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Provider', 'nvoos-graphify-ai-platform' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Model', 'nvoos-graphify-ai-platform' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Professionals', 'nvoos-graphify-ai-platform' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Tools', 'nvoos-graphify-ai-platform' ); ?></th>
							<th scope="col" class="column-actions"><?php esc_html_e( 'Actions', 'nvoos-graphify-ai-platform' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php foreach ( $assistants as $agent ) : ?>
							...
							$provider       = ! empty( $config['provider'] ) ? $config['provider'] : __( 'Default', 'nvoos-graphify-ai-platform' );
							$model          = ! empty( $config['model'] ) ? $config['model'] : __( 'Default', 'nvoos-graphify-ai-platform' );
							...
							<tr>
									...
									<div class="row-actions">
										<span class="edit">
											<a href="<?php echo esc_url( $edit_url ); ?>">
												<?php esc_html_e( 'Edit', 'nvoos-graphify-ai-platform' ); ?>
											</a>
										</span>
									</div>
								</td>
								...
								<td>
									...
									if ( empty( $professionals ) ) {
										echo '<em>' . esc_html__( 'None', 'nvoos-graphify-ai-platform' ) . '</em>';
									} else {
								...
								<td>
									...
									echo esc_html( sprintf( _n( '%d tool', '%d tools', $tool_count, 'nvoos-graphify-ai-platform' ), $tool_count ) );
									?>
								</td>
								<td>
									<button
									...
									>
										<?php esc_html_e( 'Test', 'nvoos-graphify-ai-platform' ); ?>
									</button>
								</td>
							</tr>
						<?php endforeach; ?>
			...
			<div id="wp-mcp-ai-test-modal" class="wp-mcp-ai-test-modal" style="display: none;">
				...
				<div class="wp-mcp-ai-test-modal__panel">
					<div class="wp-mcp-ai-test-modal__header">
						<h2 id="wp-mcp-ai-test-modal__title"><?php esc_html_e( 'Test Agent', 'nvoos-graphify-ai-platform' ); ?></h2>
