<?php
/**
 * Blueprints admin — dual registration in Platform + Graphify.
 *
 * @since 1.0.0
...
final class BlueprintAdmin {
	...
	public function registerPlatformSections(): void {
		\NvoosGraphifyAiPlatform\Admin\PlatformSettingsRegistry::register_tab(
			'blueprints',
			__( 'Blueprints', 'nvoos-graphify-ai-platform' )
		);

		if ( class_exists( 'NvoosGraphifyAiPlatform\Blueprints\Admin\BlueprintsDashboardSection' ) ) {
			\NvoosGraphifyAiPlatform\Admin\PlatformSettingsRegistry::register_section(
				new BlueprintsDashboardSection()
			);
		}
	}

	public function registerGraphifySections(): void {
		GraphifyRegistry::register_tab( 'blueprints', __( 'Blueprints', 'nvoos-graphify-ai-platform' ) );
	}
}
