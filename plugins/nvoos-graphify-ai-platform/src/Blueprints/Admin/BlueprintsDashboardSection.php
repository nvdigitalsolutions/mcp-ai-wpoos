 ...
 *
 * @since 1.0.0
 * @package NvoosGraphifyAiPlatform\Blueprints\Admin
 */

declare(strict_types=1);

namespace NvoosGraphifyAiPlatform\Blueprints\Admin;
...
final class BlueprintsDashboardSection extends PlatformSection {
	...
	public function get_title(): string {
		return __( 'Blueprints', 'nvoos-graphify-ai-platform' );
	}
	...
	public function get_fields(): array {
		return array(
			'blueprint_auto_apply' => array(
				'type'        => 'checkbox',
				'label'       => __( 'Auto-apply blueprints on activation', 'nvoos-graphify-ai-platform' ),
				'description' => __( 'When enabled, blueprints are automatically applied when their parent plugin or addon is activated.', 'nvoos-graphify-ai-platform' ),
			),
		);
	}
	...
	public function render_wrapper( string $page_slug = '' ): void {
		...
		<div style="display:flex;flex-wrap:wrap;gap:1rem;margin-bottom:1.5rem;">
			<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem;min-width:160px;text-align:center;">
				...
				<div style="color:#646970;"><?php esc_html_e( 'Blueprint Templates', 'nvoos-graphify-ai-platform' ); ?></div>
			</div>
			<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem;min-width:160px;text-align:center;">
				<div style="font-size:2rem;font-weight:700;color:<?php echo $auto_apply ? '#00a32a' : '#646970'; ?>;">
					<?php echo $auto_apply ? 'ON' : 'OFF'; ?>
				</div>
				<div style="color:#646970;"><?php esc_html_e( 'Auto-Apply', 'nvoos-graphify-ai-platform' ); ?></div>
			</div>
			<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem;min-width:160px;text-align:center;">
		...
		<div style="display:flex;flex-wrap:wrap;gap:0.75rem;margin-bottom:1.5rem;">
			...
			<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=ai_platform_template' ) ); ?>" class="button">
				<?php esc_html_e( 'New Template', 'nvoos-graphify-ai-platform' ); ?>
			</a>
		</div>
		...
		<h3><?php esc_html_e( 'About Blueprints', 'nvoos-graphify-ai-platform' ); ?></h3>
		<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem 1.5rem;max-width:800px;">
			<p><?php esc_html_e( 'Blueprints are pre-built configuration packages that set up agents, tools, skills, workflows, and graph schemas in a single operation. Think of them as "starter kits" for specific use cases — e-commerce, content marketing, customer support, etc.', 'nvoos-graphify-ai-platform' ); ?></p>
			<p><?php esc_html_e( 'The blueprint library grows as addons register their blueprints. Auto-apply mode automatically deploys a blueprint when its parent addon is activated, ensuring new features are ready immediately.', 'nvoos-graphify-ai-platform' ); ?></p>
		</div>
		<?php
	}
