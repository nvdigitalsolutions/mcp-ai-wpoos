...
final class ProfessionsDashboardSection extends PlatformSection {
	...
	public function get_title(): string {
		return __( 'Profession Templates', 'nvoos-graphify-ai-platform' );
	}
	...
	public function render_wrapper( string $page_slug = '' ): void {
		...
		?>
		<h2><?php esc_html_e( 'Profession Templates', 'nvoos-graphify-ai-platform' ); ?></h2>
		...
		<div style="display:flex;flex-wrap:wrap;gap:1rem;margin-bottom:1.5rem;">
			<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem;min-width:140px;text-align:center;">
				...
				<div style="color:#646970;"><?php esc_html_e( 'Total Templates', 'nvoos-graphify-ai-platform' ); ?></div>
			</div>
			<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem;min-width:140px;text-align:center;">
				<div style="font-size:2rem;font-weight:700;color:#2271b1;"><?php echo absint( count( $categories ) ); ?></div>
				<div style="color:#646970;"><?php esc_html_e( 'Categories', 'nvoos-graphify-ai-platform' ); ?></div>
			</div>
			<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem;min-width:140px;text-align:center;">
				<div style="font-size:2rem;font-weight:700;color:<?php echo $post_type_exists ? '#00a32a' : '#d63638'; ?>;">
					<?php echo $post_type_exists ? '✓' : '✗'; ?>
				</div>
				<div style="color:#646970;"><?php esc_html_e( 'CPT Registered', 'nvoos-graphify-ai-platform' ); ?></div>
			</div>
		</div>
		...
		<div style="display:flex;flex-wrap:wrap;gap:0.75rem;margin-bottom:1.5rem;">
			...
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=nvoos-graphify-ai-platform-add-agent' ) ); ?>" class="button">
				<?php esc_html_e( 'Create Agent from Template', 'nvoos-graphify-ai-platform' ); ?>
			</a>
		</div>

		<h3><?php esc_html_e( 'What are Professions?', 'nvoos-graphify-ai-platform' ); ?></h3>
		<div style="background:#fff;border:1px solid #c3c4c7;padding:1rem 1.5rem;margin-bottom:1.5rem;max-width:800px;">
			<p><?php esc_html_e( 'Professions are reusable agent templates. Each profession defines a role description, default AI provider/model, tool assignments, skill sets, and knowledge base. When you create a new agent, you can start from a profession template to jump-start configuration.', 'nvoos-graphify-ai-platform' ); ?></p>
			<p><?php esc_html_e( 'Professions are stored as posts in the base plugin and are shared across agents. Edit a profession to update all agents created from it.', 'nvoos-graphify-ai-platform' ); ?></p>
		</div>

		<?php if ( ! empty( $categories ) ) : ?>
			<h3><?php esc_html_e( 'Categories', 'nvoos-graphify-ai-platform' ); ?></h3>
		...
		<?php if ( ! empty( $professions ) ) : ?>
			<h3><?php esc_html_e( 'Recent Templates', 'nvoos-graphify-ai-platform' ); ?></h3>
			<table class="wp-list-table widefat fixed striped" style="max-width:900px;">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Profession', 'nvoos-graphify-ai-platform' ); ?></th>
						<th><?php esc_html_e( 'Category', 'nvoos-graphify-ai-platform' ); ?></th>
						<th><?php esc_html_e( 'Created', 'nvoos-graphify-ai-platform' ); ?></th>
						<th><?php esc_html_e( 'Actions', 'nvoos-graphify-ai-platform' ); ?></th>
					</tr>
				</thead>
				<tbody>
		...
		<?php elseif ( $post_type_exists ) : ?>
			<div style="background:#fff;border:1px solid #c3c4c7;padding:2rem;text-align:center;max-width:800px;">
				<h3><?php esc_html_e( 'No profession templates yet', 'nvoos-graphify-ai-platform' ); ?></h3>
				<p><?php esc_html_e( 'Create profession templates to jump-start agent creation with pre-configured roles, tools, and knowledge bases.', 'nvoos-graphify-ai-platform' ); ?></p>
				<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=mcp_ai_profession' ) ); ?>" class="button button-primary">
					<?php esc_html_e( 'Create First Profession', 'nvoos-graphify-ai-platform' ); ?>
				</a>
			</div>
		<?php endif; ?>
		<?php
