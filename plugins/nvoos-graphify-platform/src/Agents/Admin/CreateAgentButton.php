...
final class CreateAgentButton {
	...
	public function enqueueScripts( string $hook ): void {
		...
		wp_enqueue_style(
			'nvoos-graphify-platform-agents-create-button',
			NVOOS_GRAPHIFY_PLATFORM_URL . 'assets/css/agents-create-button.css',
			array(),
		...
		wp_enqueue_script(
			'nvoos-graphify-platform-agents-create-button',
			NVOOS_GRAPHIFY_PLATFORM_URL . 'assets/js/agents-create-button.js',
			array( 'jquery' ),
			NVOOS_GRAPHIFY_PLATFORM_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'buttonText' => __( 'Build AI Agent', 'nvoos-graphify-platform' ),
			)
		);
	}
	...
	public function handleAjaxCreate(): void {
		...
		if ( empty( $title ) || empty( $professions ) || empty( $regions ) ) {
			wp_send_json_error( array( 'message' => __( 'Title, professions, and regions are required.', 'nvoos-graphify-platform' ) ) );
		}

		// Use the create_assistant tool from the base plugin's tool registry.
		if ( ! class_exists( 'WP_MCP_AI_Tool_Registry' ) ) {
			wp_send_json_error( array( 'message' => __( 'Tool registry not available.', 'nvoos-graphify-platform' ) ) );
		}
		...
		if ( ! $tool ) {
			wp_send_json_error( array( 'message' => __( 'Create agent tool not available.', 'nvoos-graphify-platform' ) ) );
		}
	...
	public function handleAjaxBuildFromConversation(): void {
		...
		if ( empty( $conversation_json ) ) {
			wp_send_json_error( array( 'message' => __( 'No conversation data provided.', 'nvoos-graphify-platform' ) ) );
		}

		$conversation = json_decode( $conversation_json, true );

		if ( ! is_array( $conversation ) || empty( $conversation['messages'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid conversation data.', 'nvoos-graphify-platform' ) ) );
		}

		$assistant_config = self::extractConfigFromConversation( $conversation['messages'] );

		if ( empty( $assistant_config['title'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Could not determine a title for the agent. Please describe what you want the agent to be called.', 'nvoos-graphify-platform' ) ) );
		}
	...
	public function handleAjaxUploadAttachment(): void {
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions to upload files.', 'nvoos-graphify-platform' ) ) );
		}

		if ( empty( $_FILES['file'] ) ) {
			wp_send_json_error( array( 'message' => __( 'No file provided.', 'nvoos-graphify-platform' ) ) );
		}
		...
		if ( ! in_array( $file_ext, $allowed_types, true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid file type. Allowed types: txt, md, pdf, doc, docx.', 'nvoos-graphify-platform' ) ) );
		}
