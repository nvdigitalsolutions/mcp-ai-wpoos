...
class Test_NVOOS_SaaS_Controller_Audit_Log extends WP_UnitTestCase {
	...
	public function test_invalid_channel_falls_back_to_internal() {
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		$entry = $log->record( array( 'channel' => 'arbitrary_garbage', 'action' => 'x', 'status' => 'ok' ) );
		$this->assertSame( 'internal', $entry['channel'] );
	}

	public function test_invalid_status_falls_back_to_error() {
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		$entry = $log->record( array( 'channel' => 'internal', 'action' => 'x', 'status' => 'sideways' ) );
	...
	public function test_message_is_truncated_at_512_chars() {
		...
		$entry   = $log->record( array( 'channel' => 'internal', 'action' => 'x', 'status' => 'ok', 'message' => $payload ) );
		$this->assertSame( 512, strlen( $entry['message'] ) );
	}

	public function test_get_recent_returns_newest_first() {
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		$log->record( array( 'channel' => 'internal', 'action' => 'first', 'status' => 'ok' ) );
		$log->record( array( 'channel' => 'internal', 'action' => 'second', 'status' => 'ok' ) );
		$log->record( array( 'channel' => 'internal', 'action' => 'third', 'status' => 'ok' ) );
		$recent = $log->get_recent( 10 );
	...
	public function test_ring_buffer_trims_oldest_entries() {
		add_filter( 'nvoos_saas_controller_audit_log_max_entries', function () { return 3; } );
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		for ( $i = 0; $i < 5; $i++ ) {
			$log->record( array( 'channel' => 'internal', 'action' => 'a' . $i, 'status' => 'ok' ) );
		}
		$recent = $log->get_recent( 10 );
	...
	public function test_filter_can_suppress_entry() {
		...
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		$result = $log->record( array( 'channel' => 'internal', 'action' => 'x', 'status' => 'ok' ) );
	...
	public function test_clear_removes_all_entries() {
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		$log->record( array( 'channel' => 'internal', 'action' => 'x', 'status' => 'ok' ) );
	...
	public function test_corrupt_option_value_is_ignored() {
		...
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		$this->assertSame( 0, $log->count() );
		$entry = $log->record( array( 'channel' => 'internal', 'action' => 'x', 'status' => 'ok' ) );
		$this->assertNotNull( $entry );
		$this->assertSame( 1, $log->count() );
	}
	...
	public function test_get_recent_with_offset() {
		$log = NVOOS_SaaS_Controller_Audit_Log::instance();
		for ( $i = 0; $i < 5; $i++ ) {
			$log->record( array( 'channel' => 'internal', 'action' => 'item_' . $i, 'status' => 'ok' ) );
		}
