...
> **Status:** v0.1.0 — Phases 2, 3, 4, 5a & 5b landed (WP-Admin & REST plumbing + credentials wizard with live preflight + read-only Reconcile-Plan generator + audit log & smoke tester + HITL-gated Apply step). The remaining phase (5c — drift detector) will land in a follow-up PR.

## What's available today (Phases 2 / 3 / 4 / 5a / 5b)
...
- **Top-level admin menu** — `WP-Admin → NV oOS SaaS` (capability: `manage_options`) with four tabs:
  ...
  - **Operations** — **Apply (HITL-gated)** panel (Preview → Apply, single-use 15-minute token, per-resource result table), **Run Smoke Tests** button (live Cloudflare workers list + plan dry-run + base-plugin liveness), and the most recent 50 audit-log entries with a **Clear Audit Log** action.
...
- **Worker upload (Phase 5d)** — multipart upload of the built `worker/dist/index.js` plus bindings metadata, behind the same HITL token gate as Phase 5b.
