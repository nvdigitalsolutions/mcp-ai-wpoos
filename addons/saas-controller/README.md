...
> **Status:** v0.1.0 — Phases 2, 3, 4, 5a, 5b & 5c landed (WP-Admin & REST plumbing + credentials wizard with live preflight + read-only Reconcile-Plan generator + audit log & smoke tester + HITL-gated Apply step + drift detector). The remaining phase (5d — Worker upload) will land once the build pipeline produces `worker/dist/index.js`.

## What's available today (Phases 2 / 3 / 4 / 5a / 5b / 5c)
...
- **Top-level admin menu** — `WP-Admin → NV oOS SaaS` (capability: `manage_options`) with four tabs:
  ...
  - **Operations** — **Apply (HITL-gated)** panel (Preview → Apply, single-use 15-minute token, per-resource result table), **Drift Detector** panel (Run Drift Check → colour-coded synced/drift/unknown/error verdict + pinned-vs-deployed fingerprint table), **Run Smoke Tests** button (live Cloudflare workers list + plan dry-run + base-plugin liveness), and the most recent 50 audit-log entries with a **Clear Audit Log** action. A red drift banner is also rendered admin-wide on every NV oOS SaaS screen whenever the cached drift state is `drift`.
...
- **Mutating Cloudflare client** (`NVOOS_SaaS_Controller_Cloudflare_Mutating_Client`) — Phase 5b companion that exposes only the writes the Apply step needs: `POST …/d1/database`, `POST …/storage/kv/namespaces`, `POST …/ai-gateway/gateways`. Each call records exactly one entry in the audit log (success or failure). Worker script upload is intentionally out of scope until Phase 5d (which depends on the built `worker/dist/index.js` artefact).
- **Apply engine** (`NVOOS_SaaS_Controller_Apply_Engine`) — consumes a plan and applies its `creates[]` to Cloudflare under a single-use HITL token. The token is a 32-byte URL-safe random string; only its SHA-256 hash is persisted (as a transient with a 15-minute default TTL, filterable via `nvoos_saas_controller_apply_token_ttl`). Returns per-resource result rows (`{ kind, target, status: ok|error|skipped, message, detail? }`) plus an aggregate summary. `plan.updates[]` rows and `kind=worker` creates are marked `skipped` with an explanatory message — Phase 5d will land Worker upload. `plan.orphans[]` are never deleted automatically.
- **Drift detector** (`NVOOS_SaaS_Controller_Drift_Detector`) — compares the deployed Cloudflare Worker against the addon's pinned `worker/dist/index.js` fingerprint. Reads the pin from `worker/drift-manifest.json` (`{ expected_sha256, expected_etag, version, built_at, worker_dist_path }`). Comparison precedence is **etag** (Cloudflare's own content fingerprint) then **sha256** (response-body hash). Returns `{ ok, status: synced|drift|unknown|error, worker_name, expected_sha256, expected_etag, actual_sha256, actual_etag, message, duration_ms, ts }`, persists the result in option `nvoos_saas_controller_last_drift_check`, and records one audit-log entry per run on channel `internal`. Until Phase 5d's build pipeline populates the manifest, both fingerprints are `null` and the detector returns `status=unknown` (never `drift`) — guaranteeing no false-positive banners on a fresh install.
...
- **Worker upload (Phase 5d)** — multipart upload of the built `worker/dist/index.js` plus bindings metadata, behind the same HITL token gate as Phase 5b. Will also populate `worker/drift-manifest.json` so the Phase 5c drift detector flips from `status=unknown` to `status=synced` on the first successful upload.
