/**
 * esbuild configuration for the NV oOS Comic Reader addon.
 *
 * Produces:
 *   assets/dist/comic-reader.js   — IIFE bundle
 *   assets/dist/comic-reader.css  — Extracted stylesheet
 *
 * @since 0.1.0
 */
...
const buildOptions = {
	...
	bundle:      true,
	outfile:     path.join( outDir, 'comic-reader.js' ),
	format:      'iife',
	globalName:  'NVoOSComicReader',
	platform:    'browser',
	...
	plugins:     [
		wpI18nPlugin,
		{
			...
			setup( build ) {
				build.onEnd( () => {
					const def = path.join( outDir, 'comic-reader.css' );
...
if ( isWatch ) {
	esbuild.context( buildOptions ).then( ( ctx ) => {
		ctx.watch();
		console.log( '[comic-reader] Watching for changes…' );
	} );
} else {
