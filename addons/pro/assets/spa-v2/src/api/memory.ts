/**
 * Pro SPA v2 — typed wrappers around `mcp-ai/v1/chat-memory`.
 *
...
export function memoryScopeStorageKey( assistantId: number | string ): string {
	return `nvoos-pro-spa.memory-scope.${ assistantId || 0 }`;
}
...
export class MemoryClient {
	...
	private async request< T = unknown >(
		...
		if ( ! response.ok ) {
			...
			throw new Error(
				detail ||
					__( 'Memory request failed (status %d).', 'nvoos-pro-spa' ).replace(
						'%d',
...
/**
 * Extract contexts from REST success wrapper.
 */
