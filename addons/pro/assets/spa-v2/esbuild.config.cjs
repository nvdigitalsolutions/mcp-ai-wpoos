/**
 * esbuild configuration for NV oOS Pro SPA v2 addon.
 *
 * Produces:
 *   assets/dist/pro-spa.js   — IIFE bundle
 *   assets/dist/pro-spa.css  — Extracted stylesheet
 *
 * @since 2.0.0
 */
...
const buildOptions = {
	...
	bundle:      true,
	outfile:     path.join( outDir, 'pro-spa.js' ),
	format:      'iife',
	globalName:  'NVoOSProSpa',
	platform:    'browser',
	...
	plugins:     [
		wpI18nPlugin,
		{
			...
			setup( build ) {
				build.onEnd( () => {
					const def = path.join( outDir, 'pro-spa.css' );
...
if ( isWatch ) {
	esbuild.context( buildOptions ).then( ( ctx ) => {
		ctx.watch();
		console.log( '[pro-spa-v2] Watching for changes…' );
	} );
} else {
