...
( function ( $ ) {
	...
	var config = window.wpMcpAiCrmBlueprints || {};
	...
	function installBlueprint( slug, $btn, $spinner ) {
		...
		$.post(
			config.ajaxUrl,
			{
				action: 'wp_mcp_ai_crm_install_blueprint',
				blueprint_slug: slug,
				nonce: config.nonce,
			}
		)
			.done( function ( response ) {
				if ( response.success ) {
					...
					var $card = $btn.closest( '.crm-bp-card' );
					$card.find( '.crm-bp-meta' ).append(
						'<span class="crm-bp-tag installed">' + config.i18n.installed + '</span>'
					);
	...
	function toggleDetails( slug, $btn ) {
		var $card    = $btn.closest( '.crm-bp-card' );
		var $details = $card.find( '.crm-bp-details' );
		...
		$.post(
			config.ajaxUrl,
			{
				action: 'wp_mcp_ai_crm_get_blueprint_details',
				blueprint_slug: slug,
				nonce: config.nonce,
			}
		)
			.done( function ( response ) {
				...
				if ( response.success && response.data ) {
					...
					html += '<div class="crm-bp-details" style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #c3c4c7;">';
					...
					if ( bp.tools && bp.tools.length ) {
						...
						$.each( bp.tools, function ( i, tool ) {
							html += '<span class="crm-bp-tag" style="font-size: 11px;">' + escHtml( tool ) + '</span>';
						} );
						html += '</div>';
					}
				...
				$card.append( html );
				$card.find( '.crm-bp-details' ).slideDown( 200 );
	...
	$( document ).on( 'click', '.crm-bp-install-btn', function () {
		var $btn     = $( this );
		var slug     = $btn.data( 'slug' );
		var $card    = $btn.closest( '.crm-bp-card' );
		var $spinner = $card.find( '.crm-bp-spinner' );

		installBlueprint( slug, $btn, $spinner );
	} );

	$( document ).on( 'click', '.crm-bp-view-btn', function () {
		var $btn = $( this );
		var slug = $btn.data( 'slug' );
