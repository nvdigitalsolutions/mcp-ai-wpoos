...
class Test_Healthcare_Imaging extends WP_UnitTestCase {
	...
	public function setUp(): void {
		...
		if ( ! post_type_exists( 'mcp_ai_imaging_study' ) ) {
			register_post_type( 'mcp_ai_imaging_study', array( 'public' => false, 'label' => 'imaging' ) );
		}
	...
	private function make_study( $args = array() ) {
		$args = wp_parse_args(
			...
			array(
				...
				'patient_id'  => 'PAT-1',
				'series'      => array(
					array( 'series_instance_uid' => '1.2.3.4.5.6', 'modality' => 'CT', 'description' => 'axial', 'instance_count' => 50 ),
				),
			)
		);
		$post_id = self::factory()->post->create( array( 'post_type' => 'mcp_ai_imaging_study', 'post_title' => $args['description'] ) );
	...
	public function test_compare_imaging_studies_diff() {
		$a = $this->make_study( array( 'modality' => 'CT', 'study_date' => '20260101' ) );
		$b = $this->make_study(
			array(
				'modality'   => 'CT',
				'study_date' => '20260201',
				'series'     => array(
					array( 'series_instance_uid' => 'a', 'instance_count' => 50 ),
					array( 'series_instance_uid' => 'b', 'instance_count' => 30 ),
				),
			)
		);

		$tool = new WP_MCP_AI_Tool_Compare_Imaging_Studies();
		$out  = $tool->execute( array( 'prior_study_id' => $a, 'current_study_id' => $b ) );
	...
	public function test_compare_imaging_studies_missing() {
		$tool = new WP_MCP_AI_Tool_Compare_Imaging_Studies();
		$out  = $tool->execute( array( 'prior_study_id' => 9999, 'current_study_id' => 9998 ) );
		$this->assertWPError( $out );
	}
	...
	public function test_attach_radiology_report_missing_body() {
		$study_id = $this->make_study();
		$tool     = new WP_MCP_AI_Tool_Attach_Radiology_Report();
		$out      = $tool->execute( array( 'study_id' => $study_id, 'findings' => '', 'impression' => '' ) );
		$this->assertWPError( $out );
	}
	...
	public function test_hanging_protocol_filter() {
		add_filter(
			...
			static function ( $p ) {
				$p['CT'] = array( 'name' => 'site override', 'layout' => '4x1', 'stages' => array() );
				return $p;
			}
		);
		$tool = new WP_MCP_AI_Tool_Get_Imaging_Hanging_Protocol();
