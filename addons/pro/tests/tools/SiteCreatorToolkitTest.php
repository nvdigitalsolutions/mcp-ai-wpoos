<?php
/**
 * Batch test for Site Creator Toolkit Pro tools.
 *
 * @package WP_MCP_AI_Pro
 * @subpackage Tests
 * @group tools
 * @group pro
 * @group site-creator-toolkit
 */

class SiteCreatorToolkitTest extends WP_UnitTestCase {

	/** @var int */
	protected $user_id;

	/** @var array */
	protected static $dirs = array( 'site-creator-toolkit' );
	...
	public static function provide_all_tools() {
		$d = array();
		if ( ! defined( 'WP_MCP_AI_PRO_PATH' ) ) {
			// Data providers run before setUp(); guard against missing constant.
			return $d;
		}
	...
	protected function safe_new( $cn ) {
		try {
			...
			// Catch both \ReflectionException and \Error (e.g. missing traits).
			$this->markTestSkipped();
			return null;
		}
	}
