	...
	class WP_MCP_AI_EZuite_CCT_Manager {
		...
		protected function sync_jetengine_field_definitions( $missing_columns ) {
			...
			$request['name']        = ! empty( $existing_args['name'] )
				? $existing_args['name']
				: __( 'EZuite Inventory', 'mcp-ai-wpoos-pro' );
		...
		public function get_cached_items( $filters = array() ) {
			...
			foreach ( array( 'sku', 'warehouse', 'supplier', 'connection_id' ) as $exact_field ) {
			...
			$numeric_order = array( 'quantity', 'reorder_point', 'woo_product_id', 'cost_price' );
		...
		public function upsert( $ezuite_item ) {
			...
			if ( $existing && ! empty( $existing['_ID'] ) ) {
				$ezuite_item['_ID'] = absint( $existing['_ID'] );
			}
			...
			$result = $handler->update_item( $ezuite_item );
			...
			if ( ! $result ) {
				return new WP_Error(
					'wp_mcp_ai_ezuite_upsert_failed',
					__( 'Failed to write EZuite item to the CCT cache.', 'mcp-ai-wpoos-pro' )
				);
			}
		...
		public function update_woo_product_id( $cct_item_id, $woo_product_id ) {
			...
			$item = $factory->db->get_item( $cct_item_id );
			if ( ! $item ) {
				return new WP_Error(
			...
			if ( ! $handler ) {
				return new WP_Error(
					'wp_mcp_ai_ezuite_jetengine_not_ready',
					__( 'JetEngine CCT item handler is not available.', 'mcp-ai-wpoos-pro' )
				);
			}
		...
		public static function maybe_register_cct() {
			...
			$toolkit_settings = get_option( 'wp_mcp_ai_ezuite_toolkit_settings', array() );
			...
			// lock forever.
			$lock_key = 'wp_mcp_ai_ezuite_cct_creation_lock_' . $slug;
		...
		protected static function maybe_cleanup_duplicate_ccts( $module, $slug ) {
			...
			$flag = 'wp_mcp_ai_ezuite_cct_dup_cleanup_' . $slug;
					...
					if ( function_exists( 'wp_mcp_ai_log' ) ) {
						wp_mcp_ai_log(
							sprintf(
								...
								__( 'EZuite duplicate CCT cleaned up: removed %1$s row #%2$d, kept #%3$d.', 'mcp-ai-wpoos-pro' ),
								$slug,
								$dup_id,
								$keep_id
							),
