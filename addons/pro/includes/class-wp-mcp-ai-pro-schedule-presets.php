		...
		public static function get_media_presets() {
			return array(
				...
				'weekly_blog_topic_research' => array(
					...
					'schedule_data' => array(
						'assistant_config' => array(
							'message' => 'Research trending blog topics for the coming week. STEP 1: Use get_recent_posts (status=publish, limit=20) to review what has already been published on this site. Note the topics, angles, and formats already covered. STEP 2: Use research_blog_post to explore current industry trends, competitor content, and keyword opportunities. STEP 3: Cross-reference external research against existing content — AVOID suggesting topics that overlap significantly with already-published posts. Instead, prioritise: content gaps where the site has no coverage, fresh angles that complement (not repeat) existing posts, adjacent subjects that build on what is already published, and internal linking opportunities to related existing content. Analyse: 1) Trending topics in our niche with search volume estimates. 2) Content gaps our competitors are not covering. 3) Seasonal or timely topics relevant to our audience. 4) Long-tail keyword opportunities with low competition. 5) Suggested titles with primary and secondary keywords — flag any that overlap with existing site content so the editor can see the distinction. 6) Recommended content format (how-to, listicle, case study, opinion, comparison) for each topic. Generate a prioritised editorial brief for 3-5 blog post ideas with target word count, suggested headings, primary image recommendations, and a suggested DALL-E prompt for each topic\'s featured image.',
						),
				...
				'weekly_blog_post_writer'    => array(
					...
					'schedule_data' => array(
						'assistant_config' => array(
							'message' => 'Draft a complete blog post for this week. STEP 1: Use get_recent_posts (status=publish, limit=20) to review existing content and identify a topic that fills a genuine gap or offers a fresh angle — avoid repeating what has already been published. STEP 2: Use research_blog_post to perform deep research on the selected topic. STEP 3: BEFORE creating the post, use generate_openai_image to create a professional featured image — prompt: "Professional blog featured image for: {your article title}", size: "1792x1024", model: "dall-e-3", quality: "hd". Capture the attachment_id from the result. STEP 4: Use create_post with featured_image_id set to the attachment_id from Step 3 to generate a publish-ready draft. The post must include: 1) Compelling SEO-optimised title (H1) with primary keyword. 2) Meta description (150-160 chars) with call-to-action. 3) Introduction that hooks the reader with a statistic, question, or story. 4) Well-structured body with H2/H3 subheadings following the inverted pyramid. 5) 3-5 data points or statistics with citations. 6) Suggested image placements with alt-text descriptions. 7) Internal links to 2-3 related posts (reference the actual post titles/URLs discovered in STEP 1). 8) Schema.org Article markup. 9) Author bio snippet. 10) Call-to-action at the end. Set post status to draft for human review before publishing.',
						),
				...
				'blog_editorial_calendar'    => array(
					...
					'schedule_data' => array(
						'assistant_config' => array(
							'message' => 'Generate a 30-day editorial calendar for the blog. STEP 1: Use get_recent_posts (status=publish, limit=30) to review existing content and identify which topics, formats, and angles have already been covered recently. STEP 2: Use research_blog_post to explore current industry trends, competitor publishing patterns, and seasonal content opportunities. STEP 3: Cross-reference external research against existing content — avoid scheduling topics that overlap with posts already published. Instead, plan topics that fill genuine gaps or offer fresh angles that complement (not repeat) existing content. Structure around the topic cluster model: 1) Identify 2-3 pillar content topics for the month that the site has not recently covered. 2) Map 6-8 cluster posts that link to each pillar — reference specific existing posts where internal links make sense. 3) Assign a publishing date and time for each post (recommend Tue-Thu for B2B, Mon-Wed for B2C). 4) Include content type (how-to, listicle, case study, news, opinion). 5) Assign target keywords and search intent (informational, commercial, transactional, navigational). 6) Note seasonal hooks, industry events, or product launches. 7) Suggest social media promotion angles for each post. 8) Identify repurposing opportunities (infographic, video, podcast, email newsletter). Output as a structured calendar view with status tracking.',
						),
						'result_delivery'  => array(
							'on_success' => array(
								'channels' => array(
				...
				'content_gap_analysis'       => array(
					...
					'schedule_data' => array(
						'assistant_config' => array(
							'message' => 'Perform a weekly content gap analysis against 3-5 key competitors. STEP 1: Use get_recent_posts (status=publish, limit=50) and search_content to build an inventory of topics, keywords, and content formats already covered on this site. This is essential for accurately identifying genuine gaps. STEP 2: Use research_blog_post to explore competitor content and identify what they cover that we do not. Identify: 1) Keywords competitors rank for (top 20) that we do not — cross-reference against the site inventory from STEP 1. 2) Topics with high search volume and low competition where we have no content — verify against the inventory rather than assuming. 3) Content formats competitors use that we do not (video, infographics, podcasts, tools, calculators). 4) Questions people ask in our niche that no one is answering well (People Also Ask opportunities). 5) Content depth comparison — topics where competitor content is more comprehensive than our existing coverage. 6) Quick-win opportunities (low-effort, high-impact topics we can produce within a week). Prioritise gaps by potential traffic impact and content effort. Generate a content roadmap for the next 30 days.',
						),
						'result_delivery'  => array(
							'on_success' => array(
								'channels' => array(
