<?php
/**
 * JetEngine Custom Content Type registration for vital signs.
 *
 * Stores structured vital-sign measurements (blood pressure, heart rate,
 * temperature, weight/BMI, glucose, SpO2, respiratory rate, and kidney
 * indicators) as first-class CCT items when JetEngine is active.  Falls back
 * gracefully to options-based storage when JetEngine is absent.
 *
...
/**
 * Provision and interact with the vital_signs CCT.
 */
class WP_MCP_AI_JetEngine_Vitals_CCT {

	/**
	 * CCT slug.
	 */
	const SLUG = 'vital_signs';

	/**
	 * Base ID for meta field identifiers (43000 range).
	 */
	const FIELD_ID_BASE = 43000;
	...
	/**
	 * Insert a vital-signs record into the CCT.
	 *
	...
	/**
	 * Retrieve vital-sign records for a member, optionally filtered by date.
	 *
	...
	/**
	 * Retrieve the most recent vital-signs record for a member.
	 *
	 * @param int $member_id Member post ID.
	...
	protected static function get_registration_request() {
		$label = __( 'Vital Signs', 'mcp-ai-wpoos-pro' );
	...
	/**
	 * Field schema for the vital_signs CCT.
	 *
	 * @return array
	 */
	protected static function get_fields_schema() {
		...
		return array(
			...
			array(
				'id'          => $b + 7,
				'title'       => __( 'Legacy Entry ID', 'mcp-ai-wpoos-pro' ),
				'name'        => 'entry_id',
				'type'        => 'text',
				'search'      => true,
				'width'       => '50%',
				'default_val' => '',
				'description' => __( 'Original options-storage entry ID (vs_TIMESTAMP_RAND) for backward compatibility', 'mcp-ai-wpoos-pro' ),
			),
