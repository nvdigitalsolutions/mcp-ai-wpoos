...
class WP_MCP_AI_Chat_Channels_REST_Controller extends WP_REST_Controller {
	...
	protected function get_conversation_messages_from_cct( $contact_id, $page, $per_page, $include_metadata ) {
		...
		// When a connection_id is available, scope the query to that specific
		// connection so that conversations from different bot connections (e.g.
		// multiple Telegram bots) stay separated in the inbox. Fall back to a
		// channel + channel_contact_id only query for legacy contacts that have
		// no connection_id stored.
		if ( '' !== $connection_id ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$total = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$messages_table} WHERE channel = %s AND channel_contact_id = %s AND connection_id = %s", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name from trusted CCT helper.
					$contact['channel'],
			...
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$messages_table} WHERE channel = %s AND channel_contact_id = %s AND connection_id = %s ORDER BY message_timestamp ASC LIMIT %d OFFSET %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name from trusted CCT helper.
					$contact['channel'],
					$contact['channel_contact_id'],
					$connection_id,
					$per_page,
	 ...
	 *
	 * When a non-empty connection_id is provided the query is scoped to that
	 * specific connection so that conversations from different bot connections
	 * (e.g. multiple Telegram bots) are kept separate in the inbox.
	 *
	...
	protected function fetch_messages_from_cct( $channel, $channel_contact_id, $include_metadata = false, $connection_id = '' ) {
		...
		if ( '' !== $connection_id ) {
			...
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$messages_table} WHERE channel = %s AND channel_contact_id = %s AND connection_id = %s ORDER BY message_timestamp ASC", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$channel,
					$channel_contact_id,
					$connection_id
				),
	 ...
	 * When a non-empty connection_id is provided the query is scoped to that
	 * specific connection so that conversations from different bot connections
	 * are kept separate in the inbox.
	 *
	 * @param string $channel            Platform slug.
	 * @param string $channel_contact_id Platform contact identifier.
	 * @param bool   $include_metadata   Whether to include raw payload.
