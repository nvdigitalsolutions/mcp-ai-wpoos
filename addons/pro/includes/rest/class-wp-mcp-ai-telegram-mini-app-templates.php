 ...
 *
 * A Shopify-powered alternative to the WooCommerce-based E-Commerce Store
 * template.  Uses the `shopify_products` and `shopify_orders` tools via the
 * Remote Sites infrastructure.  Same five-tab layout (Shop, Cart, Orders, AI,
 * Settings) rendered entirely inline – no React build step required.
 *
 * Features:
 *  - Product catalog with debounced search
 *  - Shopify collection filter chips
 *  - Cart with quantity controls
 *  - Checkout via AI assistant (no direct Shopify order-creation API)
 *  - Shopify order history with financial + fulfilment status badges
 *  - Monthly spending chart (Chart.js, lazy-loaded)
 *  - AI shopping assistant chat
...
class WP_MCP_AI_TMA_Template_Shopify_Ecommerce extends WP_MCP_AI_Telegram_Mini_App_Template_Base {
	...
	/** @inheritdoc */
	public function get_description() {
		return __( 'Shop assistant with product search, collection filters, order tracking, and AI-powered recommendations. Designed for Shopify stores connected via Remote Sites.', 'mcp-ai-wpoos-pro' );
	}
	...
	public function render_html( array $ctx ) {
		...
		/* ── Theme variables (Shopify blue) ── */
		':root{--tma-btn:#5c6ac4;--tma-accent:#5c6ac4;--tma-secondary-bg:#f4f5fa;' .
			'--ec-base:14px;--ec-label:12px;--ec-heading:16px;}' .
		...
		'.ec-collection-bar{display:flex;gap:6px;padding:8px 12px;overflow-x:auto;-webkit-overflow-scrolling:touch;background:var(--tma-bg)}' .
		'.ec-collection-bar::-webkit-scrollbar{display:none}' .
		'.ec-chip{flex:0 0 auto;padding:6px 14px;border:1px solid var(--tma-border);border-radius:20px;' .
			'font-size:var(--ec-label);cursor:pointer;background:var(--tma-bg);color:var(--tma-text);white-space:nowrap}' .
		'.ec-chip.active{background:var(--tma-btn);color:var(--tma-btn-text);border-color:var(--tma-btn)}' .
		'.ec-chip:active{opacity:.7}' .
		...
		'.tma-order-meta{font-size:var(--ec-label);color:var(--tma-hint)}' .
		'.ec-status-badge{font-size:10px;padding:2px 8px;border-radius:10px;font-weight:600;border:1px solid transparent}' .
		'.ec-status-paid{background:#e8f5e9;color:#2e7d32;border-color:#a5d6a7}' .
		'.ec-status-authorized{background:#e3f2fd;color:#1565c0;border-color:#90caf9}' .
		'.ec-status-pending{background:#fff3e0;color:#e65100;border-color:#ffcc80}' .
		'.ec-status-refunded,.ec-status-voided{background:#ffebee;color:#c62828;border-color:#ef9a9a}' .
		'.ec-status-fulfilled{background:#e8f5e9;color:#2e7d32;border-color:#a5d6a7}' .
		'.ec-status-partial{background:#e3f2fd;color:#1565c0;border-color:#90caf9}' .
		'.ec-status-unfulfilled{background:#fff3e0;color:#e65100;border-color:#ffcc80}' .
		'.ec-pull-hint{text-align:center;font-size:var(--ec-label);color:var(--tma-hint);padding:8px 0}' .
		...
		'.ec-connection-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}' .
		'.ec-connection-dot.online{background:#2e7d32}' .
		'.ec-connection-dot.offline{background:#c62828}' .
		...
		'<div class="tma-shell" id="tma-shell">' .
			...
			'<header class="tma-header">' .
				...
				'<div class="tma-header-info">' .
					'<div class="tma-header-name">' . $site_name . '</div>' .
					'<div class="tma-header-status" id="ec-header-status">' . esc_html__( 'Shopify Store', 'mcp-ai-wpoos-pro' ) . '</div>' .
				'</div>' .
			'</header>' .
			...
			/* ── Collection filter chips ── */
			'<div class="ec-collection-bar" id="ec-collection-bar" style="display:none"></div>' .
		...
		'function spImgUrl(p){' .
			'if(p.images){' .
				'if(p.images.edges&&p.images.edges[0])return p.images.edges[0].node.url||p.images.edges[0].node.src||"";' .
				'if(p.images.nodes&&p.images.nodes[0])return p.images.nodes[0].url||p.images.nodes[0].src||"";' .
				'if(Array.isArray(p.images)&&p.images[0])return p.images[0].url||p.images[0].src||"";' .
			'}' .
			'if(p.image)return p.image.url||p.image.src||"";' .
			'return "";' .
		'}' .

		'function spVariant(p){' .
			'var v=null;' .
			'if(p.variants){' .
				'if(p.variants.edges&&p.variants.edges[0])v=p.variants.edges[0].node;' .
				'else if(p.variants.nodes&&p.variants.nodes[0])v=p.variants.nodes[0];' .
				'else if(Array.isArray(p.variants)&&p.variants[0])v=p.variants[0];' .
			'}' .
			'return v;' .
		'}' .

		'function spPrice(p){' .
			'var v=spVariant(p);if(!v)return 0;' .
			'if(v.priceV2)return parseFloat(v.priceV2.amount)||0;' .
			'return parseFloat(v.price)||0;' .
		'}' .

		'function spComparePrice(p){' .
			'var v=spVariant(p);if(!v)return 0;' .
			'if(v.compareAtPriceV2)return parseFloat(v.compareAtPriceV2.amount)||0;' .
			'if(v.compareAtPrice)return parseFloat(v.compareAtPrice)||0;' .
			'return 0;' .
		'}' .
		...
		'function spInStock(p){' .
			'var inv=p.totalInventory||p.total_inventory;' .
			'if(typeof inv==="number")return inv>0;' .
			'if(p.availableForSale===false)return false;' .
			'return true;' .
		'}' .

		'function spVariantId(p){' .
			'var v=spVariant(p);' .
			'return v?v.id||"":"";' .
		'}' .

		/* Normalize tool response: controller returns {success,result} */
		...
		/* ── Session init ── */
		'function ecInitSession(){' .
			'if(!VALIDATE_URL||!window.Telegram||!window.Telegram.WebApp){ecBootstrap();return;}' .
			'var initData=window.Telegram.WebApp.initData;' .
			'if(!initData){ecBootstrap();return;}' .
			'fetch(VALIDATE_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({init_data:initData})})' .
			'.then(function(r){return r.ok?r.json():null;})' .
			'.then(function(d){if(!d){ecBootstrap();return;}if(d.wp_nonce){NONCE=d.wp_nonce;}if(d.tma_token){TMA_TOKEN=d.tma_token;}ecBootstrap();})' .
		...
		'function ecApplyDisplaySettings(){' .
			...
			'}catch(e){}' .
		'}' .
		'window.ecSetFontSize=function(s){lsSet("sec_font_size",s);tmaHaptic("selectionChanged");ecApplyDisplaySettings();};' .
		'window.ecToggleCompact=function(){var c=!lsGet("sec_compact",false);lsSet("sec_compact",c);tmaHaptic("selectionChanged");ecApplyDisplaySettings();};' .
		...
		'window.ecSwitch=function(tab){' .
			...
			'if(pane)pane.classList.add("tma-active");if(btn)btn.classList.add("tma-active");' .
			'var sb=document.getElementById("ec-search-bar");if(sb)sb.style.display=tab==="products"?"":"none";' .
			'var cb=document.getElementById("ec-collection-bar");if(cb)cb.style.display=tab==="products"&&collectionsCache.length?"flex":"none";' .
			'activeTab=tab;' .
			'if(tab==="orders")ecLoadOrders(false);' .
			'if(tab==="cart")ecRenderCart();' .
			'if(tab==="assistant"&&!chatHist.length)ecChatInit();' .
		...
		/* ══════════════════════════════════════════════════════════
		   Collections
		   ══════════════════════════════════════════════════════════ */
		'function ecLoadCollections(){' .
			'ecToolCall("shopify_products",{action:"collections",first:25},function(err,d){' .
				'if(err)return;' .
				'var cols=spExtract(d,"collections");' .
				'if(!cols||!Array.isArray(cols)){cols=[];}' .
				'collectionsCache=cols;lsSet("sec_collections",cols);' .
				'ecRenderCollections();' .
			'});' .
		'}' .

		'function ecRenderCollections(){' .
			'var bar=document.getElementById("ec-collection-bar");if(!bar)return;' .
			'if(!collectionsCache.length){bar.style.display="none";return;}' .
			'bar.style.display="flex";' .
			'var html=\'<button class="ec-chip' . ( '' === '' ? ' active' : '' ) . '" onclick="ecFilterCollection(\\\'\\\')">' .
				esc_js( __( 'All', 'mcp-ai-wpoos-pro' ) ) . '</button>\';' .
			'collectionsCache.forEach(function(c){' .
				'var id=c.id||"";var title=c.title||c.name||"";' .
				'var cls=(activeCollection===id)?" active":"";' .
				'html+=\'<button class="ec-chip\'+cls+\'" onclick="ecFilterCollection(\\\'\'+escH(id)+\'\\\')">\'+escH(title)+\'</button>\';' .
			'});' .
			'bar.innerHTML=html;' .
		'}' .

		'window.ecFilterCollection=function(id){' .
			'activeCollection=id;tmaHaptic("selectionChanged");' .
			'ecRenderCollections();' .
			'if(id){' .
				'var col=collectionsCache.find(function(c){return c.id===id;});' .
				'var q=col?(col.title||""):"";' .
				'ecLoadProducts(q);' .
			'}else{ecLoadProducts();}' .
		'};' .
		...
		'function ecLoadProducts(q){' .
			...
			'if(!q&&productsCache.length){ecRenderProducts(productsCache);}' .
			'if(!q&&!productsCache.length)g.innerHTML=\'<div class="tma-empty" style="grid-column:span 2">' . esc_js( __( 'Loading…', 'mcp-ai-wpoos-pro' ) ) . '</div>\';' .
			'var args={action:q?"search":"list",first:20};' .
			'if(q)args.query=q;' .
			'ecToolCall("shopify_products",args,function(err,d){' .
				'if(err){g.innerHTML=\'<div class="tma-empty" style="grid-column:span 2">' . esc_js( __( 'Could not load products.', 'mcp-ai-wpoos-pro' ) ) . '</div>\';return;}' .
				'var ps=spExtract(d,"products");' .
				'if(!ps||!Array.isArray(ps))ps=[];' .
				'if(ps.length&&!shopCurrency){shopCurrency=spCurrency(ps[0]);}' .
				'if(!q){productsCache=ps;lsSet("sec_products_cache",ps);}' .
				'ecRenderProducts(ps);' .
			'});' .
		'}' .
		...
		'function ecRenderProducts(ps){' .
			...
			'if(!ps.length){g.innerHTML=\'<div class="tma-empty" style="grid-column:span 2">' . esc_js( __( 'No products found.', 'mcp-ai-wpoos-pro' ) ) . '</div>\';return;}' .
			'g.innerHTML=ps.map(function(p,i){' .
				'var imgUrl=spImgUrl(p);' .
				'var img=imgUrl?"<img src=\\""+escH(imgUrl)+"\\" alt=\\"\\"/>":"🛍️";' .
				'var stock=spInStock(p);' .
				'var stockCls=stock?"ec-stock-instock":"ec-stock-outofstock";' .
				'var stockTxt=stock?"' . esc_js( __( 'In Stock', 'mcp-ai-wpoos-pro' ) ) . '":"' . esc_js( __( 'Out of Stock', 'mcp-ai-wpoos-pro' ) ) . '";' .
				'var price=spPrice(p);var cmp=spComparePrice(p);' .
				'var cur=shopCurrency||spCurrency(p)||"";' .
				'var priceHtml="";' .
				'if(cmp&&cmp>price){' .
					'priceHtml=escH(cur+" "+price.toFixed(2))+"<span class=\\"ec-sale-price\\">"+escH(cur+" "+cmp.toFixed(2))+"</span>";' .
				'}else{' .
				...
				'return \'<div class="tma-product-card">' .
					...
					'<div class="tma-product-img">\'+img+\'</div>' .
					'<div class="tma-product-body">' .
						'<div class="tma-product-name">\'+escH(p.title||p.name||"Product")+\'</div>' .
						'<div class="tma-product-price">\'+priceHtml+\'</div>' .
						'\'+addBtn+\'' .
					'</div></div>\';' .
			'}).join("");' .
		...
		'document.getElementById("ec-search-input").addEventListener("input",function(e){' .
			'clearTimeout(searchTimer);var q=e.target.value;' .
			'searchTimer=setTimeout(function(){activeCollection="";ecRenderCollections();ecLoadProducts(q);},400);' .
		'});' .
		...
		'window.ecAddToCart=function(idx){' .
			'var p=productsCache[idx];if(!p)return;tmaHaptic("light");' .
			'var pid=p.id||"";' .
			'var existing=cart.find(function(c){return c.id===pid;});' .
			'if(existing){existing.qty+=1;}else{' .
				'var price=spPrice(p);' .
				'cart.push({id:pid,variantId:spVariantId(p),name:p.title||p.name||"Product",price:price,qty:1,' .
					'image:spImgUrl(p),currency:spCurrency(p)||shopCurrency||""});' .
			'}' .
			'lsSet("sec_cart",cart);ecUpdateCartBadge();' .
		'};' .
		...
		'function ecRenderCart(){' .
			...
			'if(!cart.length){' .
				...
				'if(totEl)totEl.style.display="none";if(chkEl)chkEl.style.display="none";return;' .
			'}' .
			'var total=0;var cur=cart[0].currency||shopCurrency||"";' .
			'l.innerHTML=cart.map(function(c,i){' .
				'total+=c.price*c.qty;' .
				'return \'<div class="ec-cart-item">' .
					'<div class="ec-cart-item-info"><div class="ec-cart-item-name">\'+escH(c.name)+\'</div>' .
					'<div class="ec-cart-item-price">\'+escH((c.currency||cur)+" "+c.price.toFixed(2))+\' × \'+c.qty+\'</div></div>' .
					'<div class="ec-cart-qty">' .
						'<button onclick="ecCartQty(\'+i+\',-1)">−</button>' .
						'<span>\'+c.qty+\'</span>' .
						'<button onclick="ecCartQty(\'+i+\',1)">+</button>' .
				...
				'</div>\';' .
			'}).join("");' .
			'if(totEl){totEl.style.display="flex";document.getElementById("ec-cart-total-val").textContent=(cur?cur+" ":"")+total.toFixed(2);}' .
			'if(chkEl)chkEl.style.display="block";' .
		'}' .
		...
		'window.ecCartQty=function(i,d){' .
			...
			'cart[i].qty+=d;' .
			'if(cart[i].qty<1)cart.splice(i,1);' .
			'lsSet("sec_cart",cart);ecUpdateCartBadge();ecRenderCart();' .
		'};' .

		'window.ecCartRemove=function(i){' .
			'tmaHaptic("light");cart.splice(i,1);lsSet("sec_cart",cart);ecUpdateCartBadge();ecRenderCart();' .
		'};' .
		...
		/* Checkout – send cart to AI assistant (Shopify has no direct order-creation tool) */
		'window.ecCheckout=function(){' .
			'tmaHaptic("medium");' .
			'var total=cart.reduce(function(s,c){return s+c.price*c.qty;},0);' .
			'var cur=cart.length?cart[0].currency||shopCurrency||"":"";' .
			'var items=cart.map(function(c){return c.name+" x"+c.qty;}).join(", ");' .
			'var checkoutMsg="' . esc_js( __( 'I would like to checkout with the following items', 'mcp-ai-wpoos-pro' ) ) .
				': "+items+". ' . esc_js( __( 'Total', 'mcp-ai-wpoos-pro' ) ) . ': "+(cur?cur+" ":"")+total.toFixed(2)+". ' .
				esc_js( __( 'Please help me complete this order.', 'mcp-ai-wpoos-pro' ) ) . '";' .
			'ecSwitch("assistant");' .
			'if(!chatHist.length)ecChatInit();' .
			'chatHist.push({role:"user",content:checkoutMsg});' .
			'ecAppendMsg("user",checkoutMsg,false);' .
			...
			'.then(function(d){' .
				'var data=d&&d.data;' .
				'var rep=(data&&data.choices&&data.choices[0]&&data.choices[0].message&&data.choices[0].message.content)||' .
					'(data&&data.content)||(data&&data.response)||"' . esc_js( __( 'Your order has been submitted to our team. We will contact you shortly!', 'mcp-ai-wpoos-pro' ) ) . '";' .
				'el.innerHTML=ecRenderMd(rep);chatHist.push({role:"assistant",content:rep});' .
				'lsSet("sec_chat_hist",chatHist.slice(-50));' .
				'cart=[];lsSet("sec_cart",cart);ecUpdateCartBadge();' .
			'})' .
			'.catch(function(){' .
				'el.textContent="' . esc_js( __( 'Connection error. Please try again.', 'mcp-ai-wpoos-pro' ) ) . '";' .
			'});' .
		'};' .
		...
		'function ecLoadOrders(force){' .
			'var l=document.getElementById("ec-orders-list");if(!l)return;' .
			'if(!force&&!orders.length){orders=lsGet("sec_orders_cache",[]);}' .
			'if(orders.length&&!force){ecRenderOrders(orders);return;}' .
			'l.innerHTML=\'<div class="tma-empty">' . esc_js( __( 'Loading orders…', 'mcp-ai-wpoos-pro' ) ) . '</div>\';' .
			'ecToolCall("shopify_orders",{action:"list",first:20},function(err,d){' .
				'if(err){l.innerHTML=\'<div class="tma-empty">' . esc_js( __( 'Could not load orders.', 'mcp-ai-wpoos-pro' ) ) . '</div>\';return;}' .
				'var os=spExtract(d,"orders");' .
				'if(!os||!Array.isArray(os))os=[];' .
				'if(os.length&&!shopCurrency&&os[0].total_price){shopCurrency=os[0].total_price.currencyCode||"";}' .
				'orders=os;lsSet("sec_orders_cache",os);ecRenderOrders(os);' .
			'});' .
		'}' .

		'function ecFinancialClass(s){' .
			'switch((s||"").toLowerCase()){' .
				'case "paid":return "ec-status-paid";' .
				'case "authorized":return "ec-status-authorized";' .
				'case "pending":return "ec-status-pending";' .
				'case "refunded":return "ec-status-refunded";' .
				'case "voided":return "ec-status-voided";' .
				'default:return "ec-status-pending";' .
			'}' .
		'}' .

		'function ecFulfillClass(s){' .
			'switch((s||"").toLowerCase()){' .
				'case "fulfilled":return "ec-status-fulfilled";' .
				'case "partial":case "partially_fulfilled":case "in_progress":return "ec-status-partial";' .
				'default:return "ec-status-unfulfilled";' .
			'}' .
		'}' .
		...
		'function ecRenderOrders(os){' .
			...
			'if(!os.length){l.innerHTML=\'<div class="tma-empty">' . esc_js( __( 'No orders found.', 'mcp-ai-wpoos-pro' ) ) . '</div>\';ecHideChart();return;}' .
			'l.innerHTML=os.map(function(o){' .
				'var fin=(o.financial_status||o.financialStatus||"pending").toLowerCase();' .
				'var ful=(o.fulfillment_status||o.fulfillmentStatus||o.displayFulfillmentStatus||"unfulfilled").toLowerCase();' .
				'var tp=o.total_price||o.totalPriceSet&&o.totalPriceSet.shopMoney||{};' .
				'var amt=tp.amount||"0";var cur=tp.currencyCode||shopCurrency||"";' .
				...
				'return \'<div class="tma-order-item">' .
					'<div class="tma-order-header">' .
						...
						'<span class="ec-status-badge \'+ecFinancialClass(fin)+\'">\'+escH(fin)+\'</span>' .
					'</div>' .
					'<div class="tma-order-meta">\'+escH(date)+\' · <span class="ec-status-badge \'+ecFulfillClass(ful)+\'">\'+escH(ful)+\'</span></div>' .
					'<div class="tma-order-total">\'+escH(totalStr)+\'</div>' .
				'</div>\';' .
			'}).join("");' .
			'ecRenderSpendChart(os);' .
		'}' .
		...
		'function ecRenderSpendChart(os){' .
			'ecLoadChartJs(function(){' .
				...
				'var monthly={};' .
				'os.forEach(function(o){' .
					'var d=o.created_at||o.createdAt||"";if(!d)return;' .
					'var dt=new Date(d);var m;' .
					'if(!isNaN(dt.getTime())){m=dt.getFullYear()+"-"+("0"+(dt.getMonth()+1)).slice(-2);}' .
					'else if(/^\\d{4}-\\d{2}/.test(d)){m=d.substring(0,7);}' .
					'else{return;}' .
					'var tp=o.total_price||o.totalPriceSet&&o.totalPriceSet.shopMoney||{};' .
					'var t=parseFloat(tp.amount||0);' .
					'if(!monthly[m])monthly[m]=0;monthly[m]+=t;' .
				'});' .
				'var keys=Object.keys(monthly).sort();' .
				'if(!keys.length){ecHideChart();return;}' .
				...
				'spendChartInst=new Chart(cv.getContext("2d"),{type:"line",data:{labels:labels,datasets:[{' .
					'label:"' . esc_js( __( 'Spending', 'mcp-ai-wpoos-pro' ) ) . '",' .
					'data:data,borderColor:"#5c6ac4",backgroundColor:"rgba(92,106,196,0.1)",' .
					'fill:true,tension:0.3,pointRadius:3' .
				'}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true}}}});' .
			'});' .
		'}' .
		...
		'function ecChatInit(){' .
			...
			'}else{' .
				'var ctx="[' . esc_js( __( 'Shopify store context', 'mcp-ai-wpoos-pro' ) ) . '] ' . esc_js( __( 'Site', 'mcp-ai-wpoos-pro' ) ) . ': "+SITE_NAME+", ' .
					...
					esc_js( __( 'Recent orders', 'mcp-ai-wpoos-pro' ) ) . ': "+orders.length;' .
				'chatHist.push({role:"system",content:ctx});' .
				'ecAppendMsg("bot","' . esc_js( __( 'Hi! I\'m your Shopify shopping assistant. I can help you find products, track orders, and answer questions about our store.', 'mcp-ai-wpoos-pro' ) ) . '",false);' .
			'}' .
		'}' .
		...
		'function ecRenderSettings(){' .
			...
			'if(ci){' .
				'if(SHOPIFY_CONNECTION_ID){' .
					'ci.innerHTML=\'<span class="ec-connection-dot online"></span>' . esc_js( __( 'Shopify Store', 'mcp-ai-wpoos-pro' ) ) . '\';' .
				'}else{' .
					'ci.innerHTML=\'<span class="ec-connection-dot offline"></span>' . esc_js( __( 'Not Connected', 'mcp-ai-wpoos-pro' ) ) . '\';' .
				'}' .
			'}' .
			'var ds=document.getElementById("ec-data-summary");' .
			'if(ds)ds.textContent="' . esc_js( __( 'Cached products', 'mcp-ai-wpoos-pro' ) ) . ': "+productsCache.length+", ' .
				...
				esc_js( __( 'Chat messages', 'mcp-ai-wpoos-pro' ) ) . ': "+chatHist.length;' .
			'var cd=document.getElementById("ec-currency-display");' .
			'if(cd){cd.textContent=shopCurrency||"\\u2014";}' .
		'}' .

		'window.ecSyncFromServer=function(){' .
			'tmaHaptic("medium");ecLoadCollections();ecLoadProducts();ecLoadOrders(true);' .
		'};' .
		...
		'function ecDoClear(){' .
			'try{' .
				...
				'localStorage.removeItem("sec_orders_cache");' .
				'localStorage.removeItem("sec_cart");' .
				'localStorage.removeItem("sec_chat_hist");' .
				'localStorage.removeItem("sec_collections");' .
				'localStorage.removeItem("sec_font_size");' .
				'localStorage.removeItem("sec_compact");' .
			'}catch(e){}' .
			'productsCache=[];orders=[];cart=[];chatHist=[];collectionsCache=[];activeCollection="";shopCurrency="";' .
			'ecUpdateCartBadge();ecRenderSettings();ecRenderCollections();tmaHaptic("notificationSuccess");' .
		'}' .
		...
		'orders=lsGet("sec_orders_cache",[]);' .
		'collectionsCache=lsGet("sec_collections",[]);' .
		'ecUpdateCartBadge();' .
		'ecApplyDisplaySettings();' .

		'if(productsCache.length)ecRenderProducts(productsCache);' .
		'if(collectionsCache.length)ecRenderCollections();' .
