...
- **Writes to:** outbound HTTPS to the NV oOS Cloud gateway (default `https://cloud.nvoos.com/v1`), provider-response telemetry via the Base cost-tracking observer (custom response headers carry wholesale cost so the billing observer can record the 7% retail markup).
