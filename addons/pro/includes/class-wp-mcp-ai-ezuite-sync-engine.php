...
if ( ! class_exists( 'WP_MCP_AI_EZuite_Sync_Engine' ) ) {
	...
	class WP_MCP_AI_EZuite_Sync_Engine {
		...
		public static function schedule_recurring_syncs() {
			...
			$last_stored_interval = (int) get_option( 'wp_mcp_ai_ezuite_sync_scheduled_interval', 0 );
			...
			if ( empty( $sync_connections ) ) {
				...
				if ( $needs_reschedule ) {
					update_option( 'wp_mcp_ai_ezuite_sync_scheduled_interval', $interval_seconds );
				}
				return;
			}
			...
			// Each connection needs its own action because the EZuite API
