...
- **Tool execution fails in band.** `POST /tools/execute/{slug}` answers HTTP 200 with `{ successful: false, error: ... }`. `execute_tool()` converts that to `WP_Error` (`wp_mcp_ai_composio_account_auth_required` for auth-class failures, `wp_mcp_ai_composio_tool_failed` otherwise). Never treat a 200 as success without that check.
