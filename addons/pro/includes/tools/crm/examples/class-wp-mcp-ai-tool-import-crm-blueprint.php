...
class WP_MCP_AI_Tool_Import_CRM_Blueprint implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! file_exists( $file ) ) {
			return new WP_Error( 'not_found', __( 'Blueprint file not found.', 'mcp-ai-wpoos-pro' ) ); }
		$json = file_get_contents( $file );
		$data = json_decode( $json, true );
		if ( ! $data ) {
			return new WP_Error( 'invalid_json', __( 'Blueprint file is invalid JSON.', 'mcp-ai-wpoos-pro' ) ); }
		$name     = $data['name'] ?? ucwords( str_replace( '-', ' ', $bp ) );
		$existing = get_page_by_title( $name, OBJECT, 'mcp_ai_assistant' );
		if ( $existing && empty( $arguments['overwrite'] ) ) {
			return new WP_Error( 'duplicate', sprintf( __( 'An assistant named "%s" already exists. Set overwrite=true to replace.', 'mcp-ai-wpoos-pro' ), $name ) ); }
		...
		if ( ! empty( $data['meta'] ) ) {
			foreach ( $data['meta'] as $k => $v ) {
				update_post_meta( $assistant_id, sanitize_key( $k ), $v ); }
		}
		update_post_meta( $assistant_id, '_blueprint_source', $bp );
		return array(
			'success'      => true,
			'message'      => sprintf( __( 'Blueprint "%1$s" imported as assistant #%2$d.', 'mcp-ai-wpoos-pro' ), $name, $assistant_id ),
			'blueprint'    => $bp,
