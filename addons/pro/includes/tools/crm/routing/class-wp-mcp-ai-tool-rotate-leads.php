...
class WP_MCP_AI_Tool_Rotate_Leads implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() { return 'rotate_leads'; }
	public function get_name() { return __( 'Rotate Leads', 'mcp-ai-wpoos-pro' ); }
	public function get_description() { return __( 'Bulk-reassign unowned or stale leads across the routing pool using the configured strategy.', 'mcp-ai-wpoos-pro' ); }
	public function get_parameters_schema() {
		return array(
			'type' => 'object',
			'properties' => array(
				'lead_status'      => array( 'type' => 'string', 'default' => 'new', 'description' => __( 'Reassign only leads with this status.', 'mcp-ai-wpoos-pro' ) ),
				'lifecycle_stage'  => array( 'type' => 'string', 'default' => '', 'description' => __( 'Optional filter by lifecycle stage.', 'mcp-ai-wpoos-pro' ) ),
				'max_leads'        => array( 'type' => 'integer', 'default' => 100, 'minimum' => 1, 'maximum' => 1000, 'description' => __( 'Maximum number of leads to reassign.', 'mcp-ai-wpoos-pro' ) ),
				'dry_run'          => array( 'type' => 'boolean', 'default' => false, 'description' => __( 'If true, returns the list of leads that WOULD be reassigned without making changes.', 'mcp-ai-wpoos-pro' ) ),
			),
		);
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		$meta_q[] = array( 'key' => 'lead_status', 'value' => $lead_status );
		if ( ! empty( $arguments['lifecycle_stage'] ) ) {
			$meta_q[] = array( 'key' => 'lifecycle_stage', 'value' => sanitize_key( $arguments['lifecycle_stage'] ) );
		}
		...
		foreach ( $q->posts as $lead_id ) {
			...
			if ( ! $new_owner ) { continue; }

			if ( ! $dry_run ) {
				update_post_meta( $lead_id, 'contact_owner', $new_owner );
				if ( class_exists( 'WP_MCP_AI_CRM_Audit' ) ) {
					WP_MCP_AI_CRM_Audit::record( 'lead_rotated', 'lead', $lead_id, array(
