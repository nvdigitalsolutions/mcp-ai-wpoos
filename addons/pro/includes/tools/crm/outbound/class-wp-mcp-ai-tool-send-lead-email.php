...
class WP_MCP_AI_Tool_Send_Lead_Email implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() { return 'send_lead_email'; }
	public function get_name() { return __( 'Send Lead Email', 'mcp-ai-wpoos-pro' ); }
	public function get_description() { return __( 'Send an outbound email to a lead. Requires active email consent.', 'mcp-ai-wpoos-pro' ); }
	public function get_parameters_schema() {
		return array( 'type' => 'object', 'properties' => array(
			'lead_id'     => array( 'type' => 'integer', 'description' => __( 'Lead or contact post ID.', 'mcp-ai-wpoos-pro' ) ),
			'subject'     => array( 'type' => 'string', 'description' => __( 'Email subject line.', 'mcp-ai-wpoos-pro' ) ),
			'body'        => array( 'type' => 'string', 'description' => __( 'Email body (HTML or plain text).', 'mcp-ai-wpoos-pro' ) ),
			'template_id' => array( 'type' => 'string', 'description' => __( 'Optional MJML template identifier.', 'mcp-ai-wpoos-pro' ) ),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $email ) { return new WP_Error( 'no_email', __( 'Lead has no email address.', 'mcp-ai-wpoos-pro' ) ); }
		...
		if ( ! $sent ) { return new WP_Error( 'send_failed', __( 'Email failed to send.', 'mcp-ai-wpoos-pro' ) ); }

		// Log as activity.
		$activity_id = wp_insert_post( array( 'post_type' => 'mcp_ai_crm_activity', 'post_title' => sprintf( __( 'Sent email: %s', 'mcp-ai-wpoos-pro' ), $subject ), 'post_status' => 'publish' ), true );
		...
		if ( class_exists( 'WP_MCP_AI_CRM_Audit' ) ) { WP_MCP_AI_CRM_Audit::record( 'outbound_email_sent', 'lead', $lead_id, array( 'email' => $email ) ); }

		return array( 'success' => true, 'message' => __( 'Email sent successfully.', 'mcp-ai-wpoos-pro' ), 'lead_id' => $lead_id, 'to' => $email, 'activity_id' => $activity_id );
	}
}
