...
class WP_MCP_AI_Tool_Get_Pipeline_View implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() { return 'get_pipeline_view'; }
	public function get_name() { return __( 'Pipeline View', 'mcp-ai-wpoos-pro' ); }
	public function get_description() { return __( 'Kanban-style pipeline snapshot grouped by deal stage with weighted amounts.', 'mcp-ai-wpoos-pro' ); }
	public function get_parameters_schema() {
		return array(
			'type' => 'object',
			'properties' => array(
				'deal_owner' => array( 'type' => 'integer', 'description' => __( 'Filter by assigned owner WP user ID.', 'mcp-ai-wpoos-pro' ) ),
				'per_stage'  => array( 'type' => 'integer', 'default' => 50, 'minimum' => 1, 'maximum' => 200, 'description' => __( 'Max deals per stage.', 'mcp-ai-wpoos-pro' ) ),
			),
		);
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		foreach ( $stages as $stage_id => $stage_def ) {
			$args = array(
				...
				'fields'         => 'ids',
				'meta_query'     => array( array( 'key' => 'deal_stage', 'value' => $stage_id ) ),
				'no_found_rows'  => true,
			);
			if ( $meta_q ) {
