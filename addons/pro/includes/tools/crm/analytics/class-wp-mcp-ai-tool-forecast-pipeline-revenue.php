...
class WP_MCP_AI_Tool_Forecast_Pipeline_Revenue implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() { return 'forecast_pipeline_revenue'; }
	public function get_name() { return __( 'Forecast Pipeline Revenue', 'mcp-ai-wpoos-pro' ); }
	public function get_description() { return __( 'Weighted pipeline revenue forecast by month/quarter with best-case, most-likely, and commit scenarios.', 'mcp-ai-wpoos-pro' ); }
	public function get_parameters_schema() {
		return array(
			'type' => 'object',
			'properties' => array(
				'deal_owner' => array( 'type' => 'integer' ),
				'bucket'     => array( 'type' => 'string', 'enum' => array( 'month', 'quarter' ), 'default' => 'month' ),
				'months'     => array( 'type' => 'integer', 'default' => 6, 'minimum' => 1, 'maximum' => 24, 'description' => __( 'Number of future periods to forecast.', 'mcp-ai-wpoos-pro' ) ),
			),
		);
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		$meta_q[] = array( 'key' => 'deal_stage', 'value' => array( 'closed_won', 'closed_lost' ), 'compare' => 'NOT IN' );
		if ( ! empty( $arguments['deal_owner'] ) ) {
			$meta_q[] = array( 'key' => 'deal_owner', 'value' => absint( $arguments['deal_owner'] ), 'type' => 'NUMERIC' );
		}
		...
		$buckets = array();
		$totals  = array( 'best_case' => 0, 'most_likely' => 0, 'commit' => 0 );
		...
		foreach ( $q->posts as $deal ) {
			...
			if ( ! isset( $buckets[ $key ] ) ) {
				$buckets[ $key ] = array( 'best_case' => 0, 'most_likely' => 0, 'commit' => 0, 'deal_count' => 0 );
			}
