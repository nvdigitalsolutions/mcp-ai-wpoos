 ...
 *
 * Allows AI assistants to delete medical records from the health wellness system.
 *
 * @package WP_MCP_AI
 */
...
class WP_MCP_AI_Tool_Delete_Medical_Record implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_description() {
		return __( 'Deletes a medical record from the health and wellness system. Only the record creator or users with delete_others_posts capability can delete records.', 'mcp-ai-wpoos-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'record_id' => array(
					'type'        => 'integer',
					'description' => __( 'Medical record ID to delete (required)', 'mcp-ai-wpoos-pro' ),
					'minimum'     => 1,
				),
				'force'     => array(
					'type'        => 'boolean',
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $current_user_id ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be logged in to delete medical records.', 'mcp-ai-wpoos-pro' ) );
		}
		...
		if ( ! $is_author && ! $can_delete_others ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to delete this medical record.', 'mcp-ai-wpoos-pro' ) );
		}

		// Delete the record.
		$result = wp_delete_post( $record_id, $force );
		...
		return array(
			...
			'record_id' => $record_id,
			'message'   => sprintf(
				/* translators: 1: record title, 2: action (deleted/trashed) */
				__( 'Medical record "%1$s" has been %2$s.', 'mcp-ai-wpoos-pro' ),
				$record->post_title,
