...
class WP_MCP_AI_Pro_Tool_Schedule_All_Export implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Schedule WP All Export', 'mcp-ai-wpoos-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'export_id'  => array(
					'type'        => 'integer',
					'description' => __( 'The ID of the export template to schedule.', 'mcp-ai-wpoos-pro' ),
					'minimum'     => 1,
				),
				'interval'   => array(
					'type'        => 'string',
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $user_id ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You must be logged in to schedule exports.', 'mcp-ai-wpoos-pro' ) );
		}

		if ( ! user_can( $user_id, 'manage_options' ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to schedule exports.', 'mcp-ai-wpoos-pro' ) );
		}
		...
		if ( false === $scheduled ) {
			return new WP_Error( 'wp_mcp_ai_schedule_failed', __( 'Failed to schedule export.', 'mcp-ai-wpoos-pro' ) );
		}
		...
		return array(
			'success'     => true,
			'message'     => __( 'Export scheduled successfully.', 'mcp-ai-wpoos-pro' ),
			'export_id'   => $export_id,
