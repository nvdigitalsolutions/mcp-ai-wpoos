...
class WP_MCP_AI_Pro_Tool_List_Templates {
	...
	private function format_template_cpt( $post ) {
		...
		return array(
			...
			'category'            => get_post_meta( $post->ID, 'category', true ) ? get_post_meta( $post->ID, 'category', true ) : 'custom',
			'status'              => $status_map[ $post->post_status ] ?? 'draft',
			'version'             => get_post_meta( $post->ID, 'version', true ) ? get_post_meta( $post->ID, 'version', true ) : '1.0.0',
			'usage_count'         => intval( get_post_meta( $post->ID, 'usage_count', true ) ? get_post_meta( $post->ID, 'usage_count', true ) : 0 ),
			'success_rate'        => floatval( get_post_meta( $post->ID, 'success_rate', true ) ? get_post_meta( $post->ID, 'success_rate', true ) : 0 ),
			'avg_completion_time' => intval( get_post_meta( $post->ID, 'avg_completion_time', true ) ? get_post_meta( $post->ID, 'avg_completion_time', true ) : 0 ),
			'tags'                => get_post_meta( $post->ID, 'tags', true ) ? get_post_meta( $post->ID, 'tags', true ) : array(),
			'placeholders'        => $placeholders,
