...
class WP_MCP_AI_Tool_Research_Post implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	 ...
	 *
	 * @param array  $post_data     Parsed research post data.
	 ...
	 */
	protected function export_research_document( $post_data, $output_format ) {
		$title   = ! empty( $post_data['title'] ) ? sanitize_text_field( $post_data['title'] ) : __( 'Research', 'mcp-ai-wpoos-pro' );
		...
		if ( 'pdf' === $output_format ) {
			return $this->export_as_pdf( $title, $content, $post_data );
		}

		if ( 'docx' === $output_format ) {
			return $this->export_as_docx( $title, $content, $post_data );
		}
	 ...
	 * @param array  $post_data Full post data for metadata.
	 * @return array|WP_Error Export result or error.
	 */
	protected function export_as_pdf( $title, $content, $post_data ) {
		...
		try {
			...
			// Add metadata footer.
			if ( ! empty( $post_data['sources'] ) ) {
				$html .= '<hr><h3>' . esc_html__( 'Sources', 'mcp-ai-wpoos-pro' ) . '</h3><ul>';
				foreach ( array_slice( $post_data['sources'], 0, 10 ) as $source ) {
	 ...
	 * @return array|WP_Error Export result or error.
	 */
	protected function export_as_docx( $title, $content, $post_data ) {
