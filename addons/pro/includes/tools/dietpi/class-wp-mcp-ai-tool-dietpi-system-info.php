...
if ( ! class_exists( 'WP_MCP_AI_Tool_DietPi_System_Info' ) ) {
	class WP_MCP_AI_Tool_DietPi_System_Info extends WP_MCP_AI_Tool_DietPi_Base {
		public function get_slug() { return 'dietpi_system_info'; }
		public function get_name() { return __( 'DietPi System Info', 'mcp-ai-wpoos-pro' ); }
		public function get_description() { return __( 'Get detailed system information about the Raspberry Pi including model, OS version, kernel version, DietPi version, and uptime.', 'mcp-ai-wpoos-pro' ); }
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			if ( is_wp_error( $pi_info ) ) { return $pi_info; }
			...
			$os = array(); $os_raw = is_wp_error( $os_info ) ? '' : $os_info['stdout'];
			foreach ( explode( "\n", $os_raw ) as $line ) { $p = explode( ':', trim( $line ), 2 ); if ( 2 === count( $p ) ) $os[ strtolower( $p[0] ) ] = $p[1]; }
