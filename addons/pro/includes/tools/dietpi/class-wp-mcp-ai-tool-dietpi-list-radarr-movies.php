...
if ( ! class_exists( 'WP_MCP_AI_Tool_DietPi_List_Radarr_Movies' ) ) {
	...
	class WP_MCP_AI_Tool_DietPi_List_Radarr_Movies extends WP_MCP_AI_Tool_DietPi_Base {
		...
		public function get_slug() {
			return 'dietpi_list_radarr_movies';
		}
		...
		public function get_name() {
			return __( 'List Radarr Movies', 'mcp-ai-wpoos-pro' );
		}
			...
			if ( is_array( $movies ) ) {
				foreach ( $movies as $m ) {
					$out[] = array(
						...
						'tmdb_id'   => isset( $m['tmdbId'] ) ? (int) $m['tmdbId'] : 0,
						'imdb_id'   => isset( $m['imdbId'] ) ? sanitize_text_field( $m['imdbId'] ) : '',
						'year'      => isset( $m['year'] ) ? (int) $m['year'] : 0,
