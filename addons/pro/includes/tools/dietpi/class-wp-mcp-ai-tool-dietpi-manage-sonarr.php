...
if ( ! class_exists( 'WP_MCP_AI_Tool_DietPi_Manage_Sonarr' ) ) {
	class WP_MCP_AI_Tool_DietPi_Manage_Sonarr extends WP_MCP_AI_Tool_DietPi_Base {
		public function get_slug() { return 'dietpi_manage_sonarr'; }
		public function get_name() { return __( 'Manage Sonarr', 'mcp-ai-wpoos-pro' ); }
		public function get_description() { return __( 'Manage Sonarr: trigger series refresh, rescan episodes, search for missing episodes, monitor/unmonitor series, check download queue, and view system status.', 'mcp-ai-wpoos-pro' ); }
		public function get_parameters_schema() {
			return array(
				'type' => 'object',
				'properties' => array(
					'action' => array( 'type' => 'string', 'description' => __( 'Management action.', 'mcp-ai-wpoos-pro' ), 'enum' => array( 'refresh_all', 'rescan_series', 'search_missing', 'monitor', 'unmonitor', 'queue', 'system_status', 'calendar' ) ),
					'series_id' => array( 'type' => 'integer', 'description' => __( 'Series ID (required for rescan_series, search_missing, monitor, unmonitor).', 'mcp-ai-wpoos-pro' ) ),
				),
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			switch ( $action ) {
				...
				case 'rescan_series':
					...
					if ( $sid <= 0 ) { return new WP_Error( 'wp_mcp_ai_missing_series_id', __( 'series_id is required for this action.', 'mcp-ai-wpoos-pro' ) ); }
				...
				case 'search_missing':
					$sid = $this->sanitize_int( $arguments, 'series_id' );
					if ( $sid <= 0 ) { return new WP_Error( 'wp_mcp_ai_missing_series_id', __( 'series_id is required.', 'mcp-ai-wpoos-pro' ) ); }
				...
				case 'queue':
					...
					return $this->success( sprintf( _n( '%d item in queue.', '%d items in queue.', count( $result ), 'mcp-ai-wpoos-pro' ), count( $result ) ), array( 'queue' => $result ) );
				case 'system_status':
				...
				case 'calendar':
					$start = gmdate( 'Y-m-d' ); $end = gmdate( 'Y-m-d', strtotime( '+14 days' ) );
