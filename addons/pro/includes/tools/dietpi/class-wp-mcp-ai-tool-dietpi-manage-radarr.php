...
if ( ! class_exists( 'WP_MCP_AI_Tool_DietPi_Manage_Radarr' ) ) {
	class WP_MCP_AI_Tool_DietPi_Manage_Radarr extends WP_MCP_AI_Tool_DietPi_Base {
		public function get_slug() { return 'dietpi_manage_radarr'; }
		public function get_name() { return __( 'Manage Radarr', 'mcp-ai-wpoos-pro' ); }
		public function get_description() { return __( 'Manage Radarr: trigger movie refresh, search for movies, check queue, view system status, or see upcoming releases calendar.', 'mcp-ai-wpoos-pro' ); }
		public function get_parameters_schema() {
			return array(
				'type' => 'object',
				'properties' => array(
					'action' => array( 'type' => 'string', 'description' => __( 'Management action.', 'mcp-ai-wpoos-pro' ), 'enum' => array( 'refresh_all', 'search_all', 'queue', 'system_status', 'calendar', 'diskspace' ) ),
				),
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			switch ( $action ) {
				...
				case 'calendar':
					$start = gmdate( 'Y-m-d' ); $end = gmdate( 'Y-m-d', strtotime( '+30 days' ) );
