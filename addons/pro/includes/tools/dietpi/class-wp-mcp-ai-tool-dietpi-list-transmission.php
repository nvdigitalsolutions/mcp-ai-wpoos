...
if ( ! class_exists( 'WP_MCP_AI_Tool_DietPi_List_Transmission' ) ) {
	...
	class WP_MCP_AI_Tool_DietPi_List_Transmission extends WP_MCP_AI_Tool_DietPi_Base {
		...
		public function get_slug() {
			return 'dietpi_list_transmission';
		}
		...
		public function get_name() {
			return __( 'List Transmission Torrents', 'mcp-ai-wpoos-pro' );
		}
		...
		public function get_parameters_schema() {
			return array(
				...
				'properties' => array(
					'status' => array(
						'type'        => 'string',
						'description' => __( 'Filter by torrent status. If omitted, all torrents are returned.', 'mcp-ai-wpoos-pro' ),
						'enum'        => array( 'all', 'downloading', 'seeding', 'paused', 'checking', 'queued', 'error' ),
					),
				),
			);
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			foreach ( $torrents as $t ) {
				$out[] = array(
					...
					'name'           => $t['name'],
					'status'         => (int) $t['status'],
					'percent_done'   => round( (float) $t['percentDone'] * 100, 1 ),
					'download_speed' => (int) $t['rateDownload'],
					'upload_speed'   => (int) $t['rateUpload'],
					'eta_seconds'    => (int) $t['eta'],
					'total_size'     => (int) $t['totalSize'],
					'downloaded'     => (int) $t['downloadedEver'],
					'uploaded'       => (int) $t['uploadedEver'],
					'labels'         => isset( $t['labels'] ) ? $t['labels'] : array(),
					'error'          => isset( $t['errorString'] ) ? $t['errorString'] : '',
				);
			}
			return $this->success(
				sprintf(
