<?php
/**
 * WooCommerce Product Price Update Tool
 *
 * Pro add-on tool for updating WooCommerce product prices across all
 * product types: simple, variable (parent + variations), grouped, and
...
class WP_MCP_AI_Tool_Update_Woo_Product_Price implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface, WP_MCP_AI_Tool_Safety_Profile_Interface {
	...
	public static function get_unavailable_reason() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return __( 'WooCommerce product price updates require WooCommerce to be installed and activated.', 'mcp-ai-wpoos-pro' );
		}
		...
		return __( 'WooCommerce product price update tool is not available.', 'mcp-ai-wpoos-pro' );
	}
	...
	public function get_name() {
		return __( 'Update WooCommerce Product Price', 'mcp-ai-wpoos-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties' => array(
				...
				'regular_price'  => array(
					'type'        => 'string',
					'description' => __( 'New regular price (e.g. "29.99").', 'mcp-ai-wpoos-pro' ),
				),
				'sale_price'     => array(
					'type'        => 'string',
					'description' => __( 'New sale price (must be lower than the regular price). Pass an empty string to clear the sale price.', 'mcp-ai-wpoos-pro' ),
				),
				'sale_date_from' => array(
					'type'        => 'string',
					'description' => __( 'Optional date the sale starts (Y-m-d or any valid date).', 'mcp-ai-wpoos-pro' ),
				),
				'sale_date_to'   => array(
					'type'        => 'string',
					'description' => __( 'Optional date the sale ends (Y-m-d or any valid date).', 'mcp-ai-wpoos-pro' ),
				),
				'clear_sale'     => array(
					'type'        => 'boolean',
					'description' => __( 'Set true to remove the sale price and sale dates entirely.', 'mcp-ai-wpoos-pro' ),
					'default'     => false,
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! in_array( $product->get_type(), array( 'simple', 'variable', 'variation', 'grouped', 'external' ), true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Product type "%s" is not supported for price updates.', 'mcp-ai-wpoos-pro' ),
					$product->get_type()
				)
			);
		}
		...
		if ( ! $has_price_field ) {
			return new WP_Error(
				'missing_price_field',
				__( 'At least one of regular_price, sale_price, or clear_sale is required.', 'mcp-ai-wpoos-pro' )
			);
		}
