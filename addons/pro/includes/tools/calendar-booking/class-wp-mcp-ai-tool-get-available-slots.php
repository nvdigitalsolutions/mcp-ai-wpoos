...
public function get_slug() { return 'get_available_slots'; }
public function get_name() { return __( 'Get Available Slots', 'mcp-ai-wpoos-pro' ); }
...
'type' => 'object',
'properties' => array(
'date' => array( 'type' => 'string', 'description' => __( 'Date to check (Y-m-d format)', 'mcp-ai-wpoos-pro' ) ),
'duration_minutes' => array( 'type' => 'integer', 'description' => __( 'Required duration', 'mcp-ai-wpoos-pro' ), 'default' => 60 ),
),
'required' => array( 'date' ),
);
}
public function get_capability_flags() { return array( 'pro', 'database-read', 'phase-2.6' ); }
...
return array( 'success' => true, 'date' => $date, 'duration_minutes' => $duration, 'available_slots' => $slots, 'total_slots' => count( $slots ) );
}
...
$args = array( 'post_type' => 'mcp_appointment', 'post_status' => 'publish', 'meta_query' => array( 'relation' => 'AND', array( 'key' => '_status', 'value' => array( 'confirmed', 'pending' ), 'compare' => 'IN' ), array( 'key' => '_start_time', 'value' => $slot_end, 'compare' => '<', 'type' => 'DATETIME' ), array( 'key' => '_end_time', 'value' => $slot_start, 'compare' => '>', 'type' => 'DATETIME' ) ) );
$query = new WP_Query( $args );
if ( ! $query->have_posts() ) {
$slots[] = array( 'start_time' => $slot_start, 'end_time' => $slot_end, 'available' => true );
}
