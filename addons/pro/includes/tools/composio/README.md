...
- **Auth failures are recoverable, not fatal.** An auth-class error is rewritten into `wp_mcp_ai_composio_account_auth_required` carrying a `reconnect_url`, and the verdict is recorded so auto-resolution skips the dead account next time. This includes a provider status proxied through a delivered call (`provider_status: 401`), not just Composio's own `successful: false`.
- **An account ID the caller supplied is verified before use.** A `ca_...` that Composio answers 404 for is rejected with `wp_mcp_ai_composio_unknown_account` naming the accounts that *do* exist, because the usual cause is a caller replaying a stale ID from earlier in the conversation, and sending it upstream only produces an opaque failure. Other lookup failures (rate limit, transport) still fall back to the connection's identity rather than blocking execution.
