...
class WP_MCP_AI_Pro_Tool_Create_Google_Calendar_Event implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Create Google Calendar Event', 'mcp-ai-wpoos-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'description'      => array(
					'type'        => 'string',
					'description' => __( 'Optional long-form description for the event.', 'mcp-ai-wpoos-pro' ),
				),
				'location'         => array(
					'type'        => 'string',
					'description' => __( 'Optional location for the event.', 'mcp-ai-wpoos-pro' ),
				),
				'start_time'       => array(
					'type'        => 'string',
					'description' => __( 'Event start time in ISO 8601 format or YYYY-MM-DD for all-day events.', 'mcp-ai-wpoos-pro' ),
				),
				'end_time'         => array(
					'type'        => 'string',
					'description' => __( 'Event end time in ISO 8601 format or YYYY-MM-DD for all-day events.', 'mcp-ai-wpoos-pro' ),
				),
				'duration_minutes' => array(
					'type'        => array( 'number', 'integer' ),
					'minimum'     => 1,
					'description' => __( 'Duration in minutes to use when end_time is omitted.', 'mcp-ai-wpoos-pro' ),
				),
				'timezone'         => array(
					'type'        => 'string',
					'description' => __( 'Optional IANA timezone identifier applied to the start and end time.', 'mcp-ai-wpoos-pro' ),
				),
				'calendar_id'      => array(
					'type'        => 'string',
					'description' => __( 'Override the calendar identifier. Falls back to the configured default.', 'mcp-ai-wpoos-pro' ),
				),
				'attendees'        => array(
					'type'        => 'array',
					'description' => __( 'Optional attendee list. Provide email strings or objects with email, name, and optional status.', 'mcp-ai-wpoos-pro' ),
					'items'       => array(
						'anyOf' => array(
							array(
								'type' => 'string',
				...
				'send_updates'     => array(
					...
					'description' => __( 'Controls whether attendees receive update emails (all, externalOnly, or none).', 'mcp-ai-wpoos-pro' ),
				),
				'reminders'        => array(
					'type'                 => 'object',
					'description'          => __( 'Optional reminder overrides. Supports use_default and overrides array with method and minutes.', 'mcp-ai-wpoos-pro' ),
					'properties'           => array(
						'use_default' => array(
							'type'        => 'boolean',
							'description' => __( 'Whether to use the calendar\'s default reminder settings.', 'mcp-ai-wpoos-pro' ),
						),
						'overrides'   => array(
							'type'        => 'array',
							'description' => __( 'Custom reminder definitions.', 'mcp-ai-wpoos-pro' ),
							'items'       => array(
								'type'                 => 'object',
								'properties'           => array(
									'method'  => array(
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( '' === $summary ) {
			return new WP_Error( 'wp_mcp_ai_missing_summary', __( 'An event summary is required.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
		}
		...
		if ( empty( $arguments['start_time'] ) || ! is_string( $arguments['start_time'] ) ) {
			return new WP_Error( 'wp_mcp_ai_missing_start_time', __( 'A valid start time must be provided.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
		}
		...
		if ( ! empty( $arguments['end_time'] ) ) {
			if ( ! is_string( $arguments['end_time'] ) ) {
				return new WP_Error( 'wp_mcp_ai_invalid_end_time', __( 'The event end time must be a string value.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
			}
		...
		if ( isset( $arguments['duration_minutes'] ) && '' !== $arguments['duration_minutes'] ) {
			if ( ! is_numeric( $arguments['duration_minutes'] ) ) {
				return new WP_Error( 'wp_mcp_ai_invalid_duration', __( 'The duration must be a numeric value.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
			}

			$duration_minutes = (float) $arguments['duration_minutes'];
			if ( $duration_minutes <= 0 ) {
				return new WP_Error( 'wp_mcp_ai_invalid_duration', __( 'The duration must be greater than zero.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
			}
		}

		if ( null === $end ) {
			if ( $duration_minutes <= 0 ) {
				return new WP_Error( 'wp_mcp_ai_missing_end_time', __( 'Provide an end time or duration for the event.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
			}

			if ( 'dateTime' !== $start['type'] || ! isset( $start['datetime'] ) ) {
				return new WP_Error( 'wp_mcp_ai_duration_requires_time', __( 'Durations may only be used with date/time events.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
			}
	...
	protected function resolve_access_token( array $arguments, array $context ) {
		...
		if ( empty( $credentials ) || ! is_array( $credentials ) ) {
			return new WP_Error( 'wp_mcp_ai_calendar_missing_credentials', __( 'Google Calendar credentials were not provided.', 'mcp-ai-wpoos-pro' ), array( 'status' => 500 ) );
		}
	...
	protected function exchange_service_account_token( array $credentials, array $arguments, array $context ) {
		...
		if ( '' === $client_email || '' === $private_key ) {
			return new WP_Error( 'wp_mcp_ai_calendar_invalid_credentials', __( 'Incomplete Google service account credentials.', 'mcp-ai-wpoos-pro' ), array( 'status' => 500 ) );
		}
	...
	protected function build_jwt_assertion( array $claims, $private_key ) {
		...
		if ( ! $success ) {
			return new WP_Error( 'wp_mcp_ai_calendar_signing_failed', __( 'Unable to sign the service account assertion.', 'mcp-ai-wpoos-pro' ), array( 'status' => 500 ) );
		}
	...
	protected function validate_time_range( array $start, array $end ) {
		if ( 'date' === $start['type'] && 'date' === $end['type'] ) {
			...
			if ( strcmp( $end_date, $start_date ) <= 0 ) {
				return new WP_Error( 'wp_mcp_ai_invalid_time_range', __( 'The event end date must be after the start date.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
			}
		...
		if ( ! isset( $start['datetime'], $end['datetime'] ) ) {
			return new WP_Error( 'wp_mcp_ai_invalid_time_range', __( 'Invalid start or end time provided.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
		}

		if ( $end['datetime'] <= $start['datetime'] ) {
			return new WP_Error( 'wp_mcp_ai_invalid_time_range', __( 'The event end time must be after the start time.', 'mcp-ai-wpoos-pro' ), array( 'status' => 400 ) );
		}
	...
	protected function parse_event_time( $value, $timezone, $label ) {
		...
		if ( '' === $value ) {
			return new WP_Error(
				...
				__( 'A valid date or time value must be supplied.', 'mcp-ai-wpoos-pro' ),
				array(
					'status' => 400,
					'field'  => $label,
				)
		...
		try {
			...
			return new WP_Error(
				'wp_mcp_ai_invalid_time',
				__( 'Unable to parse the supplied date/time value.', 'mcp-ai-wpoos-pro' ),
				array(
					'status' => 400,
					'field'  => $label,
				)
		...
		if ( $timezone ) {
			try {
				...
				return new WP_Error(
					...
					__( 'The provided timezone is not valid.', 'mcp-ai-wpoos-pro' ),
					array(
						'status' => 400,
						'field'  => $label,
					)
