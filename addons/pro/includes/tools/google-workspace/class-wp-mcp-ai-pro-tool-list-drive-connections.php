<?php
/**
 * Tool that lists configured Google Drive connections.
 *
 * Lets an agent discover valid connection_id values for search_drive and
 * get_drive_file instead of guessing. Credentials are never exposed — only
 * id, name, type, enabled state, the configured user email, and the optional
 * scoped folder ID are returned.
 *
...
class WP_MCP_AI_Pro_Tool_List_Drive_Connections implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'List Google Drive Connections', 'mcp-ai-wpoos-pro' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
			...
			if ( is_array( $all_connections ) ) {
				foreach ( $all_connections as $conn_id => $connection ) {
					...
					$connections[] = array(
						...
						'folder_id'       => isset( $connection['folder_id'] ) ? (string) $connection['folder_id'] : '',
					);
				}
			}
		}

		// Settings-based fallback: represents the Drive account used when connection_id is omitted.
		$settings = WP_MCP_AI_Admin_Settings::get_settings();
		if ( ! empty( $settings['google_drive_client_id'] ) && ! empty( $settings['google_drive_refresh_token'] ) ) {
			$connections[] = array(
				'id'              => 'settings',
				'name'            => __( 'NV oOS Google Drive settings (default connection)', 'mcp-ai-wpoos-pro' ),
				'connection_type' => 'google_drive',
				'enabled'         => true,
				'user_email'      => isset( $settings['google_drive_user_email'] ) ? (string) $settings['google_drive_user_email'] : '',
				'folder_id'       => '',
			);
		}
		...
		return $this->format_success_response(
			__( 'Google Drive connections listed.', 'mcp-ai-wpoos-pro' ),
			array( 'data' => $data )
		);
	}
