<?php
/**
 * Cloudways App Credentials Tool
 *
...
if ( ! class_exists( 'WP_MCP_AI_Tool_Cloudways_App_Credentials' ) ) {
	...
	class WP_MCP_AI_Tool_Cloudways_App_Credentials extends WP_MCP_AI_Tool_Cloudways_Base {
		...
		public function get_slug() {
			return 'cloudways_app_credentials';
		}

		/** {@inheritdoc} */
		public function get_name() {
			return __( 'Get App Credentials', 'mcp-ai-wpoos-pro' );
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			$path   = '/app/' . $server_id . '/' . $app_id . '/credentials';
			...
			if ( ! isset( $result['credentials'] ) || ! is_array( $result['credentials'] ) ) {
				return new WP_Error(
			...
			$data = array(
				'host'     => isset( $credentials['host'] ) ? sanitize_text_field( $credentials['host'] ) : '',
				'username' => isset( $credentials['username'] ) ? sanitize_text_field( $credentials['username'] ) : '',
				'port'     => isset( $credentials['port'] ) ? absint( $credentials['port'] ) : 22,
			);

			return $this->success(
				sprintf(
					/* translators: %d: app ID */
					__( 'Credentials retrieved for app %d.', 'mcp-ai-wpoos-pro' ),
					$app_id
				),
