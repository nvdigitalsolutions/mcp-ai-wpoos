...
class WP_MCP_AI_Tool_Get_Queued_Videos implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get Queued Videos', 'mcp-ai-wpoos-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties' => array(
				'status'   => array(
					'type'        => 'string',
					'description' => __( 'Filter by processing status.', 'mcp-ai-wpoos-pro' ),
	...
	public static function get_unavailable_reason() {
		...
		return __( 'Get Queued Videos tool is not available.', 'mcp-ai-wpoos-pro' );
	}
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $current_user_id || ! user_can( $current_user_id, 'read' ) ) {
			return new WP_Error(
				'wp_mcp_ai_forbidden',
				__( 'You do not have permission to view queued videos.', 'mcp-ai-wpoos-pro' )
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Retrieved %d queued videos.', 'mcp-ai-wpoos-pro' ),
				count( $videos )
			),
			'total'   => count( $videos ),
			'videos'  => $videos,
