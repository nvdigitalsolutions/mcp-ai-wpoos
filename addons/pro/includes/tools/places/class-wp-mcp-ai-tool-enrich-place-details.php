...
class WP_MCP_AI_Tool_Enrich_Place_Details implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() { return "enrich_place_details"; }
	public function get_name() { return __( "Enrich Place Details", "mcp-ai-wpoos-pro" ); }
	public function get_description() { return __( "Enrich a place with data from the Google Places API: ratings, phone numbers, websites, and Google Place IDs. Requires a Google Maps API key configured in plugin settings.", "mcp-ai-wpoos-pro" ); }
		...
		return array(
			...
			"properties"           => array(
				...
				"fields"             => array(
					...
					"items"       => array( "type" => "string", "enum" => array( "rating", "place_id", "phone", "website", "price_level" ) ),
				),
				"dry_run"            => array(
					"type"        => "boolean",
					"default"     => false,
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( empty( $api_key ) ) {
			return new WP_Error( "wp_mcp_ai_no_api_key", __( "Google Maps API key not configured. Add it in NV oOS → Settings → Providers → Google Maps.", "mcp-ai-wpoos-pro" ) );
		}
	...
	private function tally( $r, $i ) {
		if ( ! empty( $i["enriched"] ) ) { $r["enriched"]++; }
	...
	private function enrich( $place, $fields, $api_key, $dry_run ) {
		...
		if ( $dry_run ) {
			$item["message"] = sprintf( __( "Would search: %s", "mcp-ai-wpoos-pro" ), $place->post_title );
			return $item;
		}
		...
		if ( ! empty( $added ) ) {
			$item["enriched"] = true;
			$item["added"]    = $added;
			$item["message"]  = sprintf( __( "Added: %s", "mcp-ai-wpoos-pro" ), implode( ", ", $added ) );
		} else {
			$item["skipped"]  = true;
