...
/**
 * Enriches places with latitude/longitude via geocoding.
 *
 * @since 1.4.2
 */
class WP_MCP_AI_Tool_Enrich_Place_Coordinates implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_slug() {
		return 'enrich_place_coordinates';
	}
	...
	public function get_name() {
		return __( 'Enrich Place Coordinates', 'mcp-ai-wpoos-pro' );
	}
	...
	/**
	 * Resolve the best available geocoding provider.
	 *
	 * @return string 'google' or 'nominatim'.
	 */
	...
	private function geocode_place( $place, $provider, $dry_run ) {
		...
		if ( $dry_run ) {
			$item['message'] = sprintf(
				...
				__( 'Would geocode: %s', 'mcp-ai-wpoos-pro' ),
				$address
			);
			return $item;
		}
		...
		if ( $coords ) {
			...
			$item['message']   = sprintf(
				...
				__( 'Geocoded: %.6f, %.6f', 'mcp-ai-wpoos-pro' ),
				$coords['lat'],
				$coords['lng']
			);
		} else {
