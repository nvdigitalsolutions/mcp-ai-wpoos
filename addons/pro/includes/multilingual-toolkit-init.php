<?php
/**
 * Multi-language Content Toolkit Initialization
 *
 * Loads the Multi-language Toolkit system for translation,
 * localization, and multilingual content management including
 * RTL language support.
 *
 * @package WP_MCP_AI
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

// Check if Multi-language toolkit is enabled.
$settings   = get_option( 'wp_mcp_ai_settings', array() );
$is_enabled = ! empty( $settings['enable_multilingual_toolkit'] );
$is_base    = function_exists( 'wp_mcp_ai_is_base_version' ) && wp_mcp_ai_is_base_version();

// Only load if enabled and not in base version.
if ( $is_enabled && ! $is_base ) {
	
	// Load Multilingual admin pages.
	if ( is_admin() ) {
		// @TODO: Create admin pages in future phase.
		// require_once WP_MCP_AI_PRO_PATH . 'includes/admin/class-wp-mcp-ai-multilingual-settings-page.php';
	}
	
	// Register tools will be loaded automatically via the tools directory structure.
	// Tools are located in: addons/pro/includes/tools/multilingual/
}

/**
 * Enqueue multilingual toolkit admin styles.
 *
 * @param string $hook Current admin page hook.
 */
function wp_mcp_ai_enqueue_multilingual_toolkit_admin_styles( $hook ) {
	// Only load if toolkit is enabled.
	$settings = get_option( 'wp_mcp_ai_settings', array() );
	if ( empty( $settings['enable_multilingual_toolkit'] ) ) {
		return;
	}

	// Enqueue admin styles if available.
	$css_file = WP_MCP_AI_PRO_PATH . 'assets/css/admin-multilingual-toolkit.css';
	if ( file_exists( $css_file ) ) {
		wp_enqueue_style(
			'wp-mcp-ai-multilingual-toolkit-admin',
			WP_MCP_AI_PRO_URL . 'assets/css/admin-multilingual-toolkit.css',
			array(),
			WP_MCP_AI_PRO_VERSION
		);
	}
}
add_action( 'admin_enqueue_scripts', 'wp_mcp_ai_enqueue_multilingual_toolkit_admin_styles' );
