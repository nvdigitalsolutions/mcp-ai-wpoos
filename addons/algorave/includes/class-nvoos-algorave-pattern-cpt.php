...
class NV_oOS_Algorave_Pattern_CPT {
	...
	public static function get_pattern( $post_id ) {
		...
		return array(
			...
			'engine'      => get_post_meta( $post->ID, '_algorave_engine', true ) ?: 'strudel',
			'bpm'         => (int) get_post_meta( $post->ID, '_algorave_bpm', true ) ?: 120,
			'scale'       => get_post_meta( $post->ID, '_algorave_scale', true ) ?: 'C minor',
