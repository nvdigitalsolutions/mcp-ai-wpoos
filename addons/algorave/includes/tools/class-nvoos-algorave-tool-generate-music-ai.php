...
class NV_oOS_Algorave_Tool_Generate_Music_AI implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Generate Music (AI)', 'nvoos-algorave' );
	}
	...
	public function get_description() {
		return __( 'Generate a complete music track using an external AI music generation API (Google Lyria, Replicate, etc.). Provide a text prompt describing the desired music and receive an audio file. Use this when the user wants AI to compose and render a full audio track, not just generate pattern code.', 'nvoos-algorave' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'prompt'   => array(
					'type'        => 'string',
					'description' => __( 'Text description of the desired music (e.g. "a 30-second ambient techno loop with deep bass and ethereal pads").', 'nvoos-algorave' ),
					'minLength'   => 1,
					'maxLength'   => 2000,
				),
				'duration' => array(
					'type'        => 'integer',
					'description' => __( 'Desired duration in seconds (max 300).', 'nvoos-algorave' ),
					'default'     => 30,
					'minimum'     => 5,
					'maximum'     => 300,
				),
				'provider' => array(
					'type'        => 'string',
					'description' => __( 'AI provider to use. Leave empty to use the configured default.', 'nvoos-algorave' ),
	...
	private function generate_with_lyria( $prompt, $duration, $api_key ) {
		...
		return array(
			...
			'message'  => __( 'Music generated successfully via Google Lyria. The audio data is included in the response.', 'nvoos-algorave' ),
		);
	}
	...
	private function generate_with_replicate( $prompt, $duration, $api_key ) {
		...
		return array(
			...
			'message'       => __( 'Music generation started on Replicate. The prediction is processing — check the status with the prediction ID.', 'nvoos-algorave' ),
		);
	}
