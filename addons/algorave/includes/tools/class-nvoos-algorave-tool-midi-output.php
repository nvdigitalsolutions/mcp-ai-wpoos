...
class NV_oOS_Algorave_Tool_MIDI_Output implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'MIDI Output', 'nvoos-algorave' );
	}
	...
	public function get_description() {
		return __( 'Configure MIDI output for routing Strudel patterns to external hardware synthesizers, drum machines, or DAWs via WebMIDI. Use this when the user wants to send MIDI from the browser to external devices, list available MIDI ports, or get code examples for .midi() output. The actual MIDI device list is detected in the browser; this tool provides guidance and code patterns.', 'nvoos-algorave' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				'action'      => array(
					'type'        => 'string',
					'description' => __( 'MIDI action to perform.', 'nvoos-algorave' ),
					'enum'        => array( 'list_devices', 'generate_code', 'help' ),
					'default'     => 'help',
				),
				'device_name' => array(
					'type'        => 'string',
					'description' => __( 'Target MIDI device name for code generation (optional — uses first available if empty).', 'nvoos-algorave' ),
					'maxLength'   => 200,
				),
				'channel'     => array(
					'type'        => 'integer',
					'description' => __( 'MIDI channel (0-15).', 'nvoos-algorave' ),
					'minimum'     => 0,
					'maximum'     => 15,
					'default'     => 0,
				),
	...
	private function list_devices() {
		return array(
			...
			'message'          => __( 'MIDI device detection runs in the browser via WebMIDI. The live coder will show available devices.', 'nvoos-algorave' ),
			'instructions'     => __( 'The browser will detect available MIDI output devices. Ensure your MIDI device is connected via USB or use a virtual MIDI port (IAC Driver on macOS, MIDI Through on Linux) to route to DAWs. WebMIDI requires HTTPS or localhost.', 'nvoos-algorave' ),
		);
	}
	...
	private function generate_midi_code( $arguments ) {
		...
		return array(
			...
			'message'     => sprintf(
				...
				__( 'Generated MIDI output code targeting %s. Paste into the live coder to send MIDI.', 'nvoos-algorave' ),
				! empty( $device_name ) ? esc_html( $device_name ) : __( 'the default MIDI output', 'nvoos-algorave' )
			),
		);
	}
	...
	private function get_help() {
		return array(
			...
			'reference' => array(
				...
				'setup'     => array(
					__( 'WebMIDI requires HTTPS or localhost.', 'nvoos-algorave' ),
					__( 'Connect hardware synths via USB-MIDI.', 'nvoos-algorave' ),
					__( 'Use virtual MIDI ports for DAW routing (IAC on macOS, MIDI Through on Linux).', 'nvoos-algorave' ),
					__( 'Browser will prompt for MIDI access permission on first use.', 'nvoos-algorave' ),
				),
			),
			'message'   => __( 'MIDI output reference. Use .midi() in Strudel patterns to route audio to external devices.', 'nvoos-algorave' ),
		);
	}
