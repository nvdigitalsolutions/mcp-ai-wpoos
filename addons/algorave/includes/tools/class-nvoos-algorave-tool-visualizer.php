...
class NV_oOS_Algorave_Tool_Visualizer implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_description() {
		return __( 'Control the audio visualizer in the browser. Change visualization mode (waveform, spectrum, bars, circular), adjust colors, toggle fullscreen, or turn the visualizer on/off. Use this when the user wants to change how the audio is displayed visually during live coding.', 'nvoos-algorave' );
	}
		...
		switch ( $action ) {
			case 'set_mode':
				...
				if ( ! in_array( $mode, $valid_modes, true ) ) {
					return array(
						'success' => false,
						'error'   => __( 'Invalid mode. Use: waveform, spectrum, bars, circular, or particles.', 'nvoos-algorave' ),
					);
				}
				$result['mode']    = $mode;
