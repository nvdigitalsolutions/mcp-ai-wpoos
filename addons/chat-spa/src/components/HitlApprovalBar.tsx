/**
 * NV oOS Chat SPA — HITL Approval Bar (Phase 5).
 *
...
export interface HitlApprovalBarProps {
	...
	/** Session key from useTranscriptSession — used to filter approvals. */
	sessionId?: string;
...
export function HitlApprovalBar( {
	...
	return (
		<div
			className="nvoos-chat-spa-hitl-bar"
			role="alert"
			aria-label={ __( 'Pending approvals', 'nvoos-chat-spa' ) }
		>
			<p className="nvoos-chat-spa-hitl-bar-heading">
				{ sprintf(
					/* translators: %d: count of pending approvals */
					__( '⚠️ %d action(s) awaiting approval', 'nvoos-chat-spa' ),
					pending.length
				) }
			</p>
			<ul className="nvoos-chat-spa-hitl-list">
				{ pending.map( ( record ) => (
					<ApprovalCard
						key={ record.id }
						record={ record }
...
function ApprovalCard( {
	...
	return (
		<li className="nvoos-chat-spa-hitl-card">
			<div className="nvoos-chat-spa-hitl-card-meta">
				<strong className="nvoos-chat-spa-hitl-tool">{ record.tool }</strong>
				{ argsSummary && (
					<code className="nvoos-chat-spa-hitl-args">{ argsSummary }</code>
				) }
				{ record.reason && (
					<span className="nvoos-chat-spa-hitl-reason">{ record.reason }</span>
				) }
			</div>
			<div className="nvoos-chat-spa-hitl-card-actions">
				<button
					type="button"
					className="nvoos-chat-spa-hitl-approve"
					disabled={ isBusy }
					aria-label={
						/* translators: %s: tool name */
						sprintf( __( 'Approve %s', 'nvoos-chat-spa' ), record.tool )
					}
					onClick={ onApprove }
				>
					{ isBusy
						? __( '…', 'nvoos-chat-spa' )
						: __( '✓ Approve', 'nvoos-chat-spa' ) }
				</button>
				<button
					type="button"
					className="nvoos-chat-spa-hitl-deny"
					disabled={ isBusy }
					aria-label={
						/* translators: %s: tool name */
						sprintf( __( 'Deny %s', 'nvoos-chat-spa' ), record.tool )
					}
					onClick={ onDeny }
				>
					{ isBusy
						? __( '…', 'nvoos-chat-spa' )
						: __( '✕ Deny', 'nvoos-chat-spa' ) }
				</button>
			</div>
		</li>
	);
