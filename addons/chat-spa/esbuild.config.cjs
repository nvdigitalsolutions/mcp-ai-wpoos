/**
 * esbuild configuration for the NV oOS Chat SPA addon.
 *
 * Produces:
 *   assets/dist/chat-spa.js   — IIFE bundle
 *   assets/dist/chat-spa.css  — Extracted stylesheet
 *
 * @since 0.1.0
 */
...
bundle:      true,
outfile:     path.join( outDir, 'chat-spa.js' ),
format:      'iife',
globalName:  'NVoOSChatSpa',
platform:    'browser',
...
setup( build ) {
build.onEnd( () => {
const def = path.join( outDir, 'chat-spa.css' );
...
ctx.watch();
console.log( '[chat-spa] Watching for changes…' );
} );
} else {
