...
class NV_oOS_Graphify_Tool_Shortest_Path implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $start_label || ! $end_label ) {
			return array( 'success' => false, 'error' => __( 'Both start and end are required.', 'nvoos-graphify' ) );
		}
		...
		if ( ! $start_node ) {
			return array( 'success' => false, 'error' => sprintf( __( 'Start node "%s" not found.', 'nvoos-graphify' ), $start_label ) );
		}
		if ( ! $end_node ) {
			return array( 'success' => false, 'error' => sprintf( __( 'End node "%s" not found.', 'nvoos-graphify' ), $end_label ) );
		}
		...
		foreach ( $path_ids as $nid ) {
			...
			$path_nodes[] = $n ? array( 'node_id' => $n->node_id, 'label' => $n->label, 'type' => $n->type, 'url' => $n->url ) : array( 'node_id' => $nid );
		}
