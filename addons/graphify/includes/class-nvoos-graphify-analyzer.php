<?php
/**
 * NV oOS Graphify — Graph Analyzer
 *
...
/**
 * Graph analytics engine for NV oOS Graphify.
 *
 * @since 0.5.0
 */
class NV_oOS_Graphify_Analyzer {
	 ...
	 */
	public static function detect_communities() {
		global $wpdb;
		...
		if ( count( $edges ) < 10 ) {
			$communities = self::connected_components( $nodes, $edges );
		} else {
			$communities = self::louvain( $nodes, $edges );
		}

		// Split oversized communities.
		$communities = self::split_oversized( $communities, count( $nodes ) );
		...
		foreach ( $communities as $community_id => $member_ids ) {
			...
			foreach ( $member_ids as $node_id ) {
				NV_oOS_Graphify_DB::set_community( $node_id, sanitize_key( $community_id ) );
			}
		}

		NV_oOS_Graphify_DB::set_meta( 'community_count', count( $communities ) );
	 ...
	 */
	private static function connected_components( array $nodes, array $edges ) {
		$parent = array();
		foreach ( $nodes as $n ) {
	...
	private static function louvain( array $nodes, array $edges ) {
		...
		if ( $total_weight <= 0 ) {
			return self::connected_components( $nodes, $edges );
		}
	 ...
	 * @return array<string, string[]>
	 */
	private static function split_oversized( array $communities, $total_nodes ) {
		if ( $total_nodes <= 0 ) {
			return $communities;
		}
	 ...
	 */
	public static function get_god_nodes( $limit = 10, $type = '' ) {
		return NV_oOS_Graphify_DB::list_nodes(
			array(
				'limit'    => absint( $limit ),
				'order_by' => 'degree',
				'order'    => 'DESC',
	 ...
	 */
	public static function get_surprising_connections( $limit = 10 ) {
		global $wpdb;
		$edges_table = NV_oOS_Graphify_DB::edges_table();
		$nodes_table = NV_oOS_Graphify_DB::nodes_table();
	 ...
	 * }
	 */
	public static function get_knowledge_gaps() {
		global $wpdb;
		$nodes_table = NV_oOS_Graphify_DB::nodes_table();
		$edges_table = NV_oOS_Graphify_DB::edges_table();
	 ...
	 * @return array
	 */
	public static function get_recommendations( $limit = 10 ) {
		global $wpdb;
		$nodes_table = NV_oOS_Graphify_DB::nodes_table();
		$edges_table = NV_oOS_Graphify_DB::edges_table();
	 ...
	 */
	public static function shortest_path( $start_node_id, $end_node_id, $max_depth = 6 ) {
		...
		while ( ! empty( $queue ) ) {
			...
			$neighbors = NV_oOS_Graphify_DB::get_neighbor_ids( $current );
	...
	public static function traverse( $seed_node_id, $depth = 2, $mode = 'bfs', $max_nodes = 50 ) {
		...
		foreach ( array_unique( $node_ids ) as $nid ) {
			$node = NV_oOS_Graphify_DB::get_node( $nid );
			if ( $node ) {
				$nodes[] = $node;
			}
		}
