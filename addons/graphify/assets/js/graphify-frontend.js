/* jshint esversion: 6 */
/**
 * NV oOS Graphify — Frontend Embed
 *
 * Powers the [nvoos_graphify] shortcode and Gutenberg block on the frontend.
...
( function ( $ ) {
	 ...
	 *
	 * @param {Object} config nvoosGraphifyData_{containerId} object.
	 * @return {void}
	 */
	...
	function fetchAndRender( config, $container ) {
		...
		$.ajax( {
			...
			$container.html( '<p class="nvoos-graphify-error">Graph data unavailable.</p>' );
		} );
	}
	...
	// -------------------------------------------------------------------------
	$( function () {
		$( '[id^="nvoos-graphify-"]' ).each( function () {
			var rawId = $( this ).attr( 'id' );
			// Each container has a matching nvoosGraphifyData_{id_with_underscores} object.
			var dataKey = 'nvoosGraphifyData_' + rawId.replace( /-/g, '_' );
			if ( window[ dataKey ] ) {
