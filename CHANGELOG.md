...
- On PHP 8+ calling a disabled function throws a fatal `Error` that `@` cannot suppress — the Pro Packages admin page fataled on hosts where `exec` is disabled via `disable_functions`, and the same unguarded pattern existed in five other Pro admin pages and the OCR service. New canonical helpers in `addons/pro/includes/npm-integration-filters.php` — `wp_mcp_ai_check_nodejs_available()` and `wp_mcp_ai_get_nodejs_version()` — try `exec()` first (gated on `function_exists`) and fall back to the Process Service (`proc_open`); the six Pro admin pages (Pro Packages, Document Generation, Image Production, Media Settings, Media Toolkit, Media Command Center) now delegate to them and can never fatal. The OCR service routes `convert_pdf_to_images` / `convert_pdf_with_pdftoppm` / `extract_with_tesseract` / `determine_best_provider` through new `is_cli_tool_available()` / `run_cli_command()` helpers (exec-first, Process Service fallback, Windows-aware `where`/`which`) and guards `proc_open`/`proc_close`/`proc_terminate` in `execute_node_service_with_timeout()`. Negative settings inputs are clamped with `max( 0, (int) $value )` instead of `absint()` (which flips `-5` into `5`) across the six CPT settings pages.

### Changed — PHPUnit Repair Campaign Continuation, Wave 5 (PRs #6280–#6308)

- A fifth campaign wave (~18 test-suite PRs + grouped production fixes) kept the suite aligned after the 1.1.69 catch-up. Production fixes it carried:
- **Transcript attachments & token caps** (#6280) — the REST validator now sanitizes `display.attachments` (URL/label/downloadName/meta) instead of dropping them; the OpenAI client caps the resource-aware `max_tokens` default against the model's own window and max-output limits so small-window models aren't hard-rejected by the pre-flight guard.
- **Model routing & catalog** (#6298, #6306) — `WP_MCP_AI_Enhanced_OpenAI_Client` passes an empty model as the routing base so complexity-based light/advanced selection actually runs (feeding the settings default short-circuited it); `WP_MCP_AI_Token_Budget_Manager::get_model_limit()` retries provider-prefixed slugs without their leading provider segment (`nvidia/nemotron-…`); `WP_MCP_AI_HTTP::is_timeout_error()` recognizes the `timed-out` variant.
- **JetEngine availability & CCT tables** (#6296, #6300) — JetEngine detection now requires the `JET_ENGINE_VERSION` constant (file-scope test stubs leaked process-wide, so `function_exists('jet_engine')` alone couldn't prove the real plugin is loaded) across Shopify Sync, `get_jetengine_items`, and the JetEngine Pro tool; `WP_MCP_AI_JetEngine_CCT::is_storage_available()` gates conversation-import writer/deleter/tool on the physical CCT table; the JetEngine Pro tool's `cct_table_exists()` uses a direct SELECT probe instead of `SHOW TABLES` (MySQL 8.0's data-dictionary view misses transactional DDL); the Pro tool map is cached (`wp_mcp_ai_pro_get_tool_map()`) so the token-usage service enumerates gated-off Pro tools without rebuilding it.
- **OAuth & Auth0** (#6301) — the GitHub OAuth handler's settings-page redirect honors a `wp_mcp_ai_github_oauth_redirect_terminate` filter (test seam; production exits as before); `generate_auth0_token` validates the audience by scheme/host syntax instead of `wp_http_validate_url()` (which DNS-resolves and rejected non-publicly-resolvable identifiers).
- **REST & skill packs** (#6302) — markup REST validation failures now carry `status 400` instead of defaulting to 500; the MCP controller's legacy HTTP+SSE handshake is filterable via `wp_mcp_ai_legacy_sse_enabled` (mixed Accept headers never trigger it); the skill-pack registry drops the empty-pack early return and requires a slug (numeric-keyed entries are invalid and skipped).
- **Mesh & image tools** (#6285, #6305, #6306) — mesh peer URL validation now rejects scheme-less strings before `esc_url_raw()` silently prefixes them; `WP_MCP_AI_Tool_Image_Base` checks `user_can( $user_id, 'read_post', … )` against the acting user instead of the global current user (cron/CLI/token contexts); `safe_set_current_user()` validates positive integers via `filter_var` (absint flips negatives); `validate_image_for_product` falls back to direct `image_url`/`url` arguments when the resolver returns nothing; `scrape_product` fixes CSS→XPath descendant/predicate ordering; `web_search` preserves `published_date` from Tavily/Brave.
- **Memory, harness & logging** (#6286, #6308) — memory auto-capture writes guest observations to a shared `guest` bucket only when the site opts in via `wp_mcp_ai_memory_auto_capture_guests_allowed`, and the dedup window is only consumed once a capture is actually eligible; harness trace `meta.json` gains millisecond-precision `started_at_ms` with a 1 ms duration floor; the logger event allowlist adds `agentic_tool_execution`/`agentic_model_switched`/`agentic_messages_truncated`/`agentic_loop_limit`/`token_db_optimized`.
- **Presets & dashboards** (#6281, #6282, #6283, #6287, #6293) — the Pro dashboard security-event switch accepts hyphenated event types (`file-integrity`, `plugin-updates`, `configuration`, `security-alerts`); `web_search_validated` gains `writer`/`librarian` profession tags; the CRM Data Cleanup preset drops the unavailable `validate_contact_data` step and the CRM auto-import preset uses a proper `daily` schedule type with its hook in `schedule_data`; `WP_MCP_AI_Pro_CPT_AI_Integration::reset_for_tests()` (test seam); the fine-tune curriculum exporter skips empty input/expected structures instead of emitting `[]` rows; the Elementor editor detection honors the `action=elementor` flag in the shortcode and the template-kit import uses `static::is_available()`. Conversation-import tools (`conversation_import_detect|run|status|delete`) are added to the tool presets (#6296).
- **Admin & settings drift** (#6303) — the quiz settings page clamps negatives before capping (`max( 0, min( 100, (int) … ))`); the CPT settings base documents the clamp contract; the plugins-integration admin redirect honors a `wp_mcp_ai_plugins_integration_redirect_terminate` filter (test seam; production exits as before).
- **Test-only PRs** — #6284 (Cloudflare), #6288 (PayHere), #6289 (music generation), #6290 (OpenAI image), #6291 (Pro settings optional deps), #6292 (Content Graph AI ecosystem), #6297 (composer detection), #6299 (Gemini), #6307 (orchestration budget) plus the docs-only #6294 (test-suite cluster board) and the new CI-triage artifact `docs/developer/testing-docs/CI-TRIAGE-91771001271.md` (#6296).

### Versioning

- Bumped to 1.1.69 across plugin header, `WP_MCP_AI_VERSION` and `WP_MCP_AI_PRO_VERSION` constants, `package.json`, readme.txt Stable tag, README.md, CHANGELOG.md, QUICK_REFERENCE.md, and DOCUMENTATION_INDEX.md. Pro addon: 1.1.69. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: 1.0.3 (unchanged). nvoos-content-graph-ai: 1.0.4 (unchanged). nvoos-content-graph-ai-platform: 2.0.0 (unchanged). Docs Hub addon: **0.4.2** (unchanged). Tool count: ~303 base + ~1,263 Pro (~1,566 total; live registry authoritative — the Vision Analysis toolkit adds one Pro tool; wave 5 added no tools). Providers: 15. Addons: 27. Bundled skills: 74 base + 41 Pro. Coding-time agent skills: 53.
