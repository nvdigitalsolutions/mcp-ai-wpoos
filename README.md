...
**🆕 v1.1.83 Highlights:** A tool-guidance and reliability release. **Every base+Pro tool (~1,487 across 1,584 files) now carries model-facing usage guidance** — a new guidance interface (`when_to_use`/`when_not_to_use`/`related_tools`/`notes`) assembles a compact `[Usage: …]` suffix on the model-facing payload, legacy-format classes opt in through the legacy tool wrapper, and the guidance sniff is enforced at severity 5 so every future tool class without guidance surfaces on PRs (full-tree gate: 1,584/1,584 files, 0 warnings). **An opt-in adaptive tool cap** sizes the tools sent per model context window (site option + per-assistant override), and **`list_mcp_tools` gains lazy schema loading** (`tool_slug` + `include_schemas`). **A Pro bootstrap incomplete-install guard** keeps the site running on the base plugin when a partial Pro deploy leaves required files missing. **`send_telegram_message` auto-chunks messages over 4,096 characters.** **The Upwork pipeline delivers**: web_search-mode + credential gating (#6678), category-page dropping, SERP budget/type/recency extraction, an always-on broad second pass (#6679/#6680), and workflow deliveries now ship the **full result set** (50-item cap, filterable) with per-item URLs rendered as a single properly numbered list (#6682/#6684). **Gmail `connection_id: "settings"` resolves to the settings fallback**, and skill/OKF errors append the available options for self-correction (#6677). The **Playground Ollama demo lands at `/ollama-test-lab/` on fresh installs** (permalink seed fix) with the local `npx` run as the primary test path (#6683). npm advisories closed for adm-zip, js-yaml, and colord (#6681). Tool count: ~306 base + ~1,279 Pro (~1,585 total; unchanged). Stale build ZIPs removed: the 1.1.81 set (30 files).
...
### September 21, 2026 — Tool Guidance Everywhere, Pro Bootstrap Guard, Telegram Chunking, Upwork & Workflow Delivery
...
- 📏 **Regulatory Envelope Migration — PR #6689.** Five regulatory tools move to the canonical `WP_Error` envelope.
- 📦 **Versioning** — bumped to **1.1.83** across all version-bearing files. Pro addon: 1.1.83. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.8** (unchanged; ZIP rebuilt in-window). nvoos-content-graph-ai: **1.0.4** (unchanged; ZIP rebuilt in-window). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). nvoos-content-graph-pro: **1.0.0** (unchanged — mirror patches only, no port waves). Checkout API: **0.1.2** (unchanged). Docs Hub addon: **0.4.7** (unchanged). Comic Reader addon: **0.5.0** (unchanged). Model catalog: **v2026.09.10** (unchanged). Tool count: **~306 base + ~1,279 Pro (~1,585 total)** — unchanged, no tool registrations in-window; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative. Provider count: **15**. Addon count: **27**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **59** (unchanged — no new skills; the test-suite skill gained pattern 48 and the plugin/design-crm/playground-demos skills were updated in-window). Stale build ZIPs removed: the 1.1.81 set (30 files), superseded by the in-window 1.1.82 wp.org package builds.
