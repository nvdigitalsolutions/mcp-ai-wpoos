...
- 🧪 **PHPUnit Repair Campaign Continuation (~95 PRs, #6114–#6208).** A second cluster wave kept the suite green through the extraction/port work and PHPUnit 11 / WP 7.1 drift; the production fixes it carried are grouped in the changelog (crawler contract hardening, REST guards, LLM sanitization delegation, URL-encoded external-API queries, Veo 5s floor, settings/cache fixes, WhatsApp webhook signature rejection, HTTP 304 cached-body re-serve). The `mcp-ai-wpoos-test-suite` skill now distills **26** root-cause patterns.
