...
**🆕 v1.1.77 Highlights:** A model-refresh, base+pro & checkout-launch release. **DeepSeek refreshes to V4.1 Flash** — the catalog gains `deepseek-flash` (vision, $0.15/$0.60 off-peak), retires V4 Flash + Vision Exp, deprecates V4 Pro (sunset 2026-09-14), and the cost calculator learns **peak/off-peak pricing** (DeepSeek's Mon–Fri 01:00–04:00 + 06:00–10:00 UTC schedule). **A new Knowledge Graph Companion onboarding preset** ships — featured, graph-aware, and auto-selected on sites with a connected content graph. **Pro toolkits finally load in base+pro installs** — the load gates now honor the Pro constant, backed by a permanent 15-test base+pro regression matrix with its own CI job. **The Pro addon stops fataling WP-CLI** when activated before the base plugin. **The agent-memory CCT phantom slug is fixed** — Graphify graph builds and Pro memory retention now read the canonical `ai_agent_memories` table, so dormancy sweeps, per-user caps, and Memory Health stats start working. **The NV oOS Complete checkout launches end-to-end** — Checkout API **0.1.1** (Stripe boolean serialization fix, 424/502 error contract, connection test, `GET /health`) and Content Graph **1.0.7** (CCT source selection, trust/post-purchase modal, Stripe element + release-tag fixes). **Four npm packages patched**; **Wave G completes five more `nvoos-content-graph-pro` clusters** (law-firm, cre-debt, quiz, eca, chat-channels) and starts places. Tool count unchanged: ~303 base + ~1,265 Pro (~1,568 total). Stale 1.1.75 build ZIPs removed (30 files).
