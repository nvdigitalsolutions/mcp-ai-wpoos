...
**🆕 v1.1.85 Highlights:** An MCP Apps connection & exposure release. **Metabox diagnostics** — per-row Test Connection / Discover Tools buttons with inline protocol, handshake, server, session, latency, live tool-count, and verbatim error results; a persisted per-app status badge + tool-count chip; and Test All (#6753). **Basic auth end-to-end** with token masking (stored credentials never echoed; empty = keep) and mcpServers JSON import; the client echoes `Mcp-Session-Id`, falls back to the legacy handshake, and advertises the **negotiated protocol version** on post-initialize requests (#6753/#6754). **Bridge tools now reach the model** — the new `wp_mcp_ai_chat_effective_tools` seam appends the assistant's enabled `mcp_app_<label>_<tool>` slugs to the chat payload after attention filtering and before capability checks, and the resolved assistant ID flows through `handle_tools_list()`, `handle_tool_request()`, `execute_tool_call_internal()`, and the `list_mcp_tools` catalogue (#6755/#6758). **Same-site MCP servers dispatch in-process** via `rest_do_request()` — no outbound socket, no TLS handshake, no second PHP-FPM worker — with `rest_post_dispatch` parity, a 60 s negative discovery cache, and outbound-HTTP safety rails (#6756/#6757). **Security Center** gains an MCP App Allowed Hosts setting (#6753). **Docs Hub 0.5.1** — local-first uploads source, opt-in remote import, symlink-escape hardening, slug-named uploads folder with a one-time migration (0 blocking Plugin Check errors, #6749/#6750/#6759). **README consolidation** — one 12-release Release History section + complete Previous Releases table (#6751/#6752). Tool count: ~308 base + ~1,282 Pro (~1,590 total; unchanged — bridge slugs are dynamic chat-time registrations). Stale 1.1.83 build ZIPs removed.
...
### ✨ What's New at a Glance (v1.1.85)

- 🔌 **MCP Apps connection diagnostics (PR #6753).** Per-row Test Connection / Discover Tools buttons with inline results (negotiated protocol, handshake type, server info, session state, latency, live tool count, verbatim errors); persisted per-app status badge + tool-count chip; Test All. Basic auth end-to-end (raw `user:pass` auto-encoded or pre-encoded base64) with token masking — stored credentials never echoed into the metabox HTML, empty = keep. `Mcp-Session-Id` capture + echo, legacy-handshake fallback on `-32601`/`-32600`/session errors, JSON-RPC error-body decoding on HTTP 4xx, spec-compliant `Accept` header, mcpServers JSON import, loopback detection with a PHP-FPM deadlock warning. **Security Center → Network & Headers: MCP App Allowed Hosts** (constant hard override → filter + saved setting merged).
- 🤝 **Negotiated protocol header (PR #6754).** The client records the negotiated `protocolVersion` and advertises it post-initialize (2026-07-28 kept stateless); the `_meta` envelope is suppressed once a legacy session is established. PHPUnit-11 one-class-per-file discovery fixed — the #6753 registry/REST suites now genuinely execute.
- 🌉 **Bridge tools reach the chat payload (PR #6755).** New base `wp_mcp_ai_chat_effective_tools` filter seam in `build_tools_payload()` (after attention filtering, before capability checks; empty selection no longer short-circuits before the filters); Pro `wp_mcp_ai_mcp_apps_expose_tools()` appends the enabled apps' `mcp_app_<label>_<tool>` slugs; registry gains `collect_remote_tools()` + `get_remote_tool_slugs()`.
- ⚡ **In-process same-site bridge (PR #6756).** Same-origin MCP endpoints dispatch via `rest_do_request()` — no outbound socket, no TLS handshake, no second PHP-FPM worker (ends the worker-starvation TLS deadlock on self-hosted servers); outbound HTTP kept for remote hosts, unregistered routes, and the `wp_mcp_ai_mcp_app_disable_inprocess_bridge` filter.
- 📨 **`rest_post_dispatch` parity (PR #6757).** The in-process path re-applies the filter after `rest_do_request()` (mirroring `serve_request()`), so response-side session headers (e.g. EMCP's `Mcp-Session-Id`) reach the client and `tools/list` stops failing.
- 🧰 **Bridge tools wired into every tool surface (PR #6758).** The seam carries a third `$assistant_id` argument applied on `handle_tools_list()`, `handle_tool_request()`, `execute_tool_call_internal()`, and the `list_mcp_tools` catalogue; Pro exposure registers bridges in the local registry (idempotent, transient-cached); `rest_pre_dispatch` registers bridges before REST arg validation; 60 s negative discovery cache; `save_apps()` invalidates the `/tools` REST list cache.
- 📚 **Docs Hub 0.5.1 (sub-project, PRs #6749/#6750/#6759).** Local-first by default (`sources = ['uploads']`, zero remote calls) with the GitHub importer as an explicit opt-in service; `is_path_safe()` symlink-escape hardening; slug-named uploads folder (`nvoos-docs-hub/content/`) with a one-time non-destructive migration of the legacy `uploads/docs` path; 0 blocking Plugin Check errors.
- 📐 **README anchors + consolidation (PRs #6751/#6752).** Six VS16-fallback TOC anchors fixed; `bin/validate-readme-anchors.py` repaired to GitHub's real anchor rules + CI enforcement on README changes; one `## 📜 Release History` section keeps the 12 most recent releases in full (v1.1.84 → v1.1.73) plus a complete Previous Releases table (README 4,423 → 3,301 lines).
