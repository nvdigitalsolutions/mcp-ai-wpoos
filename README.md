...
**🆕 v1.1.73 Highlights:** A WooCommerce tool-upgrade & ecosystem-port release. **`bulk_update_products` expands variable parents** — a new `scope=all` mode pushes price/stock updates from variable parents to their variations and grouped parents to their children, re-syncs variable parents, and reports `targets[]` per input ID (`scope=product` keeps the legacy exact-ID behavior). **`update_woo_product_qty` gains a `notify` flag** (default `true`) that suppresses the low-stock/no-stock notification emails for a single write without touching the `woocommerce_low_stock`/`woocommerce_no_stock` actions. **The async job queue table bootstrap is fixed** — the queue class now boots in time to create its table (activation + first page load self-heal), ending the missing-table SQL floods. **Comic Reader jumps 0.2.0 → 0.5.0** with a Komga-parity upgrade (archive validation, Range serving, vertical/webtoon modes, ComicInfo.xml RTL detection, per-user progress). **Content Graph ecosystem Wave F2 completes** — the new **`nvoos-content-graph-pro`** standalone addon (v1.0.0) ports the Pro CRM + e-commerce surface (43 e-commerce tools), and **Docs Hub 0.4.3** completes wp.org submission prep. New `mcp-ai-wpoos-wporg-submission` coding-time skill (**55** skills now). Tool count unchanged: ~303 base + ~1,265 Pro (~1,568 total).
