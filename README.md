...
**🆕 v1.1.75 Highlights:** A scheduled-delivery & ecosystem-port release. **Scheduled Telegram delivery is fixed twice over** — inline broadcast credentials stored as JSON strings no longer fatal the array-typed broadcast tool (normalize + validate end-to-end, malformed values fail gracefully per channel), and cron delivery now falls back to the first enabled Remote Sites connection of the channel type (assistant-assigned preferred) instead of failing with "no valid credentials" — `create_pro_schedule`/`update_pro_schedule` and the schedules REST endpoint now accept `result_delivery` instead of silently dropping it. **The Content Graph memory bridge goes live** — base + ecosystem memories project into the standalone graph behind a new `wake_up_context` graph-retriever filter seam, and the Content Graph checkout now sells the **NV oOS Complete bundle** (base + Pro) with a conflict guard — nvoos-content-graph bumps **1.0.4 → 1.0.6**. **Wave F2 completes five more `nvoos-content-graph-pro` toolkits** — financial-planning (16 tools + admin slice), social-media (32 tools + settings page), all 33 MCP toolkit servers, plus remote-sites, video-production, analytics, multilingual, cloudways, dj-management, and image-production slices. Tool count unchanged: ~303 base + ~1,265 Pro (~1,568 total). Stale 1.1.73 + content-graph 1.0.4 build ZIPs removed.
...
| `.agents/skills/` | 55 coding-time agent skills for Zed editor (20 wp-* WordPress plugin development patterns + 31 design-* skills + mcp-ai-wpoos-plugin operational guide + mcp-ai-wpoos-test-suite repair guide + mcp-ai-wpoos-updates maintenance guide + mcp-ai-wpoos-wporg-submission wp.org readiness guide) |
