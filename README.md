...
**🆕 v1.1.76 Highlights:** A scheduled-delivery, checkout-launch & ecosystem-port release. **Chat result delivery goes full-featured** — every chat channel now supports the `full` report template (summary + substantive response + envelope data) with a per-channel `format` selector (Telegram `html`/`markdown`/`markdown_v2`/`plain` via the Bot API parse mode; WhatsApp/Slack/Discord/Teams `markdown`/`plain`; Messenger/Google Chat `plain`), and `full` templates no longer print the assistant's summary twice. **The Comic Creation toolkit can finally be enabled** — its settings toggle is now registered (Tools section, Pro Features subtab, memory estimator, WP-CLI). **The NV oOS Complete checkout is launch-ready** — required ToS/refund consent + buyer email at purchase, EU country selector with required billing address, Stripe Product/Price metadata + statement descriptor, manual-install-first flow, and the full commercial legal set (`docs/legal/`: privacy policy, acceptable-use policy, clickwrap guide, compliance checklist, updated ToS). **Playbook syncs stop churning attachments** — the seeder's content hash now ignores the per-second `Generated:` timestamp. **Four security sweeps** close 11 Dependabot alerts (tiptap, multer, csv-parse, react-router-dom, SVGO CVE-2026-84370, hono, vitest) and unblock the schedule-anything-spa build; the docs-hub wp.org ZIP stops shipping dev Markdown. **Wave F2 completes ten more `nvoos-content-graph-pro` clusters** — site-creator (33 tools), document-generation, regulatory-registration, healthcare (9 batches), law-firm, image-production, comic-creation, dj-management, ai-tool-builder, architect-agent, architectural-design. New `mcp-ai-wpoos-ecosystem-port` coding-time skill (**55 → 56**). Tool count unchanged: ~303 base + ~1,265 Pro (~1,568 total). Stale 1.1.74 build ZIPs removed (30 files).
