...
**🆕 v1.1.74 Highlights:** A calendar, scheduling & ecosystem-port release. **Google Calendar date queries are fixed** — every query value is now percent-encoded (the raw `+` in RFC3339 timestamps was decoded as a space, 400-ing every `time_min`/`time_max` call), and `calendar.freebusy` joins the Standard scope profile so availability checks work on a normal connection. **Result Delivery emails render HTML** — a new `WP_MCP_AI_Markdown_Converter` (escaped + `wp_kses`-allowlisted + protocol-allowlisted links) turns assistant Markdown digests into email-safe HTML behind a per-channel `format` setting (`both`/`html`/`markdown`, default `both`, zero migration). **The Pro Schedule Manager edit modal can now display and update the assistant prompt** for `assistant_run` schedules, and `update_pro_schedule` accepts `assistant_config` via MCP. **Wave F2 completes two more toolkits in `nvoos-content-graph-pro`** — the Project Management port (data layer → research/settings pages) and the calendar-booking port (data layer → admin slice). Test/CI: the perf-suite MCP-abilities failure is fixed **process-wide** in the test bootstrap, and `nvoos-content-graph-pro` lints under its own standard. Tool count unchanged: ~303 base + ~1,265 Pro (~1,568 total).
