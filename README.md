...
**🆕 v1.1.86 Highlights:** A MCP Apps × Remote Sites, Higgsfield media, and delivery-quality release. **MCP Server connections** — the new `mcp_server` Remote Sites connection type (Proposal 041, #6761) stores central credentials AES-256-CBC-encrypted, runs a real JSON-RPC handshake on Test Connection, persists tool-discovery snapshots, and maps auth types onto MCP (Elementor application passwords → `basic`); per-assistant `connection_ref` reference mode resolves credentials decrypt-on-use at chat time with auto-disable on missing import refs, and assistant export/import now redacts MCP App `token`/`oauth_data` by default. **Four new Higgsfield base tools** (#6772) — `generate_higgsfield_video` (Cinema Studio 4.0, Seedance 2.5/2.0, Wan 3.0, Kling 3.0 behind one model parameter), `generate_higgsfield_image` (SOUL V2/SOUL Cinema), `check_higgsfield_request`, `cancel_higgsfield_request` — on a shared submit/status/cancel client with backoff+jitter polling and immediate download. **`get_system_logs` becomes searchable** — optional `since`/`levels`/`search` filters over the structured buffers + file logs with a filters summary (#6768). **Scheduled digests get an `action_items` template** plus smart excerpts that prefer the response's own Summary/TL;DR/action sections over blind 80-word trims (#6773). **Tool costs reach the final response label** — a top-level cost envelope on both the agentic loop and SSE streaming, with usage/cost badges on tool bubbles (#6771). **Environment status fixed** — the always-on "no assistants published" warning and the dead default-assistant branch are repaired, and `plugin.default_provider_model` resolves the effective per-provider model (#6767). Tool count: ~312 base + ~1,282 Pro (~1,594 total; +4 base). Stale 1.1.84 build ZIPs removed.
...
### ✨ What's New at a Glance (v1.1.86)

- 🔌 **MCP Apps as a Remote Sites connection type (PR #6761, Proposal 041).** New `mcp_server` connection type with AES-256-CBC-encrypted central credentials (incl. the `mcp_oauth` blob), a real JSON-RPC handshake Test Connection, tool discovery with persisted snapshots, restricted-host enforcement, and activity logging; auth mapping `basic_auth`/`application_password` → MCP `basic`, `custom_header` → `header`, `bearer`, `oauth`. Per-assistant `connection_ref` reference mode resolves credentials decrypt-on-use at chat time (never written back to post meta); missing refs skip with error-status snapshots and imported bundles with missing refs auto-disable with a warning. Assistant export/import redacts MCP App `token`/`oauth_data` by default (opt-out filters); the metabox gains "Add from Remote Sites" reference rows + read-only "Managed in Remote Sites" rows. New coding-time skill `design-elementor-mcp-connection`.
- 🎬 **Higgsfield video & image provider (PR #6772, Proposal 042).** Four new base tools — `generate_higgsfield_video` (Cinema Studio 4.0, Seedance 2.5/2.0, Wan 3.0, Kling 3.0 behind one `model` param with per-model payload mapping/clamping + reference inputs + cinematic controls), `generate_higgsfield_image` (SOUL V2 + SOUL Cinema, batch 1|4), `check_higgsfield_request`, `cancel_higgsfield_request` — on the shared `WP_MCP_AI_Higgsfield_Client` (two-part `Key ID:SECRET` auth, submit/status/cancel lifecycle, backoff+jitter polling 2s→10s, immediate download against the 7-day retention; settings → env → constants credential chain). `lib/core` dual-layer wrappers registered via `oos-bridge`. Also fixes the TypeSafe tool schemas + Pro coverage manifest.
- 🔍 **`get_system_logs` filters (PR #6768).** Optional AND-combined `since` (relative "2h"/"30m"/"3d" or absolute ISO 8601 / `Y-m-d H:i:s` UTC), `levels` (critical/error/warning/notice/deprecated), and `search` (case-insensitive, 200-char cap) filters over the structured NV oOS buffers + file logs, with a filters summary (resolved UTC cutoff) and per-file `filtered_out` counts; `parse_since()` is the shared canonical parser; CG AI ports base-identical.
- 📨 **Action-items template + smart excerpts (PR #6773).** Scheduled digests stop blind-trimming the first 80 words: the new `action_items` template sends only the actionable section ("Action Items"/"Needs your attention"/"Next steps"), and the `summary` excerpt prefers the response's own distillation section, then the action block, then centroid-ranked sentences, with a lead trim as final fallback.
- 💰 **Tool costs in final response labels (PR #6771).** New `build_tool_cost_envelope()` emits the top-level `toolResult.cost` shape (`cost_usd`/`is_estimated`/`provider`/`model`) on the agentic loop + SSE streaming; the client's shared `aggregateToolUsageBadgeData()` aggregates it (with legacy-shape fallbacks) and tool bubbles render the same usage/cost badges.
- 🩺 **Environment status fixed (PR #6767).** The always-on "no assistants published" warning and the never-firing default-assistant branch are repaired (envelope-key mismatch, base + CG port in sync); DeepSeek joins the provider warning maps; new `plugin.default_provider_model` resolves the effective per-provider model and the default assistant entry carries its provider/model meta.
- 📦 **Versioning** — bumped to **1.1.86** across all version-bearing files. Pro addon: 1.1.86. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.8** (unchanged). nvoos-content-graph-ai: **1.0.4** (unchanged — ZIP rebuilt). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). nvoos-content-graph-pro: **1.0.0** (unchanged). Checkout API: **0.1.2** (unchanged). Docs Hub: **0.5.1** (unchanged). Comic Reader: **0.5.0** (unchanged). Model catalog: **v2026.09.22** (unchanged). Tool count: **~312 base + ~1,282 Pro (~1,594 total; +4 base — the Higgsfield quartet)**. Provider count: **15** chat providers (Higgsfield is a media-generation provider, not a model-catalog provider). Addon count: **27**. Bundled skills: **75** base + **41** Pro (unchanged). Coding-time agent skills: **59 → 60** (new `design-elementor-mcp-connection`). Stale build ZIPs removed: the 1.1.84 wp.org package set (6 files).
...
- **~1,594 tool classes** (~312 base + ~1,282 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative) registered through a singleton Tool Registry
