...
**🆕 v1.1.68 Highlights:** A front-end surfaces & stability release. **Pro SPA v2 ships a shortcode** — `[nvoos_pro_spa]` embeds the Pro chat surface (threads, drawers, tool shortcuts, OKF drawer) on any page in a chat-first embedded mode, with optional guest mode riding the guest-token machinery (proposal 033). **Hermes WebUI gains an `extensions/` tree** — a fleet monitoring + control plane extension (Python plugin API over ~35 sites) plus `backup-download`, `external-app-tab`, and `mcp-tool-shortcuts` extensions with installer + smoke tests. **Chat surfaces self-heal stale nonces** — the new `GET /mcp-ai/v1/session/nonce` endpoint mints a fresh session-bound nonce, ending the "Cookie check failed" 403s caused by full-page caching or session-token rotation on long-lived pages (Docs Hub, SPA embeds). **Docs Hub 0.4.2** fixes local-page link navigation, TOC anchors (github-slugger-exact), directory-relative "Accept fix" suggestions, `../` link validation with skip reasons, sync-failure surfacing — and stops the core emoji loader from crashing the React SPA. **Fresh installs now default cloud providers to disabled** — dropdowns list only enabled + credentialed providers, and the onboarding wizard auto-enables the provider whose key you enter. Plus: Google Calendar granted-scopes URL-corruption fix, Tiptap `mergeAttributes` prototype-pollution pins, `fast-uri` >=4.1.4 across npm trees, assistant untrash status restoration, presets gaining the missing Google/Gmail/Drive/OKF/git tools, and a jQuery UI sortable shim for post screens. Tool count unchanged: ~303 base + ~1,262 Pro (~1,565 total).
...
- 🧪 **PHPUnit Repair Campaign Continuation (~21 PRs, #6224–#6257).** A third wave kept the suite aligned with current contracts; the production fixes it carried are grouped above.
...
- **~1,565 tool classes** (~303 base + ~1,262 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative) registered through a singleton Tool Registry
...
- 🧪 **PHPUnit Repair Campaign Continuation — PRs #6223–#6257.** Third wave (~21 test-only PRs: #6224, #6227–#6231, #6234–#6241, #6245, #6247, #6248, #6251, #6252, #6254, #6257 + CI alpha-log refresh #6223) kept the suite aligned with current contracts; the production fixes it carried are listed above.
...
2. You now have the **complete version** with all ~1,565 tools (~303 base + ~1,262 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)

**What you get from the repository clone:**

- ✅ The full codebase — all ~1,565 built-in tools ready to use (~303 base + ~1,262 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
...
- **[Architecture Overview](docs/developer/architecture/ARCHITECTURE.md)** - System architecture (15 providers, ~1,565 tool classes, 36 REST controllers)
...
- **[Tool Reference](docs/reference/tools/tool-reference.md)** - All ~1,565 tools documented (~303 base + ~1,262 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
