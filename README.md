...
**🆕 v1.1.87 Highlights:** A parity-suite, image-identification, and outbound-booking release. **30 new base tools from the mcp-wordpress parity suite** (#6777) — comment/user CRUD, content/media/terms, site settings + application passwords (log-masked), and a 9-tool SEO toolkit, with capability checks, multisite guards, and new ID-handoff contracts. **Non-LLM image identification ladder** (Proposal 043, #6780/#6785) — `identify_image` orchestrates metadata → perceptual-hash lookup → classic Cloud Vision detection → layout description → optional reverse-image search without ever calling a vision LLM; five base tools join the Media Generation preset plus two Pro tools (`ocr_image_classic`, `search_similar_images`); every external rung is key-gated and fails closed. **Outbound appointment booking toolkit** (Proposal 044, #6786) — ICP lists, an hourly sequence engine with send windows/daily caps/consent attestation, DM approval boards, the angle-bank CPT with A/B tests, booking links + shortcode + a rate-limited honeypotted public booking endpoint, and three new Pro tools. **mcp-wordpress gateway addon** (#6778) — auth-gated Streamable HTTP MCP server for Cloudways Velocity. **Per-session budget warnings** (#6776) — blocked-session messages point at the real reset path, the dormant 75% hook now injects a one-shot warning via the new `wp_mcp_ai_chat_messages` filter. **Restriction notice fixed** (#6779) — no more user over-counting or stale banners. **Upwork refinements** (#6784) — in-place workflow step editing + `exclude_keywords`/tier/budget parsing. Tool count: ~347 base + ~1,287 Pro (~1,634 total; +35 base +5 Pro). Addons: 27 → 28. Stale 1.1.85 build ZIPs removed (30 files).
...
> - [Addon Inventory](docs/project/ADDON_INVENTORY.md) — what each of 28 addons does and its status
...
### ✨ What's New at a Glance (v1.1.87)

- 🔧 **mcp-wordpress parity suite — 30 new base tools (PR #6777).** The genuinely missing surface from the MIT upstream lands as native PHP: comment CRUD (`list_comments`–`delete_comment`), user CRUD (`list_users`–`delete_user`), content/media/terms (`get_post_revisions`, `get_term`, `delete_term`, `get_media`, `upload_media`, `update_media`, `delete_media`), site settings + application passwords (`get/update_site_settings` allowlist-only, `list/create/delete_application_passwords` log-masked), and a 9-tool SEO toolkit (`seo_analyze_content` through `seo_keyword_research`). Capability checks, multisite guards, and new `comment_id`/`user_id`/`attachment_id` ID-handoff contracts.
- 🖼️ **Non-LLM image identification ladder (PR #6780 + #6785, Proposal 043).** `identify_image` answers "what is on this image?" without a vision LLM — metadata → pure-PHP dHash media lookup → classic Cloud Vision detection → deterministic layout description → optional reverse-image web search, with a confidence score + escalation hint. Five new base tools join the Media Generation preset; two new Pro tools (`ocr_image_classic`, `search_similar_images`) cover tesseract OCR + Google Lens. Every external rung is key-gated and fails closed (skipped, never an HTTP request without credentials; SSRF-guarded).
- 📅 **Outbound appointment booking toolkit (PR #6786, Proposal 044).** A new Pro toolkit (`enable_outbound_booking_toolkit`) turns CRM leads + sequences into booked sales calls: ICP list building, an hourly outbound engine with send windows/daily caps/consent attestation, email + LinkedIn/Instagram DMs behind a human approval board, the `mcp_ai_oa_angle` angle bank with A/B tests, booking links + `[nvoos_oa_booking]` shortcode + a rate-limited honeypotted consent-gated public booking endpoint, and three new Pro tools (`outbound_get_pipeline_stats`, `outbound_import_leads`, `outbound_manage_angle`).
- 🌐 **mcp-wordpress gateway addon (PR #6778).** `addons/mcp-wordpress-gateway/` — an auth-gated Streamable HTTP MCP server for Cloudways Velocity (timing-safe `X-MCP-Token` auth, 1 MB body cap, deny-wins tool policy) deployed via subtree split to the `nvoos-mcp-wordpress` mirror; connects to assistants through the Remote Sites → MCP Server type.
- 🚦 **Per-session budget warnings in chat (PR #6776).** Blocked-session messages now report session state and the real reset path; the dormant 75% `wp_mcp_ai_session_limit_approaching` hook is wired to a one-shot warning on the next chat turn through the new `wp_mcp_ai_chat_messages` filter; Token Manager reset semantics documented.
- 🧹 **Restriction notice fixed (PR #6779).** The "N users have been restricted" banner no longer over-counts (one entry per user, distinct-user counting) and no longer lingers after restrictions lift (render-time sweep + lift pruning; the Pro command-center banner is fixed too).
- 🔎 **Upwork search + workflow editing (PR #6784).** `update_pro_schedule` edits workflow steps in place (no more delete-and-recreate), and `search_upwork_jobs` gains `exclude_keywords`, an `all` sentinel, quoted-OR skills, tier/budget parsing, and criteria echoing — with a byte-identical CG Pro port.
- 📦 **Versioning** — bumped to **1.1.87** across all version-bearing files. Pro addon: 1.1.87. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.8** (unchanged — explorer motion/UX + remote-source driver fixes on its own track). nvoos-content-graph-ai: **1.0.4** (unchanged). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). nvoos-content-graph-pro: **1.0.0** (unchanged — one byte-identical tool port). Checkout API: **0.1.2** (unchanged). Docs Hub: **0.5.1** (unchanged). Comic Reader: **0.5.0** (unchanged). Model catalog: **v2026.09.22** (unchanged). Tool count: **~347 base + ~1,287 Pro (~1,634 total; +35 base +5 Pro)**. Provider count: **15** chat providers (unchanged). Addon count: **27 → 28** (new `mcp-wordpress-gateway`). Bundled skills: **75** base + **41** Pro (the two Elementor copies now genuinely tracked). Coding-time agent skills: **60** (unchanged). Stale build ZIPs removed: the 1.1.85 build set (30 files).
...
### v1.1.85 — September 24, 2026

#### MCP Apps Connection & Exposure Wave, Docs Hub 0.5.1, README Consolidation

- 🔌 **MCP Apps connection diagnostics + correctness — PR #6753.** Per-row Test Connection / Discover Tools buttons with inline results (negotiated protocol, handshake type, server info, session state, latency, live tool count, verbatim errors); persisted `_wp_mcp_ai_mcp_app_status` badge + tool-count chip; Test All. Basic auth end-to-end (raw `user:pass` auto-encoded or pre-encoded base64) with **token masking** — stored credentials never echoed into the metabox HTML, empty = keep. `Mcp-Session-Id` capture + echo; legacy-handshake fallback on `-32601`/`-32600`/session errors; JSON-RPC error decoding on HTTP 4xx; spec-compliant `Accept`; `stdClass`-params coercion fix; mcpServers JSON import; loopback detection with a PHP-FPM deadlock warning. **Security Center → Network & Headers: MCP App Allowed Hosts** (constant hard override → filter + saved setting merged).
- 🤝 **Negotiated protocol header after session fallback — PR #6754.** The client advertises the **negotiated** `protocolVersion` on post-initialize requests and suppresses the 2026-only `_meta` envelope once a legacy session is established — sessionful servers no longer reject `tools/list`. Test infra: PHPUnit 11 one-class-per-file discovery fixed (the #6753 registry/REST suites now genuinely execute).
- 🌉 **Bridge tools reach the chat payload — PR #6755.** New base `wp_mcp_ai_chat_effective_tools` filter seam in `build_tools_payload()` (after attention-based filtering, before the per-tool capability check; empty selection no longer short-circuits before the filters); Pro `wp_mcp_ai_mcp_apps_expose_tools()` appends the enabled apps' `mcp_app_<label>_<tool>` slugs; registry gains `collect_remote_tools()` + `get_remote_tool_slugs()`.
- ⚡ **In-process same-site bridge — PR #6756.** Same-origin MCP endpoints dispatch via `rest_do_request()` — no outbound socket, no TLS handshake, no second PHP-FPM worker (ends the self-request TLS deadlock); outbound HTTP kept for remote hosts, unregistered routes, and the `wp_mcp_ai_mcp_app_disable_inprocess_bridge` filter.
- 📨 **`rest_post_dispatch` parity — PR #6757.** The in-process path re-applies the filter after `rest_do_request()` (mirroring `serve_request()`), so response-side session headers (e.g. EMCP's `Mcp-Session-Id`) reach the client and tool enumeration stops failing.
- 🧰 **Bridge tools on every chat tool surface — PR #6758.** The seam carries a third `$assistant_id` argument applied on `handle_tools_list()`, `handle_tool_request()`, `execute_tool_call_internal()`, and the `list_mcp_tools` catalogue filter; Pro exposure registers bridges in the local registry (idempotent, transient-cached discovery); `rest_pre_dispatch` registers bridges before REST arg validation; 60 s negative discovery cache; `save_apps()` invalidates the `/tools` REST list cache.
- 📚 **Docs Hub 0.5.1 — PRs #6749/#6750/#6759.** 0.5.0 went local-first by default (`sources = ['uploads']`, zero remote calls) with the GitHub importer as an explicit opt-in service (`enable_remote_repos`, off by default, server-side enforced); `is_path_safe()` symlink-escape hardening (0 blocking Plugin Check errors); 0.5.1 moves uploads to the slug-named `nvoos-docs-hub/content/` folder with a one-time non-destructive migration of the legacy `uploads/docs` path.
- 📐 **README anchors + consolidation — PRs #6751/#6752.** Six VS16-fallback TOC anchors fixed; `bin/validate-readme-anchors.py` repaired to GitHub's real anchor rules (leading hyphens kept, no collapsing, emoji/VS16/ZWJ stripped, full-file scan) + CI enforcement on README changes; one `## 📜 Release History` section keeps the 12 most recent releases in full plus a complete Previous Releases table (README 4,423 → 3,301 lines); the 12-entry maintenance rule codified in the README + the updates skill.
- 📦 **Versioning** — bumped to **1.1.85** across all version-bearing files. Pro addon: 1.1.85. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.8** (unchanged). nvoos-content-graph-ai: **1.0.4** (unchanged). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). nvoos-content-graph-pro: **1.0.0** (unchanged — no port waves in-window). Checkout API: **0.1.2** (unchanged). Docs Hub addon: **0.4.7 → 0.5.1** (#6749 → 0.5.0, #6750 hardening, #6759 → 0.5.1). Comic Reader addon: **0.5.0** (unchanged). Model catalog: **v2026.09.22** (unchanged — no model PRs in-window). Tool count: **~308 base + ~1,282 Pro (~1,590 total)** — unchanged, no static registrations in-window (the MCP App bridge slugs are dynamic chat-time registrations); live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative. Provider count: **15** chat providers (unchanged). Addon count: **27**. Bundled skills: **75** base + **41** Pro (unchanged). Coding-time agent skills: **59** (unchanged — the updates skill gained the README-anchor guardrail + 12-entry maintenance rule, #6751/#6752). Stale build ZIPs removed: the 1.1.83 set (30 files: 9 in `build/` incl. 3 `.sha256`, 2 in `build/optional-components/`, 19 in `build/toolkit-addons/`) + the superseded docs-hub 0.4.7 and 0.5.0 ZIPs (0.5.1 is current).
...
2. You now have the **complete version** with all ~1,594 tools (~312 base + ~1,282 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)

**What you get from the repository clone:**

- ✅ The full codebase — all ~1,594 built-in tools ready to use (~312 base + ~1,282 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
...
- **[Architecture Overview](docs/developer/architecture/ARCHITECTURE.md)** - System architecture (15 providers, ~1,594 tool classes, 36 REST controllers)
...
- **[Tool Reference](docs/reference/tools/tool-reference.md)** - All ~1,594 tools documented (~312 base + ~1,282 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
