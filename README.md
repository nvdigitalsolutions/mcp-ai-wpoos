...
**🆕 v1.1.69 Highlights:** An AI-vision & admin-compat release. **Pro gains a Vision Analysis toolkit** — the new `analyze_image_objects` tool detects and counts objects in an image (attachment/file/URL/base64) via HuggingFace OWLv2 or Ollama detection, VLM counting, and hybrid label normalization, with `annotate=true` returning a GD-drawn labeled bounding-box copy as a media attachment; the toolkit ships off by default behind a new NV oOS → Vision Analysis settings page with SSRF-guarded remote URLs. **tagDiv Newspaper admin screens are fixed** — the four SiteKit tools crashed the assistant tools metabox with a non-existent capability-flag constant (string flag arrays now), the sortable shim prints its bundled copy whenever `td_wp_admin` is enqueued, and the metabox CSS is enqueued in the head. **DeepSeek 400s on argument-less tools are gone** — 29 tools' empty `properties` now encode as `{}` instead of `[]`, and `LegacyToolAdapter` preserves object maps. **A dead-code ZipSlip guard is revived** — `count($zip)` replaces the PHP 8.x-nonexistent `ZipArchive::$num_files`, restoring path-traversal rejection in OKF bundle import. Plus: rate-limiter classification by the real HTTP verb with a separate nefarious-monitor counter, assistant-edit-screen shortcut fatal containment, model-catalog fixes (gpt-4o 128k, gemini video-capable typo, Qwen entries), and a settings-save fix that stopped leaving object-cache additions suspended (WP 7.1 inline-async tick locks). **A fifth repair wave (#6280–#6308) then hardened exec-disabled hosts** (Pro pages + OCR no longer fatal), restored complexity-based model routing, made JetEngine detection stub-proof with physical CCT-table probes, and grouped a dozen more production fixes (see Latest Updates). Tool count: ~303 base + ~1,263 Pro (~1,566 total).
...
- 🧪 **PHPUnit Repair Campaign Wave 5 (PRs #6280–#6308).** A fifth wave carried grouped production fixes: Pro admin pages + OCR no longer fatal when `exec` is disabled (`wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` exec-first + Process Service fallback, negative inputs clamped with `max(0,(int))`, #6295); transcript display attachments sanitized + OpenAI `max_tokens` capped to the model's window (#6280); complexity-based model routing restored and provider-prefixed slug limits (`nvidia/…`) resolved (#6298, #6306); JetEngine detection requires `JET_ENGINE_VERSION` and CCT checks probe the physical table (`SELECT` not `SHOW TABLES`, MySQL 8.0) (#6296, #6300); markup REST validation returns 400 and the legacy SSE handshake is filterable (#6302); image permission checks use the acting user (#6305); mesh peer URLs validated pre-sanitization (#6285); memory guest captures opt-in (#6286); logger/harness event coverage (#6308).
...
- 🧪 **PHPUnit Repair Campaign Wave 5 — PRs #6280–#6308.** A fifth wave (merged after the catch-up) carried grouped production fixes:
  - 🛡️ **exec-disabled hardening (#6295)** — Pro admin pages + OCR no longer fatal on hosts where `exec` is disabled: canonical `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` (exec-first + Process Service fallback), OCR `is_cli_tool_available()` / `run_cli_command()` (Windows-aware), `proc_*` guards, and `max(0,(int))` clamping replaces `absint()` on six CPT settings pages.
  - 🐛 **Transcripts & token caps (#6280)** — REST validator sanitizes `display.attachments`; OpenAI client caps the resource-aware `max_tokens` default against the model's window/max-output.
  - 🐛 **Model routing (#6298, #6306)** — complexity-based light/advanced routing actually runs (empty base instead of settings default); provider-prefixed slug limits (`nvidia/nemotron-…`) resolve; `timed-out` timeout variant recognized.
  - 🐛 **JetEngine & CCT (#6296, #6300)** — `JET_ENGINE_VERSION` constant required (stub-proof); physical CCT table probe (`SELECT`, MySQL 8.0-aware); cached Pro tool map (`wp_mcp_ai_pro_get_tool_map()`); conversation-import tools join presets.
  - 🐛 **REST & auth (#6301, #6302)** — markup validation 400s; filterable legacy SSE handshake (`wp_mcp_ai_legacy_sse_enabled`); Auth0 audience syntax validation (no DNS resolution); GitHub OAuth redirect terminate filter; skill-pack registry slug enforcement.
  - 🐛 **Mesh, images & memory (#6285, #6305, #6306)** — peer URL pre-sanitization validation; acting-user `read_post` checks in `WP_MCP_AI_Tool_Image_Base`; positive-int validation in `safe_set_current_user()`; scrape-product CSS→XPath ordering; `published_date` preserved from Tavily/Brave; opt-in guest memory bucket.
  - 🐛 **Logging & harness (#6308)** — agentic + token-DB log events allowlisted; millisecond-precision trace durations. Presets/dashboards: hyphenated security-event types (#6281), CRM Data Cleanup preset fix + `daily` schedule type (#6283), curriculum exporter skips empty rows (#6293), quiz settings clamp order + plugins-integration redirect terminate filter (#6303). Test-only: #6284, #6288–#6292, #6297, #6299, #6307 (+ docs #6294, CI-triage artifact #6296).
