...
**🆕 v1.1.82 Highlights:** A Playground-demos, Pro SPA, and hardening release. **Two one-click WordPress Playground demo blueprints** let anyone test-drive the plugin with zero setup: the Content Graph demo boots a seeded sci-fi universe ("Project Asteria" — 26 posts, 5 pages, 3 true orphan posts, 49 nodes / 255 edges) with a deterministic graph build, and the **NV oOS Complete demo boots the Complete bundle against your local Ollama** (Playground runs WordPress in the browser, so the plugin's `localhost:11434` endpoint is your machine — provider pre-wired, Oma assistant created, Ollama Test Lab page with the status banner + embedded Pro SPA chat). **The Pro SPA gains `cron_monitor="0"`** to kill the blocking SSE cron-status stream + REST poll on constrained hosts, and **embedded mode now seeds the model store from the assistant's real config** instead of hardcoding `gpt-4o`. **Token-tracking table creation is hardened for SQLite-backed environments** (verify-then-version, hourly retry backoff, quiet failure, graceful reads — no more per-request DB error dumps), and **schedule result delivery no longer duplicates summaries or leaks internal metadata**. The **`[ollama_status]` banner is fixed** (footer-enqueued checker JS instead of a texturize-mangled inline script). **Docs Hub 0.4.7** lands its third wp.org reviewer pass with 0 blocking Plugin Check errors. New **59th coding-time skill** (`mcp-ai-wpoos-playground-demos`). Tool count: ~306 base + ~1,279 Pro (~1,585 total; unchanged). Stale build ZIPs removed: the 1.1.80 set (30 files) + the superseded docs-hub 0.4.6 ZIP.
