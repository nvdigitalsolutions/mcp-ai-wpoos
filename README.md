...
**🆕 v1.1.70 Highlights:** A host-hardening & test-suite-stability release. **Pro admin pages and OCR no longer fatal on exec-disabled hosts** — new canonical `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` helpers probe exec-first and fall back to the Process Service (`proc_open`), and `max(0, …)` clamps replace `absint()` in the CPT/settings sanitizers. **A fifth PHPUnit repair wave (~29 test-only PRs, CI runs 91006542428 + 91771001271) closed every remaining cluster** and carried grouped production fixes: transcript attachment display metadata is preserved on save and default `max_tokens` respect the model's window; Pro dashboard event stats accept dashed event slugs; the validated `web_search` profession tags align with the base tool; schedule presets drop a phantom tool step; mesh peer URLs validate before sanitization; opt-in guest memory capture writes under a shared guest bucket; JetEngine gates require the real plugin's load marker plus a physical CCT-table probe; **complexity routing works again** (the default model no longer short-circuits it) and provider-prefixed model slugs resolve their real token limits; Auth0 audiences validate structurally; image/chart permissions check the acting user; markup REST errors return 400; skill packs reject numeric-keyed entries; agentic log events reach the recent-activity feed; webhook IDs and settings-blob reads are fixed. Tool count: ~303 base + ~1,263 Pro (~1,566 total).
