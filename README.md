...
**🆕 v1.1.83 Highlights:** A tool-guidance, Jev-decisions, and reliability release. **Every base+Pro tool (~1,487 across 1,584 files) now carries model-facing usage guidance** — a new guidance interface (`when_to_use`/`when_not_to_use`/`related_tools`/`notes`) assembles a compact `[Usage: …]` suffix on the model-facing payload, legacy-format classes opt in through the legacy tool wrapper, and the guidance sniff is enforced at severity 5 so every future tool class without guidance surfaces on PRs (full-tree gate: 1,584/1,584 files, 0 warnings; the post-window close-out covers the last three webchat tools + CG Pro member mirrors, #6740). **TypeSafe Jev joins as a first-class *decision* provider** — deliberately separate from chat providers, returning typed choice/score/noul decisions over supplied state (70–500 ms, $0.042/M input, output free) via a new decision-client contract, the OpenRouter Decisions bridge, and the new base tool **`typesafe_decide`** (canonical envelope + adversarial-state caveat); Pro gains the fail-open Jev classifier for cascade routing (`jev_routing` on the Parallel Model Dispatcher) and an opt-in research source filter (#6728). **"The Assistant Builder" meta-assistant** seeds on activation — designs, authors, and verifies other assistants (default roster 6 → 7, #6727). **ID-handoff data contracts** now annotate every ID-bearing tool family — post, cron, term, assistant, vector store, batch, Pro schedule, toolkit_cpt, medical record, plan, calendar, WPCode, session, member, and webchat room — with `produces`/`consumes` handoffs enforced by permanent manifest-driven honesty + round-trip suites (#6729–#6739). **An opt-in adaptive tool cap** sizes the tools sent per model context window, and **`list_mcp_tools` gains lazy schema loading** (`tool_slug` + `include_schemas`). **A Pro bootstrap incomplete-install guard** keeps the site running on the base plugin when a partial Pro deploy leaves required files missing. **`send_telegram_message` auto-chunks messages over 4,096 characters.** **The Upwork pipeline delivers**: web_search-mode + credential gating (#6678), category-page dropping, SERP budget/type/recency extraction, an always-on broad second pass (#6679/#6680), and workflow deliveries now ship the **full result set** (50-item cap, filterable) with per-item URLs rendered as a single properly numbered list (#6682/#6684). **Gmail `connection_id: "settings"` resolves to the settings fallback**, and skill/OKF errors append the available options for self-correction (#6677). **The Usage Monitor sub-tab saves again** (dashboard handler now applies the monitor bridge filter, #6726), and the webchat tools' latent fatal + SQL prepare warnings are fixed (#6738/#6740). The **Playground Ollama demo lands at `/ollama-test-lab/` on fresh installs** (permalink seed fix) with the local `npx` run as the primary test path (#6683). npm advisories closed for adm-zip, js-yaml, and colord (#6681). Tool count: ~307 base + ~1,279 Pro (~1,586 total; +1 base — `typesafe_decide`). Stale build ZIPs removed: the 1.1.81 and 1.1.82 sets (30 files each).
...
### ✨ What's New at a Glance (v1.1.83)

- 🧭 **Tool Description Engineering: Model-Facing Usage Guidance for Every Tool (PRs #6686, #6695, #6687–#6723).** A new `WP_MCP_AI_Tool_Usage_Guidance_Interface` (`when_to_use` / `when_not_to_use` / `related_tools` / `notes`) feeds a compact `[Usage: …]` suffix assembled by `get_model_facing_description()` on the model-facing payload (REST chat path, Tool Service `/tools`, `list_mcp_tools`); legacy-format classes opt in via a `get_usage_guidance()` method or definition key forwarded by `WP_MCP_AI_Legacy_Tool_Wrapper`. The Phase 2 sweep (37 PRs — 34 tool clusters + the wrapper passthrough + the final gate) completes the full base+pro tree — ~1,487 tools across every toolkit — and the `WPMCPAI.Tools.ToolDescriptionGuidance` sniff raises to severity 5: full-tree gate 1,584/1,584 files, 0 warnings (every future tool without guidance surfaces on PRs).
- 🎛️ **Adaptive Tool Cap + Lazy Schema Loading (PR #6686).** `WP_MCP_AI_Tool_Payload_Advisor` caps the tools sent per model context window (40 ≤128K / 64 ≤256K / 100 above) — opt-in via `wp_mcp_ai_adaptive_tool_cap` + a per-assistant three-state override, default off. `list_mcp_tools` gains `tool_slug` (single-schema on demand) and `include_schemas` (lean catalogue mode).
- 🛡️ **Pro Bootstrap Incomplete-Install Guard (PR #6677).** A partial Pro deploy (interrupted extraction, AV/SFTP exclusion) previously fataled the whole site because `class-wp-mcp-ai-pro-module-registry.php` was required unconditionally — the require is now `file_exists`-checked and degrades to an admin notice + WP_DEBUG line, keeping the site running on the base plugin.
- ✂️ **Telegram Auto-Chunking (PR #6677).** `send_telegram_message` splits >4,096-char messages at paragraph → line → hard boundaries (soft chunks keep `parse_mode`; hard splits drop it; mid-sequence failure returns `wp_mcp_ai_telegram_chunk_error` with the failed index/count). New `chunk` param (default `true`; `false` restores legacy single-send). Schedule result-delivery is covered automatically; CG Pro mirror byte-identical.
- 🔍 **Upwork Search & Workflow Delivery Series (PRs #6678, #6679, #6680, #6682).** `search_upwork_jobs` gains the web_search-mode + API credential gates (#6678 — no more missing-refresh-token scan failures); fallback drops category landing pages, extracts `job_type`/`budget`/`published` from SERP snippets, and merges an always-on broad second pass under `min(limit, 5)` jobs (#6679/#6680); new `sort`/`location` args wired to settings; derived `url`; preset stops sending the invalid `job_type: all` filter. Workflow deliveries now ship the **full result set** — the hard-coded 5-item cap becomes 50 (`wp_mcp_ai_pro_workflow_response_item_cap` filter), each item carries its URL as an indented line plus budget/contract/recency, and both markdown converters render the digest as one correctly numbered list (#6684).
- 📋 **Result-Delivery Formatting + Self-Correcting Errors (PRs #6679, #6677).** Workflow `full` deliveries render `steps` as a compact execution log (failed steps surface their error) and skip null/empty values; the literal `connection_id: "settings"` now resolves to the settings-based fallback across the four Pro Gmail tools, the Drive client, and base `search_gmail`; `load_skill`/OKF bundle errors append the available assignments/bundles for the model to self-correct.
- 🧪 **Playground Ollama Demo Permalink Fix + `npx` Primary (PR #6683).** Fresh Playground installs default to plain permalinks, so `/ollama-test-lab/` 404'd — the seed now sets `/%postname%/` and flushes rewrite rules. The local `npx -y @wp-playground/cli server` run is the documented primary test path (README "Try It on Your PC", user-guide, blueprints README with a Validation section), and the capture harness waits for the lazy seed via the REST index with a cookie jar.
- 🔒 **npm Advisory Fixes (PR #6681).** adm-zip 0.6.1 (CVE-2026-77301 + symlink-overwrite fix), js-yaml 4.3.2 (GHSA-2883-xcg3-v3hh), colord 2.10.0 (GHSA-2wm5-q62r-hmrv) — hand-edited lock entries across root/saas-controller/pro/pro-spa/media-worker; `npm install --package-lock-only --dry-run` clean in all 5 directories.
- 📏 **Canonical Error Envelope for 5 Regulatory Tools (PR #6689).** `renew_registration`, `update_reg_document`, `validate_document_checklist`, `validate_inci_ingredients`, `validate_reg_product` migrate from `success=false` failure returns to the canonical `WP_Error` envelope.

### ✨ What's New at a Glance (v1.1.82)

- 🎮 **WordPress Playground Demos — Content Graph "Project Asteria" & Complete × Ollama (PRs #6662, #6663, #6666, #6670, #6673, #6674).** Two one-click Playground blueprints: the Content Graph demo seeds a fictional sci-fi universe (26 posts + 5 pages, 4 authors, 10 tags, 3 true degree-0 orphans) and builds the graph deterministically through the public `nvoos_content_graph/initial_build` hook (validated: 49 nodes, 255 edges, 5 communities); the Complete demo boots the 38 MB bundle against the user's **local Ollama** — pre-wired provider, the Oma assistant, and the Ollama Test Lab page (`[ollama_status]` + embedded Pro SPA). The blueprint generator now auto-discovers the newest Complete bundle ZIP (no more hand-bumped pin) and the build-assets workflow regenerates the blueprint with every build; `cron_monitor="0"` keeps the demo's Pro SPA page worker-safe; a `Demo NV oOS Complete` button joins the README badge row.
- 🧰 **Pro SPA `cron_monitor` Flag (PR #6665).** `[nvoos_pro_spa cron_monitor="0"]` no-ops the blocking SSE `cron-status` stream **and** the 15-second REST poll fallback via `config.cronMonitor` — the failure cascade that exhausts Playground's WASM worker request budget on constrained hosts. Default stays `true`.
- 🎯 **Pro SPA Model Store Seeded from Assistant Config (PR #6672).** Embedded mode no longer hardcodes `{provider:'openai', model:'gpt-4o'}` and overrides the assistant's real config on every chat request — `useBootstrap.ts` seeds provider + model from the preloaded assistant config.
- 🛠️ **Token Tracking Table Hardened for SQLite Backends (PR #6669).** Verify-then-version (the schema version advances only when the table verifiably exists; existing installs self-heal), an hourly retry-backoff transient, quiet failure (suppressed probes, single `error_log` line — no more per-request HTML DB-error dumps), and graceful reads when the table is absent. Schema unchanged; ported 1:1 to `nvoos-content-graph-ai` with an ecosystem matrix.
- ✂️ **Schedule Result Delivery Duplicate Content Fixed (PR #6661).** Chat `summary` and SMS templates gain the prefix dedupe; new `delivery_safe_data()` strips the duplicated `response` copy plus `assistant_id`/`is_agentic` metadata before rendering (email/chat `full`, Paper Store + WordPress post markdown); no more empty `## Details` section.
- 📡 **`[ollama_status]` Banner Unfrozen (PR #6668).** The checker moved from a texturize-mangled inline script inside `the_content` (`&&` → `&#038;&#038;`) to a footer-enqueued script that self-locates every status div.
- 🛡️ **Assistant Portability Coverage Guards Repaired (PR #6645).** The four CI guard failures from the export/import merge: the 4 portability tools join the `assistant_management` preset, 9 new AJAX tests cover the WhatsApp handlers, both tool-class manifests + the AJAX inventory regenerated.
- 📚 **New Coding-Time Skill + Updates Track C (PRs #6664, #6656).** The **59th** skill (`mcp-ai-wpoos-playground-demos`) codifies blueprint authoring, CORS rules, the PCP built-ZIP gate, worker crash modes, and the Ollama policy matrix; the updates skill gains the weekly PR deferred-item sweep track.
- 🏥 **Docs Hub 0.4.7 (PRs #6659, #6667).** Third wp.org reviewer pass: invalid `Tested up to:` PHP header removed, rebuild cron moved to `init` (kills the early-translation notice), deactivation cleanup, third-party libraries + external services disclosures — 0 blocking Plugin Check errors.
...
- 📦 **Assistant Export/Import Across All Surfaces (PR #6628).** A canonical `WP_MCP_AI_Assistant_Portability` engine powers a versioned `nvoos-assistant` JSON bundle (format_version 1) from every interface: WP-CLI `assistant export|import` (rewritten; legacy files still import, and the old export's loss of `_wp_mcp_ai_*` config is fixed), REST `POST /mcp-ai/v1/assistants/export|import` (admin-only, schema-validated, 2 MB cap, dry-run), admin row/bulk actions + Import/Export page (skip/overwrite/duplicate), and **3 new base tools** (`export_assistant`, `import_assistant`, `duplicate_assistant`) + a **Pro blueprint export tool**. Credential hashes are never exported and stripped from imports (the backup provider now shares the same denylist — previously it included credential hashes). 34 new tests + 105 regression tests green on WP 6.9/7.1.
- 🛡️ **Security Center Usage Monitor Sub-Tab (PR #6632).** The cryptic violation notice deep-links to a new `usage_monitor` sub-tab rendering the severity-tiered triage log (latest 50 of 100), monitor status cards, emergency-shutdown recovery, and editable monitor config. Clear actions run over new REST routes (`POST /mcp-ai/v1/security/clear-violations` + `/clear-shutdown`, `manage_options` + nonce). Bug fixes: unrelated settings saves no longer silently disable the monitor, and malformed admin-edited patterns are dropped/skipped instead of warning. 80 tests / 435 assertions green.
- 🛍️ **Shopify Storefront & Global Catalog UCP Modes (PRs #6624, #6630).** Two keyless UCP MCP connection modes replace the deprecated REST Catalog API: per-merchant Storefront Catalog and cross-merchant Global Catalog (`catalog.shopify.com`), both with `tools/list` handshakes, CCT-sync rejection, and a public `/wp-json/mcp-ai/v1/ucp/agent-profile` capability route. CG Pro parity restored (Shop Domain input fix + the Catalog 401 cluster) — byte-identical ports. Catalog 401s themselves are fixed in #6623 (60-min token cap, scope validation, 401 purge-and-retry) along with the JetEngine sync-gate mismatch in System Status.
- 💻 **WP-CLI Repaired & Streaming (PRs #6625, #6626).** `wp mcp-ai provider list` and every base-class command no longer fatal on PHP 8+ (by-reference Formatter constructor → by-value `format_items()`), `wp mcp-ai chat` no longer fatals on router construction, and `--stream` now streams token-by-token (native cURL SSE for the nine raw-SSE providers, simulated chunking elsewhere, both honoring the shared streaming filters).
- 🟢 **WhatsApp Webhook Self-Tests + OKF Editor Context (PRs #6622, #6631).** Remote Sites connections gain three webhook checks (verification replay, HMAC-SHA256 signature validation, `subscribed_apps` shadow-delivery check), CG Pro re-synced byte-identical. The OKF Bundle Manager editor keeps its tab/bundle/concept context on save instead of bouncing to the Bundles tab.
- 📚 **Skills & Docs (PRs #6621, #6627, #6629).** Docs Hub code-block/heading-anchor color fixes (0.4.6 track, no bump); `design-ai-assistant-admin` rewritten to the code-verified surface plus the new `design-brand-assistant-provisioning` skill; the brand-provisioning template's 67 WPCS violations fixed.
...
- ⌨️ **Slash Commands Reworked as Declarative Tool Wrappers (PR #6604).** The toolkit manager shrinks ~10,400 → ~1,000 lines: ~76 "implementation coming soon" placeholders purged, **36 commands across 15 toolkits** remain, each mapped to a verified real tool slug. Execution delegates through a new `WP_MCP_AI_Slash_Command_Tool_Adapter` (`register_tool_command()` / `wp_mcp_ai_register_tool_command()`) to `WP_MCP_AI_Tool_Registry::execute_tool()` — capability gates, validation, sanitisation, and the canonical envelope live in the tool layer, while parsing/auth/rate-limiting/audit stay in the handler. New `WP_MCP_AI_Slash_Command_Prompts` exposes every command as an MCP prompt template (`slash.<command>` via `prompts/list` / `prompts/get`) wired into the per-toolkit MCP servers. All 19 built-in workflows chain registered, tool-backed commands; native chat commands untouched. Suites: 26/26 registry, 169/169 workflows, 5/5 prompts, 201/201 broad slash, 109/109 Pro slash + toolkit MCP servers.
- 🧠 **DeepSeek V4 Pro Restored (PR #6608).** DeepSeek's 2026-09-10 changelog confirms V4 Pro continues after September 14 with unchanged billing — the 1.1.77 deprecation reverses: `deepseek-v4-pro` → **active** ($0.66/$1.98 off-peak, sunset/fallback cleared, migration map deliberately unmapped). The `nvoos-content-graph-ai` mirror bumps to catalog **v2026.09.10** (flash lineup + retired `chat`/`reasoner`/`coder` removed) and fixes its stale $1.74/$3.48 v4-pro UsageTracker pricing to $0.66/$1.98; `lib/core` mirrors align. Base catalog version deliberately stays v2026.09.10. Suites: lib/core 447/1,400; content-graph-ai 854/7,361 (WP 6.9 + 7.1); base model batch 175/6,671 (both).
- 🛒 **Checkout Asset Cache-Busting by File Mtime (PR #6598).** New `Schema::assetVersion()` versions every plugin-owned enqueue by the asset file's mtime (fallback to `NVOOS_CONTENT_GRAPH_VERSION` when missing) — the 1.0.7 purchase-modal hotfix had been served stale for up to a year (`Cache-Control: max-age=31536000`). Any future asset edit produces a new URL automatically; unchanged files keep long-term caching. Unit suite 98/513 OK.
- 🛒 **Vendor-Authoritative Checkout Price + $34.99 Default (PR #6603).** The purchase modal syncs its price label from the vendor `/session` response (`Intl.NumberFormat`, display-only — the vendor re-verifies server-side); `DEFAULT_PRICE_CENTS` moves 4900 → **3499** in the content-graph plugin and the checkout-api fresh-install default, pinned by `CommerceTest::defaultPriceIs3499Cents`. Test bootstrap prefers the plugin's pristine vendored wp-phpunit lib (the root bootstrap's PHPUnit-11 patch persisted to disk and fatals the locked PHPUnit 9.6).
- 🎨 **Checkout Modal Restyle (PR #6597).** Unified 40px field spec (12px padding, soft blue focus ring), custom SVG select chevron, fixed address-grid layout (`.nvoos-cg-field` wrappers), Stripe `appearance`-harmonized Payment Element, and a custom minimalist terms-consent checkbox.
- 📚 **Docs Hub 0.4.4 — wp.org Review Fixes (PR #6606).** Every review finding fixed: contributors (`vsamtani`), `== Source Code ==` readme section + rebuilt JS with a self-describing source banner, `/search` filters `source = "context"` entries for users without `manage_options`, `load_plugin_textdomain()` removed, notice scoped to the Docs Hub settings screen, staging-transient isolation, symlink-safe recursive deletion (resolved-path containment), sitemap context-slug leak closed, stale page transients invalidated. Official Plugin Check: **0 blocking errors**.
- ⚖️ **Legal Consolidation (PRs #6599, #6605, #6607).** Terms of Service consolidated (Part A products + Part B site/services/marketplace); new `API-LICENSES.md` + publishable HTML align the live api-licenses page; **two-entity seller model** — NV Digital Unlocked LLC sells the Products and runs the marketplace, NV Digital Solutions develops, owns IP, and provides services; dual-controller Privacy Policy + full `docs/legal/publish/*.html` regeneration.
- 🚀 **Content Graph 1.0.7 Released** (direct commits) — changelog marked released (tag `content-graph-v1.0.7`) + a wp.org 18-point sign-off document.

### ✨ What's New at a Glance (v1.1.77)

- 🧠 **DeepSeek V4.1 Flash Catalog Refresh (PR #6555).** New `deepseek-flash` model (vision, $0.15/$0.60 off-peak per 1K); V4 Flash + V4 Flash Vision Exp retired; V4 Pro deprecated (sunset 2026-09-14, fallback `deepseek-flash`); the migration map rewrites all six legacy DeepSeek ids; defaults, fallbacks, cost tables, dropdowns, diagnostics, and 10 Pro research/orchestration tools point at the new lineup (catalog v2026.09.10). The cost calculator gains **peak/off-peak pricing** (`PEAK_WINDOWS`, `is_peak_time()`, `get_model_pricing_at()`, `calculate_cost_at()` with injectable timestamps — legacy `calculate_cost()` untouched).
- 🧠 **Knowledge Graph Companion Preset (PR #6570).** New featured onboarding preset (listed first, `⭐ Featured` badge): graph-teaching system prompt, 20 graph-aware base tools always on, all 14 `graphify_*` tools when the Graphify addon is loaded + enabled, and auto-selection when a content graph is detected (saved selections always win).
- 🧪 **Base+Pro Regression Matrix (PR #6561).** New `tests/basepro/` (15 contract tests) + `phpunit-basepro.xml.dist` + `base-pro` CI job + `composer run test:basepro` boot the real base+pro shape — the permanent pin for gating regressions.
- 🧩 **Base+Pro Toolkit Gating Fixed (PR #6561).** Pro toolkits silently failed to load in base+pro installs (the gates read the base constant as "no Pro" even with the Pro addon active). All 21 toolkit `init.php` gates, the module registry, Telegram Mini App media listing, AI CPT management, site templates, embedded webchat, and ~550 Pro tool availability gates now use the `( ! $is_base || defined( 'WP_MCP_AI_PRO_VERSION' ) )` escape.
- 🖥 **Pro WP-CLI Load-Order Guard (PR #6585).** The Pro addon fataled every `wp` command when activated before the base plugin (`Undefined constant WP_MCP_AI_PATH`). The CLI loader now defers to `plugins_loaded` (priority 30) with a defense-in-depth guard in the CLI base command.
- 🧩 **Agent-Memory CCT Phantom Slug Fixed (PR #6591).** Graphify bridge maps, MemPalace edges, and Pro memory retention read the canonical `ai_agent_memories` table (with legacy-table fallback) — dormancy sweeps, per-user caps, expiry pruning, and Memory Health stats start working.
- 🔒 **NPM Dependency Bumps (PR #6592).** `sharp` 0.35.4, `nodemailer` 9.1.1, `joi` 18.2.8 (17.13.7 in pro/assets/spa), `postcss-selector-parser` 6.1.3/7.1.6 across root + addon lockfiles — same-major, no breaking changes.
- 🔄 **Content Graph Build Race Fix (PR #6593).** The ZIP publish job failed on three consecutive pushes (build/publish derived the plugin version from different checkouts). All three content-graph workflows now hand the version through job outputs with a per-ref concurrency guard.
- 🛒 **Checkout Launch Series Complete (PRs #6568, #6571, #6573, #6587, #6588, #6589, #6590, #6594).** Checkout API **v0.1.0 → 0.1.1**: settings-form redirect + Stripe ID round-trip fixes, "Test connection" balance probe, public `GET /health`, **Stripe boolean serialization fixed** (`true` → `"1"` broke every live PaymentIntent), and a **424/502 error contract** (Stripe rejections show in-modal; only transport/5xx redirect). Content Graph **1.0.6 → 1.0.7**: JetEngine CCT source selection + per-CCT status notes, trust/value/post-purchase modal (1-year updates, 30-day guarantee, EU waiver), health diagnostics, and the **Stripe element + release-tag URL fixes** that unblocked live purchases.
- 🌐 **Wave G — Five Clusters Complete (PRs #6551–#6584).** Law-firm, cre-debt, quiz, eca, and chat-channels toolkits complete byte-identical in `nvoos-content-graph-pro` (v1.0.0 unchanged); the places data layer lands — each with per-PR `tests/` suites.

### ✨ What's New at a Glance (v1.1.76)

- 📡 **Chat Delivery Full Report + Per-Channel Formats (PR #6525).** Chat channels (Telegram, WhatsApp, Slack, Discord, Teams, Messenger, Google Chat) now support the `full` result-delivery template — complete summary, substantive response, and structured envelope data — mirroring email. A new per-channel `format` setting (Telegram `html`/`markdown`/`markdown_v2`/`plain`; WhatsApp/Slack/Discord/Teams `markdown`/`plain`; Messenger/Google Chat `plain`) routes Telegram through `send_telegram_message` directly so the Bot API `parse_mode` works (broadcast strips markup); the cron capability waiver extends to that tool for the internal delivery context only; `MarkdownV2` reserved chars are escaped; group messages dropped by `require_mention` now log `telegram_group_mention_required_ignored`. Missing/invalid formats fall back to per-channel defaults — zero migration.
- 📡 **Duplicate-Summary Fix (PR #6548).** `full` delivery templates printed the assistant header twice (envelope summary = first 25 words of the response, then the full response). New `response_starts_with_summary()` skips the derived summary when the response already opens with it — email and chat templates both.
- 🎨 **Comic Creation Toolkit Toggle Registered (PR #6512).** The toolkit (12 tools, 4 CPTs, settings/research/consolidate pages) had shipped without its `enable_comic_creation_toolkit` toggle anywhere — it could never be enabled. Now registered in the Tools section + Pro Features subtab, memory estimator (128 MB), toolkit status/details, and WP-CLI toolkit maps.
- 🛒 **Checkout Launch Series (PRs #6507, #6520, #6523, #6550).** Required ToS/refund consent + buyer email at purchase (`receipt_email`; fill-once license columns `buyer_email`/`terms_agreed_at`, DB v3); EU country selector with required billing-address block (`buyer_country` column); Stripe Product/Price creation action + product metadata + statement descriptor; manual install promoted to the primary path; full commercial legal set in `docs/legal/` (privacy policy, AUP, clickwrap guide, compliance checklist) + updated ToS. Checkout API stays **v0.1.0**.
- 🔧 **Playbook Seeder Idempotency (direct commit `fe4d0ee880`).** The `Generated: <time> UTC` header changed the content hash every second, so every sync deleted + recreated the playbook attachment. `hash_playbook_content()` strips the timestamp before hashing — unchanged playbooks keep their attachment.
- 🔒 **Security Sweeps (PRs #6515, #6532, #6546).** 9 Dependabot alerts closed (tiptap ReDoS, multer FD-leak, csv-parse prototype replacement + Pro vendor refresh, react-router-dom open redirect/XSS); SVGO XSS bypass CVE-2026-84370 → `svgo >=4.1.0` (media-worker, pro, saas-controller, root, pro/assets/spa); `hono` `^4.13.7` + `vitest` 4.1.11 (11 lockfiles) + schedule-anything-spa build unblocked (unused imports, esbuild destructuring).
- 📦 **Docs-Hub ZIP Markdown Exclusion (PR #6504).** The `nvoos-docs-hub` wp.org ZIP no longer ships dev Markdown (`*.md` excluded, `readme.txt` kept) via a packaging tri-sync; the v0.4.3 ZIP rebuilt (39→37 entries).
- 🌐 **Wave F2 — Ten Clusters Complete (PRs #6505–#6549).** Site-creator (33 tools), document-generation, regulatory-registration, healthcare (nine batches through interop/OpenMed), and law-firm toolkits complete byte-identical in `nvoos-content-graph-pro` (v1.0.0 unchanged), plus image-production, comic-creation, dj-management, ai-tool-builder, architect-agent, and architectural-design slices — each with its own `tests/` suite.

### ✨ What's New at a Glance (v1.1.75)

- 📡 **Telegram Broadcast Credentials Fix (PR #6482).** Assistant-run schedules with chat-channel broadcast crashed with a `TypeError` (credentials string given, array expected): the edit modal stored inline credentials as a raw JSON string. New `normalize_channel_credentials()` decodes JSON → arrays (garbage → empty array → clean `missing_channel_credentials` error); `extract_credentials_from_connection()` maps the real Remote Sites schema (decrypted `api_key`/`token`); the broadcast tool coerces + validates per-channel credentials (malformed values become per-channel `failures` — saved broken schedules self-repair); the sanitizer coerces strings back to arrays at the save boundary; the JS modal parses/serializes JSON credentials and recognizes `conn_…` IDs.
- 📡 **Scheduled Delivery Credential Fallback (PR #6488).** Cron delivery to Telegram failed with "no valid credentials" while interactive chat worked. New **tier-4 fallback** resolves the first *enabled* Remote Sites connection of the channel type (assistant-assigned preferred) with destination fields merged from the schedule config; the broadcast capability gate is waived only for the internal `pro_schedule_manager_result_delivery` context (cron has no logged-in user; acting user = schedule creator); `create_pro_schedule`/`update_pro_schedule` + `POST /mcp-ai-pro/v1/schedules` accept `result_delivery` (previously silently dropped); failure diagnostics ride `error_data` (never secrets); the UI gains a Remote Sites connection dropdown (name + ID + enabled state).
- 🧠 **Content Graph Memory Bridge + NV oOS Complete Checkout (PR #6486).** The standalone plugin's Memory Bridge projects base + ecosystem memories into the standalone graph (memory nodes, `MEMBER_OF`/`OBSERVED_BY` edges, `DERIVED_FROM` post links) with advisory degradation; the base `wake_up_context` tool gains a `wp_mcp_ai_wake_up_context_graph_retriever` filter seam (`graph`/`auto` mode activates when listeners exist — not only with bundled Graphify); the orchestration dashboard detects any connected graph bridge. The Content Graph checkout now sells the **NV oOS Complete bundle** (base + Pro, `nvoos-oos-complete`) with a conflict guard (409, no download when another NV oOS copy exists). nvoos-content-graph **1.0.4 → 1.0.6**.
- 🌐 **Wave F2 — Financial-Planning Port Complete (PRs #6476, #6477).** Byte-identical into `nvoos-content-graph-pro`: data layer + yfinance service, tools batch B (16 tools — goals/literacy, market, reports/search, transactions — plus the tree-only `import_financial_planning_blueprint`), and the admin slice (CPT settings base, financial CPT settings page, account-research page, research-add probe).
- 🌐 **Wave F2 — Social-Media Port Complete (PRs #6478–#6481).** Data layer (`social_sched_post` CPT + 30-day retention) + 32 tools (downloads/posts/insights always-on; scheduling/engagement/analytics/content/capture gated) + the settings page.
- 🌐 **Wave F2 — MCP Toolkit Servers Complete (PRs #6483–#6485).** Framework init (observability card, `/.well-known/mcp` discovery, Scheduled Toolkit Server trait, file-gated slim init) + all 33 servers across batches A (19) and B (14 Phase 6+8 + DietPi).
- 🌐 **Wave F2 — Slice Batches Continue (PRs #6487, #6489–#6502).** Remote-sites (data layer, admin slice, remote-connections tools), video-production (gated tools, admin slice, blueprint import, services, exec-service tools), analytics toolkit, multilingual toolkit, cloudways (infra slice + tool batch), dj-management toolkit, and the image-production data layer + top-level tools — all byte-identical with per-PR `tests/` suites.

### ✨ What's New at a Glance (v1.1.74)

- 📅 **Google Calendar Date Queries & Free/Busy Fix (PR #6460).** Every date-filtered query failed with Google's HTTP 400 because `add_query_arg()` leaves values unencoded and the literal `+` in RFC3339 timestamps was decoded as a space — every query value is now `rawurlencode()`d (fixes all Calendar tools on the shared client), and `calendar.freebusy` joins the Standard scope profile so `check_google_calendar_availability` works on a normal connection (new OAuth grants only; the date fix needs no reconnect). Platform mirror updated identically.
- 📧 **Result Delivery Email Formats (PR #6465).** New dependency-free `WP_MCP_AI_Markdown_Converter` renders assistant Markdown digests into email-safe HTML (every text node escaped, `wp_kses` allowlist, protocol-allowlisted links — raw assistant HTML can never reach the email body; inline-only Outlook-safe styling). Per-channel `format` setting (`both` | `html` | `markdown`, default `both` — existing schedules get formatted HTML with zero migration); Nodemailer multipart/alternative + `wp_mail` fallback; sanitizer allowlists `format`; edit modal gains a format dropdown.
- 🗓️ **Schedule Manager Assistant-Prompt Editing (PR #6469).** `assistant_run` schedules can now show and edit their prompt: the edit modal renders an assistant dropdown + message textarea, `update_schedule()` merges `assistant_config` with the stored config (context/max_agentic_iterations survive partial updates) and uses the create-path validation, and `update_pro_schedule` accepts `assistant_config` via MCP. JS: 29 pre-existing `no-var` ESLint errors fixed.
- 🌐 **Wave F2: PM + Calendar-Booking Ports Complete (PRs #6450–#6472).** `nvoos-content-graph-pro` (v1.0.0) completes the **Project Management** port (data layer, core CRUD + dependencies, PARA, analytics/risk, command-center/workflow, templates/sprints, reports + blueprint import + shared installer, admin slice A, research/settings/consolidate) and the **calendar-booking** port (data layer, event tools + ICS export, services + jet-sync batches, appointment/slot extras, admin slice), plus the e-commerce admin pages batch.
- 🧪 **Test/CI Hardening (PRs #6457, #6464, #6470).** The recurring perf-suite MCP-abilities CI failure (WooCommerce `mcp-adapter` + Rank Math `mcp-oauth` third-party plugin interactions) is now neutralized **process-wide** in `tests/bootstrap.php` (the registry is a once-per-process lazy singleton, so the kill-switch filters must apply before any suite runs — supersedes #6464's per-suite guards). `plugins/nvoos-content-graph-pro/**` is excluded from the root WPCS/PHP-compat gate and lints under its own standard.

### ✨ What's New at a Glance (v1.1.73)

- 🛒 **`bulk_update_products` Variable-Scope Expansion (PR #6447).** New `scope` argument (`all` default | `product` legacy): under `scope=all`, price/stock fields expand variable parents to their variations and grouped parents to their children via `WP_MCP_AI_Woo_Price_Qty_Updater::resolve_update_targets()`, and `WC_Product_Variable::sync()` re-syncs each variable parent (direct variation IDs re-sync their parent). Status/featured/category/tag updates always apply to the selected product; `scope=product` keeps exact-ID legacy semantics. Acknowledged response-shape change: each `updated_products[]` entry reports `targets[]` per input ID + the response gains `scope`/`updated_targets` keys. Proposal 037.
- 🔕 **`update_woo_product_qty` Notify Suppression (PR #6448).** New boolean `notify` parameter (default `true`, current behavior) suppresses the WooCommerce low-stock/no-stock admin notification emails when `false` — the `woocommerce_should_send_low/no_stock_notification` filters are scoped to `__return_false` for the write and removed in a `finally` (no global-filter leak), while the `woocommerce_low_stock`/`woocommerce_no_stock` actions still fire (push triggers unaffected). Proposal 038.
- 🗄️ **Async Job Queue Table Bootstrap Fix (PR #6423).** The queue class was never wired into the boot loader, so its `plugins_loaded` init never ran and the table was never created — while the Load Guard queried the missing table on every REST dispatch (SQL-error flood after the 1.2.0 update). Now required at boot, created on activation, `dbDelta` gated behind a `DB_VERSION` option, and `get_queue_stats()` fails soft with zeroed stats. Already-updated sites self-heal on first page load.
- 📚 **Comic Reader 0.2.0 → 0.5.0 (PR #6402).** Four-phase Komga-parity upgrade: archive magic-byte validation + Range (206) serving + server-side CBZ covers (Phase 0); persisted reader settings, vertical scroll/webtoon modes, scale types, double-page rules, gestures, continue-reading shelf (Phase 1); ComicInfo.xml RTL detection, series/collection taxonomies, per-user progress (Phase 2); settings page + capability overrides (Phase 3). Fixes: WP 6.9 `rest_ensure_response()` 201s, real-path extension checks.
- 🌐 **Content Graph Ecosystem Wave F2 (PRs #6397–#6445, #6449).** The new **`nvoos-content-graph-pro`** standalone addon (v1.0.0) ports the Pro business surface byte-identical: F1 Pro-core slices, D8-compat infra, Pro validator, MCP toolkit server core, the full CRM fan-out (~28 slices), and the complete e-commerce fan-out (43 e-commerce tools). **Docs Hub 0.4.3** completes wp.org submission prep (static settings asset, text-domain fix, plugin-check CI gate, .pot).
- 🧪 **Docs & Tooling.** New `mcp-ai-wpoos-wporg-submission` coding-time skill (PCP triage, packaging exclusion tri-sync, `.wordpress-org` layout, Playwright screenshots) + the legacy plugin-check gate DB-bootstrap repair — skills now **55** (#6418); use-cases refreshed to Rev 3.0 tested against 1.1.72 (#6443); 17 broken internal links repaired (#6446); docs-hub + content-graph-pro subtree-sync workflows (#6439).
...
- 🔓 **REST Rate-Limit Unlock & Fixed-Window Timer (PR #6322).** Users blocked by the general request limiter never appeared in the Command Center Restrictions tab, so admins couldn't lift the block. Two bugs: the limiter never flagged the restriction registry, and the window was sliding (every request rewrote the transient with a fresh full-window TTL, so steady traffic kept the block alive and `retry_after` always reported the full window). `check_rate_limit()` now uses fixed-window accounting (`{count, first_seen}`, TTL never extended past window end) with honest remaining-time `retry_after`; a new `wp_mcp_ai_rest_request_rate_limit_exceeded` action makes `WP_MCP_AI_Restriction_Registry` flag a `rate_limit` restriction (auto-release at window end) — the block shows in the Restrictions tab + Token Manager with the Lift button, and lifting clears the request window. Guest (IP-keyed) blocks expire on their own.
- 🧠 **MemPalace Wing-Scope Enforcement (PR #6327).** `WP_MCP_AI_Tool_Wake_Up_Context::matches_wake_filters()` now enforces `wing`/`room` exclusions — the Graphify graph anchors only *boost* scores and never excluded out-of-scope memories, so `wake_up_context` in auto mode leaked cross-wing memories into a wing-scoped block once the Graphify addon loaded. The PR also ships four test-isolation fixes (transcript mock restore, shared registry teardown, provider-key isolation, logging cache reset).
- 🛒 **Checkout API Into the Addon Pipeline (PR #6315).** `bin/build-addon-zips.sh` now produces `build/nvoos-checkout-api-v<version>.zip`, the addon's 6 suites join `phpunit.xml.dist`, and the build workflow gains a `tests` job. The new suite exposed a latent production fatal: the token/crypto classes read `AUTH_KEY . SECURE_AUTH_KEY` directly — now `wp_salt( 'auth' ) . wp_salt( 'secure_auth' )` (identical on normal installs, stable fallback otherwise).
- 🧭 **September 2026 Model Catalog Refresh (PR #6328).** 217 → 228 models: new `gpt-5.6-sol/-terra/-luna`, `gpt-6-astra` preview, `gpt-image-2`, `claude-opus-5`, `claude-fable-5.1`, `claude-mythos-5`, `gemini-3.6/3.7/3.8-flash`, `gemini-3.5-flash-lite`, `deepseek-v4-flash-vision-exp`, `kimi-k3`, `kimi-k2.7-code`; retired DeepSeek chat/reasoner/coder + `gemini-3.1-flash` + `imagen-4` with migration-map successors; pricing drift fixes (DeepSeek v4-flash input 10×, Claude Opus inputs $15 → $5); new defaults (`gemini-3.6-flash`, `gpt-image-2`, `kimi-k3`); ~45 derived files updated; 517 model tests / 8,290 assertions green.
- 🌐 **Content Graph Ecosystem (PRs #6316–#6321, #6325).** The standalone `nvoos-content-graph` plugin ships its **1.0.4 visual experience system** — theme engine (PHP/JS byte-identical, WCAG 2.2 SC 1.4.11 ≥3:1 palettes), appearance settings tab, 24 SVG type glyphs, legend/minimap/zoom/keyboard explorer chrome, `GET /edges` route, export at 1–3×, block/shortcode attribute parity, and a checkout fallback redirect (#6318). Content Graph AI gains the assistant-builder block set (selector, tools grid, knowledge base, builder) and a validate-then-sanitize settings shell (#6316, #6317). The platform completes its **Wave E2 queue layer**: `AsyncJobQueue` → `QueueManager` → `JobQueueManager` (atomic `SELECT … FOR UPDATE SKIP LOCKED`) → `DeadLetterQueue`, byte-identical to the base stack (#6319–#6321, #6325).
- 🔗 **Connectors Links Fixed (PR #6314).** The "Settings → Connectors" links now use `admin_url( 'options-connectors.php' )` — the old `admin.php?page=connectors` target isn't a registered page.
- 🧪 **Post-K16 Singles (PR #6327, CI run 91942465749).** Order-dependent singles closed: transcript session-key mock restore, shared tool-registry teardown (Pro OKF tools were wiped by `clear_tools()`), Site Health provider-key isolation, transcript-mining logging cache reset. The `mcp-ai-wpoos-test-suite` skill grew to **40** patterns.
- 📚 **New Coding-Time Skill (PRs #6323, #6324).** `.agents/skills/mcp-ai-wpoos-updates/` codifies the two maintenance tracks — docs & release catch-up (Track A) and model catalog & config updates (Track B). Coding-time agent skills: 53 → **54**.

### ✨ What's New at a Glance (v1.1.70)

- 🛡️ **Shell-Call Hardening on exec-Disabled Hosts (PR #6295).** The Pro Packages page (and five other Pro admin pages + the OCR service) fataled on hosts with `exec` in `disable_functions` — on PHP 8+ `@` cannot suppress the disabled-function `Error`. Canonical `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` helpers probe exec-first and fall back to `WP_MCP_AI_Process_Service` (`proc_open`); the OCR service gains `is_cli_tool_available()` / `run_cli_command()` (Windows-aware) and guarded `proc_*` calls; seven CPT/settings pages clamp negative inputs with `max( 0, … )` instead of `absint()` (`absint('-5')` → `5`).
- 🐛 **Wave-5 Production Fixes (PRs #6280–#6312).** Transcript `attachments` display metadata preserved on save + default `max_tokens` capped by the model's window (#6280); Pro dashboard event stats accept underscore and dashed slugs (#6281); validated `web_search` `profession_tags` aligned (#6282); schedule presets drop a phantom `validate_contact_data` step and put the hook in `schedule_data` (#6283); mesh peer URLs validated **before** `esc_url_raw()` (#6285); opt-in guest memory capture writes under a shared `guest` bucket (#6286); JetEngine gates require `JET_ENGINE_VERSION` + a physical CCT-table probe, the JFB submissions tool restores `id`/flat params, and the token manager uses the cached Pro tool map (#6296, #6300); complexity routing restored — the default model no longer short-circuits `select_model()` (#6298); Auth0 audience validation is structural, image tools check `read_post` against the **acting user**, and user IDs validate as positive ints (#6301, #6305); markup REST errors return `400`, the legacy SSE handshake is filterable, skill packs reject numeric-keyed entries and always fire their installed action (#6302); clamp sanitizers + plugins-integration terminate seam (#6303); Elementor `is_elementor_editor_init()` + `static::is_available()` (#6304); provider-prefixed model-slug token limits, Ollama native tool-calls fix, `published_date` preservation, `wp_mcp_ai_missing_image` (#6306); agentic events reach the recent-activity feed + millisecond trace durations (#6308); K16 bundle — `sanitize_key`-stable webhook IDs, settings-repository blob fallback for runtime gates, acting-user chart `unfiltered_html`, social-publish date reset, PSO inflections, site-builder h1–h6, schema-verifier `required` (#6311); curriculum exporter skips empty structures (#6293).
- 🧪 **PHPUnit Repair Campaign, Wave 5 (~29 test PRs, #6280–#6312).** Two CI triage runs (`CI-TRIAGE-91006542428.md` + `CI-TRIAGE-91771001271.md`) drove the clusters closed: transcripts, charts/dashboards, professions, presets, Cloudflare, mesh, memory, CPT management, PayHere, Mubert, OpenAI Images, composer drift, Content Graph AI, pro schedule AJAX, model routing, Gemini, JetEngine/Graphify stubs, OAuth, REST/skill packs, admin settings, Elementor, permission gates, HTTP clients, orchestration budget, logging, K16 misc singles. The `mcp-ai-wpoos-test-suite` skill's cluster board is compacted to one completed wave-5 block (37 patterns unchanged).

### ✨ What's New at a Glance (v1.1.69)

- 👁️ **Vision Analysis Toolkit (Pro, PR #6267).** `analyze_image_objects` detects and counts objects per category — `detection` (HuggingFace OWLv2 / Ollama vision), `vlm` (OpenAI / Anthropic / Gemini JSON-enforced counting), and `hybrid` (detector owns counts, VLM only renames mislabeled categories) — with `annotate=true` returning a GD box-annotated copy as a media attachment. Gated by a new NV oOS → Vision Analysis settings page (`enable_vision_analysis_toolkit`, off by default) with SSRF URL-guards. See [`docs/toolkits/vision-analysis-toolkit.md`](docs/toolkits/vision-analysis-toolkit.md) + [`docs/proposals/vision-analysis-object-counting-tool.md`](docs/proposals/vision-analysis-object-counting-tool.md). Pro tool count +1.
...
- 🔧 **DeepSeek Empty-Schema 400 Fix (PR #6272).** Argument-less tools now emit `properties: {}` (never `[]`) across 29 files; `LegacyToolAdapter` preserves object maps and upgrades empty arrays; the AI Tool Builder scaffold emits `new stdClass()` for parameterless tools.
- 🔒 **ZipSlip Guard Revived (PR #6270).** `ZipArchive::$num_files` doesn't exist on PHP 8.x, so the guard loop never ran — unsafe archives imported successfully. `count( $zip )` now drives all five entry loops (OKF bundle manager + four Pro admin pages), restoring path-traversal rejection.
- 🐛 **Fixes.** Rate limiter classified by the dispatching request's real HTTP verb with a separate nefarious-monitor counter and dual-shape chat hook tolerance (#6265); one broken tool's shortcut tasks can't fatal the assistant edit screen (#6271); model catalog: gpt-4o 128k context, `gemini-2.0-flash` video-capable typo, claude-sonnet-4-6/gpt-4o active status, two Qwen entries (#6274); settings save no longer leaves object-cache additions suspended (WP 7.1 inline-async tick locks, #6277).
- 🧪 **PHPUnit Repair Campaign Continuation (~9 test PRs, #6260–#6276).** Fourth wave: count-tokens params, continuation seam, SSE handler, deterministic cache tests, coverage manifest, incorrect-usage notices, admin registration, subtab sanitizers, 14 Veo suites. The `mcp-ai-wpoos-test-suite` skill now distills **37** root-cause patterns.

### ✨ What's New at a Glance (v1.1.68)

- 🧩 **Pro SPA v2 Shortcode & Embedded Mode (Pro, PR #6256).** New `[nvoos_pro_spa]` shortcode mounts the Pro SPA v2 in chat-first embedded mode — `AgentPanel` + drawers, tool shortcuts, OKF drawer, router-free, no admin routes — with per-instance `data-config` attributes and optional guest mode via the base guest-token machinery. New `WP_MCP_AI_Pro_SPA_Shortcode` + `Pro_SPA_Config` + module-registry entry + `spa-v2` block. See [`docs/project/proposals/033-pro-spa-v2-shortcode-proposal.md`](docs/project/proposals/033-pro-spa-v2-shortcode-proposal.md).
- 🖥️ **Hermes Dashboard Fleet Extensions (PR #6249).** New top-level `extensions/` tree: a fleet monitoring + control plane extension (`nv-oos-fleet` — Python plugin API over ~35 sites), plus `backup-download`, `external-app-tab`, and `mcp-tool-shortcuts` WebUI extensions with installer + smoke tests. Plans: [`docs/developer/integration/hermes-dashboard-extensions-plan.md`](docs/developer/integration/hermes-dashboard-extensions-plan.md).
- 🔄 **Chat-Bubble Stale-Nonce Self-Heal (PR #6225).** `GET /mcp-ai/v1/session/nonce` returns a fresh session-bound `wp_rest` nonce minted from the request's own auth cookie (`no-cache`); the chat client silently retries `403 rest_cookie_invalid_nonce` failures from full-page caching or session-token rotation on long-lived SPA pages.
- 📚 **Docs Hub 0.4.2 (PRs #6246, #6253).** Internal links on local pages resolve to `#/slug` hash routes; TOC anchors mirror github-slugger exactly; "Accept fix" suggestions are directory-relative and `../` links are validated with containment checks; skipped rows show reasons; sync failures surface "Atomic swap failed"; the emoji loader no longer crashes the React SPA. Addon bump 0.4.1 → 0.4.2.
- 🔌 **Providers Disabled by Default on Fresh Installs (PR #6255).** OpenAI/Anthropic/Gemini default to disabled; dropdowns list only enabled + credentialed providers; the onboarding wizard auto-enables the provider whose key you enter.
- 🐛 **Fixes.** Google Calendar granted-scopes `%20` corruption (#6226); Tiptap `mergeAttributes` prototype pollution — canvas/document editor pin tiptap 3.30.4 (#6250); `fast-uri` >=4.1.4 across npm trees (#6244); assistant untrash restores the pre-trash status (#6239); presets gain missing Google/Gmail/Drive/OKF/git tools (#6242); `wp_mcp_ai_seed_task_templates` AJAX wired (#6243); jQuery UI sortable shim for post screens (#6258); agent-identity canonical-ID int cast (#6232); token-budget catalog dedup + restored `wp_mcp_ai_model_tpm_limit` filter seam (#6233).
- 🧪 **PHPUnit Repair Campaign Continuation (~21 PRs, #6224–#6257, + #6259 merged upstream).** A third wave kept the suite aligned with current contracts; the production fixes it carried are grouped above (including the late-merged `wp_mcp_ai_attachment_segment_provider` filter — providers without a remote file API, e.g. Ollama, resolve attachments to local references).
- 🧹 **Housekeeping.** Stale 1.1.67 build ZIPs removed (27 files).

### ✨ What's New at a Glance (v1.1.67)

- 🧩 **Content Graph AI Platform v2.0.0 Standalone (PR #6123).** The extracted `nvoos-content-graph-ai-platform` addon now carries its own business logic — Waves A–C + Blueprints deliver the namespace-bridged admin UI (`PlatformDashboard`, `PlatformSettingsRegistry`), skill/slash-command/agent bridges, harness router, a 74-skill bundled-skills pack with third-party notices, a knowledge base, and a dedicated PHPUnit matrix. See [`docs/project/plans/content-graph-platform-extraction-plan.md`](docs/project/plans/content-graph-platform-extraction-plan.md).
- 🧱 **Base+Pro → Content Graph Ecosystem Port, Wave D + D-UI (PR #6142).** The AI runtime lands in `nvoos-content-graph-ai`: chat core (optimizer, caches, rate limiter, summarizer, threads, attachments, transcripts, ChatKit), providers (Zai, Google Maps, Realtime ×3, RabbitMQ, STDIO, file services), model management + analytics/token tracking, security guards, and D-UI assistant pages + blocks/widgets/guest tokens/memory/CLI/MCP JSON-RPC. Content Graph AI bumps to 1.0.4. Tracked in [`docs/project/ecosystem-port-tracker.md`](docs/project/ecosystem-port-tracker.md).
- 📧 **Google Workspace Gmail + Drive Read Tools (Pro, PRs #6151–#6152).** Six new Pro tools — `get_gmail_message`, `get_gmail_thread`, `list_gmail_connections`, `modify_gmail_message` (destructive-ops gated) and `get_drive_file`, `list_drive_connections` — backed by new `WP_MCP_AI_Pro_Gmail_Client` / `WP_MCP_AI_Pro_Google_Drive_Client` clients on the shared `includes/google/` foundation. Pro tool count +6.
- 🧪 **PHPUnit Repair Campaign Continuation (~100 PRs, #6114–#6208, #6209–#6222).** A second cluster wave kept the suite green through the extraction/port work and PHPUnit 11 / WP 7.1 drift; the production fixes it carried are grouped in the changelog (crawler contract hardening, REST guards, LLM sanitization delegation, URL-encoded external-API queries, Veo 5s floor, settings/cache fixes, WhatsApp webhook signature rejection, HTTP 304 cached-body re-serve, Veo async-job completion ordering, token-budget display, Shopify client-availability filter seam, Paper Store array-field queries). The `mcp-ai-wpoos-test-suite` skill now distills **26** root-cause patterns.
- 🧹 **Housekeeping.** Stale 1.1.66 build ZIPs removed; re-created 1.1.65 ZIPs re-removed.
...
- 💳 **Checkout API Addon + Content Graph Paid Checkout.** New `addons/checkout-api/` (v0.1.0, vendor-server only) — Stripe session/verify endpoints, encrypted secrets, custom-table license store, signed expiring ZIP downloads, webhook license issuance/revocation, per-IP rate limiting — plus the paid-checkout client in the free `nvoos-content-graph` plugin (Stripe Payment Element, verify, license, one-flow install; no Stripe keys in the plugin). Addon count: 27. (PR #6063)
...
- 🧬 **Artifact Evolution Phases A–G (Base + Pro).** The Continual Harness Evolver and Meta-Harness become a complete, gated, Darwinian evolution loop for skills, prompts, and roles: competing artifact populations with fitness-weighted parent selection, failure-case replay + post-mutation verification, learning-log-aware mutators, a pre-commit admission gate (structural / harmlessness / marginal-gain critics), holdout-gated deployment with shadow A/B serving and drift-triggered rollback, and a cross-cutting governor with a human approval queue and per-artifact lineage graphs. The `evolve_harness` ↔ Evolver contract is repaired, and every layer is opt-in (default off) with switches in **Settings → Orchestration Layer**. See [`docs/project/proposals/007-artifact-evolution.md`](docs/project/proposals/007-artifact-evolution.md). (PRs #5923, #5925)
- 🧩 **Pro Addons Page (Pro).** New registry-driven **NV oOS Pro Dashboard → Addons** page installs and activates standalone addons whose ZIPs ship in the plugin's `build/` directory in one click — nonce + `install_plugins` + allowlist gated, with non-WordPress components (Media Worker, Cloud Worker, Tenant Router, Schedule Anything SPA) listed read-only. (PR #5924)
- ⚡ **Chat Storage Worker Offload.** Conversation saves at or above `wp_mcp_ai_storage_worker_threshold` (default 10,000 chars) offload the expensive `JSON.stringify` to a browser storage worker; small saves, unload flushes, and worker failures fall back to the synchronous main-thread write, and the threshold filter is the kill switch. See [`docs/project/proposals/032-chat-web-workers-wiring-implementation-plan.md`](docs/project/proposals/032-chat-web-workers-wiring-implementation-plan.md). (PR #5928)
- 🐛 **DeepSeek Empty-Schema Fix.** Empty tool-schema property maps now encode as `{}` (never `[]`) at all three payload boundaries, fixing DeepSeek 400s; legacy tools are wrapped before the first register attempt so the fail-loud registry log only fires for genuinely unregisterable classes. (PR #5926)
- 📚 **OKF Skill-Knowledge Conformance.** All 91 bundled SKILL.md files gained the OKF v0.2-required `type: Skill` frontmatter and the four missing cross-linked reference files were restored — the OKF Validate tab no longer reports missing concepts. (PR #5919)
...
- 🔧 **Pro SPA v2 Slash Commands.** `/chat`, `/new`, `/clear`, `/copy`, `/export` now insert reliably from the slash-command drawer. (PR #5920)
...
- 🔒 **Restricted-User Flagging & Unblocking.** Ephemeral rate-limit and token-budget blocks become persistent, reviewable restriction records with auto-expiry and audit logging — one-click lift on the Token Manager "Restricted Users" panel (Base), a full **Restrictions** tab on the Pro Command Center, REST/AJAX/WP-CLI operations (`wp mcp-ai restrictions list|lift|add`), IETF rate-limit response headers, and filterable chat rate limits (`wp_mcp_ai_chat_rate_limit` / `wp_mcp_ai_chat_rate_limit_window`). See [`docs/features/security/user-restrictions.md`](docs/features/security/user-restrictions.md). (PR #5901)
- 📥 **Conversation Import to Transcript CCT (Full, JetEngine).** New `includes/conversation-import/` subsystem imports ChatGPT, Google Takeout Gemini, Claude, ShareGPT, and OpenAI fine-tuning JSONL exports into the JetEngine `ai_chat_transcripts` CCT — four new tools (`conversation_import_detect|run|status|delete`), an admin upload/preview page, WP-CLI, GDPR export/erase, and optional memory mining. See [`docs/user-guides/conversation-import.md`](docs/user-guides/conversation-import.md). (PR #5898)
- 🧩 **Tool Schema Normalization.** Tool argument schemas are normalized before provider payloads (DeepSeek client, REST `/tools` output, Tool Service, ChatOrchestrator), preventing provider streaming errors; the registry logs registrations skipped for a missing tool contract. (PR #5903)
- 🐛 **WP_Error Fatal Fixes** in the memory layer and REST chat-memory paths (`mine_agent_memory`, `retrieve_agent_memory`, `wake_up_context`, request guard). (PR #5905)
- 📦 **nvoos-content-graph v1.0.3** — wp.org resubmission with WPCS/review fixes and `.wordpress-org` screenshot assets. (PRs #5897, #5899, #5904)
...
- 🔌 **Composio Connect (Pro).** New `addons/pro/includes/composio/` subsystem bridges WordPress to the Composio MCP gateway: OAuth connection flow with state nonce, API client, trigger bridge, and signed webhook controller. Six new beta tools — `composio_list_tools`, `composio_get_tool_schema`, `composio_list_connected_accounts`, `composio_create_connect_link`, `composio_execute_tool`, `composio_manage_triggers` — plus remote-sites admin and metabox panels. See [`docs/composio-connect.md`](docs/composio-connect.md). (PR #5889)
- ⚙️ **OOS Runtime Consolidation (Phases 0–5.8).** The OOS engine now reaches parity with the legacy path — tool-surface and security-gate parity, an event-sourced session log, opt-in shadow mode (parallel parity runs serving the legacy result, zero user exposure) with `wp mcp-ai oos parity` CLI, canary routing per assistant, scoped tools with a compaction seam, Pro composition & child binding (`wp mcp-ai composition`), and single-path telemetry. See [`docs/project/proposals/029-oos-orchestration-runtime-consolidation-implementation-plan.md`](docs/project/proposals/029-oos-orchestration-runtime-consolidation-implementation-plan.md). (PR #5881)
- 🏷️ **Standalone Plugins Renamed to Content Graph.** `plugins/nvoos-graphify` → `nvoos-content-graph`, `nvoos-graphify-ai` → `nvoos-content-graph-ai`, `nvoos-graphify-ai-platform` → `nvoos-content-graph-ai-platform` (v1.0.2) with matching workflow and build-script renames. The `addons/graphify/` knowledge-graph addon is unchanged. (PR #5890)
- 🐛 **Security Center Refresh Fix.** `wp-api` script now enqueued on the security tab — posture refresh, IP tests, snapshot restore, compliance export, and self-test no longer throw `apiFetch is not defined`. (PR #5887)
- 🔒 **deepmerge-ts CVE-2026-40345** overridden in the media-worker and Pro packages. (PR #5888)
...
- 🏢 **Media Worker v3.0.0 — Multi-Tenant Shared Worker Mode (v2.4.0).** `SITE_TOKENS` per-site isolation (fail-closed auth, namespaced files/queues/rate limits), per-site rate-limit overrides, `SITE_TOKENS_PREVIOUS` rotation. (PR #5866)
- 🔑 **Phase 2 — Per-Site Provider Keys & Observability.** `SITE_PROVIDER_KEYS` per-tenant AI provider credentials (with `PROVIDER_KEYS_STRICT` fail-closed mode), per-site usage counters, grouped temp TTLs, cluster-mode boot warnings, and a k6 load-test kit with split decision table. (PR #5868)
- 📐 **Phase 3 — Scale Without Breaking Changes.** Multisite per-blog worker tokens, plugin-side Media Worker Usage Reporter (opt-in daily cron), `SITE_TOKEN_<SLUG>` / `SITE_PROVIDER_KEYS_<SLUG>` env merges, opt-in Redis rate-limit store (`RATE_LIMIT_REDIS=1`), `PROVIDER_KEYS_FILE` hot-reload, manual Velocity deploy workflow. All opt-in — defaults unchanged.
- 🔄 **Worker Routing Expansion.** Document generation, OCR, video frames, health charts, email, QR/translate/PDF, and vectorization now route through the connected worker with local fallbacks; Pro settings lists worker-routed packages; new `probe-wordpress.php` connectivity probe.
- 🔐 **Rotation, Canvas v3, Cloudways.** Zero-downtime token rotation (`WORKER_API_TOKEN_PREVIOUS`), Canvas v3 napi prebuilds (compiler-free installs), Cloudways readiness hardening, live route bug fixes. (PRs #5863–#5865, #5867)
- 🤖 **Hermes Tooling.** WebUI MCP server (`bin/hermes-mcp-server.js`) to drive a Hermes agent from Zed (sessions, synchronous chat, skill sync), SSH MCP bridge, hardened env-file parser, and the Zed Hermes Console profile. (PR #5862 + follow-ups)

### ✨ What's New at a Glance (v1.1.55)

- 🔧 **MCP Agent Compatibility & Reliability.** JSON-RPC errors now return HTTP 200 so agent SDKs that drop non-2xx bodies relay tool errors instead of hanging. New legacy HTTP+SSE transport with credential-bound sessions for SSE-only clients. Tool rate limiter is settings-driven with credential-token exemption; GET/HEAD no longer consume the request quota; raw `cred_*` authorization headers accepted; async tool polling bounded at ~45s with inline kick of stuck jobs. (PR #5859)
- 🚀 **Hermes Fleet Operator Addon.** New `addons/fleet-operator/` — scoped `op_` operator credentials with audience binding, expiry, rate limits, and instant revocation so a supervisor agent (Hermes or any MCP/A2A host) can operate the site within an allowlist. Server-side `tools/list` scoping + `tools/call` enforcement. Ships admin page, WP-CLI commands, Hermes config generator, 3-skill nvoos pack, runbook, and tests. (PR #5858)
- 🔒 **Media Worker v2.2.0 Security Hardening.** Timing-safe `X-Site-Token` auth, SSRF guard, sandboxed Puppeteer launcher, rate limiting, Helmet headers, split health endpoints, request logging, graceful shutdown. `WP_MEDIA_WORKER_TOKEN` constant support in the plugin sidecar. Cloudways Velocity deployment guide + CI. (PR #5857)
- 🗄️ **Database Connection Pooling Stance (Proposal 023).** RabbitMQ-aware gating of Action Scheduler fallback and DB polling cron, atomic concurrency-guard slot tracking (`mcp_ai_concurrency_slots` table + daily cleanup), PDO persistent connections in Graphify, configurable queue worker batch size, and Site Health checks for MySQL pool, queue depth, and RabbitMQ health. (PR #5855)
- 📦 **PostCSS 8.5.26** (GHSA-6g55-p6wh-862q) and media-worker subtree sync workflow (PR #5856).
...
The plugin works standalone with **~307 base tools** and optionally extends through the **Pro addon**, which adds **~1,279 Pro tools** for advanced integrations (WooCommerce, JetEngine, social media APIs, GitHub, Google services — including Google Calendar and the new Gmail/Drive read tools — Shopify, QuickBooks Desktop, Yahoo Fantasy Sports, ESPN Fantasy, ECA management, CRE Debt & Securitization, Cloudways server management, CRM lead/deal/customer lifecycle, support ticket management, multichannel inbound/outbound messaging, Composio Connect, vision analysis object counting) and exec-based tools (FFmpeg, WP-CLI, Python rembg, Jukebox), bringing the total to **~1,586 built-in tools** (~307 base + ~1,279 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative).

> **Note on Tool Count:** Tools include base WordPress operations, content management, media generation, research capabilities, and optional third-party integrations. The base version (~307 tools) works standalone. The full version requires the Pro addon and provides ~1,586 total tools including specialized toolkits for e-commerce, social media, analytics, document generation, vehicle estimation, image validation, JetEngine MCP, A2A agent delegation, CRE Debt & Securitization, Cloudways infrastructure management, CRM lead/deal/customer lifecycle + support tickets + multichannel, MCP Apps, Composio Connect, vision analysis, and more. Live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative.
...
- **~1,586 tool classes** (~307 base + ~1,279 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative) registered through a singleton Tool Registry
...
## 🆕 Latest Updates (v1.1.83 — September 2026)

### September 21, 2026 — Tool Guidance Everywhere, Jev Decisions, Assistant Builder, ID-Handoff Contracts
...
## 🆕 Latest Updates (v1.1.82 — September 2026)

### September 18, 2026 — WordPress Playground Demos, Pro SPA Fixes, Token-Tracking & Delivery Hardening
...
### September 15, 2026 — Assistant Portability, Shopify UCP Catalog, Security Usage Monitor, WP-CLI Repair
...
### September 12, 2026 — Slash-Command Rework, DeepSeek V4 Pro Restoration, Checkout Hardening, Legal Consolidation
...
## 🆕 Latest Updates (v1.1.77 — September 2026)

### September 11, 2026 — DeepSeek V4.1 Flash, Base+Pro Gating, Checkout Launch
...
## 🆕 Latest Updates (v1.1.76 — September 2026)

### September 10, 2026 — Chat Delivery Formats, Checkout Launch, Security Sweeps, Wave F2 Completions
...
## 🆕 Latest Updates (v1.1.75 — September 2026)

### September 9, 2026 — Telegram Delivery Fixes, Memory Bridge + Complete Checkout, Wave F2 Toolkit Completions
...
## 🆕 Latest Updates (v1.1.74 — September 2026)

### September 8, 2026 — Calendar Query Fix, Email Formats, Assistant-Prompt Editing, PM + Calendar Ports
...
## 🆕 Latest Updates (v1.1.73 — September 2026)

### September 8, 2026 — Woo Tool Upgrades, Queue Bootstrap Fix, Wave F2 + Content Graph Pro
...
### September 5, 2026 — Rate-Limit Unlock, September Model Catalog, Content Graph 1.0.4, Wave E2 Queue Layer

- 🔓 **Rate-Limit Unlock + Fixed-Window Timer — PR #6322.** The general REST request limiter (`check_rate_limit()`) now uses fixed-window accounting (`{count, first_seen}` transient payload, TTL never extended past window end, legacy integer transients normalized on read) and returns the remaining time in `retry_after`. When the limit trips, the new `wp_mcp_ai_rest_request_rate_limit_exceeded` action makes `WP_MCP_AI_Restriction_Registry` flag a `rate_limit` restriction (scope `rest`, auto-release at window end) — the block now appears in the Command Center Restrictions tab and Token Manager with the existing Lift button, and lifting deletes the `wp_mcp_ai_rate_limit_user_{id}` transient for a fresh window. Guest (IP-keyed) blocks are not user-attached and expire on their own (`docs/features/security/user-restrictions.md`).
- 🧠 **MemPalace Wing-Scope Fix + Post-K16 Singles — PR #6327.** `WP_MCP_AI_Tool_Wake_Up_Context::matches_wake_filters()` enforces `wing`/`room` exclusions (the Graphify graph anchors only *boost* scores — they never excluded out-of-scope memories, leaking cross-wing memories into wing-scoped blocks). Test-only isolation from CI run 91942465749: transcript repository mock restored unconditionally, shared tool-registry instance restored untouched (tearDown wiped one-shot bootstrap tools like the Pro OKF tools), Site Health connectivity tests unset the other provider keys, transcript-mining logging resets the settings cache.
- 🛒 **Checkout API Pipeline + Salt Fix — PR #6315.** `build/nvoos-checkout-api-v<version>.zip` joins the addon build, `addons/checkout-api/tests` joins the main PHPUnit gate, and the build workflow gains a `tests` job. Production fix: the token/crypto classes derive `wp_salt( 'auth' ) . wp_salt( 'secure_auth' )` instead of reading `AUTH_KEY . SECURE_AUTH_KEY` directly (latent fatal on salt-less installs).
- 🧭 **September 2026 Model Catalog — PR #6328.** 217 → 228 models (catalog version 2026.07.14 → 2026.09.05): new gpt-5.6 family, `gpt-6-astra` preview, `gpt-image-2`, `claude-opus-5`, `claude-fable-5.1`, `claude-mythos-5`, gemini-3.6/3.7/3.8-flash, `gemini-3.5-flash-lite`, `gemini-3.1-flash-image-preview`, `deepseek-v4-flash-vision-exp`, `kimi-k3`, `kimi-k2.7-code`; retired `deepseek-chat`/`-reasoner`/`-coder` (API retired 2026-07-24), `gemini-3.1-flash` (sunset 2026-09-01), `imagen-4` (shutdown 2026-08-17) with migration-map successors; pricing drift fixes (deepseek-v4-flash input 10×, Claude Opus inputs $15 → $5); `gemini-2.5-flash-image` sunset 2026-10-02, `claude-sonnet-4-6` sunset cleared; defaults `gemini-3.6-flash` / `gpt-image-2` / `kimi-k3`. ~45 derived files; `lib/core/`, `lib/wordpress-adapter/`, and the Content Graph AI plugin keep their own mirrored tracks.
- 🌐 **Content Graph v1.0.4 Visual Experience — PR #6318.** Theme engine (one token registry, PHP/JS byte-identical, WCAG ≥3:1 palettes), appearance settings tab, 24 SVG type glyphs, interactive legend/minimap/zoom cluster/fullscreen/keyboard, `GET /edges` REST route with relationship color families, export at 1–3×, `[nvoos_graph]` + block parity attributes, three visual filters, and a checkout-fallback redirect to the vendor product page.
- 🌐 **Content Graph AI Blocks + Settings Shell — PRs #6316, #6317.** `assistant-selector`, `tools-grid`, `knowledge-base`, `assistant-builder` blocks (the Build Assistant Prompt tab embeds the tools grid + KB); `SettingsValidator`/`AiSection`/`SettingsRegistry` settings shell (validate-then-sanitize, real validations on provider-selection + chat-settings).
- 🌐 **Platform Wave E2 Queue Layer — PRs #6319–#6321, #6325.** `AsyncJobQueue` (new `nvoos_content_graph_ai_platform/async_job_executors` filter), `QueueManager`, `JobQueueManager` (atomic `SELECT … FOR UPDATE SKIP LOCKED`), `DeadLetterQueue` — byte-identical ports completing the platform queue pipeline; drive-by fix extracts `AgentCapabilityBoundaryHooks` to its own PSR-4 file (standalone fatal).
- 🔗 **Connectors Links — PR #6314.** Providers-tab notice + API-key credential-source hint now use `admin_url( 'options-connectors.php' )` (the core Connectors screen) instead of the unregistered `admin.php?page=connectors`.
- 📚 **New Coding-Time Skill — PRs #6323, #6324.** `.agents/skills/mcp-ai-wpoos-updates/` codifies Track A (docs & release catch-up) + Track B (model catalog & config updates); skill count 53 → **54**; YAML frontmatter fix follow-up.
- 📦 **Versioning** — bumped to **1.1.71** across all version-bearing files. Pro addon: 1.1.71. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.4** (was 1.0.3). nvoos-content-graph-ai: **1.0.4** (unchanged). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). Checkout API: **v0.1.0** (unchanged). Docs Hub addon: **0.4.2** (unchanged). Tool count: ~303 base + ~1,263 Pro (~1,566 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **27**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **54**. Stale 1.1.68 + 1.1.69 build ZIPs removed.

## 🆕 Latest Updates (v1.1.70 — September 2026)

### September 5, 2026 — exec-Disabled Host Hardening + Fifth PHPUnit Repair Wave

- 🛡️ **Shell-Call Hardening — PR #6295.** Disabled `exec`/`proc_open` no longer fatals the Pro Packages / Document Generation / Image Production / Media pages or the OCR service: `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` (new, `addons/pro/includes/npm-integration-filters.php`) fall back to `WP_MCP_AI_Process_Service`; OCR routes through `is_cli_tool_available()` / `run_cli_command()` with guarded `proc_*` calls; `max( 0, (int) $value )` replaces `absint()` for `assistant_id`, time limits, and passing scores across seven CPT/settings pages.
- 🐛 **Wave-5 Fixes — PRs #6280, #6281, #6282, #6283, #6285, #6286, #6293.** Transcript `display.attachments` preserved (sanitizer kept the `{url, label, downloadName, meta}` shape) + model-aware `max_completion_tokens` caps (#6280); Pro dashboard `get_monitoring_event_stats()` accepts both slug styles (#6281); validated `web_search` profession tags (#6282); preset fixes — phantom `validate_contact_data` step removed, hook moved to `schedule_data.hook` (#6283); mesh URLs validated pre-sanitization (#6285); guest bucket for opt-in memory capture (#6286); curriculum exporter skips empty structures (#6293).
- 🐛 **Wave-5 Fixes — PRs #6296, #6298, #6300.** `WP_MCP_AI_JetEngine_CCT::is_storage_available()` physical-table probe + deleter/writer/tool gating + `conversation_import_*` preset additions (#6296); complexity routing restored — enhanced client passes the explicit model only, falling back to the site default only when routing is disabled (#6298); `JET_ENGINE_VERSION` requirement on JetEngine gates, transactional-DDL-safe `cct_table_exists()`, JFB `id` + flat params, cached `wp_mcp_ai_pro_get_tool_map()` feeding the token-manager listing (#6300).
- 🐛 **Wave-5 Fixes — PRs #6301, #6302, #6303, #6304, #6305.** Auth0 audience validated structurally (identifiers, not endpoints — the Auth0 domain keeps the SSRF guard) + GitHub OAuth `wp_mcp_ai_github_oauth_redirect_terminate` seam (#6301); skill-pack slug normalization + `wp_mcp_ai_skill_pack_installed` for empty packs + markup REST `400` annotations + filterable `wp_mcp_ai_legacy_sse_enabled` (#6302); clamp sanitizers + `wp_mcp_ai_plugins_integration_redirect_terminate` seam + section-ID/tab contract updates (#6303); `is_elementor_editor_init()` + `static::is_available()` (#6304); acting-user `read_post` checks + positive-int user validation + Auth0 structural audience check (#6305).
- 🐛 **Wave-5 Fixes — PRs #6306, #6308, #6311.** `timed-out` timeout needle + provider-prefix-stripped model limits (`nvidia/nemotron-…` → `nemotron-3`) + Ollama native tool-call preservation + `css_to_xpath()` predicates + `published_date` + `wp_mcp_ai_missing_image` (#6306); agentic `*` + `token_db_optimized` allowlist events + `started_at_ms` trace durations (#6308); K16 — `sanitize_key`-stable webhook IDs with dotted event names, settings-repository blob fallback (request guard / security manager / destructive-ops gate now see dashboard-saved settings), acting-user chart `unfiltered_html`, social-publish `post_date` reset, PSO keyword inflections, site-builder heading-tag allowlist, schema-verifier `required` independence (#6311).
- 🧪 **PHPUnit Repair Campaign, Wave 5 — PRs #6280–#6312 (~29 test-only).** Clusters closed from two triage runs (`docs/developer/testing-docs/CI-TRIAGE-91006542428.md`, `CI-TRIAGE-91771001271.md`): chat transcripts, charts/dashboards, professions/teams, presets/remote connections, Cloudflare, mesh, memory, CPT management, PayHere, Mubert, OpenAI Images, composer drift ×2, Content Graph AI, pro schedule AJAX, model routing, Gemini, JetEngine/Graphify stub isolation (`tests/helpers/jetengine-stubs.php`), OAuth/credentials, REST + skill packs (K15 upload isolation in #6312), admin/settings, Elementor, permission gates, HTTP/provider clients, orchestration budget, logging/events, K16 misc singles. Cluster board compacted in the `mcp-ai-wpoos-test-suite` skill (37 patterns unchanged).
- 📦 **Versioning** — bumped to **1.1.70** across all version-bearing files. Pro addon: 1.1.70. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.3** (unchanged). nvoos-content-graph-ai: **1.0.4** (unchanged). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). Docs Hub addon: **0.4.2** (unchanged). Tool count: ~303 base + ~1,263 Pro (~1,566 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **27**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **53**.

## 🆕 Latest Updates (v1.1.69 — September 2026)

### September 4, 2026 — Vision Analysis Toolkit, tagDiv Admin Compat, DeepSeek Schema Fix, ZipSlip Guard Revival

- 👁️ **Vision Analysis Toolkit — PR #6267.** New Pro toolkit in `addons/pro/includes/tools/vision-analysis/`: `analyze_image_objects` (detection/vlm/hybrid modes, `annotate=true` GD bounding-box copy as a media attachment) + `WP_MCP_AI_HF_Vision_Inference_Service::count_objects()`, `WP_MCP_AI_Vision_Count_Normalizer`, `WP_MCP_AI_Vision_VLM_Client`, `WP_MCP_AI_Vision_Annotator`, and the NV oOS → Vision Analysis settings page (off by default, SSRF-guarded). `bin/run-tests-docker.sh` now composer-installs dev dependencies for production-only clones.
- 🐛 **tagDiv Admin Compat & Metabox Crashes — PRs #6266, #6278.** SiteKit tools return string capability-flag arrays (the undefined `CAPABILITY_CAN_USE_IF_ADMIN` constant killed the tools metabox at `sitekit_get_adsense` on nugl.com); the sortable fallback prints its bundled copy whenever `td_wp_admin` is enqueued (print-queue detection) at `admin_head` + `wp_print_footer_scripts` priority 1; the media script chain is forced as direct head tags from core-registered handles; tools metabox CSS enqueued on `admin_enqueue_scripts`.
- 🔧 **DeepSeek Empty-Schema 400 — PR #6272.** Argument-less tools now emit `properties: {}` across 29 files (Gmail/Drive list tools, Cloudways ×4, DietPi ×7, `wait_for_user`, `okf_list_bundles`, WebChat status, graphify, embedded abilities, `ProfessionStatsTool`, content-graph); `LegacyToolAdapter` preserves object maps + upgrades empty arrays; scaffold template emits `new stdClass()`.
- 🔒 **ZipSlip Guard Revival — PR #6270.** `count( $zip )` replaces the PHP 8.x-nonexistent `$zip->num_files` in all five entry loops (OKF bundle manager, Comic/Media Consolidate pages, Skill Manager ×2) — archive import rejects path traversal again (probe confirmed `../evil.md` previously imported).
- 🐛 **Fixes — PRs #6265, #6271, #6274, #6277.** Rate limiter accepts the dispatching request's HTTP method (internal dispatches classified by their real verb, GET/HEAD exempt) and the nefarious monitor uses its own namespaced counter so a shared counter can't halve the chat budget; `wp_mcp_ai_before_chat_request` subscribers tolerate the legacy 2-arg emitter shape. Assistant edit screen: `collect_prebuilt_shortcut_tasks()` catches `\Throwable` per tool and coerces non-array filter results. Model catalog: `gpt-4o` → 128k, `gemini-2.0-flash` back in the video-capable list, `claude-sonnet-4-6`/`gpt-4o` restored to active, two Qwen Hugging Face entries added. Settings save reads the cache-suspension state *before* suspending so it restores correctly — WP 7.1 inline-async tick locks no longer jam after a settings save.
- 🧪 **PHPUnit Repair Campaign Continuation — PRs #6260, #6262–#6276.** Fourth wave (~9 test-only PRs): #6260 count-tokens params, #6262 continuation seam, #6263 chat SSE handler, #6264 deterministic cache expiration, #6268 coverage manifest + slug integrity, #6269 incorrect-usage notices (9 suites), #6273 admin registration drift, #6275 subtab sanitizer routing, #6276 14 Veo suites. The `mcp-ai-wpoos-test-suite` skill grew to **37** patterns; the plugin skill documents the third rate-limit counter.
- 📦 **Versioning** — bumped to **1.1.69** across all version-bearing files. Pro addon: 1.1.69. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.3** (unchanged). nvoos-content-graph-ai: **1.0.4** (unchanged). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). Docs Hub addon: **0.4.2** (unchanged). Tool count: ~303 base + ~1,263 Pro (~1,566 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **27**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **53**.

## 🆕 Latest Updates (v1.1.68 — September 2026)

### September 3, 2026 — Pro SPA v2 Shortcode, Hermes Fleet Extensions, Stale-Nonce Self-Heal, Docs Hub 0.4.2, Provider Defaults

- 🧩 **Pro SPA v2 Shortcode & Embedded Mode — PR #6256.** `[nvoos_pro_spa]` embeds the Pro SPA v2 chat surface on the front end (chat-first embedded mode: threads, drawers, tool shortcuts, OKF drawer; router-free, no admin routes), with per-instance `data-config` attributes and optional guest mode through the base guest-token machinery behind the "Allow Guest Access" setting. Proposal 033 + `addons/pro/README.md`.
- 🖥️ **Hermes Dashboard Fleet Extensions — PR #6249.** New `extensions/` tree: `nv-oos-fleet` fleet monitoring + control plane (Python plugin API, `sites.yaml` inventory, dashboard bundle), `backup-download`, `external-app-tab`, `mcp-tool-shortcuts` WebUI extensions, `install.sh`, and backend smoke tests. Plans in `docs/developer/integration/`.
- 🔄 **Stale REST Nonce Self-Heal — PR #6225.** `GET /mcp-ai/v1/session/nonce` mints a fresh session-bound nonce from the request's own auth cookie (nonce/capability-free by design, `no-cache`/`no-store`); the chat client retries failed `rest_cookie_invalid_nonce` 403s instead of showing "Cookie check failed" on full-page-cached or long-lived SPA pages. `docs/reference/api/rest-api.md` updated.
- 📚 **Docs Hub 0.4.2 — PRs #6246, #6253.** Local-page `.md` links rewrite to `#/slug` hash routes (anchors preserved); TOC slugs match github-slugger exactly (0 mismatches across ~63,000 headings); "Accept fix" computes targets relative to the source directory (case-preserving); `../` links validated with root-containment checks; broken-link entries carry `slug`/`source_type` + skip reasons; sync/rebuild reports "Atomic swap failed" when the staging cache is unwritable; dead 404 route removed; the core emoji loader is disabled on docs-browser pages (its MutationObserver was crashing React's DOM). Addon bump 0.4.1 → 0.4.2.
- 🔌 **Provider Enable Defaults — PR #6255.** Fresh installs now disable OpenAI/Anthropic/Gemini by default; provider dropdowns list only enabled + credentialed providers (`get_available_providers()`); the onboarding wizard auto-enables the provider whose key is entered; plugin + test-suite skills updated in-commit.
- 🐛 **Fixes — PRs #6226, #6232, #6233, #6239, #6242, #6243, #6244, #6250, #6258.** Calendar granted-scopes URL corruption (sanitizer keeps space-delimited scope lists; legacy `%20` grants normalized back to spaces); agent-identity resolver casts alias-table canonical IDs to int; token-budget catalog dedup (`gpt-4.1`, `gemini-2.5-flash`) + restored `wp_mcp_ai_model_tpm_limit` filter seam on the CCT → catalog fallback path; assistant untrash restores the pre-trash status via `wp_untrash_post_status` (published assistants stay published, CCT sync resumes); tool presets gain the missing OKF enrich/router, Google Calendar, Gmail/Drive, `composio_manage_accounts`, and `git_inspect`/`git_change` tools; `wp_mcp_ai_seed_task_templates` AJAX action registered; `fast-uri` override >=4.1.4 across root/Pro/SaaS npm trees; Tiptap pinned to 3.30.4 in canvas-toolkit + document-editor (mergeAttributes prototype pollution); jQuery UI sortable shim on post edit screens for third-party admin scripts.
- 🧪 **PHPUnit Repair Campaign Continuation — PRs #6223–#6257, #6259.** Third wave (~21 test-only PRs: #6224, #6227–#6231, #6234–#6241, #6245, #6247, #6248, #6251, #6252, #6254, #6257 + CI alpha-log refresh #6223) kept the suite aligned with current contracts; the production fixes it carried are listed above, plus the late-merged `wp_mcp_ai_attachment_segment_provider` filter for providers without a remote file API (#6259).
- 🧹 **Housekeeping.** Stale 1.1.67 build ZIPs removed from `build/`, `build/optional-components/`, `build/toolkit-addons/` (27 files).
- 📦 **Versioning** — bumped to **1.1.68** across all version-bearing files. Pro addon: 1.1.68. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.3** (unchanged). nvoos-content-graph-ai: **1.0.4** (unchanged). nvoos-content-graph-ai-platform: **2.0.0** (unchanged). Docs Hub addon: **0.4.2** (bumped this window). Tool count: ~303 base + ~1,262 Pro (~1,565 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **27**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **53**.

## 🆕 Latest Updates (v1.1.67 — September 2026)

### September 1–2, 2026 — Content Graph Platform Extraction v2.0.0, Ecosystem Port Wave D + D-UI, Google Workspace Read Tools

- 🧩 **Content Graph Platform Extraction — PR #6123.** `plugins/nvoos-content-graph-ai-platform/` ships standalone at **v2.0.0**: Waves A–C + Blueprints (namespace-bridged admin UI, skill/slash-command/agent bridges, platform dashboard + settings registry, harness `ToolRouterHarness`), a 74-skill bundled-skills pack (`data/bundled-skills/`), knowledge base (`data/knowledge-base/`), and `.github/workflows/phpunit-platform.yml`. Plan: `docs/project/plans/content-graph-platform-extraction-plan.md`; gaps: `MIGRATION-GAPS.md`.
- 🧱 **Ecosystem Port Wave D + D-UI — PR #6142.** The additive Base+Pro → Content Graph port lands the AI runtime in `nvoos-content-graph-ai` — chat runtime core (D1), providers beyond the 13 (D2), model management + analytics/token tracking (D3), security guards (D4), plus D-UI admin pages, chat blocks, Elementor widgets, guest tokens, agent memory, WP-CLI, chat compat route, MCP JSON-RPC controller — with `.github/workflows/phpunit-ai.yml`. **Content Graph AI bumps 1.0.3 → 1.0.4.** Tracker: `docs/project/ecosystem-port-tracker.md`.
- 📧 **Google Workspace Gmail + Drive Read Tools — PRs #6151, #6152.** Six new Pro tools (Gmail: `get_gmail_message`, `get_gmail_thread`, `list_gmail_connections`, `modify_gmail_message`; Drive: `get_drive_file`, `list_drive_connections`) with two new clients on the shared `includes/google/` foundation; `search_gmail`/`search_drive` updated.
- 🧪 **PHPUnit Repair Campaign Continuation — PRs #6114–#6208, #6209–#6222.** ~100 cluster PRs (second wave) kept the suite green; production fixes carried: assistant-directory REST empty-title guard (#6141), null-safe tool-error reporting (#6186), LLM sanitization delegated to the validator (#6187), `display`-metadata persistence (#6172), crawler job-contract hardening + `wp_mcp_ai_crawl4ai_auto_spawn_cron` filter (#6124, #6125), create-post taxonomy guards (#6115), toolkit-registry live-singleton resolution (#6117), slash-command handler fixes (#6122, #6140), URL-encoded external-API queries (#6156), Veo 5-second floor (#6180), token-tier caching guards (#6178), credential-resolver cache invalidation (#6119), cache-helper option-cache eviction (#6134), settings dashboard/registry fixes (#6139, #6144), orchestration-dashboard menu (#6160), tool-multiplier visibility (#6189), content-format filter seams (#6203), CRM workflow-preset canonical schema (#6208), Pro Composer class-ambiguity rename (#6198), PM assistant metabox (#6174), profession seeder envelope (#6197), destructive-ops gate canonical settings read (#6151), WhatsApp webhook signature rejection without an app secret (#6192), Graphify/Content Graph HTTP 304 cached-body re-serve (#6161), Veo async-job completion ordering (#6211), orchestration-renderer token-budget display (#6213), Shopify Sync client-availability filter seam (#6215), slash-command workflow metrics guard (#6216) + parallel/conditional block rendering (#6218), Paper Store array-field query matching (#6222), systemic PHPUnit 11 / WP 7.1 env-compat fixes (#6196). Test-suite skill refreshed to 26 patterns (#6154).
- 🧹 **Housekeeping.** Stale 1.1.66 build ZIPs + `.sha256` removed; re-created 1.1.65 ZIPs re-removed.
- 📦 **Versioning** — bumped to **1.1.67** across all version-bearing files. Pro addon: 1.1.67. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.3** (unchanged). nvoos-content-graph-ai: **1.0.4** (bumped this window). nvoos-content-graph-ai-platform: **2.0.0** (new standalone). Tool count: ~303 base + ~1,262 Pro (~1,565 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **27**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **53**.
...
### August 22–23, 2026 — Artifact Evolution Phases A–G, Pro Addons Page, Storage Worker Offload, Test-Suite Exit Traps

- ✅ **Artifact Evolution Phases A–G — PRs #5923, #5925 (52 files, +11,666/-75 lines).** The Continual Harness Evolver + Meta-Harness become a complete gated Darwinian self-improvement loop for skills, prompts, and roles (proposal 007). Phase A repairs the broken `evolve_harness` ↔ Evolver contract (`analyze_failures()`, component-scoped `evolve()` with enforced `dry_run`, constructor normalization) and adds opt-in evolved-prompt consumption (`WP_MCP_AI_Evolved_Prompt_Resolver`) + evolved-skill merge into the Skill Registry, Refiner output sanitization/PII scrubbing, and a $5/hr default budget. Phases B–G add artifact-scoped eval suites with failure-case replay and post-mutation verification, artifact populations with parent sampling, learning-log-aware mutators, a pre-commit admission gate (three critics — contamination control), holdout-gated deployment with shadow A/B and drift rollback, and a governor with human approval queue + lineage graphs + an artifact-governance metabox on the assistant screen. New **Settings → Orchestration Layer** switches bridge the evolution filters. Every layer is opt-in and defaults off. 16 new/expanded test suites. Reference: [`docs/project/proposals/007-artifact-evolution.md`](docs/project/proposals/007-artifact-evolution.md) + implementation plan; self-evolution governance section in [`docs/operations/compliance/EU_AI_ACT_2026.md`](docs/operations/compliance/EU_AI_ACT_2026.md).
- ✅ **Pro Addons Admin Page — PR #5924 (7 files, +1,086/-1 lines).** New registry-driven **NV oOS Pro Dashboard → Addons** page (`/wp-admin/admin.php?page=wp-mcp-ai-addons`, `WP_MCP_AI_Addons_Page`, 813 lines) installs and activates standalone addons whose ZIPs ship in `build/` — nonce + `install_plugins` + allowlist gated; non-WordPress components listed read-only. AJAX test matrix included.
- ✅ **Chat Storage Worker Offload (proposal 032 v2) — PR #5928 (30 files, +1,198/-210 lines).** Conversation saves ≥ `wp_mcp_ai_storage_worker_threshold` (default 10,000 chars) offload `JSON.stringify` to the browser `storage-worker.js` via `wpMcpAiStorageUtil`; small saves, unload flushes, and worker failures fall back to the synchronous main-thread write with quota-retry. Per-call threshold overrides, `storageWorkerUrl`/`storageWorkerThreshold` localized on every chat surface, and a filter kill switch (0 = off). LLM worker manager URL resolution fixed (localized workerUrl → pluginUrl fallback → explicit error). Reference: [`docs/project/proposals/032-chat-web-workers-wiring-implementation-plan.md`](docs/project/proposals/032-chat-web-workers-wiring-implementation-plan.md).
- ✅ **DeepSeek Empty-Schema Fix — PR #5926 (6 files, +253/-17 lines).** Schema normalization produced `"properties": []` which DeepSeek rejects ("[] is not of type 'object'"); empty maps now encode as `{}` at all three payload boundaries (REST, Tool Service, ChatOrchestrator), and legacy tools are wrapped before the first register attempt so the fail-loud registry log no longer fires for successfully wrapped classes.
- ✅ **OKF Skill-Knowledge Conformance — PR #5919 (91 SKILL.md files + 4 reference files).** `type: Skill` frontmatter added to every bundled SKILL.md lacking it; reference files restored for wp-plugin-dto, wp-plugin-presenter, wp-action-scheduler, and mcp-builder; `.agents/skills` sync now recursive + covers all project-owned design skills. The OKF Validate tab no longer reports missing concepts.
- ✅ **Test-Suite Exit Traps — PR #5929 (40 files, +2,121/-472 lines).** New `bin/sweep-tests.php` (parallel 6-worker sweep, 180s/file cap, report + custom report path), AJAX test contracts in `tests/bootstrap.php`, and 24+ test files aligned. Production fixes kept minimal: bare-exit guards in four admin handlers (`WP_MCP_AI_TESTS_RUNNING` seams), duplicate CRM workflow-rule class renamed, toolkit MCP scope check now passes null (non-OAuth) scope and resource URIs skip `esc_url_raw()` (`nvoos://` scheme preserved), SSE/Veo polling-limit filters added, export-provider `log_action()` public. Reference: [`docs/developer/testing-docs/TEST-SUITE-REMAINING-FIXES-PLAN.md`](docs/developer/testing-docs/TEST-SUITE-REMAINING-FIXES-PLAN.md).
- ✅ **Pro SPA v2 Slash-Command Composer — PR #5920 (8 files, +520/-109 lines).** `/chat`, `/new`, `/clear`, `/copy`, `/export` insert reliably from the SlashCommandsDrawer (also openable from the AgentPanel button); base `assets/js/slash-commands.js` fixes. Fix history: [`docs/history/2026/fixes/pro-spa-v2-slash-command-composer-fix.md`](docs/history/2026/fixes/pro-spa-v2-slash-command-composer-fix.md).
- 📦 **Versioning** — bumped to **1.1.63** across all version-bearing files. Pro addon: 1.1.63. Media Worker: **v3.2.0** (unchanged). nvoos-content-graph: **1.0.3** (unchanged). Tool count: ~303 base + ~1,249 Pro (~1,552 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **26**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **52**.
...
### August 20–21, 2026 — Restricted-User Flagging, Conversation Import, Tool Schema Normalization

- ✅ **Restricted-User Flagging & Unblocking — PR #5901 (26 files, +3,568/-20 lines).** New `WP_MCP_AI_Restriction_Registry` (`includes/class-wp-mcp-ai-restriction-registry.php`) converts ephemeral rate-limit and token-budget blocks into persistent, enumerable restriction records (user meta + `wp_mcp_ai_active_restrictions` index, daily cleanup cron, audit-logged). The OOS rate-limiter adapter fires `wp_mcp_ai_rate_limit_exceeded`; chat rate limits are filterable (`wp_mcp_ai_chat_rate_limit` / `wp_mcp_ai_chat_rate_limit_window`). Admin surfaces: Token Manager "Restricted Users" panel (Base) + Pro Command Center **Restrictions** tab; REST `GET /mcp-ai/v1/restrictions`, `GET|POST /mcp-ai/v1/users/{id}/restrictions`, `DELETE /mcp-ai/v1/users/{id}/restrictions/{type}`; AJAX lift actions; `wp mcp-ai restrictions list|lift|add`; IETF rate-limit headers; new `restriction_registry_on` posture signal. Full reference: [`docs/features/security/user-restrictions.md`](docs/features/security/user-restrictions.md).
- ✅ **Conversation Import to Transcript CCT — PR #5898 (38 files, +8,791/-4 lines).** New `includes/conversation-import/` subsystem imports ChatGPT `conversations.json` (incl. ZIP), Google Takeout Gemini activity, Claude `conversations.jsonl`, ShareGPT, and OpenAI fine-tuning JSONL into the JetEngine `ai_chat_transcripts` CCT — one row per conversation — with format detection, tree/branch linearization, idempotent dedupe, dry-run, batching, and checkpoint resume. Four new tools (`conversation_import_detect|run|status|delete`), an admin upload/preview page with progress reporting, `wp mcp-ai conversation-import detect|import|status|delete`, GDPR exporter/eraser + retention coverage, media sideloading, and opt-in memory mining via `mine_agent_memory`. Five PHPUnit suites. Guide: [`docs/user-guides/conversation-import.md`](docs/user-guides/conversation-import.md).
- ✅ **Tool Schema Normalization — PR #5903 (5 files, +162/-14 lines).** Tool argument schemas are normalized before being embedded in provider payloads — `WP_MCP_AI_DeepSeek_Client`, REST `/tools` schema output, `WP_MCP_AI_Tool_Service`, and the OOS `ChatOrchestrator` — preventing provider streaming failures from non-compliant schemas; the registry now logs tool registrations skipped for a missing tool contract.
- ✅ **WP_Error Fatal Fixes — PR #5905 (8 files, +216/-50 lines).** Memory tools (`mine_agent_memory`, `retrieve_agent_memory`, `wake_up_context`), the request guard, and the REST chat-memory controller now handle `WP_Error` returns before array/object access.
- ✅ **nvoos-content-graph v1.0.3 wp.org Resubmission — PRs #5897, #5899, #5904.** WPCS fixes for the renamed plugins, wp.org review fixes, readme/README polish, `.wordpress-org` screenshot assets, and the standalone build ZIP.
- 📦 **Versioning** — bumped to **1.1.60** across all version-bearing files. Pro addon: 1.1.60. Media Worker: **v3.2.0** (unchanged). Tool count: ~300 base + ~1,247 Pro (~1,547 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **26**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **52**.
...
### August 18, 2026 — Composio Connect, OOS Runtime Consolidation, Content Graph Rename

- ✅ **Composio Connect Integration — PR #5889 (29 files, +5,318/-2 lines).** New `addons/pro/includes/composio/` subsystem bridges WordPress to the Composio MCP gateway: OAuth connection flow with state nonce (`WP_MCP_AI_Composio_Auth_Handler`), API client (`WP_MCP_AI_Composio_Client`), trigger bridge (`WP_MCP_AI_Composio_Trigger_Bridge`), and signed webhook controller (`WP_MCP_AI_Composio_Webhook_Controller`). Six new beta tools — `composio_list_tools`, `composio_get_tool_schema`, `composio_list_connected_accounts`, `composio_create_connect_link`, `composio_execute_tool`, `composio_manage_triggers` — plus remote-sites admin and metabox panels (connected-apps badge). 4 test classes (+887 lines). Full details: [`docs/composio-connect.md`](docs/composio-connect.md), [`docs/toolkits/composio-connect.md`](docs/toolkits/composio-connect.md).
- ✅ **OOS Runtime Consolidation Phases 0–5.8 — PR #5881 (8 commits).** The OOS engine reaches parity with the legacy path: Phase 0–2 parity foundations (`includes/bootstrap/oos-bridge.php`, tool-surface + security-gate contracts), Phase 3 event-sourced session log (`SessionLog`/`SessionEvent`/`SessionTelemetry` + observer), Phase 4 shadow mode (`includes/oos/` parallel parity runs serving the legacy result; `wp mcp-ai oos parity` CLI) + canary routing (per-assistant `_wp_mcp_ai_engine` opt-in), Phase 5 scoped tools (`ToolScope`) + compaction seam (`CompactionProvider`) + Pro composition & child binding (`addons/pro/includes/composition/`), and Phase 5.8 telemetry single-path. Full details: [`docs/project/proposals/029-oos-orchestration-runtime-consolidation-implementation-plan.md`](docs/project/proposals/029-oos-orchestration-runtime-consolidation-implementation-plan.md).
- ✅ **Graphify Standalone Plugins → Content Graph — PR #5890.** `plugins/nvoos-graphify` → `nvoos-content-graph` (v1.0.2), `nvoos-graphify-ai` → `nvoos-content-graph-ai` (v1.0.2), `nvoos-graphify-ai-platform` → `nvoos-content-graph-ai-platform` (v1.0.2), with matching build/sync workflow renames and `bin/build-*-content-graph*.sh` scripts. The `addons/graphify/` knowledge-graph addon is unchanged.
- ✅ **Security Center wp.apiRequest Fix — PR #5887 (1 file, +8 lines).** The security tab's inline scripts (posture refresh, IP tests, snapshot restore, compliance export, self-test) call `wp.apiRequest()` but `wp-api` was never enqueued there. Now enqueued when `?tab=security` is active. See [`docs/history/2026/fixes/security-center-wp-api-request-fix.md`](docs/history/2026/fixes/security-center-wp-api-request-fix.md).
- ✅ **deepmerge-ts CVE-2026-40345 — PR #5888 (4 files, +30/-6 lines).** Override pinned in `addons/media-worker/package.json` and `addons/pro/package.json`.
- 📦 **Versioning** — bumped to **1.1.58** across all version-bearing files. Pro addon: 1.1.58. Media Worker: **v3.0.0** (unchanged). Tool count: ~265 base + ~1,243 Pro (~1,508 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative). Provider count: **15**. Addon count: **26**. Bundled skills: **74** base + **41** Pro. Coding-time agent skills: **51**.
...
### August 14, 2026 — Media Worker Multi-Tenancy & Scale (v2.4.0 → v3.0.0), Worker Routing, Hermes Tooling

- ✅ **Media Worker Multi-Tenant Shared Worker Mode — PR #5866 (21 files, +1,349/-304 lines).** `SITE_TOKENS` JSON map switches the worker into per-site isolation: every `/api/*` request must carry a token belonging to a known site (fail-closed with `AUTH_MODE=strict`), and temp files, job queues, and rate limits are namespaced per site slug via the new `src/utils/site-paths.js`. Per-site rate-limit overrides (`RATE_LIMIT_*_<SITE>`), `SITE_TOKENS_PREVIOUS` rotation overlap, audit logs tagged `site=<slug>`. Specs: [`026-media-worker-multi-tenancy-sidecar-proposal.md`](docs/project/proposals/026-media-worker-multi-tenancy-sidecar-proposal.md) and [`027-media-worker-multi-tenancy-phase2-spec.md`](docs/project/proposals/027-media-worker-multi-tenancy-phase2-spec.md). (PR #5866)
- ✅ **Media Worker Phase 2 — Per-Site Provider Keys & Observability — PR #5868 (8 files, +366/-50 lines).** `SITE_PROVIDER_KEYS` JSON map resolves per-tenant AI provider credentials before falling back to the shared pool (`PROVIDER_KEYS_STRICT=1` disables the fallback); `tenants.usage` per-site provider usage counters; grouped temp TTLs (`TEMP_TTL_UPLOAD/VIDEO/BROWSER/DOC`) with storage stats; cluster-mode boot warnings and a k6 load-test kit under [`addons/media-worker/bin/load-test/`](addons/media-worker/bin/load-test/README.md). (PR #5868)
- ✅ **Media Worker Phase 3 — Scale Without Breaking Changes (14 files, +602/-63 lines).** Worker bumped to **v3.0.0**. W1: per-blog worker tokens on WordPress multisite (additive option chain in the sidecar trait). W2: new `WP_MCP_AI_Media_Worker_Usage_Reporter` (`includes/class-wp-mcp-ai-media-worker-usage-reporter.php`) — daily cron snapshots `tenants.usage` into an option and fires `wp_mcp_ai_media_worker_usage_updated` (opt-in, off by default). W3: `SITE_TOKEN_<SLUG>` / `SITE_PROVIDER_KEYS_<SLUG>` env-var merges over the JSON maps (env wins). W4: opt-in Redis rate-limit store (`RATE_LIMIT_REDIS=1` + `rate-limit-redis` optional dep) for PM2 cluster mode. W5: `PROVIDER_KEYS_FILE` hot-reload with atomic replace and malformed-update rejection. W6 revised: permissive-mode boot notice only — the strict-path default flip is deferred (open Q5). W7: manual Velocity deploy workflow. 59 unit tests passing. Plan: [`028-media-worker-phase3-proposal.md`](docs/project/proposals/028-media-worker-phase3-proposal.md).
...
2. You now have the **complete version** with all ~1,586 tools (~307 base + ~1,279 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)

**What you get from the repository clone:**

- ✅ The full codebase — all ~1,586 built-in tools ready to use (~307 base + ~1,279 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
...
- **[Architecture Overview](docs/developer/architecture/ARCHITECTURE.md)** - System architecture (15 providers, ~1,586 tool classes, 36 REST controllers)
...
- **[Tool Reference](docs/reference/tools/tool-reference.md)** - All ~1,586 tools documented (~307 base + ~1,279 Pro; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
