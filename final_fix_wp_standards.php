<?php
/**
 * Final fix: Remove extra spaces after => to match standard "key => value" format
 */

function fix_extra_spaces_after_arrow( $file_path ) {
    ...
    // Fix extra spaces after => - should be single space
    // Pattern: => followed by multiple spaces
...
foreach ( $files as $file ) {
    if ( fix_extra_spaces_after_arrow( $file ) ) {
        $fixed_count++;
    }
}

echo "Total files cleaned: $fixed_count\n";
