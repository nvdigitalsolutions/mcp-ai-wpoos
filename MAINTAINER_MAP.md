...
wp_mcp_ai_pro_init()   (at plugins_loaded priority 15)
  ...
  ├─ Register Pro tool classes (~1,256 Pro tools via pro.php class-loader)
...
|---|---|---|
| `WP_MCP_AI_BASE_VERSION` | `true` | `true` = base-only (~303 tool classes); `false` = full mode (~1,559 tools: ~303 base + ~1,256 Pro) |
...
│   │
│   ├─ tools/                  ← Tool classes (~1,559 total implementations registered through the singleton registry; live count authoritative)
...
| **Tools** | ~303 core tools | +~1,256 Pro tools = **~1,559 total** |
