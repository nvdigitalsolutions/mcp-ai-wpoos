...
|---|---|---|
| `WP_MCP_AI_BASE_VERSION` | `true` | `true` = base-only (~303 tool classes); `false` = full mode (~1,568 tools: ~303 base + ~1,265 Pro) |
...
│   │
│   ├─ tools/                  ← Tool classes (~1,568 total implementations registered through the singleton registry; live count authoritative)
...
| **Tools** | ~303 core tools | +~1,265 Pro tools = **~1,568 total** |
