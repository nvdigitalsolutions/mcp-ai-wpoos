...
> Last reviewed: **September 27, 2026** · Version: **2.41**
...
├── class-wp-mcp-ai-tool-registry.php   ← Tool registry singleton (~1,634 tools total; live count is authoritative)
...
- **Constants:** `WP_MCP_AI_BASE_VERSION = true` (~347 base tool classes) or `false` (~1,634 total; live count via `WP_MCP_AI_Tool_Registry::get_tools()` is authoritative)
...
- **Vision Analysis toolkit (Pro)** (v1.1.69, PR #6267) — new `analyze_image_objects` tool (`addons/pro/includes/tools/vision-analysis/`) counts objects per category via HF OWLv2/Ollama detection, VLM counting, or hybrid label normalization; `annotate=true` returns a GD box-annotated attachment. Gated by `enable_vision_analysis_toolkit` (off by default) with a NV oOS → Vision Analysis settings page; remote image URLs pass the SSRF URL guard. Pro tool count +1 → ~1,263.
- **tagDiv admin compat** (v1.1.69, PRs #6266/#6278) — the four SiteKit tools crashed the assistant tools metabox via the non-existent `CAPABILITY_CAN_USE_IF_ADMIN` constant (string flag arrays now, AdSense includes `financial-impact`); the sortable shim prints a bundled jQuery UI 1.14.2 copy whenever `td_wp_admin` is enqueued (print-queue detection — a registered+enqueued core handle can still fail to execute) and at both `admin_head`/`wp_print_footer_scripts` when the core handle won't load; the media script chain is forced as direct head tags from core-registered handles; tools metabox CSS enqueued on `admin_enqueue_scripts`.
- **DeepSeek empty-schema fix** (v1.1.69, PR #6272) — argument-less tools must encode `properties: {}` (never `[]`; DeepSeek 400s otherwise) across 29 files; `LegacyToolAdapter` preserves object maps + upgrades empty arrays; the AI Tool Builder scaffold emits `new stdClass()`.
- **ZipSlip guard revival** (v1.1.69, PR #6270) — `ZipArchive::$num_files` doesn't exist on PHP 8.x, so the guard loops never ran (dead code); `count( $zip )` now drives the OKF bundle manager + four Pro admin page loops.
...
- **Shopify UCP mode-aware tool routing** (v1.1.81, PR #6634) — `storefront_catalog`/`global_catalog` connections drive live UCP queries (`search_catalog`/`lookup_catalog`/`get_product`) in `shopify_products`; UCP results are never cached (no transients/CCT) and marked `live: true`; admin-only tools refuse catalog connections (`wp_mcp_ai_shopify_catalog_mode_admin_only`); `shopify_catalog` is the unified live catalog tool (`get_product`, batch `ids` + `not_found`, `lookup_by_variant`); buyer `context` allowlisted/sanitized, opaque `cursor` passthrough, clamps 250/50/10; `remote_shopify_connection` validates UCP modes via the MCP `tools/list` handshake. **Image cards** (PR #6638) — `images[]` + chat-rendered markdown card (10-card cap) on every product-returning path via a shared `WP_MCP_AI_Shopify_Product_Normalizers` trait. CG Pro ports byte-identical.
- **FlowHub Remote Sites resolution + proxy** (v1.1.81, PRs #6635/#6637) — a shared resolver chain (explicit `connection_id` → toolkit settings → configured sync connections → first enabled FlowHub connection) ends the "credentials are not configured" failure; live requests honor the connection proxy via `http_api_curl`; `record_health_metric()` is public for connection-bound base requests. New base helper `WP_MCP_AI_FlowHub_Connection_Helper`.
- **JobNavigator CRM adoption + Gmail reply poller** (v1.1.81, PRs #6636/#6640/#6641) — 5 new Pro tools (`bulk_move_deal_stages`, `create_tracked_link`, `record_crm_reply`, `get_crm_handover`, `get_pipeline_digest`); deal stage history (`stage_changed_at`, undoable no-op-preserving moves); `create_lead` email dedup + canonical company linking + source snapshots; email/sentiment reply signals + settings-gated auto-disqualify; won-deal lead release cascade. `WP_MCP_AI_CRM_Gmail_Reply_Poller` (cron) classifies unread Gmail replies with sentiment + optional positive-sentiment stage advancement; `record_crm_reply::apply()` is the cron-safe core. Digest scheduling recipe `addons/pro/config/pipeline-digest-recipe.json` (Workflow Builder + Pro Schedule Manager). CG Pro ports byte-identical (#6640).
- **OpenTerminal financial toolkit resilience** (v1.1.81, PR #6639) — 8 new Pro tools (`market_screener`, `macro_data_fetcher`, `economic_calendar_fetcher`, `earnings_calendar_fetcher`, `options_chain_fetcher`, `crypto_market_data`, `portfolio_transaction_log`, `price_alerts` daily cron); provider fallback chains + stale-while-revalidate caching in the yfinance service; keyless microservice auth; PHP technical indicators; news de-dup; portfolio transaction CPT with P&L. CG Pro port byte-identical. **Tool counts +13 Pro → ~306 base + ~1,279 Pro, ~1,585 total**.
- **Multi-recipient result-delivery email** (v1.1.81, PR #6643) — comma/semicolon/whitespace lists normalized on save and in `sanitize_result_delivery()` via `sanitize_email_recipients()` (per-address sanitize, dedupe, empty-list → `missing_email_recipient`); `send_email()` fans out via Nodemailer + `wp_mail()`; legacy `notify_email` stays single-address.
- **WordPress Playground demos** (v1.1.82, PRs #6662/#6663/#6666/#6670/#6673/#6674) — two one-click blueprint demos: Content Graph "Project Asteria" (`plugins/nvoos-content-graph/blueprints/`, deterministic build via the public `nvoos_content_graph/initial_build` hook, 3 true degree-0 orphans) and NV oOS Complete × local Ollama (root `blueprints/ollama-demo.*`; the browser worker's `localhost:11434` **is the user's machine**). `bin/generate-ollama-blueprint.php` auto-discovers the newest Complete bundle ZIP (`--bundle-url=` override); `build-assets.yml` regenerates the blueprint with every build. New skill `mcp-ai-wpoos-playground-demos` (59th) codifies the crash modes + CORS/PCP rules.
- **Pro SPA `cron_monitor` flag + model-store seeding** (v1.1.82, PRs #6665/#6672) — `[nvoos_pro_spa cron_monitor="0"]` flows through `WP_MCP_AI_Pro_SPA_Config::build()` into `config.cronMonitor` and no-ops both the SSE cron-status stream and the REST poll (constrained-host kill switch; default `true`). `useBootstrap.ts` seeds the model store from the preloaded assistant config — the hardcoded `{provider:'openai', model:'gpt-4o'}` override that broke Ollama-wired assistants in embedded mode is gone.
- **Token-tracking table hardening** (v1.1.82, PR #6669) — verify-then-version (schema-version option advances only when the table verifiably exists; existing installs self-heal), hourly retry-backoff transient (`wp_mcp_ai_token_tracking_table_retry`), quiet failure (suppressed probes + single `error_log` line), graceful reads (empty results when absent). Schema unchanged; ported 1:1 to `plugins/nvoos-content-graph-ai/src/Analytics/TokenTrackingDatabase.php`.
- **Result-delivery duplicate-content fix** (v1.1.82, PR #6661) — `delivery_safe_data()` strips the duplicated `response` copy + `assistant_id`/`is_agentic` before rendering (email/chat `full` + Paper Store/WordPress markdown); chat `summary`/SMS templates gain the `response_starts_with_summary()` dedupe; no empty `## Details` section.
- **`[ollama_status]` banner fix + portability guards** (v1.1.82, PRs #6668/#6645) — shortcodes render markup only: the checker JS is footer-enqueued (`wp_add_inline_script`) because inline scripts inside `the_content` get texturized (`&&` → `&#038;&#038;`). The four assistant-portability tools joined the `assistant_management` preset; 9 WhatsApp AJAX tests + regenerated manifests close the CI guard gap. **Docs Hub 0.4.7** (v1.1.82, #6659/#6667) — cron on `init`, deactivation cleanup, 0 blocking PCP errors. **Counts unchanged:** ~306 base + ~1,279 Pro (~1,585 total); skills 58 → 59.
- **Tool Description Engineering — model-facing usage guidance** (v1.1.83, PRs #6686/#6695/#6687–#6723) — `WP_MCP_AI_Tool_Usage_Guidance_Interface` declares `when_to_use`/`when_not_to_use`/`related_tools`/`notes`; `WP_MCP_AI_Tool_Registry::get_model_facing_description()` assembles the `[Usage: …]` suffix on the model-facing payload (REST chat path, Tool Service `/tools`, `list_mcp_tools`). Legacy-format classes opt in via a `get_usage_guidance()` method or `usage_guidance` definition key forwarded by `WP_MCP_AI_Legacy_Tool_Wrapper` (method wins). The Phase 2 sweep completes the full base+pro tree (~1,487 tools; 1,584/1,584 files clean) and the `WPMCPAI.Tools.ToolDescriptionGuidance` sniff is enforced at **severity 5** in `phpcs.xml.dist` — every new tool class without guidance surfaces on PRs (warnings; CI error thresholds unaffected). Opt-in **adaptive tool cap** (`WP_MCP_AI_Tool_Payload_Advisor`: 40 ≤128K / 64 ≤256K / 100 above; site option `wp_mcp_ai_adaptive_tool_cap` + per-assistant three-state override; default off) and **lazy schema loading** (`tool_slug` + `include_schemas` on `list_mcp_tools`).
- **Pro bootstrap incomplete-install guard** (v1.1.83, PR #6677) — `mcp-ai-wpoos-pro.php` now `file_exists`-checks the `class-wp-mcp-ai-pro-module-registry.php` require and degrades to `wp_mcp_ai_pro_incomplete_install_notice` + a WP_DEBUG line (mirroring the vendor-incomplete guard) — partial deploys keep the site running on the base plugin. **Telegram auto-chunking** — `send_telegram_message` splits >4,096-char messages at paragraph → line → hard boundaries (hard splits drop `parse_mode`; `wp_mcp_ai_telegram_chunk_error` carries the failed chunk index/count; `chunk=false` restores legacy single-send). **Gmail `connection_id="settings"`** resolves to the settings-based fallback across the four Pro Gmail tools, the Drive client, and base `search_gmail`. **Skill/OKF self-correction** — `load_skill` not-assigned appends `Assigned skills: …`; `okf_bundle_not_found` appends `Available bundles: …` (new `list_bundle_names()`); CG Pro/CG AI mirrors byte-identical.
- **Upwork search + workflow delivery series** (v1.1.83, PRs #6678–#6680/#6682/#6684) — `search_upwork_jobs` gains the web_search-mode + API credential gates (byte-identical with `score_upwork_job`/`draft_upwork_proposal`), drops category landing pages, extracts `job_type`/`budget`/`published` from SERP snippets, always runs the broad second pass under `min(limit, 5)` jobs, and accepts `sort` (`recency`|`best_match`) + `location` args wired to settings `default_location`/`default_sort`. Workflow deliveries ship the **full 50-item result set** (`wp_mcp_ai_pro_workflow_response_item_cap` filter) with per-item URLs + budget/contract/recency; `envelope_data_to_text()`/`envelope_data_to_markdown()` render `steps` as a compact execution log and skip null/empty values; both markdown converters fold indented continuation lines into a single `<ol>` (digests render 1–10, not every item as "1.").
- **Playground Ollama demo permalink fix + npx primary** (v1.1.83, PR #6683) — fresh Playground installs default to plain permalinks, so `/ollama-test-lab/` 404'd; the seed now switches to `/%postname%/` + flushes rewrite rules. The local `npx -y @wp-playground/cli server` run is the documented primary test path; `bin/capture-real-page.sh` waits for the lazy seed via the REST index and follows the auto-login 302 with a cookie jar (`/verify-out/` gitignored).
- **npm advisories + regulatory envelope** (v1.1.83, PRs #6681/#6689) — adm-zip 0.6.1 (CVE-2026-77301 + symlink-overwrite fix), js-yaml 4.3.2 (GHSA-2883-xcg3-v3hh), colord 2.10.0 (GHSA-2wm5-q62r-hmrv) across root/saas-controller/pro/pro-spa/media-worker locks (hand-edited, `--package-lock-only --dry-run` clean); five regulatory tools (`renew_registration`, `update_reg_document`, `validate_document_checklist`, `validate_inci_ingredients`, `validate_reg_product`) migrate to the canonical `WP_Error` envelope. **Counts (pre-Jev):** ~306 base + ~1,279 Pro (~1,585 total); skills stay 59 — the post-window merge of #6728 adds +1 base (see below).
- **TypeSafe Jev decision provider + Pro integrations** (v1.1.83 post, PR #6728) — Jev joins as a first-class *decision* provider deliberately separate from chat: `Interface_WP_MCP_AI_Decision_Client` + `WP_MCP_AI_Typesafe_Client` (typed choice/score/noul over state; 429/`retry-after`; versioned-model logging; `test_connection()`), OpenRouter Decisions bridge (`create_decision()`, filterable endpoint, actionable 404), and the **new base tool `typesafe_decide`** (canonical envelope + two-gate sanitisation, adversarial-state caveat, `manage_options`-gated; `ai_ml` preset + coverage manifest, #6743). Pro: fail-open `WP_MCP_AI_Pro_Jev_Classifier` (`decide()`/`classify_prompt()`/`filter_sources_by_relevance()`, kill-switch `wp_mcp_ai_jev_classifier_enabled`), opt-in dispatcher `jev_routing` (REST `compare-models` passthrough), opt-in `enable_jev_research_filter` on `research_eca`/`generate_research_report`; fixes the pre-existing dispatcher `chat_completion()` bug. **Tool counts +1 base → ~307 base + ~1,279 Pro, ~1,586 total**. Model catalog → v2026.09.21 (+3 Jev decision entries).
- **"The Assistant Builder" meta-assistant** (v1.1.83 post, PR #6727) — a pre-configured meta-assistant (7-phase build workflow, 10-component prompt framework, 20 base slugs + Pro `export_assistant_blueprint`) appended to `get_default_assistants()` (roster 6 → 7); idempotent `install_assistant_builder_assistant()` + one-shot `admin_init` backfill (`wp_mcp_ai_assistant_builder_backfilled`).
- **P3 ID-handoff data contracts** (v1.1.83 post, PRs #6729–#6738, closure #6739) — every ID-bearing tool family declares `produces`/`consumes` handoffs (D1: exact envelope key names) with the manifest as single source of truth (`tests/fixtures/tool-contract-manifest.php`) and permanent L1 honesty + L2 round-trip suites. Landed: `post_id` (additive key on create/save/get_post), `cron_job_id`, `term_id`, `assistant_id`, `vector_store_id` (promoted from `data.id`), `batch_id`, `schedule_id` (6 Pro tools, manifest `scope=pro`), `item_id` (`toolkit_cpt`), `record_id` (medical records, both trees byte-identical), `plan_id` + `post_id`/`assistant_id` expansions, `event_id`/`snippet_id`, `session_id`/`member_id`/`room_id`. In-wave fixes: `format_code_prettier` envelopes → canonical `WP_Error` (#6737); webchat `log_activity()` → `log_event( 'activity', … )` (#6738).
- **Usage monitor save fix + webchat close-out** (v1.1.83 post, PRs #6726/#6740) — `handle_save_settings()` now applies the `wp_mcp_ai_admin_settings_sanitize` bridge filter with raw input pre-sanitize (Usage Monitor sub-tab persists again; unrelated saves unchanged); `get_webchat_messages` queries rebuilt with explicit placeholders (phpcs warnings gone); the last three webchat tools + CG Pro member mirrors gain usage guidance; the CG interface copy gains the guidance interface (standalone-resolution fatal fix). Residual CG mirror drift (~957 files) → issue #6741.
- **MCP Apps as a Remote Sites connection type + reference mode** (v1.1.86, PR #6761, Proposal 041) — new `mcp_server` Remote Sites connection type: central credentials stored AES-256-CBC-encrypted (incl. the `mcp_oauth` blob) and resolved **decrypt-on-use** (never written back to post meta); real JSON-RPC handshake Test Connection + tool-discovery snapshots + restricted-host enforcement + activity logging; auth mapping `basic_auth`/`application_password` → MCP `basic` (Elementor application passwords), `custom_header` → `header`, `bearer`, `oauth`. Assistant MCP App entries carry a `connection_ref` (non-secret pointer) — missing refs skip with error-status snapshots and imported bundles with missing refs auto-disable (`wp_mcp_ai_mcp_apps_validate_imported_refs`). Portability: `token`/`oauth_data` redacted on export + stripped on import (opt-out filters); stored credentials survive overwrite imports. New coding-time skill `design-elementor-mcp-connection` (59 → 60). Deferred per proposal 041: server-card discovery, admin OAuth flow, adopt-into-Remote-Sites.
- **Higgsfield video/image provider** (v1.1.86, PR #6772, Proposal 042) — four new base tools: `generate_higgsfield_video` (Cinema Studio 4.0, Seedance 2.5/2.0, Wan 3.0, Kling 3.0 behind one `model` param; per-model payload mapping/clamping; image/video/audio references; cinematic controls), `generate_higgsfield_image` (SOUL V2 + SOUL Cinema, batch 1|4, `style_id` gating), `check_higgsfield_request`, `cancel_higgsfield_request`. Shared `WP_MCP_AI_Higgsfield_Client`: two-part `Key ID:SECRET` auth, submit/status/cancel lifecycle, backoff+jitter polling 2s→10s, immediate download (7-day retention), settings → env → constants credential chain; 480 s async-executor timeout override; `check_video_status` resolves `async_*` job IDs; `lib/core` dual-layer wrappers via `oos-bridge` (core registry — not counted). Dotted model IDs sanitized with `sanitize_text_field` **not** `sanitize_key`. **Tool counts +4 base → ~312 base + ~1,282 Pro, ~1,594 total.**
- **`get_system_logs` filters** (v1.1.86, PR #6768) — optional AND-combined `since` (relative "2h"/"30m"/"3d" or absolute ISO 8601 / `Y-m-d H:i:s` UTC; bare number = minutes; malformed → `wp_mcp_ai_invalid_since`), `levels` (critical/error/warning/notice/deprecated), and `search` (case-insensitive, 200-char, mb-safe) over the structured NV oOS buffers + file logs; filters summary (resolved UTC cutoff) + per-file `filtered_out`; `parse_since()` is the public static canonical parser shared by the Symfony-validated variant; CG AI ports base-identical.
- **Action-items template + smart excerpts** (v1.1.86, PR #6773) — scheduled digests stop blind-trimming the first 80 words: new `action_items` delivery template (email + chat) sends only the actionable section ("Action Items"/"Needs your attention"/"Next steps") with graceful fallbacks; `summary` excerpts prefer the response's own distillation section ("Summary"/"TL;DR"/"Key points"/"Results"), then the action block, then centroid-ranked sentences (MEAD-style centrality × positional decay), then a lead trim.
- **Tool costs in final response labels** (v1.1.86, PR #6771) — `build_tool_cost_envelope()` emits the top-level `toolResult.cost` shape (`cost_usd`/`is_estimated`/`provider`/`model`) on the non-streaming agentic loop + SSE streaming (live `tool_result` events); the client's shared `aggregateToolUsageBadgeData()` aggregates it with nested/legacy-shape fallbacks and tool bubbles render the same badges.
- **Environment status fixes** (v1.1.86, PR #6767) — `build_warnings()` now unwraps the `environment`-keyed summary before reading (`summarise_assistants()` wrapped its payload; the "no assistants published" warning always fired and the default-assistant branch could never fire); DeepSeek joined the provider warning maps; new `plugin.default_provider_model` resolves the effective per-provider model (not the OpenAI-only `default_model`); the default assistant entry carries its provider/model meta.
- **mcp-wordpress parity suite — 30 new base tools** (v1.1.87, PR #6777) — the missing surface from the MIT upstream lands as native PHP (base plugin, PHP 7.4, WP core APIs only): comment CRUD (`list_comments`/`get_comment`/`create_comment`/`update_comment`/`delete_comment`), user CRUD (`list_users`/`create_user`/`update_user`/`delete_user`), content/media/terms (`get_post_revisions`, `get_term`, `delete_term`, `get_media`, `upload_media` URL/path/base64, `update_media`, `delete_media`), site settings + app passwords (`get_site_settings`/`update_site_settings` allowlist-only, `list_application_passwords`, `create_application_password` log-masked, `delete_application_password`), and a 9-tool SEO toolkit (`seo_analyze_content` … `seo_keyword_research`; the two network tools reuse the Brave key). Capability checks, multisite guards, chat-client restriction trait on sensitive write tools; new `comment_id`/`user_id`/`attachment_id` data-contract families (manifest + L1/L2). Gap matrix: `docs/developer/mcp-wordpress-tool-parity.md`.
- **Non-LLM image identification ladder** (v1.1.87, PR #6780 + #6785, Proposal 043) — five new base tools: `identify_image` (orchestrates metadata → pure-PHP dHash media lookup → classic Cloud Vision detection → deterministic layout description → optional reverse-image web search; confidence + escalation hint, never calls a vision LLM), `get_image_metadata`, `find_similar_media` (dHash + post-meta cache + daily backfill cron), `detect_image_content` (Cloud Vision normalized envelope), `describe_image_layout` (bounding boxes → text-mode Set-of-Mark). Two new Pro tools: `ocr_image_classic` (tesseract-only via the shared OCR service) + `search_similar_images` (Bing Visual Search / SerpApi Google Lens; new `va_*` Vision Analysis settings). Shared `WP_MCP_AI_Cloud_Vision_Client` extracted from `vision_object_localization` (behavior-neutral). Every external rung key-gated and fails closed (skipped, never an HTTP request without keys; SSRF guard). The five base tools join the Media Generation preset (#6785 — fixes the presets coverage test). **Tool counts +5 base +2 Pro.**
- **Outbound appointment booking toolkit** (v1.1.87, PR #6786, Proposal 044) — Pro toolkit gated by `enable_outbound_booking_toolkit`: ICP list building (CSV → `mcp_ai_lead`, ICP-scored, sequence enrollment), hourly cron engine over `mcp_ai_sequence` steps (send windows / daily caps / consent attestation), email + LinkedIn/Instagram DMs via approval board or webhook hand-off, `mcp_ai_oa_angle` angle bank (champion/challenger A/B + weekly promotion), `mcp_ai_oa_booking` links + `[nvoos_oa_booking]` shortcode + rate-limited honeypotted consent-gated public booking REST endpoint → `mcp_appointment`; pipeline dashboard + Slack + daily digest. **Three new Pro tools**: `outbound_get_pipeline_stats`, `outbound_import_leads`, `outbound_manage_angle`. **Tool counts +3 Pro.**
- **mcp-wordpress gateway addon** (v1.1.87, PR #6778) — new `addons/mcp-wordpress-gateway/` (Node, Streamable HTTP MCP server on pinned docdyhr/mcp-wordpress internals, no stdio/child processes): Express `/healthz` + `/mcp` with timing-safe `X-MCP-Token` (≥32 chars, rotation overlap) + 1 MB body cap; `MCP_TOOLS_ALLOW`/`MCP_TOOLS_DENY` policy at tool registration (deny wins; filtered tools absent from tools/list + rejected on tools/call); subtree-split deploy to the `nvoos-mcp-wordpress` mirror for Cloudways Velocity; connects via the Remote Sites → MCP Server type.
- **Session-budget warnings + bundled Elementor skills** (v1.1.87, PR #6776) — blocked-session messages now report session state + the real reset path (Restricted Users / `wp mcp-ai restrictions lift`); the dormant 75% `wp_mcp_ai_session_limit_approaching` hook is wired (`handle_session_limit_approaching()` logs once + flags; `inject_session_budget_warning()` = one-shot system notice on the next turn); new `wp_mcp_ai_chat_messages` filter on both chat paths; Token Manager reset semantics + the 20% safety buffer documented. The two untracked Elementor bundled skills (`design-elementor-mcp-connection`, `design-elementor-template-kits`) are committed — 75 tracked base bundled skills. **Deferred to the ecosystem-port track:** the CG AI `ToolTokenLimits.php` mirror (OI-9).
- **Restriction admin notice fixed** (v1.1.87, PR #6779) — one queue entry per user (re-flag refreshes), render-time sweep (expired windows + unrestricted users + legacy dedupe + option prune, renders nothing when empty), `lift()` prunes on full lift, new `count_active_users()` (distinct users) also fixes the Pro command-center banner.
- **Upwork search + in-place workflow editing** (v1.1.87, PR #6784) — `update_pro_schedule` gains `workflow_steps` (tool_slug/arguments/label, replace semantics; invalid_schedule_type / invalid_workflow_steps guards); `search_upwork_jobs` gains `exclude_keywords` (Upwork boolean NOT; `-\"term\"` in fallback), `all` sentinel normalization, quoted-OR skills, tier + budget-range snippet parsing (year-number guard), criteria echoing + noise notice; preset carries academic exclusions; byte-identical CG Pro port; `design-pro-schedule-manager` + `design-crm` skills updated.
- **Sub-project flags** (v1.1.87) — content-graph 1.0.8 (unchanged): explorer hover focus + tooltips, eased camera, debounced search, opt-in edge-flow animation, Okabe-Ito palette + the `ensure_contrast()` gamut-walk fix (#6782); Remote Sources driver audit — Generic REST edge-mapping/pagination/honest probe, Wikidata/SPARQL double-encoding, SPARQL `LIMIT 1` probe, failure envelopes, select rendering + slug autofill (#6783) — own track, not edited here.
- **MCP Apps connection & exposure wave** (v1.1.85, PRs #6749–#6759) — **#6753:** metabox diagnostics (Test Connection / Discover Tools / status badge / Test All), basic auth end-to-end with token masking (stored credentials never echoed; empty = keep), `Mcp-Session-Id` capture + echo, legacy-handshake fallback, mcpServers import, loopback detection, and the Security Center **MCP App Allowed Hosts** setting (constant → filter + saved merge). **#6754:** the client advertises the **negotiated** `protocolVersion` post-initialize and suppresses the `_meta` envelope in legacy sessions (also fixed PHPUnit-11 one-class-per-file discovery). **#6755:** new base `wp_mcp_ai_chat_effective_tools` seam in `build_tools_payload()` (after attention filtering, **before** capability checks) + Pro `wp_mcp_ai_mcp_apps_expose_tools()`. **#6756/#6757:** same-origin MCP endpoints dispatch in-process via `rest_do_request()` (no outbound socket/TLS/extra worker; outbound-HTTP rails for remote hosts/unregistered routes/disable filter) with `rest_post_dispatch` re-applied for response-side session headers. **#6758:** the seam's `$assistant_id` flows through `handle_tools_list()`/`handle_tool_request()`/`execute_tool_call_internal()`/the `list_mcp_tools` catalogue + `rest_pre_dispatch` registration + 60 s negative discovery cache + `save_apps()` cache invalidation. **Docs Hub 0.5.1** (#6749/#6750/#6759): local-first uploads source, opt-in remote import, symlink hardening, slug-named uploads folder + one-time migration. **README anchors + consolidation** (#6751/#6752): validator repaired to GitHub's real rules + CI enforcement; one 12-release Release History section + complete Previous Releases table. **Tool counts unchanged** (~308 base + ~1,282 Pro, ~1,590 total; bridge slugs are dynamic chat-time registrations). Catalog stays v2026.09.22.
- **TypeSafe Jev enhancement wave — plan 040 Phases 0–2** (v1.1.84, PRs #6745–#6747) — **Phase 0 fidelity:** noul criteria + structured EntryType fields (recursive two-gate walk); bounded 429/5xx retries honouring `retry-after` on the native client + OpenRouter decisions bridge (4xx/transport never retried); the bridge defaults to `typesafe/jev-1.13` and normalises aliases; opt-in advisory decision cache (`enable_typesafe_cache`, 300 s TTL, `cached: true` + zeroed usage, endpoint/base/model/payload keying); `typesafe_endpoint` setting + filter; `jev-preview` alias; `min_confidence`/`weights`/token warnings + `prompt_tokens`/`completion_tokens` usage aliases on `typesafe_decide`. **Phase 1:** new base tool **`typesafe_guardrail`** (one noul per hazard category → advisory pass/review/block; `ai_ml` preset + coverage manifest) + bundled skill `mcp-ai-wpoos-jev-decisions` (bundled skills 74 → 75). **Phase 2 (Pro):** fail-open guest-chat guardrail (`enable_jev_guest_guardrail` on the Layer I `wp_mcp_ai_pre_chat_message` filter, vetoes only high-confidence blocks); `check_citations()` advisory `citation_checks` on `generate_research_report`/`research_eca`; three new `manage_options`-gated Pro tools (`typesafe_rerank`, `typesafe_eval` + `WP_MCP_AI_Pro_Jev_Eval`, `typesafe_skill_select`). **#6745:** `typesafe_decide`'s declared capability matches the enforced admin gate (metadata-only, CI-pinned). **Tool counts +1 base +3 Pro → ~308 base + ~1,282 Pro, ~1,590 total.** Model catalog → v2026.09.22 (+`jev-preview`). Deferred per the plan: extraction tools, the CG port cluster, NV Cloud passthrough (OpenRouter Decisions GA).
...
- **DeepSeek V4 Pro restored** (v1.1.78, PR #6608) — DeepSeek's 2026-09-10 changelog continues V4 Pro past Sep 14: `deepseek-v4-pro` back to **active** ($0.66/$1.98 off-peak, sunset/fallback cleared, migration map deliberately unmapped); `nvoos-content-graph-ai` mirror bumps its catalog to v2026.09.10 (retired `chat`/`reasoner`/`coder` removed, legacy ids → `deepseek-flash`) + UsageTracker v4-pro pricing corrected $1.74/$3.48 → $0.66/$1.98; `lib/core` CostCalculator/TokenBudgetManager/CountTokensTool/SuggestBestModelTool aligned. Base catalog version stays v2026.09.10.
- **Checkout hardening series** (v1.1.78, PRs #6597/#6598/#6603) — `Schema::assetVersion()` cache-busts every content-graph enqueue by file mtime (fixes the invisible 1.0.7 hotfix — year-long `Cache-Control` on `?ver=1.0.7`); purchase-modal price syncs from the vendor `/session` response (display-only, vendor re-verifies) with `DEFAULT_PRICE_CENTS` 4900 → **3499**; minimalist modal restyle. Content-graph test bootstrap prefers its own vendored wp-phpunit lib (root-bootstrap PHPUnit-11 patch persisted to disk and fatals locked PHPUnit 9.6).
- **Docs Hub 0.4.4** (v1.1.78, PR #6606) — all wp.org review findings fixed: `/search` context-source filtering behind `manage_options`, `== Source Code ==` readme section + source-banner JS, contributors, textdomain cleanup, notice scoping, staging-transient isolation, symlink-safe recursive deletion (resolved-path containment), sitemap context-slug leak closed, stale page transients invalidated; PCP gate 0 blocking errors.
- **Legal consolidation** (v1.1.78, PRs #6599/#6605/#6607) — ToS unified (Part A products / Part B site+services+marketplace, then three parts for the two-entity seller model: **NV Digital Unlocked LLC** sells Products + runs the marketplace, **NV Digital Solutions** develops, owns IP, provides services); `API-LICENSES.md` + publishable HTML aligned; dual-controller Privacy Policy; full `docs/legal/publish/*.html` regenerated.
- **Content Graph 1.0.7 released** (v1.1.78, direct commits `4d7a97df5c`/`fda3e99c04`) — changelog marked released (tag `content-graph-v1.0.7`) + wp.org 18-point sign-off document. Tool counts unchanged (~303 base + ~1,265 Pro, ~1,568 total).
- **DeepSeek V4.1 Flash model refresh** (v1.1.77, PR #6555) — catalog v2026.09.10: new `deepseek-flash` (vision, $0.15/$0.60 off-peak per 1K); `deepseek-v4-flash` + `deepseek-v4-flash-vision-exp` retired; `deepseek-v4-pro` deprecated (sunset 2026-09-14, fallback `deepseek-flash`, corrected $0.66/$1.98); migration map rewrites all six legacy DeepSeek ids; defaults/fallbacks/cost tables/dropdowns/diagnostics + 10 Pro research/orchestration tools point at the new lineup. Cost calculator gains peak/off-peak support (`PEAK_WINDOWS`, `is_peak_time()`, `get_model_pricing_at()`, `calculate_cost_at()` — injectable timestamps; legacy `calculate_cost()`/`get_model_pricing()` stay time-independent and are opt-in untouched). `lib/core/` mirrors + `plugins/nvoos-content-graph-ai/` keep their own tracks (flagged, not edited).
- **Knowledge Graph Companion preset** (v1.1.77, PR #6570) — new featured `knowledge_graph` onboarding preset (listed first, temperature 0.3): 20 graph-aware base tools always included, all 14 `graphify_*` tools appended when the bundled Graphify addon is loaded + enabled (`wp_mcp_ai_onboarding_graph_tools_active` seam), auto-selected via `is_content_graph_detected()` when the standalone content-graph plugin / Graphify addon / any graph memory-bridge retriever exists (saved selections win); `⭐ Featured` badge + full-width featured card.
- **Base+pro gating fix + regression matrix** (v1.1.77, PR #6561) — Pro toolkits silently failed to load in base+pro installs (gates read `wp_mcp_ai_is_base_version()` as "no Pro" even with `WP_MCP_AI_PRO_VERSION` defined). All 21 toolkit `init.php` gates, module registry, Telegram Mini App media listing, AI CPT management, site templates, embedded webchat, and ~550 Pro tool `is_available()`/`get_unavailable_reason()` gates now use `( ! $is_base || defined( 'WP_MCP_AI_PRO_VERSION' ) )`. New `tests/basepro/` (15 contract tests) + `phpunit-basepro.xml.dist` + `base-pro` CI job + `composer run test:basepro`.
- **Pro WP-CLI load-order guard** (v1.1.77, PR #6585) — `wp_mcp_ai_pro_load_cli_commands()` defers the CLI require loop to `plugins_loaded` 30 when `WP_MCP_AI_PATH` is undefined (Pro activated before base); CLI base command returns early on undefined constant. Fixes the live `Undefined constant WP_MCP_AI_PATH` fatal that killed every `wp` command.
- **Agent-memory CCT canonical slug** (v1.1.77, PR #6591) — Graphify bridge maps/MemPalace guard/slug guesser and Pro memory retention move from phantom `ai_chat_agent_memories` to canonical `ai_agent_memories` (retention reads `jet_cct_ai_agent_memories` first, legacy-table fallback) — graph builds map memory fields/edges and dormancy sweeps, per-user caps, expiry pruning, and Memory Health stats start working. The next CG port wave must pick up the bridge change (byte-identity verifier will flag drift).
- **Checkout launch series complete** (v1.1.77, PRs #6568/#6571/#6573/#6587/#6588/#6589/#6590/#6594) — Checkout API **0.1.0 → 0.1.1**: settings-form redirect + Stripe ID round-trip, nonce-protected balance probe, public `GET /health`, **Stripe boolean stringification** (`true` → literal `'true'`; PHP form encoding broke every live PaymentIntent), **424/502 error contract** (Stripe 4xx → 424 in-modal, no redirect; transport/5xx → 502 → fallback). Content Graph **1.0.6 → 1.0.7**: JetEngine CCT Sources section + `excluded_cct_slugs` (#6557), per-CCT status notes (#6588), trust/value/post-purchase modal with 1-year updates (#6571), `GET /payments/health` diagnostics (#6573), Stripe `payment` element + `nvdigital-oos-v{VERSION}` release-tag URLs (#6594). Sub-project docs catch-up (#6590).
- **Security / CI sweeps** (v1.1.77, PRs #6592/#6593) — sharp 0.35.4, nodemailer 9.1.1, joi 18.2.8 (17.13.7 in pro/assets/spa), postcss-selector-parser 6.1.3/7.1.6 across root + addon lockfiles (same-major); `@ai-sdk/provider-utils` cross-major deferred. Content-graph ZIP publish race fixed (version/artifact via job outputs + per-ref concurrency guard) in all three build workflows.
- **Ecosystem Wave G — five toolkit ports complete + places starts** (v1.1.77, PRs #6551–#6584) — `nvoos-content-graph-pro` (1.0.0, unchanged) completes law-firm (intake/litigation/compliance-ethics/document-automation/research-analytics/admin), cre-debt (7 batches), quiz, eca (data→tools→REST→admin), and chat-channels (data→tools→REST A/B→admin); the places data layer lands. All byte-identical with per-PR `tests/` suites. Tool counts unchanged (~303 base + ~1,265 Pro, ~1,568 total).
- **Chat delivery full report + per-channel formats** (v1.1.76, PR #6525) — chat channels support the `full` template (summary + substantive response + envelope data, mirroring email); new per-channel `format` setting with allowlists (Telegram `html`/`markdown`/`markdown_v2`/`plain`; WhatsApp/Slack/Discord/Teams `markdown`/`plain`; Messenger/Google Chat `plain`). Telegram delivery routes through `send_telegram_message` directly (the broadcast tool strips HTML) so `parse_mode` works — the cron capability waiver extends to `wp_mcp_ai_send_telegram_message_capability` for the internal delivery context; `send_telegram_message` accepts `MarkdownV2` with reserved-char escaping; group messages dropped by `require_mention` now log `telegram_group_mention_required_ignored` (previously silent). Schema extensions only — no new tool slugs.
- **Duplicate summary skip** (v1.1.76, PR #6548) — assistant-run envelopes derive `summary` from the response's first 25 words and the email/chat `full` templates prepended it again above the full response. `response_starts_with_summary()` normalizes tags/whitespace + strips the trailing ellipsis and skips the summary when the response already opens with it (non-derived summaries still prepend).
- **Comic Creation toggle + playbook seeder idempotency** (v1.1.76, PR #6512 + direct `fe4d0ee880`) — the toolkit's `enable_comic_creation_toolkit` toggle is now registered (Tools section field list, Pro Features subtab, toolkit memory estimator 128 MB, `get_individual_toolkit_status()`/`get_toolkit_details()`, both WP-CLI toolkit maps — the 12 tools/4 CPTs shipped earlier). The profession playbook seeder's `hash_playbook_content()` strips the `Generated: <time> UTC` header before hashing so unchanged playbooks keep their attachment instead of deleting/recreating it every sync.
- **Checkout launch-complete series** (v1.1.76, PRs #6507/#6520/#6523/#6550) — required ToS/refund consent checkbox + required buyer-email on the purchase modal (`receipt_email` on the PaymentIntent); EU country selector (EU-27, filterable) reveals a required billing-address block (Stripe `billing_details`); Stripe Product/Price creation admin action + product/price IDs and statement descriptor attached as PaymentIntent metadata. License DB v3 stores consent timestamp, buyer email, buyer country (fill-once, `dbDelta` migrates incl. webhook-issued licenses). Manual install promoted to the primary documented path (`/verify` returns `download_url`). Commercial legal docs in `docs/legal/` (ToS §8.4 AI-output disclaimer + Florida governing law, refund policy, CCPA/GDPR privacy, AI-specific AUP, clickwrap implementation, compliance checklist). Checkout API version constant stays **0.1.0**.
- **Security / dependency sweeps** (v1.1.76, PRs #6515/#6532/#6546/#6504) — 9 Dependabot alerts (tiptap 3.30.5 ReDoS, multer 2.3.0 FD-leak, csv-parse 7.0.2 prototype replacement incl. Pro vendor refresh, react-router-dom 6.30.6 across canvas-toolkit/document-editor/media-worker/pro/SPAs); SVGO XSS CVE-2026-84370 floor `>=4.1.0`; svgo/hono/vitest bumps across 11 lockfiles + schedule-anything-spa build blockers; extract-zip/adm-zip have no patched release — recommended for dismissal. docs-hub ZIP packaging excludes `*.md` (keep `readme.txt`; rebuilt v0.4.3 ZIP 39→37 entries).
- **Ecosystem Wave F2 — ten toolkit ports complete + new port skill** (v1.1.76, PRs #6505–#6549 + direct `7b9195d1ac`) — `nvoos-content-graph-pro` (1.0.0, unchanged) completes the comic-creation (12 tools), dj-management, ai-tool-builder, architect-agent, architectural-design, site-creator (**33 tools**), document-generation, regulatory-registration, healthcare (9 batches), and law-firm ports — all byte-identical, monolith-gated seams degrade to `WP_Error` standalone, per-PR `tests/` suites. New coding-time skill `.agents/skills/mcp-ai-wpoos-ecosystem-port/` (base+pro→CG-Pro port-loop playbook); skill count **55 → 56**. Tool counts unchanged (~303 base + ~1,265 Pro, ~1,568 total).
- **Telegram broadcast credentials fix** (v1.1.75, PR #6482) — inline chat-channel credentials stored as raw JSON strings fataled the array-typed `unified_channel_broadcast` parameter. `normalize_channel_credentials()` decodes JSON strings to arrays (non-array garbage → empty array → clean `missing_channel_credentials` `WP_Error`); `extract_credentials_from_connection()` maps the real Remote Sites schema (decrypts `api_key` / `token`); the broadcast tool coerces/validates per-channel credentials (malformed → per-channel `failures` — saved broken schedules self-repair); the sanitizer coerces strings back to arrays at the save boundary.
- **Scheduled delivery credential fallback** (v1.1.75, PR #6488) — cron delivery resolved credentials in only three tiers; new tier-4 fallback resolves the first *enabled* Remote Sites connection of the channel type (assistant-assigned preferred) with destination fields merged from the schedule config. The broadcast capability gate is waived only for the internal `pro_schedule_manager_result_delivery` context (`wp_mcp_ai_unified_channel_broadcast_capability`; cron has no logged-in user; acting user = schedule creator). `create_pro_schedule`/`update_pro_schedule` + `POST /mcp-ai-pro/v1/schedules` accept `result_delivery` (silent drop fixed). Failure diagnostics ride `WP_Error::$error_data` (never secrets).
- **Content Graph memory bridge + NV oOS Complete checkout** (v1.1.75, PR #6486) — the standalone `nvoos-content-graph` Memory Bridge projects `wp_mcp_ai_memory_stored` + `nvoos_content_graph/memory_stored` into the standalone graph (advisory degradation — bridge failures never break memory writes). The base `wake_up_context` tool gains the `wp_mcp_ai_wake_up_context_graph_retriever` filter seam; `graph`/`auto` mode activates when listeners exist (not only bundled Graphify; byte-identical with none). The orchestration dashboard detects any connected graph bridge. The Content Graph checkout sells the Complete bundle (base + Pro, product id `nvoos-oos-complete`) with a conflict guard (409 when another NV oOS copy exists). nvoos-content-graph **1.0.4 → 1.0.6**.
- **Ecosystem Wave F2 — five toolkit ports complete** (v1.1.75, PRs #6476–#6502) — `nvoos-content-graph-pro` (1.0.0, unchanged) completes the financial-planning port (16 tools + admin slice), the social-media port (32 tools + settings page), and the mcp-servers port (all 33 servers: framework init + batch A 19 + batch B 14), plus remote-sites, video-production, analytics, multilingual, cloudways, dj-management, and image-production (data layer + top-level tools) slices. All byte-identical; monolith-gated seams degrade to `WP_Error` standalone; per-PR `tests/` suites. Tool counts unchanged (~303 base + ~1,265 Pro, ~1,568 total).
- **Google Calendar query encoding + free/busy scope** (v1.1.74, PR #6460) — `WP_MCP_AI_Google_Calendar_Client::request()` must `rawurlencode()` every query value before `add_query_arg()` (WP leaves values unencoded; Google decodes the raw `+` in RFC3339 offsets as a space → HTTP 400 on every date-filtered query); `calendar.freebusy` is in the Standard scope profile (new OAuth grants only — existing connections keep stored scopes until reconnect). The platform `GoogleCalendarClient`/`GoogleCalendarScopes` mirror the fix.
- **Result Delivery email formats** (v1.1.74, PR #6465) — new `WP_MCP_AI_Markdown_Converter` (`addons/pro/includes/services/`) renders assistant Markdown to email-safe inline-styled HTML: every text node HTML-escaped + `wp_kses` allowlist + protocol-allowlisted links (raw assistant HTML never reaches the email body). `format_email()` emits `html_body` + Markdown `text/plain`; per-channel `format` setting (`both` default | `html` | `markdown`) with Nodemailer multipart/alternative and a `wp_mail` single-content-type fallback.
- **Schedule Manager assistant-prompt editing** (v1.1.74, PR #6469) — the edit modal renders assistant dropdown + message textarea for `assistant_run` schedules; `update_schedule()` merges `assistant_config` with the stored config (`context`/`max_agentic_iterations` survive partial updates) and validates like the create path; `update_pro_schedule` MCP schema gains `assistant_config` (no new tool slugs — counts unchanged).
- **Test bootstrap process-wide guards** (v1.1.74, PRs #6464/#6470) — the WordPress Abilities registry is a once-per-process lazy singleton, so third-party kill-switch filters (`mcp_adapter_create_default_server`, `wpmedia_mcp_oauth_server_enabled`) live in `tests/bootstrap.php` before any suite runs; per-suite `setUp()` guards are brittle (stale PR bases reorder suites). #6470 supersedes #6464.
- **Ecosystem Wave F2 — PM + calendar-booking port completion** (v1.1.74, PRs #6450–#6472) — `nvoos-content-graph-pro` (1.0.0, unchanged) completes the Project Management port (data layer → research/settings pages) and the calendar-booking port (data layer → admin slice), plus the e-commerce admin pages batch. CI: `plugins/nvoos-content-graph-pro/**` excluded from the root WPCS + PHP-compat gate — the plugin lints under its own standard (#6457). Tool counts unchanged (~303 base + ~1,265 Pro, ~1,568 total).
- **`bulk_update_products` variable-scope expansion** (v1.1.73, PR #6447) — new `scope` argument (`all` default | `product` legacy): price/stock fields expand variable parents to their variations and grouped parents to their children via `WP_MCP_AI_Woo_Price_Qty_Updater::resolve_update_targets()`, `WC_Product_Variable::sync()` re-syncs variable parents (direct variation IDs re-sync their parent); status/featured/category/tag always apply to the selected product; `scope=product` keeps exact-ID legacy semantics. Acknowledged response-shape change: `updated_products[]` entries report `targets[]` per input ID + new `scope`/`updated_targets` keys (canonical envelope unchanged). Proposal 037.
- **`update_woo_product_qty` notify flag** (v1.1.73, PR #6448) — `notify` boolean (default `true`) suppresses low/no-stock *emails* when `false`: `apply_stock_quantity( $notify )` scopes the `woocommerce_should_send_low/no_stock_notification` filters to `__return_false` (priority 999) for the write and removes them in a `finally` (no global leak); the `woocommerce_low_stock`/`woocommerce_no_stock` actions still fire (push triggers unaffected). Proposal 038.
- **Async job queue table bootstrap fix** (v1.1.73, PR #6423) — `WP_MCP_AI_Async_Job_Queue` is required at boot (side-effect file) so its `plugins_loaded` init runs in time; activation creates the table; `dbDelta` gated behind a `DB_VERSION` option; `use_custom_table()` `SHOW TABLES` probe; `get_queue_stats()` fails soft with zeroed stats (Load Guard can never spam SQL errors). New `tests/test-async-job-queue-table-resilience.php`.
- **Comic Reader 0.5.0** (v1.1.73, PR #6402) — Komga-parity four-phase upgrade (0.2.0 → 0.5.0): archive magic-byte validation + owner-scoped delete + Range serving + CBZ covers; vertical/webtoon modes + scale types + double-page rules; ComicInfo.xml RTL detection + series/collection taxonomies + per-user progress; settings page + capability overrides. Addon tests wired into the root bootstrap.
- **Ecosystem port Wave F2 + `nvoos-content-graph-pro` 1.0.0** (v1.1.73, PRs #6397–#6445, #6449) — third ecosystem plugin ports the Pro business surface byte-identical (F1 Pro-core, D8-compat infra, validator, MCP toolkit server core, full CRM + e-commerce fan-out — 43 e-commerce tools); monolith-gated seams degrade to `WP_Error` standalone. Docs Hub **0.4.3** wp.org prep (#6397/#6403). Coding-time skills → **55** (new `mcp-ai-wpoos-wporg-submission` skill, #6418). Tool counts unchanged (~303 base + ~1,265 Pro, ~1,568 total).
...
- **Checkout API pipeline + salt fix** (v1.1.71, PR #6315) — `build/nvoos-checkout-api-v<version>.zip` joins the addon build and `addons/checkout-api/tests` joins the main PHPUnit gate; token/crypto classes derive `wp_salt( 'auth' ) . wp_salt( 'secure_auth' )` instead of raw `AUTH_KEY . SECURE_AUTH_KEY` (latent fatal on salt-less installs). Connectors links → `options-connectors.php` (#6314). Coding-time skills 53 → **54** (new `mcp-ai-wpoos-updates` skill, #6323/#6324). Test-suite skill → **40** patterns.
- **exec-disabled host hardening** (v1.1.70, PR #6295) — on PHP 8+ a disabled function throws a fatal `Error` that `@` cannot suppress; canonical `wp_mcp_ai_check_nodejs_available()` / `wp_mcp_ai_get_nodejs_version()` helpers (`addons/pro/includes/npm-integration-filters.php`) probe exec-first with a `WP_MCP_AI_Process_Service` (`proc_open`) fallback across six Pro admin pages + the OCR service (`is_cli_tool_available()`/`run_cli_command()`); `max( 0, (int) $value )` clamps replace `absint()` in seven CPT/settings sanitizers (never `absint()` — it flips negatives into positives).
- **Fifth PHPUnit repair wave** (v1.1.70, PRs #6280–#6312) — ~29 test-only PRs driven by two CI triage runs (`docs/developer/testing-docs/CI-TRIAGE-91006542428.md`, `CI-TRIAGE-91771001271.md`) closed clusters K1–K16 and carried grouped production fixes: transcript `attachments` metadata preserved + model-window-capped default tokens (#6280); dashboard event-slug tolerance (#6281); validated `web_search` profession tags (#6282); preset fixes (#6283); mesh URL validation before `esc_url_raw()` (#6285); opt-in guest memory bucket `DEFAULT_GUEST_AGENT_ID` (#6286); `reset_for_tests()` CPT-integration seam (#6287); curriculum exporter empty-structure skips (#6293); JetEngine `JET_ENGINE_VERSION` gates + `is_storage_available()` physical-table probe + transactional-DDL-safe `cct_table_exists()` + JFB `id`/flat params + cached `wp_mcp_ai_pro_get_tool_map()` (#6296/#6300); complexity routing restored — the enhanced OpenAI client passes the explicit model only (#6298); Auth0 audience structural validation + GitHub OAuth terminate seam (#6301); markup REST `400` annotations + `wp_mcp_ai_legacy_sse_enabled` filter + skill-pack slug normalization + install action for empty packs (#6302); clamp sanitizers + plugins-integration terminate seam (#6303); `is_elementor_editor_init()` + `static::is_available()` (#6304); acting-user image `read_post` + positive-int user validation (#6305); `timed-out` needle + provider-prefix-stripped model limits + Ollama native tool-call preservation + `published_date` (#6306); agentic event allowlist + `started_at_ms` trace durations (#6308); K16 bundle — `sanitize_key`-stable webhook IDs, settings-repository blob fallback, acting-user chart `unfiltered_html`, social-publish date reset, PSO inflections, h1–h6 allowlist, schema-verifier `required` independence (#6311); skill-suite upload isolation (#6312). The test-suite skill board is compacted (37 patterns unchanged); residual findings tracked in `docs-catch-up-open-items.md` OI-4.
- **Pro SPA v2 shortcode + embedded mode** (v1.1.68, PR #6256) — new `[nvoos_pro_spa]` shortcode (`WP_MCP_AI_Pro_SPA_Shortcode`, `WP_MCP_AI_Pro_SPA_Config`, module-registry entry) mounts the Pro SPA v2 chat-first embedded mode on the front end (router-free, drawers/tool shortcuts/OKF drawer); guest mode rides the base guest-token machinery behind the "Allow Guest Access" setting. Proposal 033.
- **Hermes dashboard fleet extensions** (v1.1.68, PR #6249) — new top-level `extensions/` tree (fleet monitoring + control plane `nv-oos-fleet` Python plugin API, `backup-download`, `external-app-tab`, `mcp-tool-shortcuts`, `install.sh`, smoke tests); plans in `docs/developer/integration/`.
- **Stale REST nonce self-heal** (v1.1.68, PR #6225) — `GET /mcp-ai/v1/session/nonce` mints a fresh session-bound nonce from the request's own auth cookie (nonce/capability-free by design, `no-cache`/`no-store`); the chat client retries `403 rest_cookie_invalid_nonce` failures instead of showing "Cookie check failed". Keep the justification comment on the `__return_true` callback.
- **Docs Hub 0.4.2** (v1.1.68, PRs #6246/#6253) — local-page link resolution to `#/slug` routes, github-slugger-exact anchors, directory-relative "Accept fix" suggestions, `../` validation + containment checks (`nvoos_docs_hub_fixer_allowed_roots`), slug-based source resolution + skip reasons, atomic-swap sync failure surfacing, and the shortcode disables the core emoji loader on docs-browser pages (MutationObserver crashed React's DOM).
- **Provider enable defaults** (v1.1.68, PR #6255) — OpenAI/Anthropic/Gemini enable flags now default to **false** on fresh installs; provider dropdowns list only enabled + credentialed providers (`WP_MCP_AI_Model_Config::get_available_providers()`); the onboarding wizard auto-enables the provider whose key is entered.
- **Third PHPUnit repair wave + grouped fixes** (v1.1.68, PRs #6223–#6258) — ~21 test-only PRs kept the suite aligned. Production fixes: calendar granted-scopes sanitizer keeps space-delimited scope lists + normalizes legacy `%20` grants (#6226); agent-identity resolver casts alias-table canonical IDs to int (#6232); token-budget catalog dedup (`gpt-4.1`, `gemini-2.5-flash`) + restored `wp_mcp_ai_model_tpm_limit` filter seam on the CCT → catalog fallback path (#6233); assistant untrash restores the pre-trash status via `wp_untrash_post_status` (#6239); tool presets gain the missing OKF/Calendar/Gmail/Drive/Composio/git tools and exclude test doubles (#6242); `wp_mcp_ai_seed_task_templates` AJAX registered (#6243); `fast-uri` >=4.1.4 across npm trees (#6244); Tiptap pinned to 3.30.4 in canvas-toolkit + document-editor (`mergeAttributes` prototype pollution, #6250); jQuery UI sortable shim enqueues core `jquery-ui-sortable` at priority 5 on post edit screens (#6258); test-suite skill refreshed to **27** patterns.
...
| `.context/image-identification.md` | Working on the image identification ladder (identify_image + Cloud Vision / perceptual-hash / OCR tools) |
