...
> Last reviewed: **September 4, 2026** · Version: **2.22**
...
- **Fifth PHPUnit repair wave** (v1.1.69 wave 5, PRs #6280–#6308) — ~18 test-suite PRs + grouped production fixes: exec-disabled host hardening (canonical Node.js/OCR CLI helpers, exec-first + Process Service fallback, `max(0,(int))` sanitizer clamps, #6295); transcript `display.attachments` sanitization + model-aware `max_tokens` caps (#6280); complexity-based model routing restored (empty base, #6298) + provider-prefixed slug limits (#6306); JetEngine detection requires `JET_ENGINE_VERSION` and CCT probes use direct SELECT + `is_storage_available()` (MySQL 8.0 transactional DDL, #6296/#6300); markup validation 400s + filterable legacy SSE (`wp_mcp_ai_legacy_sse_enabled`) (#6302); Auth0 audience syntax checks + GitHub OAuth redirect terminate filter (#6301); acting-user image permission checks + positive-int `safe_set_current_user()` (#6305); pre-sanitization mesh peer URL validation (#6285); opt-in guest memory bucket (#6286); millisecond-precision harness traces + agentic/token-DB log events (#6308); conversation-import tools join presets; CI-triage artifact `docs/developer/testing-docs/CI-TRIAGE-91771001271.md`.
